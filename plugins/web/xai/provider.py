...
``{title, url, description, position}`` rows every other Alex web
provider produces.
...
prefers Alex-managed xAI Grok OAuth (via ``alex auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.alex/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``alex tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `alex auth` to sign in with "
