...
Let the alex agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where alex runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `alex meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `alex meet` CLI |
...
| `cli.py` | `alex meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$ALEX_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `alex meet node {run,list,approve,remove,status,ping}` |
...
alex meet install                                      # pip + Chromium
alex meet setup                                        # preflight
alex meet auth                                         # optional
alex meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
alex meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.alex/.env
alex meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
alex meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
alex meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.alex/.env
alex meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, alex will **not** switch your system audio input automatically — the
...
alex plugins enable google_meet
alex meet node run --display-name my-mac --host 0.0.0.0 --port 18789
