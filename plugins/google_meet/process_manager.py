...
session-scoped state file under ``$ALEX_HOME/workspace/meetings/.active.json``
...
# File + directory layout (under $ALEX_HOME):
#
#   workspace/meetings/
...
def _root() -> Path:
    return Path(get_alex_home()) / "workspace" / "meetings"
...
def start(
    ...
    *,
    ...
    If a bot is already running for this alex install, leave it first —
    ...
    env = os.environ.copy()
    env["ALEX_MEET_URL"] = url
    env["ALEX_MEET_OUT_DIR"] = str(out)
    env["ALEX_MEET_GUEST_NAME"] = guest_name
    if headed:
        env["ALEX_MEET_HEADED"] = "1"
    if auth_state:
        env["ALEX_MEET_AUTH_STATE"] = auth_state
    if duration:
        env["ALEX_MEET_DURATION"] = duration
    # v2: realtime mode + passthroughs. The bot defaults to transcribe
    # mode if ALEX_MEET_MODE isn't set, matching v1 behavior.
    if mode:
        env["ALEX_MEET_MODE"] = mode
    if realtime_model:
        env["ALEX_MEET_REALTIME_MODEL"] = realtime_model
    if realtime_voice:
        env["ALEX_MEET_REALTIME_VOICE"] = realtime_voice
    if realtime_instructions:
        env["ALEX_MEET_REALTIME_INSTRUCTIONS"] = realtime_instructions
    if realtime_api_key:
        env["ALEX_MEET_REALTIME_KEY"] = realtime_api_key
