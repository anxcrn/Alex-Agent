/**
 * Alex Kanban — Dashboard Plugin
 *
 * Board view for the multi-agent collaboration board backed by
 * ~/.alex/kanban.db. Calls the plugin's backend at /api/plugins/kanban/
 * and tails task_events over a WebSocket for live updates.
 *
 * Plain IIFE, no build step. Uses window.__ALEX_PLUGIN_SDK__ for React +
...
(function () {
  ...
  const SDK = window.__ALEX_PLUGIN_SDK__;
  if (!SDK) return;
  ...
  const COLUMN_DOT = {
    triage: "alex-kanban-dot-triage",
    todo: "alex-kanban-dot-todo",
    ready: "alex-kanban-dot-ready",
    running: "alex-kanban-dot-running",
    blocked: "alex-kanban-dot-blocked",
    done: "alex-kanban-dot-done",
    archived: "alex-kanban-dot-archived",
  };
  ...
  const MIME_TASK = "text/x-alex-task";
  ...
  const DOCS_URL = "https://alex-agent.charan vankudoth.com/docs/user-guide/features/kanban";
  const DOCS_TUTORIAL_URL = "https://alex-agent.charan vankudoth.com/docs/user-guide/features/kanban-tutorial";
  ...
  const LS_BOARD_KEY = "alex.kanban.selectedBoard";
  ...
  function renderMarkdown(src) {
    ...
    html = html.replace(/\u0000CODE(\d+)\u0000/g, (_m, i) =>
      `<pre class="alex-kanban-md-code"><code>${escapeHtml(blocks[Number(i)])}</code></pre>`,
    );
    return html;
  }
  ...
  function sanitizeMarkdownAttrs(tag, attrs) {
    ...
    if (tag === "pre" && /\sclass=(["'])alex-kanban-md-code\1/i.test(attrs)) {
      return ' class="alex-kanban-md-code"';
    }
    return "";
  }
  ...
  function MarkdownBlock(props) {
    ...
    if (!enabled) {
      return h("pre", { className: "alex-kanban-pre" }, props.source || "");
    }
    return h("div", {
      className: "alex-kanban-md",
  ...
  // standard `drop` event and our `alex-kanban:drop` event.
  // -------------------------------------------------------------------------
  ...
  function attachTouchDrag(el, taskId) {
    ...
    function onDown(e) {
      ...
      proxy.classList.add("alex-kanban-touch-proxy");
      ...
      function move(ev) {
        ...
        if (target !== lastTarget) {
          if (lastTarget) lastTarget.classList.remove("alex-kanban-column--drop");
          if (target) target.classList.add("alex-kanban-column--drop");
          lastTarget = target;
        }
      }
      function up() {
        ...
        if (lastTarget) {
          lastTarget.classList.remove("alex-kanban-column--drop");
          const status = lastTarget.getAttribute("data-kanban-column");
          const isTrash = lastTarget.hasAttribute("data-kanban-trash");
          if (isTrash) {
            lastTarget.dispatchEvent(new CustomEvent("alex-kanban:delete", {
              detail: { taskId },
              bubbles: true,
            }));
          } else if (status) {
            lastTarget.dispatchEvent(new CustomEvent("alex-kanban:drop", {
  ...
  function KanbanPage() {
    ...
    useEffect(function () {
      ...
      function openWs() {
        ...
        // loopback. Reading window.__ALEX_SESSION_TOKEN__ directly (the old
   ...
   const deleteTask = useCallback(function (taskId) {
    ...
    return h(ErrorBoundary, null,
      h("div", { className: "alex-kanban flex flex-col gap-4" },
        h(BoardSwitcher, {
          board: board,
          boardList: boardList,
  ...
  function AttentionStrip(props) {
    ...
    return h("div", {
      className: cn(
        "alex-kanban-attention",
        "alex-kanban-attention--" + topSev,
      ),
    },
      h("div", { className: "alex-kanban-attention-bar" },
        h("span", { className: "alex-kanban-attention-icon" },
          topSev === "critical" ? "!!!" : topSev === "error" ? "!!" : "⚠"),
        h("span", { className: "alex-kanban-attention-text" },
          diagTasks.length === 1
      ...
      expanded
        ? h("div", { className: "alex-kanban-attention-list" },
            diagTasks.map(function (task) {
              ...
              return h("div", {
                key: task.id,
                className: cn(
                  "alex-kanban-attention-row",
                  "alex-kanban-attention-row--" + sev,
                ),
              },
                h("span", { className: "alex-kanban-attention-row-sev" },
                  sev === "critical" ? "!!!" : sev === "error" ? "!!" : "⚠"),
                h("span", { className: "alex-kanban-attention-row-id" }, task.id),
                h("span", { className: "alex-kanban-attention-row-title" },
                  task.title || tx(t, "untitled", "(untitled)")),
                h("span", { className: "alex-kanban-attention-row-meta" },
                  task.assignee ? "@" + task.assignee : tx(t, "unassigned", "unassigned"),
                  " \u00b7 ",
                  kinds.length > 0 ? kinds.join(", ") : tx(t, "diagnostic", "diagnostic"),
                ),
                h("button", {
                  className: "alex-kanban-attention-row-btn",
  ...
  function DiagnosticActionButton(props) {
    ...
    const cls = cn(
      "alex-kanban-diag-action-btn",
      action.suggested ? "alex-kanban-diag-action-btn--suggested" : "",
    );
    ...
    return h("span", { className: cls + " alex-kanban-diag-action-btn--unknown" },
      label);
  }
  ...
  function DiagnosticCard(props) {
    ...
    const execAction = function (action) {
      ...
      if (action.kind === "comment") {
        ...
        const ta = document.querySelector(".alex-kanban-drawer-comment-row input, .alex-kanban-drawer-comment-row textarea");
        if (ta) {
    ...
    const sevClass = "alex-kanban-diag--" + (diag.severity || "warning");
    return h("div", { className: cn("alex-kanban-diag", sevClass) },
      h("div", { className: "alex-kanban-diag-header" },
        h("span", { className: "alex-kanban-diag-sev" },
          diag.severity === "critical" ? "!!!" :
          diag.severity === "error" ? "!!" : "\u26a0"),
        h("span", { className: "alex-kanban-diag-title" },
          diag.title),
      ),
      h("div", { className: "alex-kanban-diag-detail" },
        diag.detail),
      diag.data && Object.keys(diag.data).length > 0
        ? h("div", { className: "alex-kanban-diag-data" },
            Object.keys(diag.data).map(function (k) {
              const v = diag.data[k];
              if (Array.isArray(v) && v.length > 0 && typeof v[0] === "string" &&
                  v[0].indexOf("t_") === 0) {
                // Task-id list — render as chips.
                return h("div", { key: k, className: "alex-kanban-diag-data-row" },
                  h("span", { className: "alex-kanban-diag-data-key" }, k + ":"),
                  v.map(function (x) {
                    return h("code", {
                      key: x, className: "alex-kanban-event-phantom-chip",
                    }, x);
                  }),
                );
              }
              return h("div", { key: k, className: "alex-kanban-diag-data-row" },
                h("span", { className: "alex-kanban-diag-data-key" }, k + ":"),
                h("span", { className: "alex-kanban-diag-data-val" },
      ...
      reassignAction
        ? h("div", { className: "alex-kanban-diag-reassign-row" },
            h("span", { className: "alex-kanban-diag-reassign-label" },
              tx(t, "reassignTo", "Reassign to:")),
            h("select", {
              className: "alex-kanban-recovery-select",
        ...
        : null,
      h("div", { className: "alex-kanban-diag-actions" },
      ...
      msg
        ? h("div", {
            className: cn(
              "alex-kanban-diag-msg",
              msg.ok ? "alex-kanban-diag-msg--ok" : "alex-kanban-diag-msg--err",
            ),
          }, msg.text)
        : null,
    );
  ...
  function DiagnosticsSection(props) {
    ...
    return h("div", { className: "alex-kanban-section" },
      h("div", { className: "alex-kanban-section-head-row" },
        h("span", { className: "alex-kanban-section-head" },
          hasOpenDiags
            ? h("span", { className: "alex-kanban-section-head-warning" },
                `\u26a0 ${tx(t, "diagnostics", "Diagnostics")} (${diags.length})`)
            : tx(t, "diagnostics", "Diagnostics"),
        ),
        h("button", {
          className: "alex-kanban-section-toggle",
          onClick: function () { setOpen(function (x) { return !x; }); },
          type: "button",
        }, open ? tx(t, "hide", "Hide") : tx(t, "show", "Show")),
      ),
      open
        ? h("div", { className: "alex-kanban-diag-list" },
  ...
  function DocsLink() {
    return h("a", {
      ...
      className: "alex-kanban-docs-link",
      title: "Open Alex Kanban docs in a new tab",
      "aria-label": "Alex Kanban documentation",
    }, "?");
  }
  ...
  function OrchestrationPanel() {
    ...
    // each triage card (or runs `alex kanban decompose <id>`) and tasks
    ...
    return h(Card, { className: "p-3" },
      h(CardContent, { className: "p-2 flex flex-col gap-3" },
        ...
        msg ? h("div", {
          className: msg.ok ? "alex-kanban-msg-ok" : "alex-kanban-msg-err",
        }, msg.text) : null,
  ...
  function BoardSwitcher(props) {
    ...
    if (!shouldShow) {
      return h("div", {
        className: "alex-kanban-boardswitcher-compact",
    ...
    return h("div", { className: "alex-kanban-boardswitcher" },
      h("div", { className: "alex-kanban-boardswitcher-inner" },
  ...
  function NewBoardDialog(props) {
    ...
    return h("div", {
      className: "alex-kanban-dialog-backdrop",
      onClick: function (e) { if (e.target === e.currentTarget) props.onCancel(); },
    },
      h("form", {
        className: "alex-kanban-dialog",
        onSubmit: onSubmit,
      },
        h("div", { className: "alex-kanban-dialog-title" },
        ...
        h("div", { className: "alex-kanban-dialog-actions" },
          h(Button, {
            type: "button",
  ...
  function BoardToolbar(props) {
    ...
    return h("div", { className: "flex flex-wrap items-end gap-3" },
      ...
      h("div", { className: "flex flex-col gap-1",
                 title: "Filter by assigned Alex profile. Profiles are the named agent identities that claim and work on tasks." },
  ...
  function BulkActionBar(props) {
    ...
    return h("div", { className: "alex-kanban-bulk" },
      h("span", { className: "alex-kanban-bulk-count" },
      ...
      h("div", { className: "alex-kanban-bulk-priority",
      ...
      h("div", { className: "alex-kanban-bulk-reassign",
                 title: "Reassign selected tasks to a different Alex profile. Pick a profile (or unassign) and click Apply." },
        h(Select, Object.assign({
          value: assignee,
          className: "h-7 text-xs",
      ...
      h("label", { className: "alex-kanban-bulk-reclaim-first", title: "Reclaim any active claims before reassigning" },
        h(Checkbox, {
  ...
  function TrashDropZone(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("alex-kanban:delete", onTouchDelete);
      return function () { el.removeEventListener("alex-kanban:delete", onTouchDelete); };
    }, [props.onDelete]);
    ...
    return h("div", {
      ...
      className: cn(
        "alex-kanban-trash",
        dragOver ? "alex-kanban-trash--drop" : "",
        props.draggingTaskId ? "alex-kanban-trash--active" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("span", { className: "alex-kanban-trash-icon" }, "🗑️"),
      h("span", { className: "alex-kanban-trash-label" },
  ...
  function BoardColumns(props) {
    const handleDragStart = useCallback(function (e) {
      const card = e.target.closest && e.target.closest(".alex-kanban-card");
      if (!card) return;
    ...
    return h("div", { className: "alex-kanban-columns", onDragStart: handleDragStart, onDragEnd: handleDragEnd },
  ...
  function Column(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("alex-kanban:drop", onTouchDrop);
      return function () { el.removeEventListener("alex-kanban:drop", onTouchDrop); };
    ...
    return h("div", {
      ...
      className: cn(
        "alex-kanban-column",
        dragOver ? "alex-kanban-column--drop" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("div", { className: "alex-kanban-column-header",
                 title: colHelp || "" },
        h(Checkbox, {
          className: "alex-kanban-col-check",
        ...
        h("span", { className: cn("alex-kanban-dot", COLUMN_DOT[props.column.name]) }),
        h("span", { className: "alex-kanban-column-label" },
          colLabel || props.column.name),
        h("span", { className: "alex-kanban-column-count",
                    title: `${props.column.tasks.length} task${props.column.tasks.length === 1 ? "" : "s"} in this column` },
          props.column.tasks.length),
        h("button", {
          type: "button",
          className: "alex-kanban-column-add",
          title: tx(t, "createTask", "Create task in this column"),
          onClick: function () { setShowCreate(function (v) { return !v; }); },
        }, showCreate ? "×" : "+"),
      ),
      h("div", { className: "alex-kanban-column-sub" },
        colHelp || ""),
      ...
      h("div", { className: "alex-kanban-column-body" },
        props.column.tasks.length === 0
          ? h("div", { className: "alex-kanban-empty" }, tx(t, "noTasks", "— no tasks —"))
          : lanes
            ? lanes.map(function (lane) {
                return h("div", { key: lane.assignee, className: "alex-kanban-lane" },
                  h("div", { className: "alex-kanban-lane-head" },
                    h("span", { className: "alex-kanban-lane-name" }, lane.assignee),
                    h("span", { className: "alex-kanban-lane-count" }, lane.tasks.length),
                  ),
  ...
  function stalenessClass(task) {
    ...
    if (age >= tier.red)   return "alex-kanban-card--stale-red";
    if (age >= tier.amber) return "alex-kanban-card--stale-amber";
    return "";
  }
  ...
  function TaskCard(props) {
    ...
    const handleDragStart = function (e) {
      ...
      const selectedCards = document.querySelectorAll(".alex-kanban-card--selected");
      if (selectedCards.length > 1 && props.selected) {
        const ghost = document.createElement("div");
        ghost.className = "alex-kanban-drag-ghost";
    ...
    return h("div", {
      ...
      className: cn(
        "alex-kanban-card",
        props.selected ? "alex-kanban-card--selected" : "",
        props.failed ? "alex-kanban-card--failed" : "",
        props.draggingSource ? "alex-kanban-card--dragging-source" : "",
        stalenessClass(t),
      ),
      draggable: true,
      tabIndex: 0,
      ...
      h(Card, null,
        h(CardContent, { className: "alex-kanban-card-content" },
          h("div", { className: "alex-kanban-card-row" },
            h("label", {
              className: "alex-kanban-card-check-wrap",
              title: tx(i18n, "selectForBulk", "Select for bulk actions"),
              onClick: function (e) { e.stopPropagation(); },
            },
              h(Checkbox, {
                className: "alex-kanban-card-check",
            ...
            h("span", { className: "alex-kanban-card-id",
                        title: `Task id: ${t.id}. Use this id with kanban_show, /kanban show, or alex kanban show.` }, t.id),
            t.warnings && t.warnings.count > 0
              ? h("span", {
                  className: cn(
                    "alex-kanban-warning-badge",
                    "alex-kanban-warning-badge--" + (t.warnings.highest_severity || "warning"),
                  ),
                  title: (
            ...
            needsAssignee
              ? h(Badge, {
                  variant: "outline",
                  className: "alex-kanban-needs-assignee",
          ...
          h("div", { className: "alex-kanban-card-title" },
            t.title || tx(i18n, "untitled", "(untitled)")),
          h("div", { className: "alex-kanban-card-row alex-kanban-card-meta" },
            t.assignee
              ? h("span", { className: "alex-kanban-assignee",
                            title: `Assigned to Alex profile @${t.assignee}` }, "@", t.assignee)
              : h("span", { className: "alex-kanban-unassigned",
                            title: needsAssignee
            ...
            t.comment_count > 0
              ? h("span", { className: "alex-kanban-count",
            ...
            t.link_counts && (t.link_counts.parents + t.link_counts.children) > 0
              ...
              : null,
            h("span", { className: "alex-kanban-ago",
  ...
  function InlineCreate(props) {
    ...
    return h("div", { className: "alex-kanban-inline-create" },
      h("textarea", {
        value: title,
      ...
      h("div", { className: "flex gap-2" },
        h(Input, {
          ...
          title: props.columnName === "triage"
            ? "Alex profile that will spec this task (default: the dispatcher's configured specifier). Leave blank to let the dispatcher pick."
            : "Alex profile to assign. Leave blank and the dispatcher will pick from available profiles when the task is Ready.",
  ...
  function TaskDrawer(props) {
    ...
    return h("div", { className: "alex-kanban-drawer-shade", onClick: props.onClose },
      h("div", {
        className: "alex-kanban-drawer",
        onClick: function (e) { e.stopPropagation(); },
      },
        h("div", { className: "alex-kanban-drawer-head" },
          h("span", { className: "text-xs text-muted-foreground" }, props.taskId),
          h("button", {
            type: "button",
            onClick: props.onClose,
            className: "alex-kanban-drawer-close",
        ...
        data ? h("div", { className: "alex-kanban-drawer-comment-row" },
          h(Input, {
            value: newComment,
  ...
  function AttachmentsSection(props) {
    ...
    return h("div", { className: "alex-kanban-section" },
      h("div", { className: "alex-kanban-section-head" },
      ...
      atts.length === 0
        ...
        : atts.map(function (a) {
            return h("div", {
              ...
              h("button", {
                type: "button",
                className: "alex-kanban-attachment-link truncate",
                title: a.filename,
  ...
  function TaskDetail(props) {
    ...
    return h("div", { className: "alex-kanban-drawer-body" },
      h("div", { className: "alex-kanban-drawer-title" },
        h("span", { className: cn("alex-kanban-dot", COLUMN_DOT[t.status]) }),
        props.editing
          ? h(TitleEditor, {
              initial: t.title || "",
          ...
          : h("span", {
              className: "alex-kanban-drawer-title-text",
              title: tx(i18n, "clickToEdit", "Click to edit"),
              onClick: function () { props.setEditing(true); },
            }, t.title || tx(i18n, "untitled", "(untitled)")),
      ),
      h("div", { className: "alex-kanban-drawer-meta" },
      ...
      t.result ? h("div", { className: "alex-kanban-section" },
        h("div", { className: "alex-kanban-section-head" }, tx(i18n, "result", "Result")),
      ...
      h("div", { className: "alex-kanban-section" },
        h("div", { className: "alex-kanban-section-head" },
        ...
        comments.length === 0
          ...
          : comments.map(function (c) {
              return h("div", { key: c.id, className: "alex-kanban-comment" },
                h("div", { className: "alex-kanban-comment-head" },
                  h("span", { className: "alex-kanban-comment-author" }, c.author || "anon"),
                  h("span", { className: "alex-kanban-comment-ago" },
      ...
      h("div", { className: "alex-kanban-section" },
        ...
        events.slice().reverse().slice(0, 20).map(function (e) {
          ...
          return h("div", {
            ...
            className: cn(
              "alex-kanban-event",
              isDiag ? "alex-kanban-event--hallucination" : "",
            ),
          },
            isDiag
              ? h("div", { className: "alex-kanban-event-header" },
                  h("span", { className: "alex-kanban-event-warning-icon" }, "⚠"),
                  h("span", { className: "alex-kanban-event-warning-label" },
                    getDiagnosticEventLabel(i18n, e.kind) || e.kind),
                  h("span", { className: "alex-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                )
              : h("div", { className: "alex-kanban-event-header-plain" },
                  h("span", { className: "alex-kanban-event-kind" }, e.kind),
                  h("span", { className: "alex-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                ),
            isDiag && phantoms.length > 0
              ? h("div", { className: "alex-kanban-event-phantom-row" },
                  h("span", { className: "alex-kanban-event-phantom-label" },
                    tx(i18n, "phantomIds", "Phantom ids:")),
                  phantoms.map(function (pid) {
                    return h("code", {
                      key: pid,
                      className: "alex-kanban-event-phantom-chip",
                    }, pid);
                  }),
                )
              : null,
            e.payload && !isDiag
              ? h("code", { className: "alex-kanban-event-payload" },
    ...
    return h("div", { className: "alex-kanban-section" },
      h("div", { className: "alex-kanban-section-head-row" },
        ...
        !showAll
          ? h("button", {
              type: "button",
              onClick: function () { setExpanded(true); },
              className: "alex-kanban-edit-link",
      ...
      visible.map(function (r) {
        const outcomeClass = r.ended_at
          ? `alex-kanban-run--${r.outcome || r.status || "ended"}`
          : "alex-kanban-run--active";
        return h("div", { key: r.id, className: cn("alex-kanban-run", outcomeClass) },
          h("div", { className: "alex-kanban-run-head" },
            h("span", { className: "alex-kanban-run-outcome" },
              r.ended_at ? (r.outcome || r.status || tx(t, "ended", "ended")) : tx(t, "active", "active")),
            h("span", { className: "alex-kanban-run-profile" },
              r.profile ? `@${r.profile}` : tx(t, "noProfile", "(no profile)")),
            h("span", { className: "alex-kanban-run-elapsed" }, fmtElapsed(r)),
            h("span", { className: "alex-kanban-run-ago" },
              timeAgo ? timeAgo(r.started_at) : ""),
          ),
          r.summary
            ? h("div", { className: "alex-kanban-run-summary" }, r.summary)
            : null,
          r.error
            ? h("div", { className: "alex-kanban-run-error" }, r.error)
            : null,
          (r.metadata && Object.keys(r.metadata).length > 0)
            ? (function () {
                ...
                return h("details", {
                    className: "alex-kanban-run-meta-block",
                    open: !collapsed,
                  },
                  h("summary", { className: "alex-kanban-run-meta-label" }, "Metadata"),
                  h("code", { className: "alex-kanban-run-meta" }, json),
                );
              })()
            : null,
        );
  ...
  function WorkerLogSection(props) {
    ...
    if (state.loading) {
      ...
      body = h("pre", { className: "alex-kanban-pre alex-kanban-log" },
        data.content || "(empty)");
    }
  ...
  function MetaRow(props) {
    return h("div", { className: "alex-kanban-meta-row" },
      h("span", { className: "alex-kanban-meta-label" }, props.label),
      h("span", { className: "alex-kanban-meta-value" }, props.value),
    );
  }
  ...
  function TitleEditor(props) {
    ...
    return h("div", { className: "alex-kanban-edit-row" },
      h(Input, {
  ...
  function AssigneeEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "alex-kanban-meta-row" },
        h("span", { className: "alex-kanban-meta-label" }, tx(t, "assignee", "Assignee")),
        h("span", {
          className: "alex-kanban-meta-value alex-kanban-editable",
  ...
  function PriorityEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "alex-kanban-meta-row" },
        h("span", { className: "alex-kanban-meta-label" }, tx(t, "priority", "Priority")),
        h("span", {
          className: "alex-kanban-meta-value alex-kanban-editable",
          onClick: function () { setEditing(true); },
  ...
  function BodyEditor(props) {
    ...
    return h("div", { className: "alex-kanban-section" },
      h("div", { className: "alex-kanban-section-head-row" },
        h("span", { className: "alex-kanban-section-head" }, tx(t, "description", "Description")),
        editing
      ...
      editing
        ? h("textarea", {
            className: "alex-kanban-textarea",
            value: v,
            rows: 8,
            onChange: function (e) { setV(e.target.value); },
          })
  ...
  function DependencyEditor(props) {
    ...
    return h("div", { className: "alex-kanban-section" },
      h("div", { className: "alex-kanban-section-head" }, tx(t, "dependencies", "Dependencies")),
      h("div", { className: "alex-kanban-deps-row" },
        h("span", { className: "alex-kanban-deps-label" }, tx(t, "parents", "Parents:")),
        h("div", { className: "alex-kanban-deps-chips" },
          (links.parents || []).length === 0
            ? h("span", { className: "alex-kanban-deps-empty" }, tx(t, "none", "none"))
            : (links.parents || []).map(function (id) {
                return h("span", { key: id, className: "alex-kanban-dep-chip" },
                  id,
                  h("button", {
                    type: "button",
                    className: "alex-kanban-dep-chip-x",
      ...
      h("div", { className: "alex-kanban-deps-row" },
        h("span", { className: "alex-kanban-deps-label" }, tx(t, "children", "Children:")),
        h("div", { className: "alex-kanban-deps-chips" },
  ...
  function StatusActions(props) {
    ...
    return h("div", null,
      h("div", { className: "alex-kanban-actions" },
        specifyButton,
        decomposeButton,
  ...
  function HomeSubsSection(props) {
    ...
    return h("div", { className: "alex-kanban-section" },
      ...
      h("div", { className: "alex-kanban-home-subs" },
        channels.map(function (hc) {
          ...
          return h(Button, {
            ...
            className: hc.subscribed
              ? "alex-kanban-home-sub alex-kanban-home-sub--on"
              : "alex-kanban-home-sub",
          }, label);
        })
      )
    );
  ...
  if (window.__ALEX_PLUGINS__ && typeof window.__ALEX_PLUGINS__.register === "function") {
    window.__ALEX_PLUGINS__.register("kanban", KanbanPage);
  }
})();
