# Alex Achievements Performance Spec (Post-Hackathon)

Status: Draft (no code changes yet)
Owner: alex-achievements plugin
...
### `GET /api/plugins/alex-achievements/achievements`
...
### `POST /api/plugins/alex-achievements/rescan` (optional)
Manual refresh trigger.
Prefer async trigger + immediate status response.

### `GET /api/plugins/alex-achievements/scan-status` (optional new)
Reports scan state for UX/ops.

### Removed
- `GET /api/plugins/alex-achievements/overview`
...
- Persist plugin data under:
  - `~/.alex/plugins/alex-achievements/scan_snapshot.json`

### Phase 2
- Incremental scan checkpoints with per-session fingerprints.
- Persist checkpoint data under:
  - `~/.alex/plugins/alex-achievements/scan_checkpoint.json`
...
- `~/.alex/plugins/alex-achievements/`
