...
Optional Alex observability plugin that maps Alex observer hooks to
...
not replace Alex Agent's planner, tools, memory, model provider routing, or
CLI UX. Instead, this plugin lets Alex emit NeMo Relay lifecycle events for
the work Alex already owns: sessions, turns, provider/API calls, tool calls,
approval prompts, and delegated subagents.

With this plugin enabled, Alex Agent can:

- Preserve Alex execution as NeMo Relay scopes, LLM spans, tool spans, and
  mark events.
...
```bash
alex plugins enable observability/nemo_relay
```

The `ALEX_NEMO_RELAY_*` environment variables below only configure an
already-enabled plugin. They do not enable plugin discovery by themselves.

For isolated test homes, enable the plugin in the same `ALEX_HOME` that the
agent run will use:

```bash
env ALEX_HOME=/tmp/alex-nemo-relay-test \
  alex plugins enable observability/nemo_relay
```

Runs started with `--ignore_user_config` skip the enabled-plugin state from
`ALEX_HOME`, so local E2E tests should omit that flag unless the test harness
loads `observability/nemo_relay` explicitly another way.

`ALEX_HOME` is the Alex profile/config home used by both
`alex plugins enable ...` and the later `alex chat ...` run. If unset,
Alex uses the user's default home, usually `~/.alex`. For isolated smoke
tests, choose any writable temporary directory and use the same value for every
command in that test:

```bash
export ALEX_HOME=/tmp/alex-nemo-relay-test
alex plugins enable observability/nemo_relay
alex chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```

For source checkouts, make sure the `alex` command you run is built from the
...
uv run alex plugins enable observability/nemo_relay
uv run alex chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```
...
```bash
uv build --wheel
python -m pip install --force-reinstall dist/alex_agent-*.whl
...
The plugin can configure exporters directly from `ALEX_NEMO_RELAY_*`
...
```bash
export ALEX_NEMO_RELAY_ATOF_ENABLED=1
export ALEX_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=.nemo-relay/atof
export ALEX_NEMO_RELAY_ATIF_ENABLED=1
export ALEX_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=.nemo-relay/atif
```

Optional overrides:

- `ALEX_NEMO_RELAY_ATOF_FILENAME`
- `ALEX_NEMO_RELAY_ATOF_MODE` (`append` or `overwrite`)
- `ALEX_NEMO_RELAY_ATIF_FILENAME_TEMPLATE`
- `ALEX_NEMO_RELAY_ATIF_AGENT_NAME`
- `ALEX_NEMO_RELAY_ATIF_AGENT_VERSION`
- `ALEX_NEMO_RELAY_ATIF_MODEL_NAME`
- `ALEX_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE` (`embedded` by default; set `all` to also write standalone child files)

### NeMo Relay Component Config

To initialize NeMo Relay from a component config, create a `plugins.toml` file
and point Alex at it:

```bash
export ALEX_NEMO_RELAY_PLUGINS_TOML=.nemo-relay/plugins.toml
```
...
When `ALEX_NEMO_RELAY_PLUGINS_TOML` is set and initializes successfully, NeMo
Relay owns exporter lifecycle through that config. The direct
`ALEX_NEMO_RELAY_ATOF_*` fallback setup is skipped. If the same
`plugins.toml` observability config enables `atif`, the direct
`ALEX_NEMO_RELAY_ATIF_*` fallback setup is also skipped so Alex does not
double-export trajectories on teardown. If `plugins.toml` initialization fails,
Alex keeps the direct env-var fallbacks active for that run.
...
exposes `llm.execute(...)` / `tools.execute(...)`, Alex routes LLM and tool
...
For the full generic Alex middleware contract, see
...
export ALEX_HOME=/tmp/alex-nemo-relay-docs/alex-home
mkdir -p "$ALEX_HOME"

cat > "$ALEX_HOME/config.yaml" <<'YAML'
model:
  provider: custom
  default: qwen3.6:35b
...
This run starts a parent Alex session, delegates to a child subagent, has the
child call `terminal`, and writes both ATOF and ATIF.

```bash
export ALEX_NEMO_RELAY_ATOF_ENABLED=1
export ALEX_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/alex-nemo-relay-docs/subagent/atof
export ALEX_NEMO_RELAY_ATOF_FILENAME=nested-subagent-atof.jsonl
export ALEX_NEMO_RELAY_ATOF_MODE=overwrite
export ALEX_NEMO_RELAY_ATIF_ENABLED=1
export ALEX_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/alex-nemo-relay-docs/subagent/atif
export ALEX_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='nested-subagent-atif-{session_id}.json'
export ALEX_NEMO_RELAY_ATIF_AGENT_NAME='Alex Agent E2E'
export ALEX_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
export ALEX_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE=all
...
{"kind":"mark","name":"alex.subagent.start","metadata":{"parent_session_id":"docs-parent-session","session_id":"docs-child-session","subagent_id":"sa-0-docs","child_role":"leaf"}}
...
```json
{
  ...
  "session_id": "docs-parent-session",
  "agent": {"name": "Alex Agent E2E", "version": "docs-example", "model_name": "qwen3.6:35b"},
  "steps": [
    {
      "source": "agent",
...
message. Alex dispatches the read-only tools as one batch, and NeMo Relay
records both tool invocations.

```bash
mkdir -p /tmp/alex-nemo-relay-docs/workdir
printf 'docs_parallel_alpha_function\n' > /tmp/alex-nemo-relay-docs/workdir/alpha.txt
printf 'docs_parallel_beta_function\n' > /tmp/alex-nemo-relay-docs/workdir/beta.txt
cd /tmp/alex-nemo-relay-docs/workdir

export ALEX_NEMO_RELAY_ATOF_ENABLED=1
export ALEX_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/alex-nemo-relay-docs/parallel/atof
export ALEX_NEMO_RELAY_ATOF_FILENAME=parallel-tools-atof.jsonl
export ALEX_NEMO_RELAY_ATOF_MODE=overwrite
export ALEX_NEMO_RELAY_ATIF_ENABLED=1
export ALEX_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/alex-nemo-relay-docs/parallel/atif
export ALEX_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='parallel-tools-atif-{session_id}.json'
export ALEX_NEMO_RELAY_ATIF_AGENT_NAME='Alex Agent E2E'
export ALEX_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
...
- Alex sessions map to `agent` scopes.
- Alex API request hooks map to `llm` scope start/end events.
- Alex tool hooks map to `tool` scope start/end events.
...
The `observability/nemo_relay` plugin uses Alex execution middleware to hand
...
```bash
export ALEX_NEMO_RELAY_PLUGINS_TOML=/tmp/alex-middleware-test/plugins.toml
```

When the adaptive component is enabled and the installed NeMo Relay runtime
exposes `llm.execute(...)` and `tools.execute(...)`, Alex routes execution
...
Alex provider call
  -> llm_execution middleware
    -> nemo_relay.llm.execute(...)
      -> Alex provider adapter next_call(...)

Alex tool call
  -> tool_execution middleware
    -> nemo_relay.tools.execute(...)
      -> Alex tool dispatcher next_call(...)
```
...
middleware for a local Alex run. This path requires a NeMo Relay runtime that
...
```bash
export ALEX_HOME=/tmp/alex-middleware-test/alex-home
mkdir -p "$ALEX_HOME" /tmp/alex-middleware-test/nemo-relay
...
cat > /tmp/alex-middleware-test/nemo-relay/plugins.toml <<'TOML'
version = 1
...
[components.config.atof]
enabled = true
output_directory = "/tmp/alex-middleware-test/atof"
filename = "middleware-events.jsonl"
mode = "overwrite"

[components.config.atif]
enabled = true
output_directory = "/tmp/alex-middleware-test/atif"
filename_template = "middleware-trajectory-{session_id}.json"
agent_name = "Alex Middleware E2E"
agent_version = "local"
...
export ALEX_NEMO_RELAY_PLUGINS_TOML=/tmp/alex-middleware-test/nemo-relay/plugins.toml
