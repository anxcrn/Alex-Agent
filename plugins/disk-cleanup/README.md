...
Auto-tracks and cleans up ephemeral files created during Alex Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$ALEX_HOME` and `/tmp/alex-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `ALEX_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `ALEX_HOME` or `/tmp/alex-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$ALEX_HOME/disk-cleanup/` is itself excluded
- `$ALEX_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
