"""disk_cleanup — ephemeral file cleanup for Alex Agent.
...
Rules:
  ...
  - empty dirs    → always delete (under ALEX_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly ALEX_HOME and /tmp/alex-*
Never touches: ~/.alex/logs/ or any system directory.
"""
...
try:
    ...
    def get_alex_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("ALEX_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".alex").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$ALEX_HOME/logs/``."""
    return get_alex_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$ALEX_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under ALEX_HOME or ``/tmp/alex-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    alex_home = get_alex_home()
    try:
        path.resolve().relative_to(alex_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/alex-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("alex-"):
        return True
    return False
...
_EMPTY_DIR_PROTECTED_TOP_LEVEL = frozenset({
    ...
    "alex-agent", "backups", "profiles", ".worktrees",
})
...
# Paths under $ALEX_HOME that must NEVER be deleted by quick(),
...
def _is_protected_cron_path(p: Path) -> bool:
    ...
    """
    # Lazily build the set once per process so ALEX_HOME is resolved
    # exactly once.
    if not _PROTECTED_CRON_PATHS:
        alex_home = get_alex_home()
        for parent in ("cron", "cronjobs"):
            base = alex_home / parent
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside ALEX_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under ALEX_HOME, but never recurse into known
    # durable state trees.  Some installs place the Alex checkout, venv,
    # and desktop build under ALEX_HOME; a full rglob over that tree can
    # stall the gateway event loop for minutes.
    alex_home = get_alex_home()
    empty_removed = 0
    sweep_stack: List[Tuple[Path, bool]] = []
    try:
        for top in alex_home.iterdir():
            if (
                top.is_dir()
...
def guess_category(path: Path) -> Optional[str]:
    ...
    alex_home = get_alex_home()
    try:
        rel = path.resolve().relative_to(alex_home)
        top = rel.parts[0] if rel.parts else ""
        if top in {
            "disk-cleanup", "logs", "memories", "sessions", "config.yaml",
            "skills", "plugins", ".env", "USER.md", "MEMORY.md", "SOUL.md",
            "auth.json", "alex-agent",
        }:
            return None
        ...
        # Path isn't under ALEX_HOME (e.g. /tmp/alex-*) — fall through.
        pass
