"""SimpleX Chat platform adapter (Alex plugin).
...
This adapter ships as a Alex platform plugin under
``plugins/platforms/simplex/``. The Alex plugin loader scans the
...
discoverable and ``alex setup`` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside Alex — no
...
class SimplexAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig, **kwargs):
        ...
        self._text_batch_delay = float(
            os.getenv("ALEX_SIMPLEX_TEXT_BATCH_DELAY", "0.8")
        )
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``alex setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / groups /
    auto-accept / home channel. Writes to ``~/.alex/.env`` via
    ``alex_cli.config``.
    """
    print()
    ...
    try:
        ...
        print(
            "alex_cli.config not available; set SIMPLEX_* vars manually in "
            "~/.alex/.env"
        )
        return
