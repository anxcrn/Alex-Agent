...
Credential storage mirrors every other Alex channel:

    * runtime SDK creds  -> ``~/.alex/.env``  (``PHOTON_PROJECT_ID`` =
      project id, ``PHOTON_PROJECT_SECRET``) via ``save_env_value``
    * management metadata -> ``~/.alex/auth.json`` under
...
try:
    import httpx
except ImportError:  # pragma: no cover - httpx is a alex dependency
...
# registers Alex as its own client_id.
...
# Default name of the project Alex provisions for the operator.
DEFAULT_PROJECT_NAME = "Alex Agent"
...
# ---------------------------------------------------------------------------
# auth.json helpers — share the file with the rest of alex-agent.

def _auth_json_path() -> Path:
    """Resolve ``~/.alex/auth.json`` honouring the active Alex profile."""
    try:
        from alex_constants import get_alex_home
        return Path(get_alex_home()) / "auth.json"
    except Exception:
        return Path(os.path.expanduser("~/.alex")) / "auth.json"
...
def load_project_credentials() -> Tuple[Optional[str], Optional[str]]:
    ...
    Precedence: process env (``~/.alex/.env`` is loaded into the gateway's
...
def store_project_credentials(
    *,
    ...
    The runtime SDK creds land in ``~/.alex/.env`` via the same
...
def _persist_runtime_env(spectrum_project_id: str, project_secret: str) -> None:
    """Write the SDK creds to ``~/.alex/.env`` (canonical runtime store).
    ...
    try:
        ...
        logger.warning("photon: alex_cli.config unavailable — skipping .env write")
        return
    try:
...
def print_credential_summary(emit: Any = print) -> None:
    ...
    labels["device_token"] = (
        ...
        else "✗ missing (run `alex photon setup`)"
    )
    ...
    labels["phone_number"] = (
        phone if phone else "✗ missing (run `alex photon setup --phone ...`)"
    )
    labels["assigned_phone_number"] = (
        assigned if assigned else "✗ missing (run `alex photon setup`)"
    )
...
def credential_summary() -> Dict[str, str]:
    ...
    def _present_phone() -> str:
        ...
        return phone or "✗ missing (run `alex photon setup --phone ...`)"

    def _present_assigned_phone() -> str:
        _phone, assigned = load_user_numbers()
        return assigned or "✗ missing (run `alex photon setup`)"
