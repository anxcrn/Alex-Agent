...
Small Node helper that bridges Alex Agent to Photon's Spectrum SDK
(`spectrum-ts`).  Alex is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-Alex-Sidecar-Token`)
...
The Alex plugin's `alex photon setup` command runs `npm install`
here automatically.
