// Alex Agent — Photon Spectrum sidecar
//
...
//
// Inbound  (gRPC -> Alex): the SDK's `app.messages` async iterator is a
...
//   gateway connects.
// Outbound (Alex -> gRPC): `/send` drives `space.send(...)`; `/typing`
//   sends the documented `typing("start" | "stop")` content builder.
//
// Protocol (all requests require `X-Alex-Sidecar-Token: ${TOKEN}`):
...
//                          default off — toggle with `alex photon telemetry`)
  ...
  streamRestartTimer = setTimeout(() => {
    ...
    console.error(
      `photon-sidecar: upstream stream degraded for ${degradedForMs}ms; ` +
        "exiting so Alex can restart the Photon adapter"
    );
    process.exit(75);
...
// instead of a cryptic module-resolution error during import. Apply Alex'
...
function inboundStreamErrorMessage(e) {
  ...
  // iMessage internal error. Local Alex allowlists cannot cause or fix this:
  ...
  if (
    ...
    out +=
      ...
      "this is upstream of Alex, so inbound iMessages may not be delivered " +
...
const server = http.createServer(async (req, res) => {
  if (!tokenOk(req.headers["x-alex-sidecar-token"])) {
  ...
  try {
    ...
    if (req.url === "/send-attachment") {
      ...
      // overrides only when Alex supplied them so a known-good
