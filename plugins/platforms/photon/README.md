...
This plugin connects Alex Agent to iMessage (and other Spectrum
...
TypeScript-only, Alex runs it inside a small supervised Node sidecar and
...
- **Outbound**: `send` / `send_typing` / reaction tapbacks are loopback POSTs
  ...
  `/unreact`), authenticated with a shared `X-Alex-Sidecar-Token`.
...
There is no separate `login` command; like every other Alex channel,
...
Run `alex photon status` to see what's configured.

## Credentials

Runtime SDK credentials live in `~/.alex/.env` (the same place every other
...
Management metadata lives in `~/.alex/auth.json` under `credential_pool`:
...
| `PHOTON_TELEMETRY`        | false                      | Spectrum SDK telemetry — toggle with `alex photon telemetry on\|off` (restart the gateway to apply) |
...
6. Verify end-to-end: `alex photon status`, a DM and a group roundtrip,
