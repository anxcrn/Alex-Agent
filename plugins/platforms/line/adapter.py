"""
LINE Messaging API platform adapter for Alex Agent.
...
to Alex Agent. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``ALEX_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            alex_home = Path(get_alex_home()).resolve()
        except Exception:
            alex_home = Path.home().joinpath(".alex").resolve()
...
def is_connected(config) -> bool:
    """Surface in ``alex status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``alex status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``alex setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.alex/.env`` via ``alex_cli.config``.
    """
    print()
    ...
    try:
        from alex_cli.config import get_env_var, set_env_var
    except ImportError:
        print("alex_cli.config not available; set LINE_* vars manually in ~/.alex/.env")
        return
