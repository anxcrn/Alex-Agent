"""
Email platform adapter for the Alex gateway.

Allows users to interact with Alex by sending emails.
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        ...
        imap.xatom(
            "ID",
            f'("name" "alex-agent" "version" "{_alex_version}" '
            '"vendor" "charan vankudoth" '
            '"support-email" "noreply@charan vankudoth.com")',
        )
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # send-helper, and `alex config show` already read. Without the
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if missing:
            message = (
                ...
                + ", ".join(missing)
                + ". Set it via `alex gateway setup` (env) or platforms.email "
                "in config.yaml."
            )
    ...
    def _send_email(
        ...
        subject = ctx.get("subject", "Alex Agent")
        ...
        msg_id = f"<alex-{uuid.uuid4().hex[:12]}@{self._address.split('@')[1]}>"
...
# alex_cli/gateway.py, and the _send_email dispatch in
