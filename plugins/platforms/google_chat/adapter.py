...
# ``model_tools`` import time, so ``alex status``, ``alex chat``, etc.
...
# ``alex_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "Alex is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a alex MessageType.
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${ALEX_HOME}/google_chat_thread_counts.json so
        ...
        try:
            ...
            _alex_home = _get_alex_home()
        except (ModuleNotFoundError, ImportError):
            _alex_home = _Path.home() / ".alex"
        self._thread_count_store = _ThreadCountStore(
            _alex_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "Alex is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("ALEX_HOME", str(_Path.home() / ".alex"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${ALEX_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a alex MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Alex is thinking…' marker message.
        ...
        "Alex is thinking…" stuck in chat forever, plus one card that
        ...
        body: Dict[str, Any] = {"text": "Alex is thinking…"}
        if thread_id:
            body["thread"] = {"name": thread_id}
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Alex is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Alex is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Alex is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``alex setup``.

    The setup wizard at ``alex_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.alex/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. alex-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.alex/.env")
    print_info("Restart the gateway: alex gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``alex cron`` running as a
    separate process from ``alex gateway``).  Without this hook,
...
def register(ctx) -> None:
    """Plugin entry point — called by the Alex plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'alex-agent[google_chat]'",
        ...
        # the "Alex is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Alex is thinking…' marker message "
