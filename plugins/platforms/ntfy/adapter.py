"""ntfy platform adapter (Alex plugin).
...
``plugins/platforms/ntfy/``. The Alex plugin loader scans the
    ...
    platforms:
      ntfy:
        ...
        extra:
          ...
          topic: "alex-in"              # subscribe topic (incoming)
          publish_topic: "alex-out"     # optional — defaults to topic
...
_ECHO_TAG = "alex-agent"  # tag added to outgoing messages for echo-loop prevention
...
async def _standalone_send(
    ...
    *,
    ...
    ``alex cron`` running standalone). Without this hook,
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install httpx   # already a Alex dependency",
        # Env-driven auto-configuration: seeds PlatformConfig.extra so
        # env-only setups show up in `alex gateway status` without
