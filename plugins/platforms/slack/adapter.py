...
class SlackAdapter(BasePlatformAdapter):
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        tokens_file = get_alex_home() / "slack_tokens.json"
        ...
        try:
            ...
            # Alex does not currently need to route them into the agent loop.
            ...
            # /model work directly without /alex prefix). A single regex
            ...
            # manifest (see `alex slack manifest`). In Socket Mode, Slack
            ...
            from alex_cli.commands import slack_native_slashes
            import re as _re
            ...
            if _slash_names:
                ...
                _slash_pattern = _re.compile(r"^/alex$")

            @self._app.command(_slash_pattern)
            async def handle_alex_command(ack, command):
    ...
    async def create_handoff_thread(
        ...
        try:
            ...
            if client is None:
                return None
            seed_text = (
                f":thread: Alex handoff — *{(name or 'session').strip()[:80]}*"
            )
    ...
    def _dm_top_level_threads_as_sessions(self) -> bool:
        ...
        own Alex session — matching the per-thread behavior channels already
                    ...
                    "elements": [
                        {
                            ...
                            "style": "primary",
                            "action_id": "alex_approve_once",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Allow Session"},
                            "action_id": "alex_approve_session",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Always Allow"},
                            "action_id": "alex_approve_always",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                    ...
                    "elements": [
                        {
                            ...
                            "style": "primary",
                            "action_id": "alex_confirm_once",
                            "value": value,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Always Approve"},
                            "action_id": "alex_confirm_always",
                            "value": value,
                        },
                        {
                            "type": "button",
    ...
    async def _handle_slash_confirm_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "alex_confirm_once": "once",
            "alex_confirm_always": "always",
            "alex_confirm_cancel": "cancel",
        }
    ...
    async def _handle_approval_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "alex_approve_once": "once",
            "alex_approve_session": "session",
            "alex_approve_always": "always",
            "alex_deny": "deny",
        }
    ...
    async def _handle_slash_command(self, command: dict) -> None:
        ...
        The legacy ``/alex <subcommand> [args]`` form is preserved for
        backward compatibility with older workspace manifests and for users
        who want a single entry point for free-form questions (``/alex
        ...
        if slash_name in {"alex", ""}:
            # Legacy /alex <subcommand> [args] routing + free-form questions.
            # Empty slash_name falls into this branch for backward compat
            # with any caller that didn't populate command["command"].
            from alex_cli.commands import slack_subcommand_map

            subcommand_map = slack_subcommand_map()
            subcommand_map["compact"] = "/compress"
            # Guard against whitespace-only text where ``text`` is truthy but
            # ``text.split()`` returns ``[]`` (e.g. user sends ``/alex   ``).
        ...
        # "Running /cmd…" ack shown by handle_alex_command).
        # Only stash for COMMAND events (text starts with "/") — free-form
        # questions via "/alex <question>" must produce public replies so
...
# ``alex_cli/{setup,gateway}.py``, and the ``_send_slack`` dispatch in
...
def interactive_setup() -> None:
    ...
    offers to set a home channel. Replaces ``alex_cli/setup.py::_setup_slack``.
    """
    ...
    def _write_slack_manifest_and_instruct() -> None:
        """Generate the Slack manifest, write it under ALEX_HOME, and print
        paste-into-Slack instructions. Failures are non-fatal."""
        try:
            from alex_cli.slack_cli import _build_full_manifest
            from alex_constants import get_alex_home
            import json as _json

            manifest = _build_full_manifest(
                bot_name="Alex",
                bot_description="Your Alex agent on Slack",
            )
            target = Path(get_alex_home()) / "slack-manifest.json"
            ...
            print_info(
                "   Re-run `alex slack manifest --write` anytime to refresh after "
                "Alex adds new commands."
            )
        except Exception as e:
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                "list? (recommended after `alex update`)",
                True,
            ):
    ...
    print()
    print_info("   Full guide: https://alex-agent.charan vankudoth.com/docs/user-guide/messaging/slack/")
    print()
...
def _is_connected(config) -> bool:
    ...
    Looks up via ``alex_cli.gateway.get_env_value`` at call time (not via the
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'alex-agent[slack]'",
        # Interactive setup wizard — replaces alex_cli/setup.py::_setup_slack
        # and the static _PLATFORMS["slack"] dict in alex_cli/gateway.py.
