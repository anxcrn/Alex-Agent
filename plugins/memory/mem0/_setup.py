...
def _save_mem0_json(alex_home: str, data: dict) -> None:
    """Merge-write to mem0.json."""
    config_path = Path(alex_home) / "mem0.json"
    existing = {}
...
def _setup_platform(alex_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    schema = [
        ...
        {"key": "user_id", "description": "User identifier", "default": "alex-user"},
        {"key": "agent_id", "description": "Agent identifier", "default": "alex"},
    ...
    provider.save_config(provider_config, alex_home)

    if env_writes:
        _write_env(Path(alex_home) / ".env", env_writes)
...
def _setup_oss(alex_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    if not flags.get("_mode_from_flag"):
        _setup_oss_interactive(alex_home, config)
        return
    ...
    user_id = flags.get("user_id") or os.getenv("USER", "alex-user")
    ...
    if env_writes:
        _write_env(Path(alex_home) / ".env", env_writes)
    _save_mem0_json(alex_home, {"mode": "oss", "user_id": user_id, "agent_id": "alex", "oss": oss_config})
...
def _prompt_api_key(label: str, env_var: str, alex_home: str) -> str:
...
_PGVECTOR_CONTAINER = "alex-pgvector"
_PGVECTOR_IMAGE = "pgvector/pgvector:pg17"
_PGVECTOR_PASSWORD = "alex"
...
def _setup_oss_interactive(alex_home: str, config: dict) -> None:
    ...
    if llm_def["needs_key"]:
        key = _prompt_api_key(llm_def["label"], llm_def["env_var"], alex_home)
        if key:
    ...
    if embedder_def["needs_key"] and embedder_id != llm_id:
        key = _prompt_api_key(f"{embedder_def['label']} embedder", embedder_def["env_var"], alex_home)
        if key:
    ...
    user_id = input(f"  User ID [{os.getenv('USER', 'alex-user')}]: ").strip()
    user_id = user_id or os.getenv("USER", "alex-user")

    agent_id = input("  Agent ID [alex]: ").strip()
    ...
    if env_writes:
        _write_env(Path(alex_home) / ".env", env_writes)
    _save_mem0_json(alex_home, {"mode": "oss", "user_id": user_id, "agent_id": agent_id, "oss": oss_config})
...
def post_setup(alex_home: str, config: dict) -> None:
    """Entry point called by alex memory setup framework.
    ...
    if flags["mode"] == "oss":
        ...
        _setup_oss(alex_home, config, flags)
        return

    if flags["mode"] == "platform":
        _setup_platform(alex_home, config, flags)
        return
