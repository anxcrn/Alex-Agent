...
Behavioral settings live in `$ALEX_HOME/mem0.json` (set them via `alex memory setup`). Only the secret `MEM0_API_KEY` belongs in `~/.alex/.env`.

| Key | Default | Description |
|-----|---------|-------------|
| `mode` | `platform` | `platform` (Mem0 Cloud) or `oss` (self-hosted) |
| `user_id` | `alex-user` | User identifier on Mem0 |
...
```bash
alex memory setup mem0 --mode oss \
...
```bash
alex memory setup mem0 --mode oss --oss-llm-key sk-...
```

Or edit `$ALEX_HOME/mem0.json` directly:
```json
{
  "mode": "oss",
  "oss": {
    ...
    "vector_store": {"provider": "qdrant", "config": {"path": "~/.alex/mem0_qdrant"}}
  }
}
```
...
```bash
alex memory setup mem0 --mode platform --api-key sk-...
```
...
- Check `user_id` matches between sessions (`$ALEX_HOME/mem0.json`).
