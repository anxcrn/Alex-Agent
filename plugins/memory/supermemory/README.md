...
- Supermemory API key from [app.supermemory.ai/integrations?connect=alex](http://app.supermemory.ai/integrations?connect=alex)
...
Config file: `$ALEX_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `alex` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `alex-{identity}` → `alex-coder`). |
...
All Supermemory API calls send `x-sm-source: alex`, and document writes stamp
...
Use `{identity}` in the `container_tag` to scope memories per Alex profile:

```json
{
  "container_tag": "alex-{identity}"
}
```

For a profile named `coder`, this resolves to `alex-coder`. The default profile resolves to `alex-default`. Without `{identity}`, all profiles share the same container.
