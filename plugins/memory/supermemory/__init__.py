...
_API_KEY_URL = "http://app.supermemory.ai/integrations?connect=alex"
...
def _load_supermemory_config(alex_home: str) -> dict:
    config = _default_config()
    config_path = Path(alex_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, alex_home: str) -> None:
    config_path = Path(alex_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class _SupermemoryClient:
    def __init__(self, api_key: str, timeout: float, container_tag: str, search_mode: str = "hybrid"):
        ...
        self._client = Supermemory(
            ...
            timeout=timeout,
            max_retries=0,
            default_headers={"x-sm-source": "alex"},
        )

    def _merge_metadata(self, metadata: Optional[dict]) -> dict:
        # sm_source routes Alex writes into the "Alex" Space in the Supermemory
        ...
        merged = {"sm_source": "alex", **(metadata or {})}
...
def _resolve_container_tag_for_setup(alex_home: str, *, identity: str = "default") -> str:
    config = _load_supermemory_config(alex_home)
...
def _probe_supermemory_connection(api_key: str, alex_home: str, *, identity: str = "default") -> dict:
    config = _load_supermemory_config(alex_home)
    status = {
        "ok": False,
        "error": "",
        "container_tag": _resolve_container_tag_for_setup(alex_home, identity=identity),
        "profile_facts": 0,
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `alex memory setup`.
        # All other options are documented for $ALEX_HOME/supermemory.json
    ...
    def save_config(self, values, alex_home):
        ...
        _save_supermemory_config(sanitized, alex_home)
    ...
    def get_status_config(self, provider_config: dict) -> dict:
        ...
        api_key = os.environ.get("SUPERMEMORY_API_KEY", "")
        status = _probe_supermemory_connection(api_key, alex_home)
    ...
    def post_setup(self, alex_home: str, config: dict) -> None:
        ...
        from alex_cli.memory_setup import _prompt, _write_env_vars
        ...
        if env_writes:
            _write_env_vars(Path(alex_home) / ".env", env_writes)
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        from alex_constants import get_alex_home
        self._alex_home = kwargs.get("alex_home") or str(get_alex_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._alex_home)
