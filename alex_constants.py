"""Shared constants for Alex Agent.
...
_UNSET = object()
_ALEX_HOME_OVERRIDE: ContextVar[str | object] = ContextVar(
    "_ALEX_HOME_OVERRIDE", default=_UNSET
)


def set_alex_home_override(path: str | Path | None) -> Token:
    """Set a context-local Alex home override and return its reset token.

    This is for in-process, per-task scoping.  It deliberately does not mutate
    ``os.environ`` because that is shared by every thread in the process.
    """
    value: str | object = _UNSET if path is None else str(path)
    return _ALEX_HOME_OVERRIDE.set(value)


def reset_alex_home_override(token: Token) -> None:
    """Restore the previous context-local Alex home override."""
    _ALEX_HOME_OVERRIDE.reset(token)


def get_alex_home_override() -> str | None:
    """Return the active context-local Alex home override, if any."""
    override = _ALEX_HOME_OVERRIDE.get()
    if override is _UNSET or not override:
        return None
    return str(override)


def _get_platform_default_alex_home() -> Path:
    """Return the platform-native default Alex home path."""
    if sys.platform == "win32":
        local_appdata = os.environ.get("LOCALAPPDATA", "").strip()
        base = Path(local_appdata) if local_appdata else Path.home() / "AppData" / "Local"
        return base / "alex"
    return Path.home() / ".alex"


def get_alex_home() -> Path:
    """Return the Alex home directory (default: platform-native path).

    Reads ALEX_HOME env var, falls back to the platform-native default.
    This is the single source of truth — all other copies should import this.

    When ``ALEX_HOME`` is unset but an ``active_profile`` file indicates
    ...
    if not _profile_fallback_warned:
        try:
            fallback_home = _get_platform_default_alex_home()
        ...
        if active and active != "default":
            ...
            msg = (
                f"[ALEX_HOME fallback] ALEX_HOME is unset but active "
                f"profile is {active!r}. Falling back to {fallback_home}, which "
                f"is the DEFAULT profile — not {active!r}. Any data this "
                f"process writes will land in the wrong profile. The "
                f"subprocess spawner should pass ALEX_HOME explicitly "
                f"(see issue #18594)."
            )
            try:
    ...
    return _get_platform_default_alex_home()


def get_default_alex_root() -> Path:
    """Return the root Alex directory for profile-level operations.

    In standard deployments this is the platform-native Alex home
    (``~/.alex`` on POSIX, ``%LOCALAPPDATA%\\alex`` on native Windows).

    In Docker or custom deployments where ``ALEX_HOME`` points outside
    ``~/.alex`` (e.g. ``/opt/data``), returns ``ALEX_HOME`` directly
    — that IS the root.

    In profile mode where ``ALEX_HOME`` is ``<root>/profiles/<name>``,
    returns ``<root>`` so that ``profile list`` can see all profiles.
    Works both for standard (``~/.alex/profiles/coder``) and Docker
    (``/opt/data/profiles/coder``) layouts.

    Import-safe — no dependencies beyond stdlib.
    """
    native_home = _get_platform_default_alex_home()
    env_home = os.environ.get("ALEX_HOME", "")
    if not env_home:
        return native_home
    env_path = Path(env_home)
    try:
        env_path.resolve().relative_to(native_home.resolve())
        # ALEX_HOME is under ~/.alex (normal or profile mode)
        return native_home
    except ValueError:
        pass
    ...
    # Not a profile path — ALEX_HOME itself is the root
    return env_path
...
def _get_packaged_data_dir(name: str) -> Path | None:
    ...
    Used to discover bundled skills/optional-skills when Alex is installed
...
def get_optional_skills_dir(default: Path | None = None) -> Path:
    ...
    tree and expose it via ``ALEX_OPTIONAL_SKILLS``.
    """
    override = os.getenv("ALEX_OPTIONAL_SKILLS", "").strip()
    if override:
        return Path(override)
    ...
    if default is not None:
        return default
    return get_alex_home() / "optional-skills"
...
def get_optional_mcps_dir(default: Path | None = None) -> Path:
    ...
    package tree and expose it via ``ALEX_OPTIONAL_MCPS``.
    """
    override = os.getenv("ALEX_OPTIONAL_MCPS", "").strip()
    if override:
        return Path(override)
    ...
    if default is not None:
        return default
    return get_alex_home() / "optional-mcps"
...
def get_bundled_skills_dir(default: Path | None = None) -> Path:
    ...
    Resolution order:
        1. ``ALEX_BUNDLED_SKILLS`` env var (Nix wrapper / explicit override)
        2. Wheel-installed ``<sysconfig data>/skills`` (pip install path)
        3. Caller-supplied ``default`` (typically the source-checkout path)
        4. ``<ALEX_HOME>/skills`` last-resort
    """
    override = os.getenv("ALEX_BUNDLED_SKILLS", "").strip()
    if override:
        return Path(override)
    ...
    if default is not None:
        return default
    return get_alex_home() / "skills"


def get_alex_dir(new_subpath: str, old_name: str) -> Path:
    """Resolve a Alex subdirectory with backward compatibility.
    ...
    Args:
        new_subpath: Preferred path relative to ALEX_HOME (e.g. ``"cache/images"``).
        old_name: Legacy path relative to ALEX_HOME (e.g. ``"image_cache"``).
...
def iter_alex_node_dirs(home: Path | None = None) -> list[Path]:
    """Return Alex-managed Node.js directories in preferred lookup order.

    Windows installs from ``scripts/install.ps1`` unpack portable Node directly
    into ``%LOCALAPPDATA%\\alex\\node``. POSIX installs use
    ``$ALEX_HOME/node/bin``. Include both shapes on every platform so mixed
    or migrated installs still work.
    """
    root = home or get_alex_home()
    dirs = [root / "node"]
    bin_dir = root / "node" / "bin"
    # NOTE: keep this ordering in sync with alexManagedNodePathEntries() in
...
_ALEX_NODE_TARGET_MAJOR = int(os.environ.get("ALEX_NODE_TARGET_MAJOR", "22"))
...
def node_tool_runnable(path: str | None) -> bool:
    ...
    Alex-managed Node trees live under ``$ALEX_HOME/node`` (or a profile's
    ``ALEX_HOME``). A partial upgrade or interrupted install can leave
    ``bin/npm`` behind while ``lib/cli.js`` is missing — the wrapper exists but
    immediately throws ``MODULE_NOT_FOUND``. ``find_alex_node_executable``
    used to trust file presence alone, so ``alex update`` would pick that
...
def alex_managed_node_tree_present(home: Path | None = None) -> bool:
    """Return True when any Alex-managed node/npm/npx shim exists on disk."""
    names = set()
    for command in ("node", "npm", "npx"):
        names.update(_candidate_node_command_names(command))
    for directory in iter_alex_node_dirs(home):
        for name in names:
...
def _heal_managed_node_windows() -> bool:
    """Redownload the portable Node zip into ``%ALEX_HOME%\\node`` on Windows."""
    import re
    import tempfile
    ...
    home = get_alex_home()
    index_url = f"https://nodejs.org/dist/latest-v{_ALEX_NODE_TARGET_MAJOR}.x/"
    try:
    ...
    match = re.search(
        rf"node-v{_ALEX_NODE_TARGET_MAJOR}\.\d+\.\d+-win-{node_arch}\.zip",
        index_html,
    )
    if not match:
        return False
...
def heal_alex_managed_node() -> bool:
    """Redownload Alex-managed Node when the tree exists but is broken.
    ...
    if _managed_node_heal_attempted:
        return False
    if not alex_managed_node_tree_present():
        return False
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "ALEX_HOME": str(get_alex_home())},
            capture_output=True,
            timeout=300,
            check=False,
        )
...
def find_alex_node_executable(command: str) -> str | None:
    """Return a Alex-managed Node/npm executable path, healing broken trees."""
    names = _candidate_node_command_names(command)
    broken_present = False
    for directory in iter_alex_node_dirs():
        for name in names:
            candidate = directory / name
            if candidate.is_file() and (
                sys.platform == "win32" or os.access(candidate, os.X_OK)
                ...
                if node_tool_runnable(resolved):
                    return resolved
                broken_present = True
    if broken_present and heal_alex_managed_node():
        for directory in iter_alex_node_dirs():
            for name in names:
                candidate = directory / name
                if candidate.is_file() and (
...
def find_node_executable_on_path(command: str) -> str | None:
    ...
    directly, so prefer the launchable variants explicitly for Alex-owned
    subprocesses.
    """
    if sys.platform != "win32":
...
def find_node_executable(command: str) -> str | None:
    """Resolve a Node.js command, preferring healthy Alex-managed installs.

    This is for Alex-owned subprocesses that should not be broken by a bad,
    ...
    """
    managed = find_alex_node_executable(command)
    if managed:
        return managed
    if alex_managed_node_tree_present():
        return None
    return find_node_executable_on_path(command)


def with_alex_node_path(env: dict[str, str] | None = None) -> dict[str, str]:
    """Return *env* with Alex-managed Node directories prepended to PATH."""
    merged = dict(os.environ if env is None else env)
    existing = merged.get("PATH", "")
    parts = [p for p in existing.split(os.pathsep) if p]
    managed = [str(path) for path in iter_alex_node_dirs() if path.is_dir()]
...
def agent_browser_runnable(path: str | None) -> bool:
    ...
    next ``alex update`` — leaving a **dangling symlink** that ``which`` still
...
def display_alex_home() -> str:
    """Return a user-friendly display string for the current ALEX_HOME.

    Uses ``~/`` shorthand for readability::

        default:  ``~/.alex``
        profile:  ``~/.alex/profiles/coder``
        custom:   ``/opt/alex-custom``

    Use this in **user-facing** print/log messages instead of hardcoding
    ``~/.alex``.  For code that needs a real ``Path``, use
    :func:`get_alex_home` instead.
    """
    home = get_alex_home()
    try:
...
def _profile_home_path(env: dict[str, str] | None = None) -> str | None:
    """Return ``{ALEX_HOME}/home`` when the profile-home directory exists."""
    alex_home = get_alex_home_override() or (env or {}).get("ALEX_HOME") or os.getenv("ALEX_HOME")
    if not alex_home:
        return None
    profile_home = os.path.join(alex_home, "home")
...
def _iter_real_home_candidates(env: dict[str, str] | None = None) -> list[str]:
    ...
    env = env or {}
    candidates: list[str] = []
    explicit = str(env.get("ALEX_REAL_HOME") or os.getenv("ALEX_REAL_HOME", "")).strip()
    if explicit:
...
def get_real_home(env: dict[str, str] | None = None) -> str:
    """Return the OS user's real home directory, avoiding Alex profile HOME.

    ``ALEX_HOME`` scopes Alex state. ``HOME`` is reserved for the OS/user
    account and the many external CLIs that store credentials under ``~``.
    If a parent process is already running with ``HOME={ALEX_HOME}/home``,
...
def get_subprocess_home(env: dict[str, str] | None = None) -> str | None:
    ...
    * ``auto`` (default): host installs keep the real user HOME; containers use
      ``{ALEX_HOME}/home`` for persistent state. If a host parent already has
      HOME pointed at the profile home, repair subprocesses back to real HOME.
    * ``real``: always prefer the real OS-user HOME.
    * ``profile``: use ``{ALEX_HOME}/home`` when it exists, preserving the
...
def apply_subprocess_home_env(env: dict[str, str]) -> None:
    """Apply Alex' subprocess HOME contract to *env* in-place."""
    real_home = get_real_home(env)
    if real_home:
        env["ALEX_REAL_HOME"] = real_home
...
def get_config_path() -> Path:
    """Return the path to ``config.yaml`` under ALEX_HOME.

    Replaces the ``get_alex_home() / "config.yaml"`` pattern repeated
    in 7+ files (skill_utils.py, alex_logging.py, alex_time.py, etc.).
    """
    return get_alex_home() / "config.yaml"


def get_skills_dir() -> Path:
    """Return the path to the skills directory under ALEX_HOME."""
    return get_alex_home() / "skills"



def get_env_path() -> Path:
    """Return the path to the ``.env`` file under ALEX_HOME."""
...
def apply_ipv4_preference(force: bool = False) -> None:
    ...
    if getattr(socket.getaddrinfo, "_alex_ipv4_patched", False):
        return
    ...
    _ipv4_getaddrinfo._alex_ipv4_patched = True  # type: ignore[attr-defined]
...
def get_nexus_home() -> Path:
    """Return the base directory for the Nexus self-evolution engine."""
