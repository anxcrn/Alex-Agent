...
// (e.g. https://mission-control.tilos.com/alex/). The Python backend
// injects ``window.__ALEX_BASE_PATH__`` into index.html based on the
...
const SESSION_HEADER = "X-Alex-Session-Token";
...
async function getSessionToken(): Promise<string> {
  ...
  const injected = window.__ALEX_SESSION_TOKEN__;
  if (injected) {
    _sessionToken = injected;
    return _sessionToken;
  }
  throw new Error("Session token not available — page must be served by the Alex dashboard server");
}

/**
 * Fetch a single-use ticket for a WebSocket upgrade in gated mode.
 *
 * The dashboard's gated-mode WS auth (``alex_cli.web_server._ws_auth_ok``)
...
export async function buildWsAuthParam(): Promise<[string, string]> {
  if (window.__ALEX_AUTH_REQUIRED__) {
 ...
 *  - loopback / ``--insecure``: attach the ``X-Alex-Session-Token`` header.
 *  - gated OAuth: no token header (it's absent by design); the
 *    ``alex_session_at`` cookie rides along via ``credentials: 'include'``.
 *
 ...
 * ``window.__ALEX_SESSION_TOKEN__`` directly, so the gated-mode ticket
...
export const api = {
  ...
  updateAlex: () =>
    fetchJSON<ActionResponse>("/api/alex/update", { method: "POST" }),
  checkAlexUpdate: (force = false) =>
    fetchJSON<UpdateCheckResponse>(
      `/api/alex/update/check${force ? "?force=true" : ""}`,
    ),
...
export interface SkillHubSource {
  ...
  /** alex-index only: whether the centralized index loaded. */
  available?: boolean;
}
...
export interface StatusResponse {
   ...
   * updates are handled by the outer launcher instead of ``alex update``. */
  can_update_alex?: boolean;
  config_path: string;
