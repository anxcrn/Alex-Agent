...
export default function ChatPage({ isActive = true }: { isActive?: boolean }) {
  ...
  const [banner, setBanner] = useState<string | null>(() =>
    ...
    !window.__ALEX_SESSION_TOKEN__ &&
    !window.__ALEX_AUTH_REQUIRED__
      ? "Session token unavailable. Open this page through `alex dashboard`, not directly."
      : null,
  );
  ...
  useEffect(() => {
    ...
    const term = new Terminal({
      ...
      // even when the inner Alex TUI has enabled xterm mouse-events
    ...
    if (useWebgl) {
      try {
        ...
        console.warn(
          "[alex-chat] WebGL renderer unavailable; falling back to default",
          err,
        );
      }
    }
    ...
    // WebSocket. In gated mode (``window.__ALEX_AUTH_REQUIRED__``) this
    ...
    // forwarding them into Alex. Keyboard input, paste, and resize still
    // behave normally.
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-2 lg:flex-row lg:gap-3">
        <div className="flex min-h-0 min-w-0 flex-1 flex-col lg:flex-row lg:gap-3">
          ...
          >
            <div
              ref={hostRef}
              className="alex-chat-xterm-host min-h-0 min-w-0 flex-1"
            />
