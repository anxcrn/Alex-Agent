# nix/nixosModules.nix — NixOS module for alex-agent
#
# Two modes:
#   container.enable = false (default) → native systemd service
#   container.enable = true            → OCI container (persistent writable layer)
#
# Container mode: alex runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $ALEX_HOME/.env
# and read by alex at startup — no container recreation needed for env changes.
#
# Tool resolution: the alex wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }: {
  ...
  let
    cfg = config.services.alex-agent;
    effectivePackage =
    ...
    alex-agent = inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.default;

    # Deep-merge config type (from 0xrsydn/nix-alex-agent)
    deepConfigType = lib.types.mkOptionType {
      name = "alex-config-attrs";
      description = "Alex YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    generatedConfigFile = pkgs.writeText "alex-config.yaml" configJson;
    configFile = if cfg.configFile != null then cfg.configFile else generatedConfigFile;

    configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

    # config.yaml mode: group-writable (0660) when interactive users share this
    # ALEX_HOME via addToSystemPackages, so they can save settings through the
    ...
    documentDerivation = pkgs.runCommand "alex-documents" { } (
      ''
        mkdir -p $out
      '' + lib.concatStringsSep "\n" (
        lib.mapAttrsToList (name: value:
          ...
          else "cat > $out/${name} <<'ALEX_DOC_EOF'\n${value}\nALEX_DOC_EOF"
        ) cfg.documents
      )
    );

    containerName = "alex-agent";
    containerDataDir = "/data";     # stateDir mount point inside container
    containerHomeDir = "/home/alex";
    ...
    # alex user + sudo on first boot (writable layer persists), then
    # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
    containerEntrypoint = pkgs.writeShellScript "alex-container-entrypoint" ''
      set -eu

      ALEX_UID="''${ALEX_UID:?ALEX_UID must be set}"
      ALEX_GID="''${ALEX_GID:?ALEX_GID must be set}"

      # ── Group: ensure a group with GID=$ALEX_GID exists ──
      # Check by GID (not name) to avoid collisions with pre-existing groups
      # (e.g. GID 100 = "users" on Ubuntu)
      EXISTING_GROUP=$(getent group "$ALEX_GID" 2>/dev/null | cut -d: -f1 || true)
      if [ -n "$EXISTING_GROUP" ]; then
        GROUP_NAME="$EXISTING_GROUP"
      else
        GROUP_NAME="alex"
        if command -v groupadd >/dev/null 2>&1; then
          groupadd -g "$ALEX_GID" "$GROUP_NAME"
        elif command -v addgroup >/dev/null 2>&1; then
          addgroup -g "$ALEX_GID" "$GROUP_NAME" 2>/dev/null || true
        fi
      fi

      # ── User: ensure a user with UID=$ALEX_UID exists ──
      PASSWD_ENTRY=$(getent passwd "$ALEX_UID" 2>/dev/null || true)
      if [ -n "$PASSWD_ENTRY" ]; then
        TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
        TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
      else
        TARGET_USER="alex"
        TARGET_HOME="/home/alex"
        if command -v useradd >/dev/null 2>&1; then
          useradd -u "$ALEX_UID" -g "$ALEX_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
        elif command -v adduser >/dev/null 2>&1; then
          adduser -u "$ALEX_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
        fi
      fi
      mkdir -p "$TARGET_HOME"
      chown "$ALEX_UID:$ALEX_GID" "$TARGET_HOME"
      chmod 0750 "$TARGET_HOME"

      # Ensure ALEX_HOME is owned by the target user.
      ...
      if [ -n "''${ALEX_HOME:-}" ] && [ -d "$ALEX_HOME" ]; then
        find "$ALEX_HOME" \! -user "$ALEX_UID" -exec chown "$ALEX_UID:$ALEX_GID" {} +
      fi
      ...
      if [ ! -f /var/lib/alex-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
        ...
        touch /var/lib/alex-tools-provisioned
      fi

      if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/alex ]; then
        mkdir -p /etc/sudoers.d
        echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/alex
      ...
      if command -v setpriv >/dev/null 2>&1; then
        exec setpriv --reuid="$ALEX_UID" --regid="$ALEX_GID" --init-groups "$@"
    ...
    # so they can update without recreation. Env vars go through $ALEX_HOME/.env.
    ...
    # Default: /var/lib/alex/workspace → /data/workspace.
  ...
  in {
    options.services.alex-agent = with lib; {
      enable = mkEnableOption "Alex Agent gateway service";

      # ── Package ──────────────────────────────────────────────────────────
      package = mkOption {
        type = types.package;
        default = alex-agent;
        description = "The alex-agent package to use.";
      };
      ...
      stateDir = mkOption {
        ...
        description = "State directory. Contains .alex/ subdir (ALEX_HOME).";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          Declarative Alex config (attrset). Deep-merged across module
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          Contents are merged into $ALEX_HOME/.env at activation time.
          Alex reads this file on every startup via load_alex_dotenv().
        '';
      };
      ...
      environment = mkOption {
        ...
        default = { };
        description = ''
          Non-secret environment variables. Merged into $ALEX_HOME/.env
          ...
          options = {
            ...
            auth = mkOption {
              ...
              description = ''
                ...
                (remote MCP servers). Tokens are stored in $ALEX_HOME/mcp-tokens/.
              '';
            };
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `alex gateway`.";
      };
      ...
      extraPackages = mkOption {
        ...
        description = ''
          ...
          Implemented via the alex user's per-user profile
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the alex plugins
          directory. Each package should contain a plugin.yaml and __init__.py
          at its root. Alex discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          alex_agent.plugins entry-point group. Each package must be built
          with the same Python interpreter as alex (python312).
        '';
        example = literalExpression ''
          [
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in alex-agent's
      ...
      addToSystemPackages = mkOption {
        ...
        default = false;
        description = ''
          Add the alex CLI to environment.systemPackages and export
          ALEX_HOME system-wide (via environment.variables) so interactive
      ...
      container = {
        ...
        hostUsers = mkOption {
          ...
          default = [ ];
          description = ''
            Interactive users who get a ~/.alex symlink to the service
            stateDir. These users are automatically added to the alex group.
          '';
          example = [ "sidbin" ];
        };
      };
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      (lib.mkIf (cfg.mcpServers != { }) {
        services.alex-agent.settings.mcp_servers = lib.mapAttrs (_name: srv:
          # Stdio transport
      ...
      # ── Host CLI ──────────────────────────────────────────────────────
      # Add the alex CLI to system PATH and export ALEX_HOME system-wide
      # so interactive shells share state (sessions, skills, cron) with the
      # gateway service instead of creating a separate ~/.alex/.
      (lib.mkIf cfg.addToSystemPackages {
        environment.systemPackages = [ effectivePackage ];
        environment.variables.ALEX_HOME = "${cfg.stateDir}/.alex";
      })
      ...
      # ── Assertions ─────────────────────────────────────────────────────
      {
        ...
        in [{
          ...
          message = "services.alex-agent.extraPlugins: duplicate plugin names detected: ${toString names}. If using fetchFromGitHub, set name = \"plugin-name\" to disambiguate.";
        }];
      }
      ...
      # Wire extraPackages into the alex user's per-user profile so the
      ...
      (lib.mkIf (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != []) {
        warnings = [
          ''
            services.alex-agent: container.enable is true and container.hostUsers
            is set, but addToSystemPackages is false. Without a host-installed alex
            binary, container routing will not work for interactive users.
            Set addToSystemPackages = true or ensure alex is on PATH.
          ''
        ];
      })
      ...
      # ── Directories ───────────────────────────────────────────────────
      {
        systemd.tmpfiles.rules = [
          ...
          "d ${cfg.stateDir}/.alex        2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.alex/cron   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.alex/sessions 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.alex/logs   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.alex/memories 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.alex/plugins 2770 ${cfg.user} ${cfg.group} - -"
      ...
      {
        system.activationScripts."alex-agent-setup" = lib.stringAfter ([ "users" ] ++ lib.optional (config.system.activationScripts ? setupSecrets) "setupSecrets") ''
          # Ensure directories exist (activation runs before tmpfiles)
          mkdir -p ${cfg.stateDir}/.alex
          mkdir -p ${cfg.stateDir}/home
          mkdir -p ${cfg.workingDirectory}
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${cfg.stateDir}/.alex ${cfg.stateDir}/home ${cfg.workingDirectory}
          chmod 2770 ${cfg.stateDir} ${cfg.stateDir}/.alex ${cfg.workingDirectory}
          ...
          find ${cfg.stateDir}/.alex -maxdepth 1 \
            \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
            -exec chmod g+rw {} + 2>/dev/null || true
          for _subdir in cron sessions logs memories plugins; do
            mkdir -p "${cfg.stateDir}/.alex/$_subdir"
            chown ${cfg.user}:${cfg.group} "${cfg.stateDir}/.alex/$_subdir"
            chmod 2770 "${cfg.stateDir}/.alex/$_subdir"
            find "${cfg.stateDir}/.alex/$_subdir" -type f \
              -exec chmod g+rw {} + 2>/dev/null || true
          done
          ...
          # alex-group users can save settings via the CLI/TUI, else 0640).
          ${if cfg.configFile != null then ''
            install -o ${cfg.user} -g ${cfg.group} -m ${configYamlMode} -D ${configFile} ${cfg.stateDir}/.alex/config.yaml
          '' else ''
            ${configMergeScript} ${generatedConfigFile} ${cfg.stateDir}/.alex/config.yaml
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.alex/config.yaml
            chmod ${configYamlMode} ${cfg.stateDir}/.alex/config.yaml
          ''}

          # Managed mode marker (so interactive shells also detect NixOS management)
          touch ${cfg.stateDir}/.alex/.managed
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.alex/.managed
          chmod 0644 ${cfg.stateDir}/.alex/.managed
          ...
          ${if cfg.container.enable then ''
            cat > ${cfg.stateDir}/.alex/.container-mode <<'ALEX_CONTAINER_MODE_EOF'
    # Written by NixOS activation script. Do not edit manually.
    backend=${cfg.container.backend}
    container_name=${containerName}
    exec_user=${cfg.user}
    alex_bin=${containerDataDir}/current-package/bin/alex
    ALEX_CONTAINER_MODE_EOF
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.alex/.container-mode
            chmod 0644 ${cfg.stateDir}/.alex/.container-mode
          '' else ''
            rm -f ${cfg.stateDir}/.alex/.container-mode

            # Remove symlink bridge for hostUsers
            ${lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.alex";
              in ''
                if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${cfg.stateDir}/.alex" ]; then
                  rm -f "${symlinkPath}"
                  echo "alex-agent: removed symlink ${symlinkPath}"
                fi
              '') cfg.container.hostUsers)}
          ''}

          # ── Symlink bridge for interactive users ───────────────────────
          # Create ~/.alex -> stateDir/.alex for each hostUser so the
          ...
          ${lib.optionalString cfg.container.enable
            (lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.alex";
                target = "${cfg.stateDir}/.alex";
              in ''
                if [ -d "${symlinkPath}" ] && [ ! -L "${symlinkPath}" ]; then
                  # Real directory — back it up, then create symlink.
                  # (ln -sfn cannot atomically replace a directory.)
                  _backup="${symlinkPath}.bak.$(date +%s)"
                  echo "alex-agent: backing up existing ${symlinkPath} to $_backup"
          ...
          ${lib.optionalString (cfg.authFile != null) ''
            ${if cfg.authFileForceOverwrite then ''
              install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.alex/auth.json
            '' else ''
              if [ ! -f ${cfg.stateDir}/.alex/auth.json ]; then
                install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.alex/auth.json
              fi
            ''}
          ''}
          ...
          # Alex reads $ALEX_HOME/.env at startup via load_alex_dotenv(),
          # so this is the single source of truth for both native and container mode.
          ${lib.optionalString (cfg.environment != {} || cfg.environmentFiles != []) ''
            ENV_FILE="${cfg.stateDir}/.alex/.env"
            install -o ${cfg.user} -g ${cfg.group} -m 0640 /dev/null "$ENV_FILE"
            cat > "$ENV_FILE" <<'ALEX_NIX_ENV_EOF'
    ${envFileContent}
    ALEX_NIX_ENV_EOF
            ${lib.concatStringsSep "\n" (map (f: ''
              if [ -f "${f}" ]; then
        ...
        find ${cfg.stateDir}/.alex/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
        ...
        ${lib.concatStringsSep "\n" (map (plugin:
          ...
          in ''
            ...
            ln -sfn ${plugin} ${cfg.stateDir}/.alex/plugins/nix-managed-${name}
            chown -h ${cfg.user}:${cfg.group} ${cfg.stateDir}/.alex/plugins/nix-managed-${name}
          '') cfg.extraPlugins)}
        '';
      }
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.alex-agent = {
          description = "Alex Agent Gateway";
          ...
          serviceConfig = {
            ...
            # $ALEX_HOME/.env by the activation script. load_alex_dotenv()
            # reads them at Python startup — no systemd EnvironmentFile needed.

            ExecStart = lib.concatStringsSep " " ([
              "${effectivePackage}/bin/alex"
              "gateway"
            ] ++ cfg.extraArgs);
            ...
            # so interactive users in the alex group can read/write them.
            UMask = "0007";
      ...
      (lib.mkIf cfg.container.enable {
        ...
        systemd.services.alex-agent = {
          description = "Alex Agent Gateway (container)";
          wantedBy = [ "multi-user.target" ];
          after = [ "network-online.target" ]
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              ALEX_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
              ALEX_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
              ...
              ${containerBin} create \
                ...
                --env ALEX_UID="$ALEX_UID" \
                --env ALEX_GID="$ALEX_GID" \
                --env ALEX_HOME=${containerDataDir}/.alex \
                --env ALEX_MANAGED=true \
                --env HOME=${containerHomeDir} \
                --env MESSAGING_CWD=${containerWorkDir} \
                ${lib.concatStringsSep " " cfg.container.extraOptions} \
                ${cfg.container.image} \
                ${containerDataDir}/current-package/bin/alex gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
