...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      alex-agent = self'.packages.default;
      alexVenv = alex-agent.alexVenv;

      configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

      # Auto-generated config key reference — always in sync with Python
      configKeys = pkgs.runCommand "alex-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${alexVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "alex-cross-eval" { } (
          if failures != [] then
        ...
        build-package = pkgs.runCommand "alex-build-package" { } ''
          echo "PASS: package built at ${alex-agent}"
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify the devShell builds successfully (cross-platform).
        build-devshell = pkgs.runCommand "alex-build-devshell" { } ''
        ...
        package-contents = pkgs.runCommand "alex-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${alex-agent}/bin/alex || (echo "FAIL: alex binary missing"; exit 1)
          test -x ${alex-agent}/bin/alex-agent || (echo "FAIL: alex-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${alex-agent}/bin/alex version 2>&1 | grep -qi "alex" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "alex-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in alex alex-agent alex-acp; do
            test -x ${alex-agent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "alex-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking alex --help ==="
          ${alex-agent}/bin/alex --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${alex-agent}/bin/alex --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "alex-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${alex-agent}/share/alex-agent/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          SKILL_COUNT=$(find ${alex-agent}/share/alex-agent/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "ALEX_BUNDLED_SKILLS" ${alex-agent}/bin/alex || \
            (echo "FAIL: ALEX_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: ALEX_BUNDLED_SKILLS set in wrapper"
        ...
        bundled-plugins = pkgs.runCommand "alex-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${alex-agent}/share/alex-agent/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${alex-agent}/share/alex-agent/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "ALEX_BUNDLED_PLUGINS" ${alex-agent}/bin/alex || \
            (echo "FAIL: ALEX_BUNDLED_PLUGINS not in wrapper"; exit 1)
          echo "PASS: ALEX_BUNDLED_PLUGINS set in wrapper"
        ...
        bundled-locales = pkgs.runCommand "alex-bundled-locales" { } ''
          set -e
          echo "=== Checking bundled locales ==="
          test -d ${alex-agent}/share/alex-agent/locales || (echo "FAIL: locales directory missing"; exit 1)
          echo "PASS: locales directory exists"

          LOC_COUNT=$(find ${alex-agent}/share/alex-agent/locales -name "*.yaml" | wc -l)
          test "$LOC_COUNT" -ge 16 || (echo "FAIL: expected >=16 catalogs, found $LOC_COUNT"; exit 1)
          echo "PASS: $LOC_COUNT locale catalogs found"

          test -f ${alex-agent}/share/alex-agent/locales/en.yaml || (echo "FAIL: en.yaml missing"; exit 1)
          echo "PASS: en.yaml present"

          grep -q "ALEX_BUNDLED_LOCALES" ${alex-agent}/bin/alex || \
            (echo "FAIL: ALEX_BUNDLED_LOCALES not in wrapper"; exit 1)
          echo "PASS: ALEX_BUNDLED_LOCALES set in wrapper"

          echo "=== Rendering via the wrapper override (ALEX_BUNDLED_LOCALES) ==="
          export HOME=$(mktemp -d)
          RENDERED=$(cd "$HOME" && ALEX_BUNDLED_LOCALES=${alex-agent}/share/alex-agent/locales \
            ${alexVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "rendered: $RENDERED"
          test "$RENDERED" != "gateway.reset.header_default" || (echo "FAIL: i18n returned the raw key with ALEX_BUNDLED_LOCALES set"; exit 1)
          ...
          BARE_DIR=$(cd "$HOME" && ${alexVenv}/bin/python3 -c "from agent import i18n; print(i18n._locales_dir())")
          BARE=$(cd "$HOME" && ${alexVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "resolved dir (no env var): $BARE_DIR"
          echo "rendered: $BARE"
          test "$BARE" != "gateway.reset.header_default" || \
            (echo "FAIL: sealed venv could not resolve locales without ALEX_BUNDLED_LOCALES — data-files materialization regressed"; exit 1)
        ...
        bundled-tui = pkgs.runCommand "alex-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${alex-agent}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${alex-agent}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "ALEX_TUI_DIR" ${alex-agent}/bin/alex || \
            (echo "FAIL: ALEX_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: ALEX_TUI_DIR set in wrapper"
        ...
        # Verify ALEX_NODE is set in wrapper and points to Node 20+
        # (string-width uses the /v regex flag which requires Node 20+)
        alex-node = pkgs.runCommand "alex-node-version" { } ''
          set -e
          echo "=== Checking ALEX_NODE in wrapper ==="
          grep -q "ALEX_NODE" ${alex-agent}/bin/alex || \
            (echo "FAIL: ALEX_NODE not set in wrapper"; exit 1)
          echo "PASS: ALEX_NODE present in wrapper"

          ALEX_NODE=$(sed -n "s/^export ALEX_NODE='\(.*\)'/\1/p" ${alex-agent}/bin/alex)
          test -x "$ALEX_NODE" || (echo "FAIL: ALEX_NODE=$ALEX_NODE not executable"; exit 1)
          echo "PASS: ALEX_NODE executable at $ALEX_NODE"

          NODE_MAJOR=$("$ALEX_NODE" --version | sed 's/^v//' | cut -d. -f1)
          test "$NODE_MAJOR" -ge 20 || \
            (echo "FAIL: Node v$NODE_MAJOR < 20, TUI needs /v regex flag support"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 20"

          echo "=== All ALEX_NODE checks passed ==="
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify ALEX_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "alex-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(ALEX_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking ALEX_MANAGED guards ==="
          check_blocked "config set" ${alex-agent}/bin/alex config set model foo
          check_blocked "config edit" ${alex-agent}/bin/alex config edit
        ...
        extra-python-packages = let
          ...
          alexWithExtra = alex-agent.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "alex-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${alexWithExtra}/bin/alex || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${alexWithExtra}/bin/alex || \
          ...
          if grep -q "PYTHONPATH" ${alex-agent}/bin/alex; then
        ...
        extra-dependency-groups = let
          alexWithGroups = alex-agent.override {
            extraDependencyGroups = [ "honcho" ];
          };
        in pkgs.runCommand "alex-extra-dependency-groups" { } ''
          set -e
          ...
          echo "derivation: ${alexWithGroups}"
          echo "venv: ${alexWithGroups.alexVenv}"
        ...
        messaging-variant = pkgs.runCommand "alex-messaging-variant" { } ''
          set -e
          echo "=== Checking discord.py importable from messaging variant ==="
          ${self'.packages.messaging.alexVenv}/bin/python3 -c \
        ...
        in pkgs.runCommand "alex-config-roundtrip" {
          ...
          merge_and_load() {
            local alex_home="$1"
            export ALEX_HOME="$alex_home"
            ${configMergeScript} ${nixSettings} "$alex_home/config.yaml"
            ${alexVenv}/bin/python3 -c '
import json, sys
