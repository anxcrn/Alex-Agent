"""Centralized logging setup for Alex Agent.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.alex/logs/`` (profile-aware via ``get_alex_home()``).
...
Component separation:
    ...
    gui.log receives dashboard-side records from ``alex_cli.web_server``,
    ``alex_cli.pty_bridge``, ``tui_gateway.*``, and ``uvicorn.*``.
...
# essentially always in Alex (TUI, gateway, ``hy_memory`` server, MCP
...
from alex_constants import get_config_path, get_alex_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_alex_session_injector", False):
    ...
    _session_record_factory._alex_session_injector = True  # type: ignore[attr-defined]
...
# Used by _ComponentFilter and exposed for ``alex logs --component``.
COMPONENT_PREFIXES = {
    ...
    # ``alex logs --component gateway``.
    "gateway": ("gateway", "alex_plugins", "plugins.platforms"),
...
def setup_logging(
    *,
    ...
    """Configure the Alex logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    alex_home
        Override for the Alex home directory.  Falls back to
        ``get_alex_home()`` (profile-aware).
    log_level
    ...
    home = alex_home or get_alex_home()
    log_dir = home / "logs"
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_alex_verbose", False):
                return

    handler = logging.StreamHandler(_safe_stderr())
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._alex_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    1.  In managed mode (NixOS), the stateDir uses setgid (2770) so new files
        inherit the alex group. However, both ``_open()`` (initial creation)
    ...
    def __init__(self, *args, **kwargs):
        from alex_cli.config import is_managed
