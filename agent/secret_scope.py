...
- ``set_secret_scope(mapping)`` installs the active profile's secrets for the
  ...
  via ``copy_context()`` exactly like the ALEX_HOME override).
...
_GLOBAL_ENV_EXACT = frozenset({
    # Alex runtime / deployment
    "ALEX_HOME", "ALEX_PROFILE", "ALEX_GATEWAY_LOCK_DIR",
    "ALEX_MAX_ITERATIONS", "ALEX_MAX_TOKENS", "ALEX_API_TIMEOUT",
    "ALEX_REDACT_SECRETS", "ALEX_NOUS_TIMEOUT_SECONDS",
    "_ALEX_GATEWAY",
    # OS / interpreter
    "PATH", "HOME", "USER", "LANG", "LC_ALL", "TZ", "PWD", "SHELL", "TMPDIR",
    "VIRTUAL_ENV", "PYTHONPATH", "SSL_CERT_FILE",
    # Kanban paths (per-board, not per-profile-secret)
    "ALEX_KANBAN_DB", "ALEX_KANBAN_WORKSPACES_ROOT", "ALEX_KANBAN_BOARD",
})
_GLOBAL_ENV_PREFIXES = (
    "ALEX_KANBAN_",
    "ALEX_TELEGRAM_",   # tuning knobs (batch delays, fallback toggles) — NOT the token
...
def build_profile_secret_scope(alex_home: Path) -> Dict[str, str]:
    ...
    """
    return load_env_file(Path(alex_home) / ".env")
