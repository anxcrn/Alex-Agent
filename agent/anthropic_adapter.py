"""Anthropic Messages API adapter for Alex Agent.

Translates between Alex's internal OpenAI-style message format and
...
# Alex effort → Anthropic adaptive-thinking effort (output_config.effort).
...
def _resolve_anthropic_pool_token() -> Optional[str]:
    ...
    ``account_usage`` and ``alex models``) never mutates ``~/.alex/auth.json``
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    # 1. Alex-managed OAuth/setup token env var
    ...
    # 4. Alex credential_pool OAuth entry.
    resolved_pool_token = _resolve_anthropic_pool_token()
    if resolved_pool_token:
        return resolved_pool_token

    # 5. Regular API key, or a legacy OAuth token saved in ANTHROPIC_API_KEY.
    # This remains as a compatibility fallback for pre-migration Alex configs.
...
_ALEX_OAUTH_FILE = get_alex_home() / ".anthropic_oauth.json"
...
def run_alex_oauth_login_pure() -> Optional[Dict[str, Any]]:
    """Run Alex-native OAuth PKCE flow and return credential state."""
    import secrets
    import time
    import webbrowser
    ...
    print()
    print("Authorize Alex with your Claude Pro/Max subscription.")
    print()
    ...
    try:
        from alex_cli.auth import _can_open_graphical_browser as _can_open_gui
    except Exception:
...
def read_alex_oauth_credentials() -> Optional[Dict[str, Any]]:
    """Read Alex-managed OAuth credentials from ~/.alex/.anthropic_oauth.json."""
    if _ALEX_OAUTH_FILE.exists():
        try:
            data = json.loads(_ALEX_OAUTH_FILE.read_text(encoding="utf-8"))
            if data.get("accessToken"):
                return data
        except (json.JSONDecodeError, OSError, IOError) as e:
            logger.debug("Failed to read Alex OAuth credentials: %s", e)
    return None
...
def _convert_assistant_message(m: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # can validate the message history.  See alex-agent#13848.
    #
...
def _strip_orphaned_tool_blocks(result: List[Dict[str, Any]]) -> None:
    ...
    for m in result:
        if m["role"] == "assistant" and isinstance(m["content"], list):
            ...
            # replaying it verbatim.  See alex-agent: extended-thinking + parallel
...
def _manage_thinking_signatures(
    ...
    replayed assistant tool-call messages.  See alex-agent#13848 (Kimi) and
    alex-agent#16748 (DeepSeek).
...
def build_anthropic_kwargs(
    ...
    if is_oauth:
        ...
        #      a) bare Alex-native tools (``read_file``)  -> ``mcp__read_file``
    ...
    # silently hides reasoning text that Alex surfaces in its CLI. We
