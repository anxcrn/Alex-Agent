...
def _session_is_messaging_surface() -> bool:
    ...
    ``ALEX_SESSION_PLATFORM``; the CLI and TUI set ``ALEX_SESSION_SOURCE``
    (e.g. ``cli``, ``tui``) instead. Both are consulted via the session-context
    helper (with an ``os.environ`` fallback), alongside the ``ALEX_PLATFORM``
...
def verify_on_stop_enabled(config: dict[str, Any] | None = None) -> bool:
    ...
    Precedence: an explicit ``ALEX_VERIFY_ON_STOP`` env var wins, then an
    ...
    falls back to OFF.
    """
    env = os.environ.get("ALEX_VERIFY_ON_STOP")
    if env is not None:
    ...
    # Enable verify-on-stop automatically under planning mode if not explicitly turned off
    planning_val = (config or {}).get("planning_mode_guidance")
    if planning_val is None and isinstance(agent_cfg, dict):
        planning_val = agent_cfg.get("planning_mode_guidance")
    if planning_val is True:
        return not _session_is_messaging_surface()
...
def _run_auto_verification(session_id: str | None, cwd: Path, verify_commands: list[str]) -> None:
    import subprocess
    ...
    for cmd in verify_commands:
        try:
            ...
            record_terminal_result(
                ...
                session_id=session_id,
                exit_code=-1,
                output=f"Timeout: Command timed out after 45 seconds.\n{e.stdout or ''}\n{e.stderr or ''}",
            )
            ...
            record_terminal_result(
                ...
                session_id=session_id,
                exit_code=-1,
                output=f"Error running auto-verification: {e}",
            )
...
def build_verify_on_stop_nudge(
    *,
    ...
    # Run auto-verification for edited workspaces before querying the snapshot
    for cwd in _candidate_cwds(paths):
        from agent.coding_context import project_facts_for
        facts = project_facts_for(cwd)
        if not facts:
            continue
        ...
        if verify_commands:
            ...
            status = verification_status(session_id=session_id, cwd=cwd)
            if str(status.get("status") or "unverified") != "passed":
                _run_auto_verification(session_id, cwd, verify_commands)
    ...
    if verify_commands:
        ...
        command_instruction = (
            ...
            "`tempfile` path with a `alex-verify-` filename prefix, run it "
