...
Plugins built on Alex Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Alex
...
:class:`~alex_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Alex' main config
...
handles every provider, fallback chain, and per-task override Alex
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`alex_cli.plugins.PluginContext`
