...
``auth.json``.  They intentionally have no dependency on ``alex_cli.auth`` so
...
# Sources Alex owns and can intentionally persist in auth.json.  Everything
...
def sanitize_borrowed_credential_payload(
    ...
    Owned sources (manual entries and Alex-owned OAuth/device-code state)
