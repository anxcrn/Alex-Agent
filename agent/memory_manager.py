...
class MemoryManager:
    ...
    def add_provider(self, provider: MemoryProvider) -> None:
        ...
        _core_tool_names = set(_ALEX_CORE_TOOLS)
    ...
    def _strip_skill_scaffolding(text: str) -> Optional[str]:
        ...
        When a user invokes a /skill or /bundle, Alex expands the turn into
    ...
    def initialize_all(self, session_id: str, **kwargs) -> None:
        ...
        Automatically injects ``alex_home`` into *kwargs* so that every
        provider can resolve profile-scoped storage paths without importing
        ``get_alex_home()`` themselves.
        """
        if "alex_home" not in kwargs:
            from alex_constants import get_alex_home
            kwargs["alex_home"] = str(get_alex_home())
        for provider in self._providers:
            try:
