...
from alex_constants import get_config_path, get_skills_dir, is_termux
...
def _detect_environment(env: str) -> bool:
    ...
    if env == "kanban":
        ...
        # dispatcher sets ``ALEX_KANBAN_TASK`` / ``ALEX_KANBAN_BOARD`` in the
        ...
        # tool availability.
        if os.getenv("ALEX_KANBAN_TASK") or os.getenv("ALEX_KANBAN_BOARD"):
            result = True
        else:
            try:
        ...
        # The Alex Docker image runs s6-overlay as PID 1 (/init). s6 plants
...
def _load_raw_config() -> Dict[str, Any]:
    ...
    This module intentionally avoids importing ``alex_cli.config`` on the
...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``ALEX_PLATFORM`` or
            ``ALEX_SESSION_PLATFORM`` env vars.  Returns the global
...
# which becomes the dominant cost of ``alex`` startup when ~120 skills
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.alex/skills/`` are silently skipped.

    Cached in-process, keyed on ``config.yaml`` mtime — the function is
    called once per skill during banner / tool-registry scans, and YAML
    parsing a non-trivial config dominates ``alex`` cold-start time
    ...
    for entry in raw_dirs:
        ...
        p = Path(expanded)
        # Resolve relative paths against ALEX_HOME, not cwd
        if not p.is_absolute():
            p = (alex_home / p).resolve()
        else:
            p = p.resolve()
        if p == local_skills:
            continue
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.alex/skills/`` first, then external.
...
def is_external_skill_path(path) -> bool:
    ...
    ``skills.external_dirs`` are externally owned: Alex can discover and view
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(metadata, dict):
        metadata = {}
    alex = metadata.get("alex") or {}
    if not isinstance(alex, dict):
        alex = {}
    return {
        "fallback_for_toolsets": alex.get("fallback_for_toolsets", []),
        "requires_toolsets": alex.get("requires_toolsets", []),
        "fallback_for_tools": alex.get("fallback_for_tools", []),
        "requires_tools": alex.get("requires_tools", []),
    }
...
def iter_skill_index_files(skills_dir: Path, filename: str):
    ...
    Excludes Alex metadata, VCS, virtualenv/dependency, cache, and skill
