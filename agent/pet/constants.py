...
row taxonomy is inferred from the atlas shape so Alex can render both legacy
...
# rows (1536x1872), while older Alex/petdex sheets used 9 columns x 8 rows
...
class PetState(str, Enum):
    ...
    These are Alex' activity state names. They are not always identical to the
...
# Legacy Alex/petdex row order (top -> bottom) used by the older 8-row,
...
# Canonical Alex activity names -> accepted row-name aliases in descending
