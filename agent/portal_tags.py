...
Every Alex request that hits the Nous Portal — main agent loop, auxiliary
client (compression / titles / vision / web_extract / session_search / etc.),
and any future code path — must carry the same product-attribution tags so
Nous can attribute usage to Alex Agent and bucket it by client release.

Tag shape (sent in OpenAI-compatible ``extra_body['tags']``):

    [
        "product=alex-agent",
        "client=alex-client-v<__version__>",
    ]

The version is sourced live from ``alex_cli.__version__`` so it auto-aligns
...
``alex_cli.__version__`` is the canonical source of truth.
"""
...
def _alex_version() -> str:
    """Return the current Alex release version, e.g. ``"0.13.0"``.

    Falls back to ``"unknown"`` if ``alex_cli`` cannot be imported (should
...
def alex_client_tag() -> str:
    """Return the ``client=...`` tag for Nous Portal requests.

    Format: ``client=alex-client-v<MAJOR>.<MINOR>.<PATCH>``.
    """
    return f"client=alex-client-v{_alex_version()}"
...
def nous_portal_tags() -> List[str]:
    ...
    """
    return ["product=alex-agent", alex_client_tag()]
