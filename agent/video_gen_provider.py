...
as ``kind: backend``) or ``~/.alex/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class VideoGenProvider(abc.ABC):
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        Used by the tool layer for soft validation and by ``alex tools``
...
def _videos_cache_dir() -> Path:
    """Return ``$ALEX_HOME/cache/videos/``, creating parents as needed."""
    from alex_constants import get_alex_home

    path = get_alex_home() / "cache" / "videos"
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$ALEX_HOME/cache/videos/``.
