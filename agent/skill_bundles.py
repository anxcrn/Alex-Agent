...
Storage
-------
Bundles live in ``~/.alex/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``ALEX_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``alex bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under ALEX_HOME.

    Honors ``ALEX_BUNDLES_DIR`` for tests; falls back to
    ``<ALEX_HOME>/skill-bundles``.
    """
    override = os.environ.get("ALEX_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_alex_home() / "skill-bundles"
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `alex bundles` CLI subcommand.
# ---------------------------------------------------------------------------
