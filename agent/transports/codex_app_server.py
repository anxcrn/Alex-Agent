...
projection into Alex' display, approval bridging, transcript projection into
...
"codex_app_server"`. Alex' default tool dispatch is unchanged when this
...
class CodexAppServerClient:
    ...
    def __init__(
        ...
        if spawn_env.get("ALEX_KANBAN_TASK"):
            kanban_db = spawn_env.get("ALEX_KANBAN_DB")
            kanban_root = (
                ...
                if kanban_db
                    ...
                    "ALEX_KANBAN_ROOT",
                    os.path.join(
                        spawn_env.get("ALEX_HOME", os.path.expanduser("~/.alex")),
                        "kanban",
                    ),
                )
            )
