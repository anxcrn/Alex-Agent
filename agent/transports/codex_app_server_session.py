...
Owns one Codex thread per Alex session. Drives `turn/start`, consumes
...
# Alex' tools.terminal.security_mode → Codex's permissions profile id.
# Defaults if config is missing → workspace-write (matches Codex's own default).
_ALEX_TO_CODEX_PERMISSION_PROFILE = {
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse Alex/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per Alex session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _ALEX_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("ALEX_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            client_name="alex",
            client_title="Alex Agent",
            client_version=_get_alex_version(),
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but Alex' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Alex' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own alex-tools callback we
            # auto-accept — the user already approved Alex' tools
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Alex approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Alex returns 'once', 'session', 'always', or 'deny'.
...
def _get_alex_version() -> str:
    """Best-effort Alex version string for codex's userAgent line."""
    try:
