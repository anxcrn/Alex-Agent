...
from alex_cli.timeouts import get_provider_request_timeout
...
def _build_codex_gpt55_autoraise_notice(autoraise: Dict[str, float]) -> str:
    ...
    return (
        ...
        f"summarizing.\n"
        f"  Opt back out: alex config set compression.codex_gpt55_autoraise false"
    )
...
def init_agent(
    ...
    Args:
        ...
        skip_context_files (bool): If True, skip auto-injection of project context files
            (SOUL.md, .alex.md, AGENTS.md, CLAUDE.md, .cursorrules) from the cwd / ALEX_HOME
            into the system prompt. Use this for batch processing and data generation to avoid
            polluting trajectories with user-specific persona or project instructions.
        load_soul_identity (bool): If True, still use ~/.alex/SOUL.md as the primary
    ...
    agent._cache_ttl = "5m"
    try:
        from alex_cli.config import load_config as _load_pc_cfg
    ...
    # report cumulative micros spent.  Surfaced behind ALEX_DEV_CREDITS.
    ...
    # both live under ~/.alex/logs/.  Idempotent, so gateway mode
    # (which creates a new AIAgent per message) won't duplicate handlers.
    from alex_logging import setup_logging, setup_verbose_logging
    setup_logging(alex_home=_ra()._alex_home)
    ...
    if agent.verbose_logging:
        ...
        # on the fact that alex_logging.setup_logging() does not
        ...
        # handler level inside alex_logging.py, not here.
        pass
    ...
    if agent.api_mode == "anthropic_messages":
        ...
        if _is_bedrock_anthropic:
            ...
            if agent.provider == "minimax-oauth" and isinstance(effective_key, str) and effective_key:
                try:
                    from alex_cli.auth import build_minimax_oauth_token_provider
        ...
        try:
            from alex_cli.config import load_config as _load_br_cfg
        ...
        if api_key and base_url:
            ...
            if _routed_client is not None:
                ...
                if not getattr(agent, "_fallback_activated", False):
                    ...
                    raise RuntimeError(
                        "No LLM provider configured. Run `alex model` to "
                        "select a provider, or run `alex setup` for first-time "
                        "configuration."
                    )
    ...
    # worker (kanban_show tool is present iff ALEX_KANBAN_TASK is set).
    ...
    try:
        ...
        os.environ["ALEX_SESSION_ID"] = agent.session_id

    # Session logs go into ~/.alex/sessions/ alongside gateway sessions
    alex_home = get_alex_home()
    agent.logs_dir = alex_home / "sessions"
    agent.logs_dir.mkdir(parents=True, exist_ok=True)
    # Per-session JSON snapshot writer (~/.alex/sessions/session_{sid}.json)
    ...
    try:
        from alex_cli.config import load_config as _load_sess_cfg
    ...
    try:
        from alex_cli.config import load_config as _load_agent_config
            ...
            if _mem_provider_name and _mem_provider_name.strip():
                ...
                if agent._memory_manager.providers:
                    ...
                    try:
                        ...
                        _init_kwargs["agent_workspace"] = "alex"
                    except Exception:
                        pass
    ...
    # Universal planning-mode guidance toggle. Default True.
    agent._planning_mode_guidance = planning_mode_guidance if planning_mode_guidance is not None else bool(_agent_section.get("planning_mode_guidance", True))
    ...
    # Lets an enterprise admin append to or replace Alex' built-in
    ...
    if _config_context_length is None and _custom_providers:
        try:
            from alex_cli.config import get_custom_provider_context_length
    ...
    if _engine_name != "compressor":
        ...
        if _selected_engine is None:
            _candidate = None
            try:
                from alex_cli.plugins import get_plugin_context_engine
    ...
    if _ctx and _ctx < MINIMUM_CONTEXT_LENGTH:
        raise ValueError(
            ...
            f"by Alex Agent.  Choose a model with at least "
