...
Optional hooks (override to opt in):
  ...
  backup_paths() -> list[str]            — extra on-disk paths to include in `alex backup`
"""
...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - alex_home (str): The active ALEX_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.alex``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "alex").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'alex memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], alex_home: str) -> None:
        """Write non-secret config to the provider's native location.

        Called by 'alex memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``alex_home`` is the active ALEX_HOME directory path.
    ...
    def backup_paths(self) -> List[str]:
        """Return extra on-disk paths this provider stores OUTSIDE ALEX_HOME.

        ``alex backup`` only walks ALEX_HOME, so any provider state kept
        ...
        of the archive, and ``alex import`` restores them to their original
