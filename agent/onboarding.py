...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Alex starts and finds ``~/.openclaw/``.

    Points users at ``alex claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``alex claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Alex, run "
        "`alex claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `alex claw cleanup` (renames it to ~/.openclaw.pre-migration — "
