...
from alex_constants import get_alex_home, get_skills_dir, is_wsl
...
_ALEX_MD_NAMES = (".alex.md", "ALEX.md")


def _find_alex_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.alex.md`` or ``ALEX.md``.
...
DEFAULT_AGENT_IDENTITY = (
    "You are Alex Agent, an intelligent AI assistant created by charan vankudoth. "
...
ALEX_AGENT_HELP_GUIDANCE = (
    "You run on Alex Agent (by charan vankudoth). When the user needs help with "
    "Alex itself — configuring, setting up, using, extending, or troubleshooting "
    "it — or when you need to understand your own features, tools, or capabilities, "
    "the documentation at https://alex-agent.charan vankudoth.com/docs is your "
    "authoritative reference and always holds the latest, most up-to-date "
    "information. Load the `alex-agent` skill with skill_view(name='alex-agent') "
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.alex/kanban.db`. Your task id is in "
    "`$ALEX_KANBAN_TASK`; your workspace is `$ALEX_KANBAN_WORKSPACE`. "
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $ALEX_KANBAN_WORKSPACE` before "
    ...
    "\n"
    "- **Workspace.** `cd $ALEX_KANBAN_WORKSPACE` first. For a `worktree` kind "
    "with no `.git`, `git worktree add <path> "
    "${ALEX_KANBAN_BRANCH:-wt/$ALEX_KANBAN_TASK}` from the main repo, then "
    "cd there. For a project-linked task the workspace is a fresh "
    "`<repo>/.worktrees/<task-id>` and `$ALEX_KANBAN_BRANCH` a deterministic "
    ...
    "every assignee in a real profile (`alex profile list`, or ask the user), "
    "and express dependencies via `parents=[...]` on `kanban_create`, not prose.\n"
    "\n"
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `alex kanban <verb>` for board operations. Use "
...
#
# The alex-agent runtime already executes a batch of tool calls
...
# from Cline's TypeScript tool-surface guidance to alex-agent's Python
...
PLANNING_MODE_GUIDANCE = (
    "# Planning Mode\n"
    "When the user requests any complex task (such as writing or modifying code files, "
    "running terminal commands, or making architectural changes):\n"
    "1. You MUST first perform research on the codebase (using grep, search_files, read_file, etc.) to understand the structure.\n"
    "2. Before making any changes or running commands that mutate the environment, you MUST write or update `implementation_plan.md` at the project root outlining the proposed changes, architectural impact, and any open questions.\n"
    "3. Halt your turn immediately and request the user's feedback/approval. Do NOT call modifying tools (e.g. file writes, shell command execution) until the user explicitly responds with approval.\n"
    "4. Once approved, create or update `task.md` at the project root to track your progress as a checklist.\n"
    "5. Follow the checklist, updating `task.md` as you proceed.\n"
)
...
def computer_use_guidance(platform_name: Optional[str] = None) -> str:
    ...
    return (
        ...
        "run `alex computer-use doctor` and share the output. That command "
...
STEER_CHANNEL_NOTE = (
    ...
    "While you work, the user can send an out-of-band message that Alex "
...
PLATFORM_HINTS = {
    ...
    "tui": (
        "You are running in the Alex terminal UI (TUI). "
        "Cron jobs scheduled from this session are LOCAL-ONLY: their output is "
        "saved (viewable via cronjob action='list') but is NOT delivered back "
    ...
    "webui": (
        "You are in the Alex WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where Alex itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host Alex runs on.
    """
...
def build_environment_hints() -> str:
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where Alex itself is running. The host OS, home, and cwd "
                f"of the Alex process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where Alex "
    ...
    # Alex desktop GUI — any agent running under the desktop app should know
    # it. ALEX_DESKTOP marks the backend powering the chat; ALEX_DESKTOP_TERMINAL
    # marks a alex launched in the embedded terminal pane. Both set by main.cjs.
    _truthy = ("1", "true", "yes")
    _in_desktop = (os.getenv("ALEX_DESKTOP") or "").strip().lower() in _truthy
    _in_desktop_term = (os.getenv("ALEX_DESKTOP_TERMINAL") or "").strip().lower() in _truthy
    if _in_desktop or _in_desktop_term:
        _desktop_hint = "Runtime surface: you're running inside the Alex desktop GUI app."
        if _in_desktop_term:
            _desktop_hint += (
    ...
    # Embedder-supplied environment description. Lets a host that wraps Alex
    ...
    extra = (os.getenv("ALEX_ENVIRONMENT_HINT") or "").strip()
    if not extra:
        try:
...
def _skills_prompt_snapshot_path() -> Path:
    return get_alex_home() / ".skills_prompt_snapshot.json"
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.alex/skills/`` directory.  External dirs
    ...
    _platform_hint = (
        os.environ.get("ALEX_PLATFORM")
        or get_session_env("ALEX_SESSION_PLATFORM")
        or ""
    )
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot Alex Agent itself — its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature — load the `alex-agent` skill "
            "first. It has the actual commands (e.g. `alex config set …`, `alex tools`, "
            "`alex setup`) so you don't have to guess or invent workarounds.\n"
...
def build_nous_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    lines.extend(
        [
            ...
            "Useful commands: alex setup, alex setup tools, alex setup terminal, alex status.",
        ]
    )
    return "\n".join(lines)
...
def load_soul_md(context_length: Optional[int] = None) -> Optional[str]:
    """Load SOUL.md from ALEX_HOME and return its content, or None.
    ...
    """
    try:
        from alex_cli.config import ensure_alex_home
        ensure_alex_home()
    except Exception as e:
        logger.debug("Could not ensure ALEX_HOME before loading SOUL.md: %s", e)

    soul_path = get_alex_home() / "SOUL.md"
...
def _load_alex_md(cwd_path: Path, context_length: Optional[int] = None) -> str:
    """.alex.md / ALEX.md — walk to git root."""
    alex_md_path = _find_alex_md(cwd_path)
    if not alex_md_path:
        return ""
    try:
        content = alex_md_path.read_text(encoding="utf-8").strip()
        if not content:
            return ""
        content = _strip_yaml_frontmatter(content)
        rel = alex_md_path.name
        try:
            rel = str(alex_md_path.relative_to(cwd_path))
        except ValueError:
            pass
        content = _scan_context_content(content, rel)
        result = f"## {rel}\n\n{content}"
        return _truncate_content(
            result, ".alex.md", context_length=context_length,
            read_path=str(alex_md_path),
        )
    except Exception as e:
        logger.debug("Could not read %s: %s", alex_md_path, e)
        return ""
...
def build_context_files_prompt(
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .alex.md / ALEX.md  (walk to git root)
      2. AGENTS.md / agents.md   (cwd only)
      3. CLAUDE.md / claude.md   (cwd only)
      4. .cursorrules / .cursor/rules/*.mdc  (cwd only)

    SOUL.md from ALEX_HOME is independent and always included when present.
    ...
    project_context = (
        _load_alex_md(cwd_path, context_length)
    ...
    # SOUL.md from ALEX_HOME only — skip when already loaded as identity
    if not skip_soul:
