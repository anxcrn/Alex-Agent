...
Alex supports multiple gateway processes running concurrently — one per profile
...
needed. On every other profile gateway, add to `~/.alex/config.yaml`:
...
Or set the env var: `ALEX_KANBAN_DISPATCH_IN_GATEWAY=false`
