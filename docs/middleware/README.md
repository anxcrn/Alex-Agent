...
Alex middleware is the behavior-changing companion to observer hooks.
...
selection, or handoff to runtimes such as NeMo Relay without changing Alex'
planner, model provider adapters, tool registry, memory, or CLI UX.

With middleware enabled, plugins can:

- Rewrite LLM provider request kwargs before Alex calls the provider.
- Rewrite tool arguments before guardrails, approval checks, hooks, and tool
  execution see them.
- Wrap the actual LLM execution callback while preserving Alex retry,
  streaming, interrupt, and hook behavior.
- Wrap the actual tool execution callback while preserving Alex guardrails,
...
- `telemetry_schema_version`: currently `alex.observer.v1`
- `middleware_schema_version`: currently `alex.middleware.v1`
...
Alex stores those trace entries in later observer hook payloads as
`middleware_trace`.
...
If multiple plugins register the same execution middleware kind, Alex runs
them as a nested chain in registration order. Middleware failures are fail-open:
Alex logs a warning and continues with the next middleware or the base
runtime path.
...
For each provider request, Alex applies middleware in this order:
...
For each tool call, Alex applies middleware in this order:

1. Parse and coerce model-provided tool arguments.
2. Apply `tool_request` middleware.
3. Run the normal Alex pre-execution path against the effective arguments:
...
```bash
alex plugins enable <plugin-name>
```

For isolated local testing, use one `ALEX_HOME` for plugin enablement and the
agent run:

```bash
export ALEX_HOME=/tmp/alex-middleware-test
mkdir -p "$ALEX_HOME"
alex plugins enable <plugin-name>
alex chat --query 'Reply exactly ok'
```
...
```bash
uv sync
uv run alex plugins enable <plugin-name>
uv run alex chat --query 'Reply exactly ok'
```
...
def tag_llm_request(**kwargs):
    ...
    extra_body.setdefault("metadata", {})["alex_middleware_demo"] = True
...
def normalize_terminal_workdir(**kwargs):
    ...
    args.setdefault("workdir", "/tmp/alex-middleware-demo")
    return {
        "args": args,
        "source": "middleware-demo",
...
Return the same response shape Alex expects from the provider adapter. Do not
...
- If execution middleware raises before calling `next_call(...)`, Alex treats
...
- If execution middleware calls `next_call(...)` successfully and then raises
  during post-processing, Alex preserves the downstream result and does not
  run the provider or tool a second time.
- If downstream provider or tool execution fails, middleware may let that error
  propagate or translate it deliberately. Alex does not convert downstream
