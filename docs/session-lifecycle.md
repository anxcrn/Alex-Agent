...
The canonical transcript store is SQLite via `SessionDB` (from `alex_state`). The
...
Counts consecutive restarts via a JSON file (`{ALEX_HOME}/restart_counts.json`). If a
...
This prevents unwanted auto-resets after `alex update`, `alex gateway restart`,
or `/restart`.
