...
When running Alex inside Docker, the default `network_mode: host` gives the
...
The Alex [SECURITY.md](../../SECURITY.md) §2 defines the trust model. The
...
#
# Usage:
#   ALEX_UID=$(id -u) ALEX_GID=$(id -g) docker compose up -d
#
...
services:
  gateway:
    ...
    environment:
      ...
      - NO_PROXY=alex,alex-dashboard,localhost
...
docker compose exec gateway \
  curl -sf --max-time 5 http://alex-dashboard:9119/health && echo "OK: internal reachable" || echo "FAIL"
...
- [SECURITY.md](../../SECURITY.md) — Alex trust model and vulnerability reporting
