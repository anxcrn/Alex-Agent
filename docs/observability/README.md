...
Alex observer hooks are the read-only telemetry contract for plugins that
...
They do not replace Alex' planner, model providers, memory, tool registry,
...
Hook callbacks are fail-open. Alex catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so Alex only builds
