...
def build_login_parser(subparsers, *, cmd_login: Callable) -> None:
    ...
    ``alex login`` was removed in favor of ``alex auth`` / ``alex model``
    (the runtime handler in ``alex_cli/auth.py::login_command`` just prints a
    deprecation message and exits).  The subparser is kept registered so that
    old scripts/aliases invoking ``alex login [--flags]`` still receive the
    ...
    - The subparser is registered WITHOUT a ``help=`` kwarg so the row is
      omitted from ``alex --help`` (argparse only lists subcommands that
    ...
    - ``--provider`` accepts ANY value (no ``choices=``) so that, e.g.,
      ``alex login --provider anthropic`` reaches the deprecation handler and
      gets pointed at ``alex model`` instead of crashing in argparse with
      ``invalid choice: 'anthropic'`` before the handler can run.
    """
    login_parser = subparsers.add_parser(
        "login",
        description=(
            "Deprecated. Use `alex auth` to manage credentials, "
            "`alex model` to select a provider, or `alex setup` for full setup."
        ),
    )
    ...
    login_parser.add_argument(
        "--provider",
        default=None,
        help="(deprecated) Provider name; ignored — see `alex model`",
    )
    login_parser.add_argument(
    ...
    login_parser.add_argument(
        "--client-id", default=None, help="OAuth client id to use (default: alex-cli)"
    )
