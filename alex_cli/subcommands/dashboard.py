"""``alex dashboard`` subcommand parser.

Extracted verbatim from ``alex_cli/main.py:main()`` (god-file Phase 2).
...
def build_dashboard_parser(
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the Alex Agent web dashboard for managing config, API keys, and sessions",
    )
    ...
    # for `alex dashboard` cmdlines and SIGTERM them directly — the same
    # path `alex update` uses to clean up stale dashboards.
    dashboard_parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running alex dashboard processes and exit",
    )
    dashboard_parser.add_argument(
        "--status",
        action="store_true",
        help="List running alex dashboard processes and exit",
    )
    # Backward-compat shim: older Alex desktop app shells (<= 0.15.x) spawn the
    # backend as `alex dashboard --no-open --tui --host ... --port ...`. The
    ...
    # showed "Alex couldn't start" with no actionable cause. Accept and silently
    ...
    # `alex dashboard register` — register a self-hosted dashboard OAuth
    # client with Nous Portal and write the client_id into ~/.alex/.env.
    # Nested subparser so bare `alex dashboard` keeps launching the server
    ...
    dashboard_register_parser = dashboard_subparsers.add_parser(
        ...
        description=(
            ...
            "ALEX_DASHBOARD_OAUTH_CLIENT_ID into ~/.alex/.env, and prints "
            "how to engage the login gate. Requires being logged in (alex setup)."
        ),
    )
    ...
    dashboard_register_parser.add_argument(
        ...
        help=(
            ...
            "https://alex.example.com/auth/callback. Omit for localhost-only use."
        ),
    )
    dashboard_register_parser.add_argument(
        "--portal-url",
        ...
        help=(
            ...
            "portal. Also settable via ALEX_DASHBOARD_PORTAL_URL. Mainly for "
