...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or alex -c.",
    ...
    "Ctrl+Z suspends Alex to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "alex -c resumes your most recent CLI session. alex -c \"project name\" resumes by title.",
    "alex -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "alex -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "alex chat -t web,terminal enables only specific toolsets for a focused session.",
    "alex chat -s github-pr-workflow preloads a skill at launch.",
    "alex chat -q \"query\" runs a single non-interactive query and exits.",
    "alex chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "alex chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "alex --yolo bypasses all dangerous command approval prompts for the entire session.",
    "alex chat --source telegram tags the session for filtering in alex sessions list.",
    "alex -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "alex doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "alex dump outputs a compact setup summary — great for bug reports.",
    "alex config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "alex config edit opens config.yaml in your default editor.",
    "alex config check scans for missing or stale configuration options.",
    "alex sessions browse opens an interactive session picker with search.",
    "alex sessions stats shows session counts by platform and database size.",
    "alex sessions prune --older-than 30 cleans up old sessions.",
    "alex skills search react --source skills-sh searches the skills.sh public directory.",
    "alex skills check scans installed hub skills for upstream updates.",
    "alex skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "alex skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "alex mcp add github --command npx adds MCP servers from the command line.",
    "alex mcp serve runs Alex itself as an MCP server for other agents.",
    "alex auth add lets you add multiple API keys for credential pool rotation.",
    "alex completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "alex logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "alex backup creates a zip backup of your entire Alex home directory.",
    "alex profile create coder creates an isolated profile that becomes its own command.",
    "alex profile create work --clone copies your current config and keys to a new profile.",
    "alex update syncs new bundled skills to ALL profiles automatically.",
    "alex gateway install sets up Alex as a system service (systemd/launchd).",
    "alex memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "alex webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: alex tools disables unused tools, alex skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "alex models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as alex -w).",
    ...
    "ALEX_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Alex tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'alex profile create coder' creates the 'coder' command.",
    "alex profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Alex prints a resume command with session ID and stats.",
    "alex sessions export backup.jsonl exports all sessions for backup or analysis.",
    "alex -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "alex skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .alex/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: alex cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "Alex runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "alex gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Alex via a local macOS server.",
    ...
    "Context files (.alex.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "Alex loads project context from .alex.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    # --- MCP ---
    "alex mcp opens an interactive picker of Nous-approved MCPs you can install in one keystroke.",
    "alex mcp catalog lists Nous-approved MCP servers shipped with the repo.",
    "alex mcp install <name> installs a catalog entry, prompts for credentials, and lets you pick which of its tools to enable.",
    ...
    "MCP servers auto-generate toolsets at runtime — alex tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.alex/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "alex chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "alex plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "alex acp runs Alex as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "ALEX_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "alex auth add nous or alex auth add openai-codex sets up OAuth-based providers.",
    ...
    "alex status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.alex/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Alex your own.",
    ...
    "Before a gateway session reset, Alex auto-flushes important facts to memory in the background.",
    ...
    "delegate_task with acp_command: 'claude' spawns Alex Agent as a child agent from any platform.",
    ...
    "The skills audit log at ~/.alex/skills/.hub/audit.log tracks every install and removal operation.",
    "Stale git worktrees are auto-cleaned: 24-72h old with no unpushed commits get pruned on startup.",
    "Profiles scope Alex state via ALEX_HOME; host tool subprocesses keep your real HOME unless terminal.home_mode is profile.",
    "ALEX_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in ALEX_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.alex/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'alex profile create ls' would be rejected.",
    "alex profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "alex claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "ALEX_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via alex tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.alex/.env.",
    "Skills from trusted repos (charan vankudoth) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.alex/skills/.hub/quarantine/ holds skills pending security review.",

    # --- Advanced Slash Commands ---
    '/steer <prompt> injects a note after the next tool call — nudge direction mid-task without interrupting.',
    '/goal <text> sets a standing Ralph-loop objective — Alex auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of Alex config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while Alex is working.',
    ...
    '/reload reloads ~/.alex/.env into the running session — pick up new API keys without restarting.',
    ...
    "ALEX_CRON_MAX_PARALLEL (default 4) caps how many cron jobs run per tick so bursts don't saturate your keys.",

    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.alex/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.alex/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',

    # --- Curator ---
    'alex curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "alex curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'alex curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'alex auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    ...
    # --- TUI & Dashboard ---
    'ALEX_TUI_RESUME=1 auto-re-attaches to the most recent TUI session on launch — handy after SSH drops.',
    "ALEX_TUI_THEME=light|dark|<hex> forces the TUI theme on terminals that don't set COLORFGBG.",
    ...
    'alex dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'alex dashboard embeds the full Alex TUI in your browser via xterm.js and a WebSocket PTY.',
    'Drop a YAML in ~/.alex/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.alex/dashboard-plugins/ — no npm build required.',
    ...
    'ALEX_BACKGROUND_NOTIFICATIONS=result only pings when background tasks finish (vs all/error/off).',
    'ALEX_WRITE_SAFE_ROOT restricts write_file/patch to directory prefixes; multiple paths via os.pathsep (: or ;).',
    'ALEX_IGNORE_RULES skips auto-injection of AGENTS.md, SOUL.md, .cursorrules, memory, and preloaded skills.',
    'ALEX_ACCEPT_HOOKS auto-approves unseen shell hooks declared in config.yaml without a TTY prompt.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.alex/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.alex/skills/ so drop-in skills appear without restarting the session.',
    ...
    'alex -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'alex chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'alex chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'alex chat --ignore-user-config skips the active user config — reproducible bug reports and CI runs.',
    "alex chat --source tool tags programmatic chats so they don't clutter alex sessions list.",
    'alex dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'alex sessions rename <ID> "new title" renames any past session; alex sessions delete <ID> removes one.',
    'alex import restores a session export or profile archive produced by sessions export or profile export.',
    'alex fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'alex pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'alex setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'alex status --deep runs the full health sweep across every component; plain alex status is the quick view.',

    # --- Agent Behavior Env Vars ---
    'ALEX_AGENT_TIMEOUT=0 disables the gateway inactivity kill for a running agent — use for long research runs.',
    'ALEX_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.alex/plugins/ — trust-gated by design.',
    "ALEX_DISABLE_FILE_STATE_GUARD=1 turns off the 'file changed since you read it' guard on patch and write_file.",
    'ALEX_ALLOW_PRIVATE_URLS=true lets web tools hit localhost and private networks — off by default in gateway mode.',
    'ALEX_OPTIONAL_SKILLS=name1,name2 auto-installs extra optional-catalog skills on first run per profile.',
    'ALEX_BUNDLED_SKILLS points at a custom bundled-skill tree — used by Homebrew and Nix packaging.',
    'ALEX_DUMP_REQUEST_STDOUT=1 dumps every API request payload to stdout instead of log files.',
    'ALEX_OAUTH_TRACE=1 logs redacted OAuth token exchange and refresh attempts for debugging provider auth.',
    'ALEX_STREAM_RETRIES (default 3) controls mid-stream reconnect attempts on transient network errors.',

    # --- Gateway Behavior Env Vars ---
    'ALEX_GATEWAY_BUSY_ACK_ENABLED=false silences the ⚡/⏳/⏩ ack messages when a user messages a busy agent.',
    'ALEX_AGENT_NOTIFY_INTERVAL (default 180s) sets how often the gateway pings with progress on long turns.',
    'ALEX_RESTART_DRAIN_TIMEOUT (default 900s) caps how long /restart waits for in-flight runs before forcing.',
    'ALEX_CHECKPOINT_TIMEOUT (default 30s) caps filesystem checkpoint creation — raise it on huge monorepos.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes Alex block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from alex sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.alex/dashboard-plugins/.',
]
