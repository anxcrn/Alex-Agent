"""
Alex Desktop (Chat GUI) uninstaller.
...
the Python agent or the user's config/data:
  ...
  1. Source-built GUI (``alex desktop`` / ``alex gui``)
     Built inside the agent checkout under ``$ALEX_HOME/alex-agent/``:
       ...
       - ``$ALEX_HOME/desktop-build-stamp.json`` (the build freshness stamp)
  ...
  2. Packaged distributable (DMG / NSIS / AppImage / deb / rpm)
     ...
     own bundled Electron + a per-user Electron ``userData`` directory:
       - macOS:   ``/Applications/Alex.app`` or ``~/Applications/Alex.app``
       - Windows: ``%LOCALAPPDATA%\\Programs\\Alex`` (NSIS per-user)
...
the app name ("Alex"), separate from ``$ALEX_HOME``:
  - macOS:   ``~/Library/Application Support/Alex``
  - Windows: ``%APPDATA%\\Alex``
  - Linux:   ``$XDG_CONFIG_HOME/Alex`` (default ``~/.config/Alex``)
...
``alex uninstall --gui`` (and friends) without paying for the full CLI.
"""
...
def desktop_userdata_dir() -> Path:
    ...
    if sys.platform == "darwin":
        return home / "Library" / "Application Support" / "Alex"
...
def source_built_gui_artifacts(alex_home: Path) -> "list[Path]":
    """GUI build artifacts produced by ``alex desktop`` inside the checkout.

    These are removable on a GUI uninstall without harming the agent: the
    Python agent runs from ``alex-agent/`` source + ``venv/`` and never
    needs the Electron build output or node_modules.
    """
    agent_root = _agent_root(alex_home)
    ...
    return [
        ...
        alex_home / "desktop-build-stamp.json",
    ]
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    electron-builder output locations for the "Alex" product.
    """
    home = Path.home()
    paths: list[Path] = []
    if sys.platform == "darwin":
        paths += [
            Path("/Applications/Alex.app"),
            home / "Applications" / "Alex.app",
        ]
    elif sys.platform == "win32":
        local = os.environ.get("LOCALAPPDATA")
        local_base = Path(local) if local else (home / "AppData" / "Local")
        paths += [
            # NSIS per-user install (perMachine=false → Programs\Alex).
            local_base / "Programs" / "Alex",
            # Older / alternate layout some builds used.
            local_base / "alex-desktop",
        ]
        program_files = os.environ.get("ProgramFiles")
        if program_files:
            # NSIS per-machine fallback (needs admin to remove).
            paths.append(Path(program_files) / "Alex")
    else:
        ...
        paths += [
            data_base / "applications" / "alex.desktop",
            data_base / "applications" / "Alex.desktop",
        ]
    return paths


def agent_is_installed(alex_home: Path) -> bool:
    """Return True when a usable Python agent install exists under ALEX_HOME.
...
def gui_is_installed(alex_home: Path) -> bool:
    """Return True when any desktop GUI artifact exists (built or packaged)."""
    for p in source_built_gui_artifacts(alex_home):
        if p.exists():
            return True
...
def gui_install_summary(alex_home: "Path | None" = None) -> dict:
    ...
    """
    home: Path = alex_home if alex_home is not None else get_alex_home()
...
def uninstall_gui(alex_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    Never touches ``alex-agent/alex_cli`` (agent source), ``venv/``, or any
    config / sessions / .env under ``$ALEX_HOME``.
    ...
    if sys.platform.startswith("linux"):
        log_info(
            ...
            "with your package manager (e.g. 'sudo apt remove alex' or "
            "'sudo dnf remove alex'). AppImage builds are a single file you "
