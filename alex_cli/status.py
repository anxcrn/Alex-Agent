"""
Status command for alex CLI.

Shows the status of all Alex Agent components.
"""
...
from alex_cli.auth import AuthError, resolve_provider
from alex_cli.colors import Colors, color
from alex_cli.config import get_env_path, get_env_value, get_alex_home, load_config
from alex_cli.models import provider_label
from alex_cli.nous_account import (
    format_nous_portal_entitlement_message,
    get_nous_portal_account_info,
)
from alex_cli.nous_subscription import get_nous_subscription_features
from alex_cli.runtime_provider import resolve_requested_provider
...
def redact_key(key: str) -> str:
    ...
    "(not set)" placeholder in dim color to match ``alex config``'s
...
def show_status(args):
    """Show status of all Alex Agent components."""
    show_all = getattr(args, 'all', False)
    deep = getattr(args, 'deep', False)

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.CYAN))
    print(color("│                 ⚕ Alex Agent Status                  │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    if nous_logged_in:
        ...
        nous_label = "not logged in (run: alex portal)"
    print(
    ...
    print(
        ...
        f"{'logged in' if codex_logged_in else 'not logged in (run: alex model)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if minimax_logged_in else 'not logged in (run: alex auth add minimax-oauth)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if xai_oauth_logged_in else 'not logged in (run: alex auth add xai-oauth)'}"
    )
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: alex model)"
    ...
    if _effective_provider_label() == "LM Studio":
        from alex_cli.models import probe_lmstudio_models
        model_cfg = config.get("model")
    ...
    try:
        from alex_cli.gateway import get_gateway_runtime_snapshot, _format_gateway_pids
    ...
    jobs_file = get_alex_home() / "cron" / "jobs.json"
    if jobs_file.exists():
        import json
        try:
    ...
    sessions_file = get_alex_home() / "sessions" / "sessions.json"
    ...
    print(color("  Run 'alex doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'alex setup' to configure", Colors.DIM))
    print()
