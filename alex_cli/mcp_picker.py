"""MCP picker — interactive `alex mcp picker` (also the default `alex mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``alex mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `alex plugin` picker UX: arrow keys to navigate, ENTER on a row
...
from alex_cli.cli_output import prompt_yes_no
from alex_cli.curses_ui import curses_single_select
from alex_cli.mcp_catalog import (
    CatalogEntry,
    CatalogError,
    catalog_diagnostics,
    install_entry,
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    print(color(
        ...
        "Start a new Alex session for changes to take effect.",
        Colors.GREEN,
    ))
...
def _configure_tools(name: str) -> None:
    ...
    """
    import argparse
    from alex_cli.mcp_config import cmd_mcp_configure
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `alex mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: alex mcp install <name>    Picker: alex mcp",
        Colors.DIM,
    ))

    # Surface manifest-version warnings so users know when their Alex is
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer Alex — run `alex update` "
...
def show_catalog() -> None:
    """`alex mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`alex mcp picker` (and default `alex mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`alex mcp install <name>` — non-interactive entry-point.
    ...
    if entry is None:
        print(color(
            ...
            "Run `alex mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
