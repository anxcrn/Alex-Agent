...
Used by ``alex kanban specify [task_id | --all]``. Takes a task that
...
* This module intentionally mirrors ``alex_cli/goals.py`` — same aux
...
ALEX_KANBAN_SPECIFY_MAX_TOKENS = max(
    1500,
    env_int("ALEX_KANBAN_SPECIFY_MAX_TOKENS", 6000),
)

logger = logging.getLogger(__name__)


_SYSTEM_PROMPT = """You are the Kanban triage specifier for the Alex Agent board.
...
def _profile_author() -> str:
    """Mirror of ``alex_cli.kanban._profile_author``. Kept local to
    avoid a circular import when kanban.py imports this module."""
    return (
        os.environ.get("ALEX_PROFILE")
    ...
    *,
    ...
    try:
        resp = client.chat.completions.create(
            ...
            temperature=0.3,
            max_tokens=ALEX_KANBAN_SPECIFY_MAX_TOKENS,
