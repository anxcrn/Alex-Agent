"""CLI subcommand: ``alex send`` — pipe text from shell scripts to any
...
def _read_message_body(
    ...
    if file_path:
        ...
        try:
            ...
            print(
                f"alex send: {file_path} is not a text file. --file reads the "
                ...
                f'  alex send --to telegram "MEDIA:{file_path}"\n'
                f'  alex send --to telegram "optional caption MEDIA:{file_path}"\n'
                "Add [[as_document]] to deliver an image as an uncompressed file:\n"
                f'  alex send --to telegram "[[as_document]] MEDIA:{file_path}"',
                file=sys.stderr,
            )
            sys.exit(_USAGE_EXIT)
        except OSError as exc:
            print(f"alex send: cannot read {file_path}: {exc}", file=sys.stderr)
            sys.exit(_USAGE_EXIT)
    ...
    *,
    ...
    if json_mode:
        ...
        if payload.get("error"):
            print(f"alex send: {payload['error']}", file=sys.stderr)
...
def _list_targets(platform_filter: Optional[str], *, json_mode: bool) -> int:
    ...
    try:
        ...
        print(f"alex send: failed to load channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT

    try:
        raw = load_directory()
    except Exception as exc:
        print(f"alex send: failed to read channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT
    ...
    if platform_filter:
        ...
        if not filtered:
            print(
                f"alex send: no targets found for platform '{platform_filter}'. "
    ...
    if not any(platforms.values()):
        ...
        print("Set one up with `alex gateway setup`, or run the gateway once so")
        print("channel discovery can populate ~/.alex/channel_directory.json.")
        return _SUCCESS_EXIT
...
def _load_alex_env() -> None:
    """Populate ``os.environ`` from ``~/.alex/.env`` AND bridge top-level
    ...
    startup that ``alex send`` must replicate when invoked standalone:

    1. ``load_dotenv(~/.alex/.env)`` — brings bot tokens into the env.
    2. Bridge top-level simple values from ``~/.alex/config.yaml`` into
    ...
    intentionally reimplement the minimum needed here so ``alex send``
...
def cmd_send(args: argparse.Namespace) -> None:
    ...
    # Bridge ~/.alex/.env and ~/.alex/config.yaml into os.environ so the
    ...
    if not target:
        print(
            "alex send: --to PLATFORM[:channel[:thread]] is required\n"
            "Examples:\n"
            "  alex send --to telegram \"hello\"\n"
            "  alex send --to discord:#ops --file report.md\n"
            "  alex send --list      # list available targets",
            file=sys.stderr,
        )
        sys.exit(_USAGE_EXIT)
    ...
    if message is None or not message.strip():
        print(
            "alex send: no message provided. Pass text as a positional "
    ...
    # Import lazily so `alex send --help` stays fast and does not pull in
...
def register_send_subparser(subparsers) -> argparse.ArgumentParser:
    ...
    parser = subparsers.add_parser(
        ...
        description=(
            "Pipe text from any shell script to any messaging platform Alex "
            "is already configured for. Reuses the gateway's platform "
            "credentials (~/.alex/.env + ~/.alex/config.yaml) — no LLM, "
        ...
        epilog=(
            ...
            "  alex send --to telegram \"MEDIA:/tmp/chart.png\"   # send a media attachment\n"
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="List available targets. Optional positional filter: `alex send --list telegram`.",
    )
