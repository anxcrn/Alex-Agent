"""
Profile management for multiple isolated Alex instances.

Each profile is a fully independent ALEX_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.alex/profiles/<name>/`` by default.

The "default" profile is ``~/.alex`` itself — backward compatible,
zero migration needed.

Usage::

    alex profile create coder          # fresh profile + bundled skills
    alex profile create coder --clone  # also copy config, .env, SOUL.md, skills
    alex profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    alex -p coder chat                 # or via flag
    alex profile use coder             # set as sticky default
    alex profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See alex_constants.get_subprocess_home().
    "home",
]
...
# Marker file written by `alex profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `alex
...
# `alex skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Three categories:
      ...
      2. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         Alex infrastructure directories that only the default profile
         (``~/.alex``) ever contains.  Gated on ``source_dir`` actually
    ...
    is_default_source = source_resolved == _get_default_alex_home().resolve()
...
# Directories/files to exclude when exporting the default (~/.alex) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "alex-agent",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
_RESERVED_NAMES = frozenset({
    "alex", "default", "test", "tmp", "root", "sudo",
})

# Alex subcommands that cannot be used as profile names/aliases
_ALEX_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the alex root, NOT to the current ALEX_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where ALEX_HOME points outside
    ``~/.alex``, profiles live under ``ALEX_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_alex_home() / "profiles"


def _get_default_alex_home() -> Path:
    """Return the default (pre-profile) ALEX_HOME path.

    In standard deployments this is ``~/.alex``.
    In Docker/custom deployments where ALEX_HOME is outside ``~/.alex``
    (e.g. ``/opt/data``), returns ALEX_HOME directly.
    """
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_alex_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the Alex installation itself or a common system binary.  "
            f"Pick a different name."
        )


def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its ALEX_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_alex_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: reserved names, alex subcommands, existing binaries in PATH.
    """
    canon = normalize_profile_name(name)
    if canon in _RESERVED_NAMES:
        return f"'{canon}' is a reserved name"
    if canon in _ALEX_SUBCOMMANDS:
        return f"'{canon}' conflicts with a alex subcommand"
...
def create_wrapper_script(name: str, target: Optional[str] = None) -> Optional[Path]:
    ...
    if is_windows:
        ...
        try:
            alex_exe = shutil.which("alex") or "alex"
            wrapper_path.write_text(f'#!/bin/sh\nexec {shlex.quote(alex_exe)} -p {profile} "$@"\n')
...
def _migrate_profile_config_if_outdated(profile_dir: Path) -> None:
    ...
    ``_config_version``) or that is simply older than the running Alex. If we
    ...
    try:
        ...
        # not be migrated. The next `alex doctor --fix` can still surface the
...
def find_alias_for_profile(profile_name: str) -> Optional[str]:
    ...
    alias whose body invokes ``alex -p <profile>``. When the alias name equals
    the profile name this is trivial, but a custom alias (``alex profile alias
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``alex profile list`` for an unrelated profile.
    """
...
def _check_gateway_running(profile_dir: Path) -> bool:
    ...
    *this* reader — which is exactly the case for a dashboard process that is
    ...
    agree.  Parameterized by ``profile_dir`` so it never mutates ``ALEX_HOME``.
    """
    try:
...
# ``config.yaml``. ``config.yaml`` is the user-facing Alex config
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_alex_home()
...
def profiles_to_serve(multiplex: bool) -> List[Tuple[str, Path]]:
    """Return the ``(profile_name, alex_home)`` pairs a gateway should serve.
    ...
    - ``multiplex=True``: returns the default profile plus every valid named
      profile under ``profiles/``, each paired with its own ALEX_HOME.
    ...
    The returned ``alex_home`` is the path to pass to
    ``set_alex_home_override`` when scoping a turn to that profile.
    """
    active = get_active_profile_name() or "default"
    if not multiplex:
        return [(active, get_profile_dir(active))]

    serve: List[Tuple[str, Path]] = [("default", _get_default_alex_home())]
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``alex update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.alex)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.alex/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
    ...
    # Keys pages, `alex -p <name> auth add`) had no file until first
    ...
    if not env_path.exists():
        try:
            env_path.write_text(
                "# Per-profile secrets for this Alex profile.\n"
    ...
    if not soul_path.exists():
        try:
            from alex_cli.default_soul import DEFAULT_SOUL_MD
            soul_path.write_text(DEFAULT_SOUL_MD, encoding="utf-8")
        except Exception:
            pass  # best-effort — don't fail profile creation over this

    # Write the opt-out marker so seed_profile_skills() and `alex update`'s
    ...
    # Cloned configs can be older than the running Alex (or predate schema
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `alex profile describe`

    # Phase 4: when running inside a container under s6, register the
    # new profile's gateway as a runtime s6 service so
    # `alex -p <profile> gateway start` can supervise it via
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches ALEX_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``alex profile create
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "ALEX_HOME": str(profile_dir)},
...
def backfill_profile_envs(quiet: bool = False) -> List[str]:
    ...
    default_env = _get_default_alex_home() / ".env"
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.alex).\n"
            "To remove everything, use: alex uninstall"
        )
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    ``ALEX_HOME`` and binds the port resolved by ``gateway/config.py``
    ...
    try:
        ...
        from alex_cli.service_manager import get_service_manager
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set ALEX_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("ALEX_HOME")
    try:
        os.environ["ALEX_HOME"] = str(profile_dir)
        from alex_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["ALEX_HOME"] = old_home
        elif "ALEX_HOME" in os.environ:
            del os.environ["ALEX_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.alex/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: alex profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from ALEX_HOME.

    Returns ``"default"`` if ALEX_HOME is not set or points to ``~/.alex``.
    Returns the profile name if ALEX_HOME points into ``~/.alex/profiles/<name>``.
    Returns ``"custom"`` if ALEX_HOME is set to an unrecognized path.
    """
    from alex_constants import get_alex_home
    alex_home = get_alex_home()
    resolved = alex_home.resolve()

    default_resolved = _get_default_alex_home().resolve()
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.alex itself — its parent is ~/ and its
        # directory name is ".alex", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: alex profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.alex itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.alex). "
            "Specify a different name: alex profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="alex_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"alex_{old_name}"
    legacy_old_host = f"alex.{old_name}"
    new_host = f"alex_{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_alex_home() / "honcho.json",
    ...
    for path in candidates:
        ...
        if isinstance(block, dict) and "aiPeer" not in block:
            if source_host.startswith("alex_"):
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a ALEX_HOME path string.

    Called early in the CLI entry point, before any alex modules
    are imported, to set the ALEX_HOME environment variable.
    """
    canon = normalize_profile_name(profile_name)
    validate_profile_name(canon)
    profile_dir = get_profile_dir(canon)
