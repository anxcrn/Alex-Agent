...
manual BotFather token copy-paste. Alex talks only to the Nous onboarding
...
DEFAULT_API_URL = "https://setup.alex-agent.charan vankudoth.com"
...
DEFAULT_MANAGER_BOT = "AlexSetupBot"

DEFAULT_BOT_NAME = "Alex Agent"
...
def generate_username_slug(length: int = 16) -> str:
    ...
    keeping ``alex_<slug>_bot`` under Telegram's 32-character username limit.
    """
...
def generate_bot_username(profile_name: Optional[str] = None) -> str:
    """Generate a secure suggested bot username like ``alex_<slug>_bot``.
    ...
    """
    _ = profile_name
    return f"alex_{generate_username_slug()}_bot"
...
def auto_setup_telegram_bot_result(
    ...
    print()
    print(f"  Contacting Alex Telegram onboarding service: {resolved_api_url}")
    sys.stdout.flush()
    pairing = create_pairing(resolved_api_url)
    if not pairing:
        print("  ✗ Could not reach the Alex Telegram onboarding service.")
