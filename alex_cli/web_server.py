"""
Alex Agent — Web UI server.

Provides a FastAPI backend serving the Vite/React frontend and REST API
endpoints for managing configuration, environment variables, and sessions.

Usage:
    python -m alex_cli.main web          # Start on http://127.0.0.1:9119
    python -m alex_cli.main web --port 8080
"""
...
from alex_cli._subprocess_compat import windows_detach_flags, windows_hide_flags
...
from alex_cli.memory_providers import (
    MemoryProvider,
    ProviderField,
    get_memory_provider,
)
...
try:
    ...
    # running `alex dashboard` needs fastapi+uvicorn; lazy install keeps
...
WEB_DIST = Path(os.environ["ALEX_WEB_DIST"]) if "ALEX_WEB_DIST" in os.environ else Path(__file__).parent / "web_dist"
...
def _start_desktop_cron_ticker(stop_event: "threading.Event", interval: int = 60) -> None:
    ...
    The scheduler tick loop normally lives in ``alex gateway run`` — but the
    desktop app spawns a ``alex dashboard`` backend, not a gateway, so a cron
    ...
    real gateway on the same ALEX_HOME — whichever process grabs the lock
    first wins the tick.
    """
...
def _warm_gateway_module() -> None:
    try:
        import alex_cli.gateway  # noqa: F401
    except Exception:
        pass
...
def _resolve_restart_drain_timeout() -> float:
    try:
        from alex_cli.gateway import _get_restart_drain_timeout
...
async def _lifespan(app: "FastAPI"):
    ...
    # Fire alex_cli.gateway import into a background thread so the event
    # loop is not blocked and ALEX_DASHBOARD_READY fires without delay.
    ...
    # Desktop-spawned backends (ALEX_DESKTOP=1) fire cron jobs themselves,
    # since the app has no gateway running the scheduler. Server `alex
    # dashboard` is unaffected — it relies on its own gateway.
    cron_stop: "threading.Event | None" = None
    cron_thread: "threading.Thread | None" = None
    if os.getenv("ALEX_DESKTOP") == "1":
...
app = FastAPI(title="Alex Agent", version=__version__, lifespan=_lifespan)
...
# ALEX_DASHBOARD_SESSION_TOKEN so its main process can authenticate the
...
# ---------------------------------------------------------------------------
_SESSION_TOKEN = os.environ.get("ALEX_DASHBOARD_SESSION_TOKEN") or secrets.token_urlsafe(32)
_SESSION_HEADER_NAME = "X-Alex-Session-Token"
...
#
# This list is defined in ``alex_cli.dashboard_auth.public_paths`` so the
...
# ---------------------------------------------------------------------------
from alex_cli.dashboard_auth.public_paths import (
...
def _require_token(request: Request) -> None:
    ...
    * **Loopback / ``--insecure`` mode** (``auth_required`` False): the
      ...
      back via ``X-Alex-Session-Token`` (or the legacy ``Bearer`` header).
      Validate it here.
...
def should_require_auth(host: str, allow_public: bool = False) -> bool:
    ...
    unauthenticated-public-dashboard hole behind the June 2026 ``alex-0day``
...
async def _dashboard_auth_gate(request: Request, call_next):
    from alex_cli.dashboard_auth.middleware import gated_auth_middleware
...
async def _token_auth_seam(request: Request, call_next):
    ...
    through untouched.
    """
    from alex_cli.dashboard_auth.token_auth import token_auth_middleware
...
_SCHEMA_OVERRIDES: Dict[str, Dict[str, Any]] = {
    ...
    "updates.non_interactive_local_changes": {
        "type": "select",
        "description": (
            "When the chat app / gateway updates Alex (no terminal prompt), "
...
class ModelAssignment(BaseModel):
    ...
    # ``alex model`` custom flow collects. Honored only on the main slot for
...
def _normalize_main_model_assignment(provider: str, model: str) -> tuple[str, str]:
    ...
    - The "Change" picker sends a real Alex provider slug — fine.
    ...
    1. Vendor-name → Alex-provider mapping: when the provider string is
       not a known Alex provider/alias (e.g. ``moonshotai``, ``x-ai`` is
    ...
    """
    from alex_cli.models import _KNOWN_PROVIDER_NAMES, normalize_provider
    from alex_cli.model_normalize import normalize_model_for_provider
    ...
    if canonical not in _KNOWN_PROVIDER_NAMES and "/" in model_in:
        ...
        try:
            ...
            cur_provider = ""
        from alex_cli.models import _AGGREGATOR_PROVIDERS
...
_MANAGED_FILES_ROOT_ENV = "ALEX_DASHBOARD_FILES_ROOT"
...
def _default_alex_root_is_opt_data() -> bool:
    raw = os.environ.get("ALEX_HOME", "").strip()
    if not raw:
        return False
    try:
        from alex_constants import get_default_alex_root

        root = get_default_alex_root().expanduser().resolve(strict=False)
    except (OSError, RuntimeError):
...
def _dashboard_local_update_managed_externally() -> bool:
    """Return true when the dashboard should not offer ``alex update``.
    ...
    a container — e.g. the alex-webui image sharing the Alex source tree),
    the dashboard's ``alex update`` button is the correct update path and
    ...
    """
    if _default_alex_root_is_opt_data():
        return True
    try:
        from alex_constants import is_container
...
def _managed_files_policy(request: Request, *, create_root: bool = True) -> ManagedFilesPolicy:
    ...
    # to /opt/data only when the installation's Alex root is actually /opt/data
    # (the container/hosted layout) or when ALEX_DASHBOARD_FILES_ROOT is set.
    if _default_alex_root_is_opt_data():
...
async def fs_write_text(payload: FsWriteText):
    ...
    Mirrors the local Electron ``alex:fs:writeText`` hardening: the path is
    ...
    tmp = target.with_name(f".{target.name}.alex-tmp-{os.getpid()}")
    try:
...
async def get_status(profile: Optional[str] = None):
    ...
    # across that await. Status only resolves get_alex_home() at call time
    ...
    try:
        ...
        # Windows install the first import of alex_cli.gateway blocks the
        ...
        try:
            from alex_cli.dashboard_auth import list_providers as _list_providers
        ...
        status = {
            ...
            "can_update_alex": not _dashboard_local_update_managed_externally(),
            "gateway_running": gateway_running,
            "gateway_state": gateway_state,
...
async def get_system_stats():
    ...
    and non-sensitive (no env values, no paths beyond the alex home root).
    """
    ...
    try:
        ...
        try:
            du = psutil.disk_usage(str(get_alex_home()))
            info["disk"] = {
                "total": du.total,
                "used": du.used,
                "free": du.free,
...
async def run_curator():
    ...
    try:
        proc = _spawn_alex_action(["curator", "run"], "curator-run")
    except Exception as exc:
...
async def get_portal_status():
    ...
    auth: Dict[str, Any] = {}
    try:
        from alex_cli.auth import get_nous_auth_status
    ...
    features = []
    try:
        from alex_cli.nous_subscription import get_nous_subscription_features
...
async def run_prompt_size():
    try:
        proc = _spawn_alex_action(["prompt-size"], "prompt-size")
    except Exception as exc:
...
async def run_dump():
    try:
        proc = _spawn_alex_action(["dump"], "dump")
    except Exception as exc:
        raise HTTPException(status_code=500, detail=f"Failed: {exc}")
...
async def run_config_migrate():
    try:
        proc = _spawn_alex_action(["config", "migrate"], "config-migrate")
    except Exception as exc:
        raise HTTPException(status_code=500, detail=f"Failed: {exc}")
...
async def run_debug_share_endpoint(body: DebugShareRequest | None = None):
    ...
    *synchronous*: the whole point of ``debug share`` is the set of shareable
    ...
    """
    from alex_cli.debug import build_debug_share
...
# streamed to a per-action log file under ``~/.alex/logs/<action>.log`` so
# the dashboard can tail them back to the user.
# ---------------------------------------------------------------------------

_ACTION_LOG_DIR: Path = get_alex_home() / "logs"
...
def _spawn_alex_action(subcommand: List[str], name: str) -> subprocess.Popen:
    """Spawn ``alex <subcommand>`` detached and record the Popen handle.

    Uses the running interpreter's ``alex_cli.main`` module so the action
    ...
    cmd = [_dashboard_spawn_executable(), "-m", "alex_cli.main", *subcommand]
    ...
    popen_kwargs: Dict[str, Any] = {
        ...
        "env": {**os.environ, "ALEX_NONINTERACTIVE": "1"},
    }
    if sys.platform == "win32":
...
def _gateway_display_command(profile: Optional[str], verb: str) -> str:
    return " ".join(["alex", *_gateway_subcommand(profile, verb)])
...
def _spawn_gateway_restart(profile: Optional[str] = None) -> Tuple[subprocess.Popen, bool]:
    """Spawn ``alex gateway restart``, reusing an in-flight restart.
    ...
    concurrent ``alex gateway restart`` children race each other on the
    ...
    return _spawn_alex_action(subcommand, "gateway-restart"), False
...
async def restart_gateway(profile: Optional[str] = None):
    """Kick off a ``alex gateway restart`` in the background."""
    try:
...
async def gateway_drain(request: Request):
    ...
    active (no ``ALEX_DASHBOARD_DRAIN_SECRET``), the route is NOT a token
...
@app.post("/api/alex/update")
async def update_alex():
    """Kick off ``alex update`` in the background."""
    if _dashboard_local_update_managed_externally():
        message = (
            "Alex updates are managed outside this dashboard in "
            "containerized environments. The built-in local updater is "
            "disabled here."
        )
        _record_completed_action("alex-update", message, exit_code=1)
        return {
            "ok": False,
            "pid": None,
            "name": "alex-update",
    ...
    try:
        proc = _spawn_alex_action(["update"], "alex-update")
    except Exception as exc:
        _log.exception("Failed to spawn alex update")
...
@app.get("/api/alex/update/check")
async def check_alex_update(force: bool = False):
    """Report whether a Alex update is available, without applying it.

    Powers the dashboard's "check before you update" flow: the System page
    shows the commit-behind count and asks the user to confirm before
    ``POST /api/alex/update`` actually runs ``alex update``.

    Returns:
        install_method: 'git' | 'pip' | 'docker' | 'nixos' | 'homebrew' | ...
        current_version: installed Alex version string
    ...
    try:
        ...
        if force:
            try:
                (get_alex_home() / ".update_check").unlink()
            except OSError:
                pass
...
async def speak_text(payload: TTSSpeakRequest):
    ...
    configured in ``~/.alex/config.yaml`` under ``tts.``.
    """
...
async def get_profiles_sessions(
    ...
    from alex_state import SessionDB
    from alex_cli import profiles as profiles_mod
...
async def search_sessions(q: str = "", limit: int = 20, profile: Optional[str] = None):
    ...
    try:
        ...
        try:
            ...
            # logs, or another Alex surface. FTS can't find those unless the
...
def _normalize_config_for_web(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Alex supports ``model`` as either a bare string (``"anthropic/claude-sonnet-4"``)
...
def _memory_provider_config_path(provider: MemoryProvider) -> Path:
    return get_alex_home() / provider.name / "config.json"
...
# in alex_cli/config.py — listed here for deterministic ordering in the UI.
...
def get_model_options(profile: Optional[str] = None, refresh: bool = False):
    ...
    """
    try:
        from alex_cli.inventory import build_models_payload, load_picker_context
        ...
        # onboarding picker) sees the SAME full provider universe `alex model`
...
def get_recommended_default_model(provider: str = ""):
    ...
    Mirrors the model-curation `alex model` does so GUI onboarding lands on a
    ...
    if slug == "nous":
        try:
            ...
            from alex_cli.auth import get_provider_auth_state
...
def get_moa_models(profile: Optional[str] = None):
    ...
    try:
        from alex_cli.moa_config import normalize_moa_config
...
async def set_model_assignment(body: ModelAssignment, profile: Optional[str] = None):
    ...
    Writes to ``~/.alex/config.yaml`` — applies to **new** sessions only.
    ...
    try:
        ...
        if model and not body.confirm_expensive_model:
            try:
                from alex_cli.model_cost_guard import expensive_model_warning
...
def _apply_model_assignment_sync(
    ...
    if scope == "main":
        ...
        # post-model-selection behaviour (alex_cli/main.py
        ...
        if provider.strip().lower() == "nous":
            try:
                from alex_cli.nous_subscription import apply_nous_managed_defaults
                from alex_cli.tools_config import _get_platform_tools
        ...
        # endpoint, mirroring the ``alex model`` custom flow
        ...
        if provider.strip().lower() in {"custom", "local"} and base_url:
            try:
                from alex_cli.main import _auto_provider_name, _save_custom_provider
...
def _catalog_provider_env_metadata() -> dict:
    ...
    (i.e. the ``alex model`` universe). This is what lets the desktop Keys tab
    ...
    """
    try:
        from alex_cli.provider_catalog import provider_catalog
    except Exception:
        return {}
    ...
    try:
        from alex_cli.config import OPTIONAL_ENV_VARS as _OPT
    except Exception:
        _OPT = {}
    _non_provider_keys = {
    ...
    for d in provider_catalog():
        ...
        # Keys tab (otherwise Bedrock — a `alex model` provider — would be
...
async def get_env_vars(profile: Optional[str] = None):
    ...
    def _row(var_name: str, info: dict) -> dict:
        ...
        return {
            ...
            # CLI `alex model` picker uses (not desktop-only prefix guesses).
...
_PLATFORM_OVERRIDES: dict[str, dict[str, Any]] = {
    "telegram": {
        "name": "Telegram",
        "description": "Run Alex from Telegram DMs, groups, and topics.",
    ...
    "discord": {
        "name": "Discord",
        "description": "Connect Alex to Discord DMs, channels, and threads.",
    ...
    "slack": {
        "name": "Slack",
        "description": "Use Alex from Slack via Socket Mode. Add allowed Slack member IDs so connected bots can respond.",
    ...
    "mattermost": {
        "name": "Mattermost",
        "description": "Connect Alex to Mattermost channels and direct messages.",
    ...
    "matrix": {
        "name": "Matrix",
        "description": "Use Alex in Matrix rooms and direct messages.",
    ...
    "whatsapp": {
        "name": "WhatsApp",
        "description": "Use Alex through the bundled WhatsApp bridge with QR-based auth.",
    ...
    "homeassistant": {
        "name": "Home Assistant",
        "description": "Control your smart home from Alex via Home Assistant.",
    ...
    "dingtalk": {
        "name": "DingTalk",
        "description": "Connect Alex to DingTalk groups (钉钉).",
    ...
    "feishu": {
        "name": "Feishu / Lark",
        "description": "Use Alex inside Feishu / Lark.",
    ...
    "bluebubbles": {
        ...
        "description": "Use Alex through iMessage via a BlueBubbles server.",
    ...
    "qqbot": {
        "name": "QQ Bot",
        "description": "Connect Alex to a QQ Bot from the QQ Open Platform.",
    ...
    "yuanbao": {
        "name": "Yuanbao (元宝)",
        "description": "Connect Alex to Tencent Yuanbao.",
        "docs_url": "",
        "required_env": (),
    },
    "api_server": {
...
_MESSAGING_ENV_FALLBACKS: dict[str, dict[str, Any]] = {
    ...
    "WEIXIN_ACCOUNT_ID": {
        "description": "iLink Bot account ID obtained through QR login in alex gateway setup",
        "prompt": "iLink Bot account ID",
    },
    "WEIXIN_TOKEN": {
        "description": "iLink Bot token obtained through QR login in alex gateway setup",
...
async def start_telegram_onboarding(body: TelegramOnboardingStart):
    bot_name = (body.bot_name or "Alex Agent").strip() or "Alex Agent"
...
def _restart_gateway_after_telegram_onboarding(profile: Optional[str] = None) -> dict[str, Any]:
    ...
    saved token waits on a separate dashboard restart click, Alex appears
...
async def get_messaging_platforms(profile: Optional[str] = None):
    ...
    # all resolve against the requested profile's ALEX_HOME.
...
# the canonical ``alex auth add <provider>`` command so the dashboard
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    1. ``~/.alex/.anthropic_oauth.json`` — Alex-managed PKCE flow (what
    ...
    try:
        from agent.anthropic_adapter import (
            read_alex_oauth_credentials,
            _ALEX_OAUTH_FILE,
        )
    except ImportError:
        read_alex_oauth_credentials = None  # type: ignore
        _ALEX_OAUTH_FILE = None  # type: ignore

    alex_creds = None
    if read_alex_oauth_credentials:
        try:
            alex_creds = read_alex_oauth_credentials()
        except Exception:
            alex_creds = None
    if alex_creds and alex_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "alex_pkce",
            "source_label": f"Alex PKCE ({_ALEX_OAUTH_FILE})",
            "token_preview": _truncate_token(alex_creds.get("accessToken")),
            "expires_at": alex_creds.get("expiresAt"),
            "has_refresh_token": bool(alex_creds.get("refreshToken")),
        }
    ...
    try:
        ...
        pass
    try:
        from alex_cli.config import get_env_value
    except ImportError:
...
def _copilot_acp_status() -> Dict[str, Any]:
    ...
    Alex never claims a login state it can't verify.
    """
    return {
        "logged_in": False,
        "source": "copilot_cli",
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "alex auth add openai-codex",
    ...
    },
    {
        ...
        "flow": "external",
        "cli_command": "alex auth add qwen-oauth",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "alex auth add minimax-oauth",
    ...
    # usage credits on top of a Claude Max plan — see disclaimer in name).
    {
        ...
        "flow": "pkce",
        "cli_command": "alex auth add anthropic",
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        from alex_cli import auth as hauth
        if provider_id == "nous":
...
def _oauth_provider_disconnect_command(provider: Dict[str, Any]) -> Optional[str]:
    ...
    External providers store their credentials outside Alex, so the disconnect
...
def _oauth_provider_disconnect_hint(provider: Dict[str, Any], status: Dict[str, Any]) -> Optional[str]:
    ...
    if provider.get("flow") == "external":
        if _oauth_provider_disconnect_command(provider):
            ...
            return "Managed outside Alex — run the disconnect command to remove it."
...
def _build_oauth_catalog() -> list[Dict[str, Any]]:
    ...
    MEMBERSHIP is the union of:
      ...
      2. every accounts-tab provider in the unified ``provider_catalog()`` (the
         ``alex model`` universe) — so any OAuth/external provider added as a
    ...
    #    in lockstep with the `alex model` universe (zero-edit for new plugins).
    try:
        from alex_cli.provider_catalog import provider_catalog
        for d in provider_catalog():
            ...
            rows.append({
                ...
                "name": d.label,
                "flow": "external",
                "cli_command": f"alex auth add {d.slug}",
...
async def list_oauth_providers(profile: Optional[str] = None):
    ...
    Response shape (per provider):
        ...
        status:
          ...
          source           short slug ("alex_pkce", "claude_code", ...)
    ...
    sync with the `alex model` picker; _OAUTH_OVERRIDES supplies per-provider
...
async def disconnect_oauth_provider(
    ...
    with _profile_scope(profile):
        ...
        # Anthropic clears only the Alex-managed PKCE file and auth-store entry.
        ...
        if provider_id == "anthropic":
            cleared = False
            try:
                from agent.anthropic_adapter import _ALEX_OAUTH_FILE
                if _ALEX_OAUTH_FILE.exists():
                    _ALEX_OAUTH_FILE.unlink()
                    cleared = True
            except Exception:
                pass
            # Also clear the credential pool entry if present.
            try:
                from alex_cli.auth import clear_provider_auth
        ...
        try:
            from alex_cli.auth import clear_provider_auth, invalidate_nous_auth_status_cache
...
#          → persists to ~/.alex/.anthropic_oauth.json AND credential pool
...
# Guarded so alex web still starts if anthropic_adapter is unavailable;
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    """Persist Anthropic PKCE creds to both Alex file AND credential pool.

    Mirrors what auth_commands.add_command does so the dashboard flow leaves
    the system in the same state as ``alex auth add anthropic``.
    """
    from agent.anthropic_adapter import _ALEX_OAUTH_FILE
    payload = {
    ...
    _ALEX_OAUTH_FILE.parent.mkdir(parents=True, exist_ok=True)
    tmp_path = _ALEX_OAUTH_FILE.with_name(
        f"{_ALEX_OAUTH_FILE.name}.tmp.{os.getpid()}.{secrets.token_hex(8)}"
    )
    try:
        ...
        os.replace(tmp_path, _ALEX_OAUTH_FILE)
        try:
            _ALEX_OAUTH_FILE.chmod(stat.S_IRUSR | stat.S_IWUSR)
        except OSError:
            pass
    finally:
        try:
...
async def _start_device_code_flow(
    ...
    if provider_id == "nous":
        ...
        portal_base_url = (
            os.getenv("ALEX_PORTAL_BASE_URL")
...
# the tokens exactly like `alex auth add xai-oauth`.
...
def _xai_loopback_worker(session_id: str) -> None:
    ...
    try:
        ...
        base_url = hauth._xai_validate_inference_base_url(
            os.getenv("ALEX_XAI_BASE_URL", "").strip().rstrip("/")
...
def _add_xai_oauth_pool_entry(
    ...
    """Mirror `alex auth add xai-oauth`'s credential-pool insert.
...
def _nous_poller(session_id: str) -> None:
    ...
    try:
        ...
        with _profile_scope(_oauth_session_profile(session_id)):
            ...
            from alex_cli.auth import persist_nous_credentials
...
def _codex_full_login_worker(session_id: str) -> None:
    ...
    try:
        ...
        from alex_cli.auth import _save_codex_tokens
...
async def get_session_stats(profile: Optional[str] = None):
    """Session-store statistics for the Sessions page (mirrors `alex sessions stats`).
...
async def prune_sessions_endpoint(body: SessionPrune):
    """Delete ended sessions older than N days (mirrors `alex sessions prune`)."""
    if body.older_than_days < 1:
        raise HTTPException(status_code=400, detail="older_than_days must be >= 1")
    profile_home = _cron_profile_home(body.profile)[1] if body.profile else get_alex_home()
    db = _open_session_db_for_profile(body.profile)
    try:
...
async def list_artifacts():
    try:
        from pathlib import Path
        artifacts_dir = Path(get_alex_home()) / "artifacts"
        if not artifacts_dir.exists():
            return {"artifacts": []}
        
        files = []
        for p in artifacts_dir.glob("*.md"):
            try:
                title = ""
...
async def get_artifact(name: str):
    try:
        from pathlib import Path
        artifacts_dir = Path(get_alex_home()) / "artifacts"
        file_path = artifacts_dir / f"{name}.md"
...
async def save_artifact(request: Request, body: dict):
    try:
        from pathlib import Path
        name = body.get("name", "")
        ...
        file_path = artifacts_dir / f"{safe_name}.md"
        ...
        try:
            import json
            event_channels, event_lock = _get_event_state(request.app)
            async with event_lock:
                active_channels = list(event_channels.keys())
            for ch in active_channels:
                await _broadcast_event(request.app, ch, json.dumps({
...
# ---------------------------------------------------------------------------
# Dialogs endpoints (Interactive UI Modals)
# ---------------------------------------------------------------------------


@app.get("/api/v1/dialogs/pending")
async def list_pending_dialogs():
    try:
        from pathlib import Path
        import json
        dialogs_dir = Path(get_alex_home()) / "dialogs"
        if not dialogs_dir.exists():
            return {"dialogs": []}
            
        pending = []
        for p in dialogs_dir.glob("*.json"):
            try:
...
@app.post("/api/v1/dialogs/{dialog_id}/resolve")
async def resolve_dialog(dialog_id: str, body: dict):
    try:
        from pathlib import Path
        import json
        dialogs_dir = Path(get_alex_home()) / "dialogs"
        ...
        if not file_path.exists():
            raise HTTPException(status_code=404, detail="Dialog not found or already resolved")
            
        try:
            data = json.loads(file_path.read_text(encoding="utf-8"))
        except Exception:
            raise HTTPException(status_code=500, detail="Failed to read dialog state")
            
        data["status"] = "resolved"
        if data.get("type") == "question":
            data["answer"] = body.get("answer")
        elif data.get("type") == "permission":
            data["granted"] = body.get("granted", False)
...
async def get_logs(
    ...
    from alex_cli.logs import _read_tail, LOG_FILES

    log_name = LOG_FILES.get(file)
    if not log_name:
        raise HTTPException(status_code=400, detail=f"Unknown log file: {file}")
    log_path = get_alex_home() / "logs" / log_name
    if not log_path.exists():
        return {"file": file, "lines": []}

    try:
        from alex_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
...
def _cron_profile_home(profile: Optional[str]) -> Tuple[str, Path]:
    """Resolve a profile query value to (profile_name, ALEX_HOME)."""
    from alex_cli import profiles as profiles_mod
...
def _call_cron_for_profile(target_profile: Optional[str], func_name: str, *args, **kwargs):
    ...
    from the process ALEX_HOME at import time. The dashboard is a single
    ...
    with _CRON_PROFILE_LOCK:
        ...
        from alex_constants import (
            reset_alex_home_override,
            set_alex_home_override,
        )

        old_cron_dir = cron_jobs.CRON_DIR
        old_jobs_file = cron_jobs.JOBS_FILE
        old_output_dir = cron_jobs.OUTPUT_DIR
        token = set_alex_home_override(str(home))
        ...
        try:
            ...
            reset_alex_home_override(token)
...
#
# Wraps the same config data layer the CLI uses (alex_cli.mcp_config), so
# servers managed here show up under `alex mcp list` and vice versa.  Secrets
...
async def list_mcp_servers(profile: Optional[str] = None):
    from alex_cli.mcp_config import _get_mcp_servers
...
async def add_mcp_server(body: MCPServerCreate, profile: Optional[str] = None):
    from alex_cli.mcp_config import _get_mcp_servers, _save_mcp_server
...
async def remove_mcp_server(name: str, profile: Optional[str] = None):
    from alex_cli.mcp_config import _remove_mcp_server
...
async def test_mcp_server(name: str, profile: Optional[str] = None):
    ...
    from alex_cli.mcp_config import _get_mcp_servers, _probe_single_server
    ...
    def _probe_scoped():
        ...
        # ALEX_HOME override (see mcp_tool._wrap_with_home_override), so
...
async def install_mcp_catalog_entry(body: MCPCatalogInstall, profile: Optional[str] = None):
    ...
    # The -p subprocess rebinds ALEX_HOME-derived paths in the child.
    if entry.install is not None:
        try:
            proc = _spawn_alex_action(
...
#
# Wraps the same JSON store the CLI uses (alex_cli.webhook); the webhook
...
# spawn the real `alex gateway <verb>` so behaviour matches the CLI exactly.
...
async def start_gateway(profile: Optional[str] = None):
    try:
        proc = _spawn_alex_action(_gateway_subcommand(profile, "start"), "gateway-start")
    except HTTPException:
        raise
    except Exception as exc:
...
async def stop_gateway(profile: Optional[str] = None):
    try:
        proc = _spawn_alex_action(_gateway_subcommand(profile, "stop"), "gateway-stop")
    except HTTPException:
        raise
    except Exception as exc:
...
async def list_credential_pool():
    ...
    from alex_cli.auth import read_credential_pool
...
# `alex memory setup`).  The dashboard covers the common admin actions:
...
async def set_memory_provider(body: MemoryProviderSelect):
    ...
    if provider:
        ...
        if provider not in valid:
            raise HTTPException(
                status_code=400,
                detail=f"Unknown memory provider '{provider}'. Run `alex memory setup` to configure a new one.",
            )
...
async def run_doctor():
    try:
        proc = _spawn_alex_action(["doctor"], "doctor")
    except Exception as exc:
...
async def run_security_audit():
    try:
        proc = _spawn_alex_action(["security", "audit"], "security-audit")
    except Exception as exc:
...
async def run_backup(body: BackupRequest):
    ...
    try:
        proc = _spawn_alex_action(args, "backup")
    except Exception as exc:
...
class ImportRequest(BaseModel):
    archive: str
    # Pass --force to `alex import`. The spawned action runs with
...
async def run_import(body: ImportRequest):
    ...
    if body.force:
        args.append("--force")
    try:
        proc = _spawn_alex_action(args, "import")
    except Exception as exc:
...
async def list_hooks():
    ...
    form can offer them.
    """
    from alex_cli.config import load_config as _load_config
    from agent import shell_hooks

    try:
        from alex_cli.plugins import VALID_HOOKS
...
async def list_checkpoints():
    ...
    # Checkpoints live under <alex_home>/checkpoints/.  Surface a count +
    ...
    cp_dir = get_alex_home() / "checkpoints"
    sessions = []
    total_bytes = 0
    if cp_dir.is_dir():
...
async def prune_checkpoints():
    try:
        proc = _spawn_alex_action(["checkpoints", "prune"], "checkpoints-prune")
    except Exception as exc:
...
def _profile_cli_args(profile: Optional[str]) -> List[str]:
    ...
    Hub install/uninstall/update run in a fresh ``alex`` subprocess, and
...
async def preview_skill_hub(identifier: str = ""):
    ...
    def _run():
        from alex_cli.skills_hub import _resolve_source_meta_and_bundle
        from tools.skills_hub import create_source_router
...
class ProfileCreate(BaseModel):
    ...
    # via a subprocess scoped to the profile (`alex -p <name> skills install`)
    # because skills_hub.SKILLS_DIR is import-time-bound and the ALEX_HOME
...
def _fallback_profile_dicts(profiles_mod) -> List[Dict[str, Any]]:
    ...
    default_home = profiles_mod._get_default_alex_home()
...
def _profile_setup_command(name: str) -> str:
    ...
    return "alex setup" if name == "default" else f"{name} setup"
...
def _write_profile_model(profile_dir: Path, provider: str, model: str) -> None:
    ...
    context-local ALEX_HOME override so the write lands in the target
    ...
    """
    from alex_constants import set_alex_home_override, reset_alex_home_override

    token = set_alex_home_override(str(profile_dir))
    try:
        provider, model = _normalize_main_model_assignment(provider, model)
        cfg = load_config()
...
def _write_profile_mcp_servers(profile_dir: Path, servers: List["MCPServerCreate"]) -> int:
    ...
    Scopes ``load_config``/``save_config`` to ``profile_dir`` via the
    context-local ALEX_HOME override (same mechanism as
    ...
    """
    from alex_constants import set_alex_home_override, reset_alex_home_override
    from alex_cli.mcp_security import validate_mcp_server_entry
...
def _disable_unselected_skills(profile_dir: Path, keep: List[str]) -> int:
    ...
    install.) Scoped to the profile via the ALEX_HOME override. Returns the
    number of skills newly disabled.
    """
    from alex_constants import set_alex_home_override, reset_alex_home_override
    from alex_cli.skills_config import get_disabled_skills, save_disabled_skills
...
async def create_profile_endpoint(body: ProfileCreate):
    ...
    # profile's ALEX_HOME at import). Returns PIDs for the UI to poll.
...
async def get_active_profile_endpoint():
    ...
    ``active`` is the sticky default written by ``alex profile use`` —
    the profile new CLI invocations pick up. ``current`` is the profile
    the running dashboard/gateway is scoped to (derived from ALEX_HOME).
    """
    from alex_cli import profiles as profiles_mod
    try:
...
async def set_active_profile_endpoint(body: ProfileActiveUpdate):
    """Set the sticky active profile (mirrors ``alex profile use``).
...
async def update_profile_model_endpoint(name: str, body: ProfileModelUpdate):
    ...
    to the named profile via the ALEX_HOME override.
    """
    profile_dir = _resolve_profile_dir(name)
    provider = (body.provider or "").strip()
    model = (body.model or "").strip()
...
async def describe_profile_auto_endpoint(name: str, body: ProfileDescribeAuto):
    ...
    (``auxiliary.profile_describer``). Mirrors ``alex profile describe
    <name> --auto``.
    ...
    _resolve_profile_dir(name)
    try:
        from alex_cli import profile_describer
...
def _profile_scope(profile: Optional[str]):
    ...
    1. ``load_config``/``save_config`` resolve ``get_alex_home()`` at call
       time — the context-local override from ``set_alex_home_override``
    ...
    retargeted to the *current* ``get_alex_home()`` so writes land in the
    live home even when the import-time binding is stale (e.g. the process
    imported the modules before a ALEX_HOME override, or under test
    isolation).
    """
    requested = (profile or "").strip()
...
def _config_profile_scope(profile: Optional[str]):
    ...
    ``set_alex_home_override`` contextvar — it does NOT swap the
    ...
    which is all endpoints that resolve ``get_alex_home()`` at call time
...
async def get_skills(profile: Optional[str] = None):
    ...
    from alex_cli.skills_config import get_disabled_skills
    with _profile_scope(profile):
        config = load_config()
...
async def get_toolsets(profile: Optional[str] = None):
    from alex_cli.tools_config import (
...
async def toggle_toolset(name: str, body: ToolsetToggle, profile: Optional[str] = None):
    ...
    helper the CLI ``alex tools`` picker uses, so the GUI and CLI stay in
...
async def get_toolset_config(name: str, profile: Optional[str] = None):
    ...
    Surfaces the same provider rows the CLI ``alex tools`` picker shows
...
async def select_toolset_provider(
    ...
    core extracted from the CLI configurator — so the GUI and ``alex tools``
...
async def save_toolset_env(name: str, body: ToolsetEnvUpdate, profile: Optional[str] = None):
    ...
    Writes each ``key: value`` to ``~/.alex/.env`` via ``save_env_value`` —
    the same store ``alex tools`` writes when it prompts for keys. Keys are
...
async def run_toolset_post_setup(
    ...
    ``alex tools post-setup <key>`` and the frontend tails the log via
    ...
    ``profile`` spawns the hook as ``alex -p <profile> tools post-setup``.
    ...
    write per-profile state must see the same ALEX_HOME the rest of the
...
# (which attach to cua-driver's OWN identity, com.trycua.driver — not Alex,
...
async def grant_computer_use_permissions(profile: Optional[str] = None):
    """Spawn ``alex computer-use permissions grant`` as a background action.
...
#
# The endpoint spawns the same ``alex --tui`` binary the CLI uses, behind
...
if sys.platform.startswith("win"):
    try:
        from alex_cli.win_pty_bridge import WinPtyBridge as PtyBridge, PtyUnavailableError
    ...
    try:
        from alex_cli.pty_bridge import PtyBridge, PtyUnavailableError
        _PTY_BRIDGE_AVAILABLE = True
...
def _ws_auth_reason(ws: "WebSocket") -> tuple[Optional[str], str]:
    ...
    * ``?internal=<process-credential>`` — the process-lifetime internal
    ...
    if auth_required:
        ...
        from alex_cli.dashboard_auth.audit import AuditEvent, audit_log
        from alex_cli.dashboard_auth.ws_tickets import (
            TicketInvalid,
...
def _resolve_chat_argv(
    ...
    Default: whatever ``alex --tui`` would run.  Tests monkeypatch this
    function to inject a tiny fake command (``cat``, ``sh -c 'printf …'``)
    so nothing has to build Node or the TUI bundle.

    Session resume is propagated via the ``ALEX_TUI_RESUME`` env var —
    matching what ``alex_cli.main._launch_tui`` does for the CLI path.
    Appending ``--resume <id>`` to argv doesn't work because ``ui-tui`` does
    not parse its argv.

    ``ALEX_TUI_GATEWAY_URL`` is injected so the PTY child can attach to
    this process's in-memory ``tui_gateway`` instance instead of spawning
    its own Python gateway subprocess.

    `sidecar_url` (when set) is forwarded as ``ALEX_TUI_SIDECAR_URL`` so
    the spawned ``tui_gateway.entry`` can mirror dispatcher emits to the
    dashboard's ``/api/pub`` endpoint (see :func:`pub_ws`).

    `active_session_file` (when set) is forwarded as
    ``ALEX_TUI_ACTIVE_SESSION_FILE``. The TUI writes the current session id
    ...
    ``ALEX_HOME`` at the profile dir in the child env. Every spawned
    process (the TUI and the ``tui_gateway.entry`` it launches) resolves
    ``get_alex_home()`` from that env var at its own import, so the child
    binds the profile's config, skills, memory, and state.db from the start
    — the same propagation ``alex -p <name>`` performs. The in-process
    ``ALEX_TUI_GATEWAY_URL`` attach is SKIPPED for scoped chats: the
    ...
    """
    from alex_cli.main import PROJECT_ROOT, _make_tui_argv
    ...
    env.setdefault("ALEX_TUI_DISABLE_MOUSE", "1")
    env.setdefault("ALEX_TUI_INLINE", "1")
    env["ALEX_TUI_DASHBOARD"] = "1"

    if profile_dir is not None:
        env["ALEX_HOME"] = str(profile_dir)

    if resume:
        latest_resume, _latest_path = _session_latest_descendant(resume)
        if latest_resume:
            resume = latest_resume
        env["ALEX_TUI_RESUME"] = resume

    if sidecar_url:
        env["ALEX_TUI_SIDECAR_URL"] = sidecar_url
    ...
    # inherits the profile ALEX_HOME set above.
    if profile_dir is None:
        if gateway_ws_url := _build_gateway_ws_url():
            env["ALEX_TUI_GATEWAY_URL"] = gateway_ws_url
...
def _build_gateway_ws_url() -> Optional[str]:
    ...
    if getattr(app.state, "auth_required", False):
        from alex_cli.dashboard_auth.ws_tickets import internal_ws_credential
...
def _active_session_file_for_channel(app: "FastAPI", channel: str) -> Path:
    ...
    fd, raw_path = tempfile.mkstemp(prefix="alex-pty-active-", suffix=".json")
    os.close(fd)
    path = Path(raw_path)
    files[channel] = path
    return path
...
async def pty_ws(ws: WebSocket) -> None:
    ...
    if not _PTY_BRIDGE_AVAILABLE:
        await ws.send_text(
            ...
            "\x1b[33mInstall Alex inside WSL2 to use the dashboard's /chat "
...
# ALEX_TUI_SIDECAR_URL set in /api/pty's PTY env) and writes every
...
def _normalise_prefix(raw: Optional[str]) -> str:
    ...
    Thin re-export of :func:`alex_cli.dashboard_auth.prefix.normalise_prefix`
    ...
    """
    from alex_cli.dashboard_auth.prefix import normalise_prefix
...
def mount_spa(application: FastAPI):
    ...
    ``mission-control.tilos.com/alex/*`` -> local Caddy -> :9119), the
    proxy injects ``X-Forwarded-Prefix: /alex`` on every request. We
    rewrite the served ``index.html`` so absolute asset URLs (``/assets/...``)
    and the SPA's runtime ``__ALEX_BASE_PATH__`` honour that prefix
    ...
    def _serve_index(prefix: str = ""):
        ...
        ``prefix`` is the normalised ``X-Forwarded-Prefix`` (e.g. ``/alex``)
        ...
        ``__ALEX_AUTH_REQUIRED__`` flag lets the SPA pick the right
        ...
        if gated:
            bootstrap_script = (
                f"<script>"
                f"window.__ALEX_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
                f'window.__ALEX_BASE_PATH__="{prefix}";'
                f"window.__ALEX_AUTH_REQUIRED__={gated_js};"
                f"</script>"
            )
        else:
            bootstrap_script = (
                f'<script>window.__ALEX_SESSION_TOKEN__="{_SESSION_TOKEN}";'
                f"window.__ALEX_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
                f'window.__ALEX_BASE_PATH__="{prefix}";'
                f"window.__ALEX_AUTH_REQUIRED__={gated_js};"
                f"</script>"
    ...
    # under ``/alex`` they'd hit ``mission-control.tilos.com/fonts/...``
    # (the MC Pages app), not the Alex backend. Intercept CSS asset
...
_BUILTIN_DASHBOARD_THEMES = [
    {"name": "default",       "label": "Alex Teal",         "description": "Classic dark teal — the canonical Alex look"},
    {"name": "default-large", "label": "Alex Teal (Large)", "description": "Alex Teal with bigger fonts and roomier spacing"},
...
def _normalise_theme_definition(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # YAML in ~/.alex/, same trust level as the config file itself.
...
def _discover_user_themes() -> list:
    """Scan ~/.alex/dashboard-themes/*.yaml for user-created themes.

    Returns a list of fully-normalised theme definitions ready to ship
    to the frontend, so the client can apply them without a secondary
    round-trip or a built-in stub.
    """
    themes_dir = get_alex_home() / "dashboard-themes"
...
async def get_dashboard_themes():
    ...
    from `~/.alex/dashboard-themes/*.yaml` ship with their full
...
def _discover_dashboard_plugins() -> list:
    ...
    Checks three plugin sources (same as alex_cli.plugins):
    1. User plugins:    ~/.alex/plugins/<name>/dashboard/manifest.json
    2. Bundled plugins: <repo>/plugins/<name>/dashboard/manifest.json  (memory/, etc.)
    3. Project plugins: ./.alex/plugins/  (only if ALEX_ENABLE_PROJECT_PLUGINS)
    """
    plugins = []
    seen_names: set = set()

    from alex_cli.plugins import get_bundled_plugins_dir
    bundled_root = get_bundled_plugins_dir()
    search_dirs = [
        (get_alex_home() / "plugins", "user"),
    ...
    # ``alex_cli/plugins.py`` and the documented user contract.
    if env_var_enabled("ALEX_ENABLE_PROJECT_PLUGINS"):
        search_dirs.append((Path.cwd() / ".alex" / "plugins", "project"))
...
def _merged_plugins_hub() -> Dict[str, Any]:
    ...
    from alex_cli.plugins_cmd import (
    ...
    plugins_root_resolved = (get_alex_home() / "plugins").resolve()
    rows: List[Dict[str, Any]] = []
...
async def post_agent_plugin_install(request: Request, body: _AgentPluginInstallBody):
    _require_token(request)
    from alex_cli.plugins_cmd import dashboard_install_plugin
...
async def post_agent_plugin_enable(request: Request, name: str):
    ...
    from alex_cli.plugins_cmd import dashboard_set_agent_plugin_enabled
...
async def post_agent_plugin_update(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from alex_cli.plugins_cmd import dashboard_update_user_plugin
...
async def delete_agent_plugin(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from alex_cli.plugins_cmd import dashboard_remove_user_plugin
...
def _mount_plugin_api_routes():
    ...
    Project plugins (``./.alex/plugins/``) ship with the CWD and are
    ...
    for plugin in _get_dashboard_plugins():
        ...
        if not api_path.exists():
            ...
            continue
        try:
            module_name = f"alex_dashboard_plugin_{plugin['name']}"
...
from alex_cli.dashboard_auth.routes import router as _dashboard_auth_router  # noqa: E402
...
def _write_dashboard_ready_file(actual_port: int) -> None:
    ...
    so Electron passes ``ALEX_DESKTOP_READY_FILE`` and waits for this JSON.
    Normal CLI/dashboard launches still use the stdout READY line below.
    """
    target = os.environ.get("ALEX_DESKTOP_READY_FILE")
    if not target:
        return
...
def start_server(
    ...
    try:
        from alex_cli.nous_auth_keepalive import start_nous_auth_keepalive
    ...
    # the alex-0day MCP-persistence campaign abused unauthenticated public
    ...
    if app.state.auth_required:
        ...
        if not list_providers():
            ...
            # to register (e.g. missing ALEX_DASHBOARD_OAUTH_CLIENT_ID).
            # Each provider plugin that ships with Alex Agent exposes a
            ...
            _fix_hint = (
                ...
                "  • OAuth: run `alex dashboard register` (Nous Portal) or "
    ...
    # ALEX_DASHBOARD_READY, open the browser, *then* serve.
    #
    ...
    async def _serve():
        ...
        with server.capture_signals():
            ...
            print(f"ALEX_DASHBOARD_READY port={actual_port}", flush=True)
            print(f"  Alex Web UI → http://{host}:{actual_port}")
