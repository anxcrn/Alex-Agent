...
- ``alex_cli.providers``        -- canonical provider identity + overlays
- ``alex_cli.model_normalize``  -- per-provider name formatting
...
def is_nous_alex_non_agentic(model_name: str) -> bool:
    """Return True if *model_name* is a real Nous Alex 3/4 chat model.
    ...
    if not model_name:
        return False
    return bool(_NOUS_ALEX_NON_AGENTIC_RE.search(model_name))


def _check_alex_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* is a Nous Alex 3/4 chat model."""
    if is_nous_alex_non_agentic(model_name):
        return _ALEX_MODEL_WARNING
    return ""
...
def _load_direct_aliases() -> dict[str, DirectAlias]:
    ...
    Also reads ``model.aliases`` (set by ``alex config set model.aliases.xxx``)
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from alex_cli.model_switch import
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'alex model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'alex doctor' — config issues detected:"
        ...
        from alex_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
        from alex_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    ...
    # =================================================================
        ...
        try:
            from alex_cli.config import load_config
            from alex_cli.moa_config import exact_moa_preset_name, normalize_moa_config
    ...
    if provider_changed or explicit_provider:
        ...
        if explicit_provider and user_providers:
            from alex_cli.providers import resolve_user_provider as _ruser
    ...
    # Mirrors the same logic in alex_cli.runtime_provider.resolve_runtime_provider;
    ...
    alex_warn = _check_alex_model_warning(new_model)
    if alex_warn:
        warnings.append(alex_warn)
...
def prewarm_picker_cache_async() -> Optional["_threading.Thread"]:
    ...
    def _warm() -> None:
        try:
            from alex_cli.inventory import load_picker_context
...
def list_authenticated_providers(
    ...
    *,
    ...
    Uses the curated model lists from alex_cli/models.py (OPENROUTER_MODELS,
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from alex_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    # Build curated model lists keyed by alex provider ID
    ...
    if "ollama-cloud" not in curated:
        from alex_cli.models import fetch_ollama_cloud_models
    ...
    if "lmstudio" not in curated and (
        ...
        from alex_cli.models import fetch_lmstudio_models
    ...
    # --- 1. Check Alex-mapped providers ---
    from alex_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
    from alex_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    for alex_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        _alias_target = _PROVIDER_ALIAS_TABLE.get(alex_id)
        if (
            _alias_target
            and _alias_target != alex_id
        ...
        pconfig = PROVIDER_REGISTRY.get(alex_id)
        # Skip non-API-key auth providers here — they are handled in
        # section 2 (ALEX_OVERLAYS) with proper auth store checking.
        ...
        if not has_creds:
            try:
                from alex_cli.auth import _load_auth_store
                store = _load_auth_store()
                if store and store.get("credential_pool", {}).get(alex_id):
                    has_creds = True
            except Exception:
                pass
        if not has_creds:
        ...
        # /model picker sees the SAME list `alex model` would build, with
        # disk caching to keep the picker open snappy. Falls back to the
        # curated static list when the live fetcher returns nothing.
        model_ids = cached_provider_model_ids(alex_id)
        if not model_ids:
            model_ids = curated.get(alex_id, [])
            if alex_id in _MODELS_DEV_PREFERRED:
                model_ids = _merge_with_models_dev(alex_id, model_ids)
        total = len(model_ids)
    ...
    # --- 2. Check Alex-only providers (nous, openai-codex, copilot, opencode-go) ---
    from alex_cli.providers import ALEX_OVERLAYS
    from alex_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → Alex provider ID.
    # ALEX_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use Alex IDs ("copilot").
    _mdev_to_alex = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}

    for pid, overlay in ALEX_OVERLAYS.items():
        if pid.lower() in seen_slugs:
            continue

        # Resolve Alex slug — e.g. "github-copilot" → "copilot"
        alex_slug = _mdev_to_alex.get(pid, pid)
        if alex_slug.lower() in seen_slugs:
            continue

        # Check if credentials exist
        has_creds = False
        if overlay.auth_type == "aws_sdk":
            has_creds = _has_aws_sdk_creds_for_listing(alex_slug)
        elif overlay.extra_env_vars:
            has_creds = any(os.environ.get(ev) for ev in overlay.extra_env_vars)
        # Also check api_key_env_vars from PROVIDER_REGISTRY for api_key auth_type
        if not has_creds and overlay.auth_type == "api_key":
            for _key in (pid, alex_slug):
        ...
        if not has_creds:
            try:
                ...
                if store and (pid in providers_store or alex_slug in providers_store):
                    has_creds = True
        ...
        if not has_creds:
            try:
                ...
                pool = load_pool(alex_slug)
                if pool.has_credentials():
                    has_creds = True
            except Exception as exc:
                logger.debug("Credential pool check failed for %s: %s", alex_slug, exc)
        ...
        # configured.
        if not has_creds and alex_slug == "anthropic":
            try:
                ...
                if (alex_creds and alex_creds.get("accessToken")) or \
        ...
        if alex_slug in {"openai-codex", "copilot", "copilot-acp"}:
            ...
            model_ids = cached_provider_model_ids(alex_slug)
        # For aws_sdk providers (bedrock), use live discovery so the list
        # reflects the active region (eu.*, ap.*) not the static us.* list.
        elif overlay.auth_type == "aws_sdk":
            try:
                _ids = cached_provider_model_ids(alex_slug)
                model_ids = _ids if _ids else (curated.get(alex_slug, []) or curated.get(pid, []))
            except Exception:
                model_ids = curated.get(alex_slug, []) or curated.get(pid, [])
        elif alex_slug == "nous":
            # Nous serves a large live /v1/models catalog (vendor-prefixed
            # models from many providers, returned alphabetically). The
            # `alex model` picker deliberately shows ONLY the curated agentic
            ...
            try:
                ...
                from alex_cli.auth import get_provider_auth_state as _nous_state
            ...
            if not model_ids:
                model_ids = curated.get(alex_slug, []) or curated.get(pid, [])
                if alex_slug in _MODELS_DEV_PREFERRED:
                    model_ids = _merge_with_models_dev(alex_slug, model_ids)
        total = len(model_ids)
        top = model_ids[:max_models] if max_models is not None else model_ids

        results.append({
            "slug": alex_slug,
            "name": get_label(alex_slug),
            "is_current": alex_slug == current_provider or pid == current_provider,
            "is_user_defined": False,
            "models": top,
            "total_models": total,
            "source": "alex",
        ...
        seen_slugs.add(alex_slug.lower())
        _record_builtin_endpoint(alex_slug)

    # --- 2b. Cross-check canonical provider list ---
    # Catches providers that are in CANONICAL_PROVIDERS but weren't found
    # in PROVIDER_TO_MODELS_DEV or ALEX_OVERLAYS (keeps /model in sync
    # with `alex model`).
    try:
        from alex_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        for ep_name, ep_cfg in user_providers.items():
            ...
            # ``base_url`` is Alex's canonical write key (matches
            ...
            # Alex writes ``models:`` as a dict keyed by model id
            # (see alex_cli/main.py::_save_custom_provider); older
            ...
            if should_probe:
                try:
                    from alex_cli.models import fetch_api_models
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            if group_key not in groups:
                ...
                # Alex's own writer uses; a hyphen variant is accepted
            ...
            # active model. Alex's own writer (main.py::_save_custom_provider)
            # stores every configured model as a dict under ``models:``;
            # downstream readers (agent/models_dev.py, gateway/run.py,
            # run_agent.py, alex_cli/config.py) already consume that dict.
...
def _prepend_moa_picker_provider(providers: List[dict], current_provider: str = "") -> List[dict]:
    ...
    """
    try:
        from alex_cli.inventory import _moa_provider_row
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`alex_cli.models.fetch_openrouter_models`, which filters the
    ...
    """
    from alex_cli.models import fetch_openrouter_models
