...
Config files are stored in ~/.alex/ for easy access:
- ~/.alex/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.alex/.env         - API keys and secrets

This module provides:
- alex config          - Show current configuration
- alex config edit     - Open config in editor
- alex config set      - Set a specific value
- alex config wizard   - Re-run setup wizard
"""
...
def _backup_corrupt_config(config_path: Path) -> Optional[Path]:
    ...
    they re-run the setup wizard or ``alex config set`` (which rewrites
    ...
    ``config.yaml`` in place: alex never silently mutates the user's config,
...
def _warn_config_parse_failure(config_path: Path, exc: Exception) -> None:
    ...
    A YAML parse error in ``~/.alex/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``alex logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  alex config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
#   the next ``subprocess.run([...])`` Alex makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. Alex itself starts
#   from one of these on every restart.
# * ``NODE_OPTIONS`` / ``NODE_PATH`` — Node interpreter; affects npm,
#   ``alex update``, the TUI build.
...
#   on every plugin install / ``alex update``.
...
# * ``ALEX_HOME`` / ``ALEX_PROFILE`` / ``ALEX_CONFIG`` /
#   ``ALEX_ENV`` — Alex runtime location flags. Writing these into
...
#
# IMPORTANT: ``ALEX_*`` overall is NOT blocked. Many legitimate
# integration credentials follow that prefix (ALEX_LANGFUSE_PUBLIC_KEY,
# ALEX_SPOTIFY_CLIENT_ID, ...). The
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # Alex runtime location — never via dashboard env writer.
    # NOT a ALEX_* blanket: integration credentials (ALEX_GEMINI_*,
    # ALEX_LANGFUSE_*, ALEX_SPOTIFY_*, ...) ARE allowed.
    "ALEX_HOME", "ALEX_PROFILE", "ALEX_CONFIG", "ALEX_ENV",
})
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if key in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "PYTHONPATH, PATH, EDITOR, ...) or Alex runtime location "
            "(ALEX_HOME, ALEX_PROFILE, ...) cannot be persisted via "
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    "ALEX_TOOL_PROGRESS", "ALEX_TOOL_PROGRESS_MODE",
    ...
    # Activation is via plugins.enabled (opt-in through `alex plugins enable
    # observability/langfuse` or `alex tools → Langfuse`); credentials gate
    # the plugin at runtime.
    "ALEX_LANGFUSE_ENV",
    "ALEX_LANGFUSE_RELEASE",
    "ALEX_LANGFUSE_SAMPLE_RATE",
    "ALEX_LANGFUSE_MAX_CHARS",
    "ALEX_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
...
from alex_cli.default_soul import DEFAULT_SOUL_MD, is_legacy_template_soul
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("ALEX_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_alex_home() / ".managed"
...
def is_managed() -> bool:
    """Check if Alex is running in package-manager-managed mode.

    Two signals: the ALEX_MANAGED env var (set by the systemd service),
    or a .managed marker file in ALEX_HOME (set by the NixOS activation
...
def get_managed_update_command() -> Optional[str]:
    ...
    if managed_system == "Homebrew":
        return "brew upgrade alex-agent"
...
def _install_method_project_root(project_root: Optional[Path] = None) -> Path:
    ...
    This is the parent of ``alex_cli/`` — i.e. the git checkout for source
    installs, ``/opt/alex`` inside the published image, the venv's
    site-packages root for pip installs. It is a property of the running
    interpreter, NOT of ``$ALEX_HOME``, which is why a code-scoped stamp
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Alex was installed: 'docker', 'nixos', 'homebrew', 'git', or 'pip'.

    Resolution order:
    1. Code-scoped stamp ``<install tree>/.install_method`` (next to the
       running code) — the authoritative marker.
    2. Legacy home-scoped stamp ``$ALEX_HOME/.install_method`` — read for
       backward compatibility, but a ``docker`` value is IGNORED when we are
       not actually running inside a container (see below).
    3. ALEX_MANAGED env / .managed marker (NixOS, Homebrew)
    4. .git directory presence -> 'git'
    5. Fallback -> 'pip'

    Why the stamp is code-scoped, not home-scoped (issue: shared ``~/.alex``)
    --------------------------------------------------------------------------
    The install method describes *the binary that is running*, but
    ``$ALEX_HOME`` is a shared DATA directory — the Docker docs deliberately
    bind-mount it (``~/.alex:/opt/data``) so config/sessions/memory persist
    and can be shared with a host-side Desktop/CLI install. When a
    containerised gateway and a host install share one ``$ALEX_HOME``, a
    ...
    ``docker`` and ``alex update`` refuses to run ("doesn't apply inside the
    ...
    The supported installs self-identify via the code-scoped stamp:
      ...
      - the published ``charan vankudoth/alex-agent`` image bakes a ``docker``
        stamp into ``/opt/alex`` at build time.
    ...
    # 1. Code-scoped stamp — authoritative, immune to shared $ALEX_HOME.
    try:
    ...
    #    host install whose shared $ALEX_HOME was stamped by a co-located
    #    container, and honouring it wrongly blocks ``alex update``.
    try:
        method = (
            (get_alex_home() / ".install_method")
...
def _running_in_container() -> bool:
    """Thin wrapper around ``alex_constants.is_container`` (import-safe)."""
    try:
...
def stamp_install_method(method: str, project_root: Optional[Path] = None) -> None:
    ...
    not in ``$ALEX_HOME``, so that two installs sharing one data directory
    ...
    ``/opt/alex`` in the published image, which instead bakes the stamp at
...
def is_uv_tool_install() -> bool:
    """Return True when the *running* Alex lives in a ``uv tool`` layout.

    ``uv tool install alex-agent`` places the install at
    ``.../uv/tools/alex-agent/...`` (default ``~/.local/share/uv/tools``,
    ...
    ``alex-agent`` happens to be uv-tool-installed on the machine while
    the *active* Alex is a regular pip/venv install, causing
    ``alex update`` to upgrade the wrong copy. It would also block on a
    ...
    def _has_uv_tool_marker(path: str) -> bool:
        ...
        return "/uv/tools/alex-agent/" in norm + "/"
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method == "homebrew":
        return "brew upgrade alex-agent"
    if method == "docker":
        return "docker pull charan vankudoth/alex-agent:latest"
    if method == "pip":
        if is_uv_tool_install():
            return "uv tool upgrade alex-agent"
        import shutil
        if shutil.which("uv"):
            return "uv pip install --upgrade alex-agent"
        return "pip install --upgrade alex-agent"
    return "alex update"
...
# Long-form text for ``alex update`` / ``--check`` when running inside the
# Docker image.  Surfaced by ``cmd_update`` and ``_cmd_update_check`` in
# alex_cli/main.py; lives here so the wording stays consistent and we
...
#     — that script installs a *new* host-side Alex, it doesn't update
...
✗ ``alex update`` doesn't apply inside the Docker container.

Alex Agent runs as a published image (charan vankudoth/alex-agent), not a
git checkout — the container has no working tree to pull into.  Update by
pulling a fresh image and restarting your container instead:

  docker pull charan vankudoth/alex-agent:latest
  # then restart whatever started the container, e.g.:
  docker compose up -d --force-recreate alex-agent
  # or, for ad-hoc runs, exit the current container and `docker run` again

Verify the new version after restart:
  docker run --rm charan vankudoth/alex-agent:latest --version

Notes:
  • If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    won't move your container — pull the newer tag you actually want, or
    switch to ``:latest`` / ``:main`` for rolling updates.  See available
    tags at https://hub.docker.com/r/charan vankudoth/alex-agent/tags
  • Your config and session history live under ``$ALEX_HOME`` (``/opt/data``
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``alex update`` inside Docker.
...
def format_managed_message(action: str = "modify this Alex installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("ALEX_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this Alex installation is managed by NixOS "
            f"(ALEX_MANAGED={env_hint}).\n"
            "Edit services.alex-agent.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this Alex installation is managed by Homebrew "
            f"(ALEX_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade alex-agent"
        )

    return (
        f"Cannot {action}: this Alex installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Alex."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from ALEX_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, alex_bin
    or None if container mode is not active, we're already inside the
    container, or ALEX_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("ALEX_DEV") == "1":
        return None
    ...
    container_mode_file = get_alex_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "alex-agent")
    exec_user = info.get("exec_user", "alex")
    alex_bin = info.get("alex_bin", "/data/current-package/bin/alex")
...
# Re-export from alex_constants — canonical definition lives there.
from alex_constants import get_alex_home  # noqa: F811,E402
from utils import atomic_replace

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_alex_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_alex_home() / ".env"

def get_project_root() -> Path:
    """Get the project installation directory."""
    return Path(__file__).parent.parent.resolve()

def _resolve_alex_uid_gid() -> tuple[Optional[int], Optional[int]]:
    """Read the ALEX_UID / ALEX_GID env vars set by Docker deployments.

    Docker containers running Alex commonly set these to map the in-container
    user to a host user so volume-mounted state files end up with the right
    ownership. The entrypoint chowns the top-level ALEX_HOME once, but
    subdirectories created at runtime by ``ensure_alex_home()`` (especially
    ...
    if sys.platform == "win32":
        return None, None
    uid_str = os.environ.get("ALEX_UID", "").strip()
    gid_str = os.environ.get("ALEX_GID", "").strip()
    try:
...
def _chown_to_alex_uid(path) -> None:
    """Chown ``path`` to ``ALEX_UID:ALEX_GID`` if those env vars are set.
    ...
    directories created by :func:`ensure_alex_home` on Docker deployments.
    See #34107.
    """
    uid, gid = _resolve_alex_uid_gid()
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the alex group can
    share state with the gateway service.

    The mode can be overridden via the ALEX_HOME_MODE environment variable
    (e.g. ALEX_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse ALEX_HOME to reach a served subdirectory.
    The execute-only bit on a directory permits cd-through without exposing
    directory listings.

    Also applies ``ALEX_UID``/``ALEX_GID``-based ownership when those env
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("ALEX_HOME_MODE", "").strip()
    ...
    try:
        ...
        pass
    _chown_to_alex_uid(path)


def _is_container() -> bool:
    """Detect if we're running inside a Docker/Podman/LXC container.

    When Alex runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("ALEX_CONTAINER") or os.environ.get("ALEX_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set ALEX_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into ALEX_HOME, upgrading legacy empty templates.
...
def ensure_alex_home():
    """Ensure ~/.alex directory structure exists with secure permissions.
    ...
    if is_managed():
        ...
        try:
            _ensure_alex_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
...
def _ensure_alex_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"ALEX_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the Alex-level retry loop that wraps the whole call.  Lower
        ...
        # Universal planning-mode guidance — enforces structured implementation
        # planning before the agent mutates any files or runs complex terminal commands.
        "planning_mode_guidance": True,
        ...
        # prompt's environment-hints block. Lets a host that wraps Alex
        ...
        # identity slot (SOUL.md). Empty by default. The ALEX_ENVIRONMENT_HINT
        # env var overrides this (build-time/container mechanism).
        "environment_hint": "",
        # Coding posture — on interactive coding surfaces (CLI, TUI, desktop
        # app, ACP) in a code workspace, Alex adds a coding operating brief
    ...
    "terminal": {
        ...
        #             ALEX_HOME/home for persistent state (default)
        #   real    — force the real OS-user HOME
        #   profile — force ALEX_HOME/home when it exists (old strict
        ...
        # skipped. When empty, Alex auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Alex sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Alex
        ...
        # bundled Alex image, which drops to the `alex` user via
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Alex sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and Alex should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``alex chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.alex/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: alex sweeps the checkpoint base at startup
    ...
    # AGENTS.md, CLAUDE.md, .alex.md, or .cursorrules before Alex applies
    ...
    # single read_file page exceeds these limits, Alex truncates the
    ...
    # See tools/kanban_tools.py and alex_cli/kanban_db.py for the actual
    # implementations. Per-platform notification opt-out is handled by the
    # kanban dashboard (see ``alex dashboard`` -> Notifications).
    "kanban": {
    ...
    "auxiliary": {
        ...
        # Invoked by ``alex kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``alex kanban decompose`` and the kanban
        ...
        # ``alex profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `alex model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # Which interface bare `alex` (and `alex chat`) launches by default:
        ...
        # REPL and `--tui` (or ALEX_TUI=1) forces the TUI regardless of config.
        "interface": "cli",
        # When true, `alex --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `alex -c` muscle memory.  Default off so existing
        # users aren't surprised.  ALEX_TUI_RESUME=<id> always wins.
        "tui_auto_resume_recent": False,
        # When true (default), `alex --tui` drops a one-time hint
        ...
        # e.g. `model · 68% · ~/projects/alex`. Per-platform overrides go under
        ...
        # CLI, TUI, and desktop app. Manage with `alex pets`. Disabled until
        ...
        "pet": {
            ...
            # get_alex_home()/pets/. Empty → first installed pet.
            "slug": "",
    ...
    "dashboard": {
        ...
        # ``ALEX_DASHBOARD_OAUTH_CLIENT_ID`` and
        # ``ALEX_DASHBOARD_PORTAL_URL`` respectively — and the env var
        ...
        # ``portal_url`` defaults to https://portal.charan vankudoth.com).
        "oauth": {
        ...
        # (``ALEX_DASHBOARD_BASIC_AUTH_USERNAME`` /
        ...
        # ``ALEX_DASHBOARD_DRAIN_SECRET`` env var (the .env-is-for-secrets
        ...
        # Public URL override (env: ``ALEX_DASHBOARD_PUBLIC_URL``).
        # When set, this is the complete authority — scheme + host +
        # optional path prefix (e.g. ``https://example.com/alex``) —
    ...
    "tts": {
        ...
        "gemini": {
            ...
            # transcript section; Alex appends the live transcript when absent.
            "persona_prompt_file": "",
        },
        "xai": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.alex/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.alex/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    # response. If not, Alex feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Alex auto-pauses the goal and
    ...
    # always goes to ~/.alex/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${ALEX_SKILL_DIR} and ${ALEX_SESSION_ID} in SKILL.md
    ...
    # cron daemon.
    #
    # See `alex curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # class-level umbrellas. `alex curator run --consolidate` overrides
        ...
        # normally restored on every `alex update`, so pruning them only
        ...
        # skipped), snapshot ~/.alex/skills/ into
        # ~/.alex/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `alex curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for alex-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *Alex Agent*" header.
    ...
    "approvals": {
        ...
        # false.  TUI has its own modal overlay (ALEX_TUI_NO_CONFIRM=1 to
        # opt out there).
    ...
    # replace Alex' built-in platform hint for a single messaging platform
    ...
    # stored approval from ~/.alex/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or ALEX_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `alex doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``alex_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow Alex to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # $ALEX_HOME/plugins/<name>/) to relocate the trigger — e.g. "chronos",
        ...
        "chronos": {
            ...
            "portal_url": "https://portal.charan vankudoth.com",
        ...
        # Also overridable via ALEX_CRON_MAX_PARALLEL env var.
    ...
    # todos to ready, and fires `alex -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # one `alex` launches with no -p flag). This does not control the
        ...
        # decomposition is manual via `alex kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     alex-agent's own python (sys.executable). Maximum isolation
    ...
    #
    # Core Alex tools (terminal, read_file, write_file, patch,
    ...
    # Logging — controls file logging to ~/.alex/logs/.
    ...
    # update model picker lists without shipping a alex-agent release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://alex-agent.charan vankudoth.com/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `alex model` invocation; network failures
    ...
    "gateway": {
        ...
        # the ALEX_SCALE_TO_ZERO env stamp) AND messaging is relay-only/absent
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.alex/.env,
        ...
        # behavior: files must live under the Alex cache, under
        ...
        # user. Bridged to ALEX_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra directories from which model-emitted bare file paths may be
        # uploaded as native gateway attachments. Files inside the Alex
        # cache (~/.alex/cache/{documents,images,audio,video,screenshots})
        ...
        # to ALEX_MEDIA_ALLOW_DIRS at gateway startup. Tilde paths are
        ...
        # ALEX_MEDIA_TRUST_RECENT_FILES. Only consulted when ``strict``
        # is true; in default mode the denylist alone gates delivery.
        "trust_recent_files": True,
        # Recency window in seconds. 600 (10 min) comfortably covers a
        # multi-tool agent turn. Bridged to ALEX_MEDIA_TRUST_RECENT_SECONDS.
    ...
    # Session storage — controls automatic cleanup of ~/.alex/state.db.
    ...
    "sessions": {
        ...
        # default of ``alex sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.alex/sessions/session_{sid}.json`` on every turn
    ...
    # ``alex update`` behaviour.
    "updates": {
        # Run a full ``alex backup``-style zip of ALEX_HOME before every
        # ``alex update``.  Backups land in ``<ALEX_HOME>/backups/`` and
        # can be restored with ``alex import <path>``.  Off by default:
        # zipping a large ALEX_HOME (sessions DB, caches, skills) can add
        # minutes to every update.  The #48200 incident — a ``alex update
        ...
        "backup_keep": 5,
        # What `alex update` does with uncommitted local changes to the
    ...
    "lsp": {
        ...
        #              ``<ALEX_HOME>/lsp/bin/`` on first use.
    ...
    # `alex tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.alex/.env.
    "secrets": {
        "bitwarden": {
            ...
            # in ~/.alex/.env (or your shell) and never in config.yaml.
            ...
            # ~/.alex/bin/ on first use.  When False you must install
    ...
    "computer_use": {
        ...
        # by default upstream. Alex disables it for our users unless they
        # explicitly opt in here. When false (default), Alex sets
...
OPTIONAL_ENV_VARS = {
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'alex model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_DOMAIN": {
        "description": "Shared tool-gateway domain suffix for Nous Subscribers only, used to derive vendor hosts, e.g. charan vankudoth.com -> firecrawl-gateway.charan vankudoth.com",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the Alex auth store)",
    ...
    "SLACK_ALLOWED_USERS": {
        "description": "Comma-separated Slack member IDs allowed to use Alex, e.g. U01ABC2DEF3. Without this, Slack may connect but deny messages by default.",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @alex:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. ALEX_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #alex)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: alex-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'alex-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Alex API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Alex API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Alex API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # ALEX_TOOL_PROGRESS and ALEX_TOOL_PROGRESS_MODE are deprecated —
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.alex.config`` entries, then checks
    ...
    try:
        ...
        # should never break `alex update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Alex versions) write ``models`` as
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``alex_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'alex setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Alex won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'alex doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("ALEX_HOME", "~/.alex")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("ALEX_TOOL_PROGRESS")
            old_mode = get_env_value("ALEX_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in {"false", "0", "no"}:
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from ALEX_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in {"new", "all"}:
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from ALEX_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("ALEX_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from ALEX_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$ALEX_HOME/plugins/`` for currently installed user plugins.
            grandfathered: List[str] = []
            try:
                user_plugins_dir = get_alex_home() / "plugins"
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `alex plugins enable <name>` to activate."
                    )
    ...
    # settings in their `config.yaml`, and `alex curator status` has no
    ...
    #   3. Creates `~/.alex/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_alex_home() — old profiles that predate this
    #      migration still benefit).
    if current_ver < 23:
        try:
            curator_dir = get_alex_home() / "logs" / "curator"
    ...
    if isinstance(raw_mcp_servers, dict):
        try:
            from alex_cli.mcp_security import validate_mcp_server_entry as _validate_mcp_server_entry
        except Exception:
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: alex config set <key> <value>")
    ...
    # metadata.alex.config in their SKILL.md frontmatter.
...
def _normalize_root_model_keys(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    intuitive name OpenAI-SDK / LiteLLM users reach for, and ``alex config set``
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _alex_home / "config.yaml"`` local variable
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.alex/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.alex/config.yaml.
    ...
    (which change ``ALEX_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
def _load_config_impl(*, want_deepcopy: bool) -> Dict[str, Any]:
    with _CONFIG_LOCK:
        ...
        # cache signature so editing /etc/alex/config.yaml invalidates the
...
#   openai-codex (OAuth — alex auth) — OpenAI Codex
#   nous         (OAuth — alex auth) — Nous Portal
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.alex/config.yaml."""
    with _CONFIG_LOCK:
        if is_managed():
...
def _parse_env_value(raw_value: str) -> str:
    """Parse the small .env value subset Alex writes itself."""
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.alex/.env.
    ...
    interactive menu render (`alex tools`, `alex setup`, status
    panels). Sanitisation is O(lines × known-keys), so re-parsing the
    same file on every call was burning ~300ms of CPU per `alex tools`
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real Alex env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.alex/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.alex/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.alex/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.alex/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Alex — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Alex vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.alex/.env or environment."""
...
def show_config():
    ...
    print(color("│              ⚕ Alex Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    from alex_cli.auth import get_anthropic_key
    ...
    # Warn on stale ALEX_MAX_ITERATIONS ghost in .env that disagrees with
    ...
    try:
        _env_ghost = load_env().get("ALEX_MAX_ITERATIONS")
        if _env_ghost is not None and str(_env_ghost).strip() != str(_cfg_max_turns).strip():
            print(color(
                f"                ⚠ .env has stale ALEX_MAX_ITERATIONS={_env_ghost} "
                f"(run 'alex doctor --fix' to remove)",
                Colors.YELLOW,
            ))
    except Exception:
        pass
    ...
    print(color("  alex config edit     # Edit config file", Colors.DIM))
    print(color("  alex config set <key> <value>", Colors.DIM))
    print(color("  alex setup           # Run setup wizard", Colors.DIM))
    print()
...
def set_config_value(key: str, value: str):
    ...
    # so a fresh `alex config set model.api_base ...` lands on the canonical
    ...
    # — e.g. `alex config set model.api_key cfut_...` routes to config.yaml
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: alex config set <key> <value>")
            print()
            print("Examples:")
            print("  alex config set model anthropic/claude-sonnet-4")
            print("  alex config set terminal.backend docker")
            print("  alex config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'alex config migrate' to add them")
        ...
        print("  alex config           Show current configuration")
        print("  alex config edit      Open config in editor")
        print("  alex config set <key> <value>   Set a config value")
        print("  alex config check     Check for missing/outdated config")
        print("  alex config migrate   Update config with new options")
        print("  alex config path      Show config file path")
        print("  alex config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``alex config`` UI so users
