...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    ...
    The supported installs self-identify via the code-scoped stamp:
      ...
      - the published ``nousresearch/alex-agent`` image bakes a ``docker``
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method == "homebrew":
        return "brew upgrade alex-agent"
    if method == "docker":
        return "docker pull nousresearch/alex-agent:latest"
    if method == "pip":
...
Alex Agent runs as a published image (nousresearch/alex-agent), not a
...
Verify the new version after restart:
  docker run --rm nousresearch/alex-agent:latest --version

Notes:
  • If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    won't move your container — pull the newer tag you actually want, or
    switch to ``:latest`` / ``:main`` for rolling updates.  See available
    tags at https://hub.docker.com/r/nousresearch/alex-agent/tags
...
DEFAULT_CONFIG = {
    ...
    "model_catalog": {
        "enabled": True,
        "url": "https://alex-agent.nousresearch.com/docs/api/model-catalog.json",
