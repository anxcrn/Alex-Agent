...
Pure logic: walks a Alex config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `alex doctor` and a future `alex migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a Alex config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a Alex config and return retirement issues.
