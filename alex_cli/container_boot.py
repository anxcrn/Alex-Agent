...
``$ALEX_HOME/profiles/<name>/`` live on the persistent VOLUME, and
...
Dockerfile (Phase 4 Task 4.0). Runs as root after 01-alex-setup
(the stage2 hook) has chowned the volume and seeded $ALEX_HOME, but
...
def reconcile_profile_gateways(
    *,
    ...
    (the implicit profile that lives at the top of ``$ALEX_HOME``,
    not under ``profiles/``). The dispatcher in ``alex_cli.gateway``
    maps an empty profile suffix to ``gateway-default``, so this slot
    is what ``alex gateway start`` (no ``-p``) targets. Without it,
    bare ``alex gateway start`` inside the container would land on
    ...
    ``$ALEX_HOME/gateway_state.json`` (sibling to the profile root,
    not under ``profiles/``); stale runtime files there are swept the
    same way as for named profiles.

    Args:
        alex_home: The container's ALEX_HOME (typically /opt/data).
            Profiles live under ``<alex_home>/profiles/<name>/``;
            the default profile lives at ``<alex_home>`` itself.
    ...
    # ``alex gateway start`` (no ``-p``) has somewhere to land;
    ...
    default_prior_state = legacy_default_state or _read_desired_state(alex_home)
    default_should_start = default_prior_state in _AUTOSTART_STATES
    if not dry_run:
        _cleanup_stale_runtime_files(alex_home)
    ...
    profiles_root = alex_home / "profiles"
    if profiles_root.is_dir():
        for entry in sorted(profiles_root.iterdir()):
            if not entry.is_dir():
                continue
            # SOUL.md is always seeded by `alex profile create` (config.yaml
            # is not — that comes later via `alex setup`). Use it as the
            ...
            # via ``alex -p default-named gateway start`` if they
    ...
    if not dry_run:
        _write_reconcile_log(alex_home, actions)
    return actions
...
def _maybe_migrate_legacy_gateway_run_state(
    ...
    *,
    ...
    """
    state_file = alex_home / "gateway_state.json"
    if state_file.exists():
        return None

    if os.environ.get("ALEX_GATEWAY_NO_SUPERVISE", "").lower() in ("1", "true", "yes"):
        return None
...
def _strip_container_argv_prefix(argv: Sequence[str]) -> list[str]:
    """Strip the s6/wrapper prefix off the container argv, leaving the alex args.

    Two container-command argv shapes are handled:

    * **s6-overlay v2 / tini:** PID 1 argv is
      ``/init /opt/alex/docker/main-wrapper.sh <subcommand> [args...]``.
    * **s6-overlay v3:** PID 1 is ``s6-svscan`` and the command lives on the
      rc.init-launched process as ``/bin/sh -e
      /run/s6/basedir/scripts/rc.init top /opt/alex/docker/main-wrapper.sh
    ...
    ``alex`` invocations carry no wrapper, so fall back to peeling a bare
    ``init`` prefix. The wrapper re-execs ``alex <subcommand>``, so an
    explicit leading ``alex`` is peeled too. Shared by the legacy-gateway
    ...
    # The wrapper re-execs `alex <subcommand>`; peel an explicit alex.
    if args and Path(args[0]).name == "alex":
        args = args[1:]
    return args
...
def _is_dashboard_container(argv: Sequence[str]) -> bool:
    ...
    A dashboard-only container (``alex dashboard ...``) never spawns or
    ...
    the gateway and dashboard containers share a bind-mounted ALEX_HOME,
...
def _register_service(scandir: Path, profile: str, *, start: bool) -> None:
    ...
    try:
        ...
        # it up explicitly with `alex -p <profile> gateway start`
        ...
        # spawns will EEXIST our dirs/FIFOs and inherit alex
        # ownership, so runtime s6-svc / s6-svstat / s6-svwait calls
        # (all dispatched as the alex user) won't hit EACCES. See
...
def _write_reconcile_log(
    alex_home: Path, actions: list[ReconcileAction],
) -> None:
    """Append one line per profile to $ALEX_HOME/logs/container-boot.log.
...
def main() -> int:
    ...
    # bind-mounted ALEX_HOME, both race to flock() the same s6-log lock
    ...
    actions = reconcile_profile_gateways(
        alex_home=alex_home, scandir=scandir,
    )
    for a in actions:
        print(
