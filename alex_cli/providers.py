"""
Single source of truth for provider identity in Alex Agent.
...
2. **Alex overlays** — transport type, auth patterns, aggregator flags,
...
# -- Alex overlay ----------------------------------------------------------
# Alex-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class AlexOverlay:
    """Alex-specific provider metadata layered on top of models.dev."""
...
ALEX_OVERLAYS: Dict[str, AlexOverlay] = {
    "moa": AlexOverlay(
        transport="openai_chat",
        auth_type="virtual",
        base_url_override="moa://local",
    ),
    "openrouter": AlexOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ...
    "openai-api": AlexOverlay(
        transport="codex_responses",
        base_url_override="https://api.openai.com/v1",
        base_url_env_var="OPENAI_BASE_URL",
    ),
    "xai-oauth": AlexOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://api.x.ai/v1",
        base_url_env_var="XAI_BASE_URL",
    ),
    "qwen-oauth": AlexOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="https://portal.qwen.ai/v1",
        base_url_env_var="ALEX_QWEN_BASE_URL",
    ),
    "lmstudio": AlexOverlay(
        transport="openai_chat",
        auth_type="api_key",
    ...
    "copilot-acp": AlexOverlay(
        transport="codex_responses",
        auth_type="external_process",
        base_url_override="acp://copilot",
        base_url_env_var="COPILOT_ACP_BASE_URL",
    ),
    "github-copilot": AlexOverlay(
        transport="openai_chat",
        extra_env_vars=("COPILOT_GITHUB_TOKEN", "GH_TOKEN"),
    ),
    "anthropic": AlexOverlay(
    ...
    "minimax-oauth": AlexOverlay(
        transport="anthropic_messages",
        auth_type="oauth_external",
        base_url_override="https://api.minimax.io/anthropic",
    ),
    "minimax-cn": AlexOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_CN_BASE_URL",
    ),
    "deepseek": AlexOverlay(
        transport="openai_chat",
    ...
    "alibaba-coding-plan": AlexOverlay(
        transport="openai_chat",
        base_url_env_var="ALIBABA_CODING_PLAN_BASE_URL",
    ),
    "opencode": AlexOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_ZEN_BASE_URL",
    ),
    "opencode-go": AlexOverlay(
        transport="openai_chat",
        is_aggregator=True,
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "alex", "user-config"
...
def get_provider(name: str) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. Alex overlays (for providers not in models.dev: nous, openai-codex, etc.)
      2. models.dev catalog + Alex overlay
    ...
    overlay = ALEX_OVERLAYS.get(canonical)
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + alex extra
    ...
    if overlay is not None:
        # Alex-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
...
def determine_api_mode(provider: str, base_url: str = "") -> str:
    ...
    # Direct provider checks for providers not in ALEX_OVERLAYS
    if provider == "bedrock":
