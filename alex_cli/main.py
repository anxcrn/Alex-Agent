#!/usr/bin/env python3
"""
Alex CLI - Main entry point.

Usage:
    alex                     # Interactive chat (default)
    alex chat                # Interactive chat
    alex gateway             # Run gateway in foreground
    alex gateway start       # Start gateway as service
    alex gateway stop        # Stop gateway service
    alex gateway status      # Show gateway status
    alex gateway install     # Install gateway service
    alex gateway uninstall   # Uninstall gateway service
    alex setup               # Interactive setup wizard
    alex logout              # Clear stored authentication
    alex status              # Show status of all components
    alex cron                # Manage cron jobs
    alex cron list           # List cron jobs
    alex cron status         # Check if cron scheduler is running
    alex doctor              # Check configuration and dependencies
    alex honcho setup                    # Configure Honcho AI memory integration
    alex honcho status                   # Show Honcho config and connection status
    alex honcho sessions                 # List directory → session name mappings
    alex honcho map <name>               # Map current directory to a session name
    alex honcho peer                     # Show peer names and dialectic settings
    alex honcho peer --user NAME         # Set user peer name
    alex honcho peer --ai NAME           # Set AI peer name
    alex honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    alex honcho mode                     # Show current memory mode
    alex honcho mode [hybrid|honcho|local]  # Set memory mode
    alex honcho tokens                   # Show token budget settings
    alex honcho tokens --context N       # Set session.context() token cap
    alex honcho tokens --dialectic N     # Set dialectic result char cap
    alex honcho identity                 # Show AI peer identity representation
    alex honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    alex honcho migrate                  # Step-by-step migration guide: OpenClaw native → Alex + Honcho
    alex version             Show version
    alex update              Update to latest version
    alex uninstall           Uninstall Alex Agent
    alex acp                 Run as an ACP server for editor integration
    alex sessions browse     Interactive session picker with search

    alex claw migrate --dry-run  # Preview migration without changes
"""

# IMPORTANT: alex_bootstrap must be the very first import — it sets up
# UTF-8 stdio on Windows so print()/subprocess children don't hit
# UnicodeEncodeError with non-ASCII characters.  No-op on POSIX.
#
# Guarded against ModuleNotFoundError because ``alex_bootstrap`` is a
# top-level module registered via pyproject.toml's ``py-modules`` list.
# When the user upgrades code via ``git pull`` (or ``alex update``
# crashes between ``git reset --hard`` and ``uv pip install -e .``), the
# new code references ``alex_bootstrap`` but the editable install's
# ``.pth`` file still points at the old set of top-level modules.  Without
# this guard, alex crashes on import and the user can't run
# ``alex update`` to recover.  Missing the bootstrap means UTF-8 stdio
# setup is skipped on Windows — degraded, not broken.  POSIX is unaffected.
try:
    import alex_bootstrap  # noqa: F401
...
def _set_process_title() -> None:
    """Set the process title to 'alex' so tools like 'ps', 'top', and
    'htop' show the app name instead of 'python3.xx'.

    Purely cosmetic — non-fatal on any platform.

    Strategy (try in order):
      1. ``setproctitle`` (opt-in dep — installed via ``alex tools`` or
      ...
      4. No-op on Windows (the .exe name is already ``alex.exe``).
    """
    # Strategy 1: setproctitle (best — works on macOS, Linux, BSD)
    try:
        import setproctitle  # type: ignore[import-untyped]

        setproctitle.setproctitle("alex")
        return
    except ImportError:
        pass
    ...
    try:
        ...
        if system == "Linux":
            ...
            libc.prctl(15, b"alex", 0, 0, 0)  # PR_SET_NAME = 15
        elif system == "Darwin":
            libc = ctypes.CDLL("libc.dylib", use_errno=True)
            libc.pthread_setname_np(b"alex")
        # Windows: the .exe name is already ``alex.exe`` — nothing to do.
    except Exception:
        pass
...
# that run *before* alex_cli.config is importable. Mirrors the explicit
...
def _config_default_interface_early() -> str:
    ...
    value = "cli"
    try:
        home = os.environ.get("ALEX_HOME")
        if home:
            cfg_path = os.path.join(home, "config.yaml")
        else:
            cfg_path = os.path.join(os.path.expanduser("~"), ".alex", "config.yaml")
...
def _wants_tui_early(argv: "list[str] | None" = None) -> bool:
    ...
    ``--tui``/``ALEX_TUI=1``, then ``display.interface`` in config.
    """
    if argv is None:
        argv = sys.argv[1:]
    if "--cli" in argv:
        return False
    if os.environ.get("ALEX_TUI") == "1" or "--tui" in argv:
        return True
...
# `entry.tsx`; this is just the earlier cousin. ``ALEX_TUI_NO_EARLY_DISABLE``
# escapes the behaviour for diagnostics.
def _suppress_mouse_residue_early() -> None:
    if os.environ.get("ALEX_TUI_NO_EARLY_DISABLE") == "1":
        return
    if not _wants_tui_early():
        return
    try:
        # Skip when stdout is redirected (`alex --tui … >log`, CI capture):
...
def _print_fast_version_info() -> None:
    from alex_cli import __release_date__, __version__

    project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), os.pardir))
    print(f"Alex Agent v{__version__} ({__release_date__})")
...
def _try_termux_ultrafast_version() -> bool:
    """Handle ``alex --version`` before config/logging imports on Termux."""
    if os.environ.get("ALEX_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
...
from alex_cli.subcommands._shared import add_accept_hooks_flag as _add_accept_hooks_flag
from alex_cli.subcommands.cron import build_cron_parser
from alex_cli.subcommands.gateway import build_gateway_parser
from alex_cli.subcommands.profile import build_profile_parser
from alex_cli.subcommands.model import build_model_parser
from alex_cli.subcommands.setup import build_setup_parser
from alex_cli.subcommands.postinstall import build_postinstall_parser
from alex_cli.subcommands.whatsapp import build_whatsapp_parser
from alex_cli.subcommands.slack import build_slack_parser
from alex_cli.subcommands.login import build_login_parser
from alex_cli.subcommands.logout import build_logout_parser
from alex_cli.subcommands.auth import build_auth_parser
from alex_cli.subcommands.status import build_status_parser
from alex_cli.subcommands.webhook import build_webhook_parser
from alex_cli.subcommands.hooks import build_hooks_parser
from alex_cli.subcommands.doctor import build_doctor_parser
from alex_cli.subcommands.security import build_security_parser
from alex_cli.subcommands.dump import build_dump_parser
from alex_cli.subcommands.debug import build_debug_parser
from alex_cli.subcommands.backup import build_backup_parser
from alex_cli.subcommands.import_cmd import build_import_cmd_parser
from alex_cli.subcommands.config import build_config_parser
from alex_cli.subcommands.version import build_version_parser
from alex_cli.subcommands.update import build_update_parser
from alex_cli.subcommands.uninstall import build_uninstall_parser
from alex_cli.subcommands.dashboard import build_dashboard_parser
from alex_cli.subcommands.gui import build_gui_parser
from alex_cli.subcommands.logs import build_logs_parser
from alex_cli.subcommands.prompt_size import build_prompt_size_parser
from alex_cli.subcommands.memory import build_memory_parser
from alex_cli.subcommands.acp import build_acp_parser
from alex_cli.subcommands.tools import build_tools_parser
from alex_cli.subcommands.insights import build_insights_parser
from alex_cli.subcommands.skills import build_skills_parser
from alex_cli.subcommands.pairing import build_pairing_parser
...
def _require_tty(command_name: str) -> None:
    ...
    Interactive TUI commands (alex tools, alex setup, alex model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'alex {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any alex module import.
#
# Many modules cache ALEX_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("ALEX_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.alex/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set ALEX_HOME before imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    profile_index = None
    ...
    def _inside_mcp_add_args(index: int) -> bool:
        """True once argv reaches `alex mcp add ... --args <command argv>`.
        ...
        ``--profile``), not to Alex' own profile selector.
        """
        try:
    ...
    def _resolve_sudo_user_profile_env(name: str) -> str | None:
        """Resolve `sudo alex -p <name>` against the invoking user's home.
        ...
        candidate = home / ".alex" / "profiles" / name
        try:
            if candidate.is_dir():
    ...
    # after the subcommand (`alex chat -p coder`), so keep scanning broadly.
    ...
    # Mirrors alex_cli.profiles._PROFILE_ID_RE so we never call
    ...
    # 1.5 If ALEX_HOME is already set and no explicit flag was given, trust it
    # only when it already points to a specific profile directory.  The
    # distinguishing heuristic: a profile path has "profiles" as its immediate
    # parent directory name (e.g. ~/.alex/profiles/coder or
    # /opt/data/profiles/coder).  If ALEX_HOME points to the alex root
    # instead (e.g. systemd hardcodes ALEX_HOME=/root/.alex), we must
    # still read active_profile — the user may have switched profiles via
    # `alex profile use` and the gateway should honour that choice.
    # See issue #22502.
    alex_home_env = os.environ.get("ALEX_HOME", "")
    if profile_name is None and alex_home_env:
        if Path(alex_home_env).parent.name == "profiles":
            return

    # 2. If no flag, check active_profile in the alex root.
    #
    # EXCEPTION: a supervised s6 gateway child (exported by the container
    # run-script as ALEX_S6_SUPERVISED_CHILD=1) must NOT follow the sticky
    ...
    # reserved ``gateway-default`` slot runs bare ``alex gateway run`` to mean
    # "the root ALEX_HOME profile". If the reserved default child read
    ...
    if profile_name is None and not os.environ.get("ALEX_S6_SUPERVISED_CHILD"):
        try:
            from alex_constants import get_default_alex_root

            active_path = get_default_alex_root() / "active_profile"
    ...
    # 3. If we found a profile, resolve and set ALEX_HOME
    if profile_name is not None:
        try:
            from alex_cli.profiles import resolve_profile_env

            alex_home = resolve_profile_env(profile_name)
        except FileNotFoundError as exc:
            alex_home = _resolve_sudo_user_profile_env(profile_name)
            if not alex_home:
            ...
            # A bug in profiles.py must NEVER prevent alex from starting
            print(
                f"Warning: profile override failed ({exc}), using default",
                file=sys.stderr,
            )
            return
        os.environ["ALEX_HOME"] = alex_home
...
# Load .env from ~/.alex/.env first, then project root as dev fallback.
# User-managed env files should override stale shell exports on restart.
from alex_cli.config import get_alex_home
from alex_cli.env_loader import load_alex_dotenv

load_alex_dotenv(project_env=PROJECT_ROOT / ".env")

# Bridge security.redact_secrets from config.yaml → ALEX_REDACT_SECRETS env
# var BEFORE alex_logging imports agent.redact (which snapshots the flag at
...
try:
    ...
    _cfg_path = get_alex_home() / "config.yaml"
    if _cfg_path.exists():
        ...
        try:
            from alex_cli import managed_scope
            _early_cfg_raw = managed_scope.apply_managed_overlay(_early_cfg_raw)
        except Exception:
            pass
        if "ALEX_REDACT_SECRETS" not in os.environ:
            _early_sec_cfg = _early_cfg_raw.get("security", {})
            if isinstance(_early_sec_cfg, dict):
                _early_redact = _early_sec_cfg.get("redact_secrets")
                if _early_redact is not None:
                    os.environ["ALEX_REDACT_SECRETS"] = str(_early_redact).lower()
...
# Initialize centralized file logging early — all `alex` subcommands
...
try:
    from alex_logging import setup_logging as _setup_logging
...
if _FORCE_IPV4_EARLY:
    try:
        from alex_constants import apply_ipv4_preference as _apply_ipv4

        _apply_ipv4(force=True)
    except Exception:
        pass  # best-effort — don't crash if alex_constants not importable yet
...
from alex_cli import __version__, __release_date__

# Provider model-selection wizard flows extracted to alex_cli/model_setup_flows.py
# (god-file decomposition Phase 2). Re-imported here so select_provider_and_model and
# existing test monkeypatches (alex_cli.main._model_flow_*) keep resolving unchanged.
from alex_cli.model_setup_flows import (
...
def _termux_bundled_skills_stamp_path() -> Path:
    return get_alex_home() / "skills" / ".termux_bundled_sync_stamp"


def _termux_bundled_skills_sync_needed() -> bool:
    if not _is_termux_startup_environment():
        return True
    if os.environ.get("ALEX_TERMUX_FORCE_SKILLS_SYNC") == "1":
        return True
    try:
...
def _termux_should_prefetch_update_check() -> bool:
    if not _is_termux_startup_environment():
        return True
    return os.environ.get("ALEX_TERMUX_PREFETCH_UPDATES") == "1"
...
def _has_any_provider_configured() -> bool:
    ...
    from alex_cli.config import get_env_path, get_alex_home, load_config
    from alex_cli.auth import get_auth_status
    ...
    if isinstance(model_cfg, dict):
        ...
        _model_name = ""
    _has_alex_config = _model_name and _model_name != _DEFAULT_MODEL
    ...
    auth_file = get_alex_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
...
def _resolve_last_session(source: str = "cli") -> Optional[str]:
    ...
    db = None
    try:
        from alex_state import SessionDB
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        container_info: dict with backend, container_name, exec_user, alex_bin
        cli_args: the original CLI arguments (everything after 'alex')
    """

    backend = container_info["backend"]
    container_name = container_info["container_name"]
    exec_user = container_info["exec_user"]
    alex_bin = container_info["alex_bin"]
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f"  }}];\n"
                    f"\n"
                    f"Or run: sudo alex {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo alex {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def _print_tui_exit_summary(
    ...
    print(f"  alex --tui --resume {target}")
    if title:
        print(f'  alex --tui -c "{title}"')
    print()
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @alex/ink is missing or node_modules is behind package-lock.json.
    ...
    *root* (``ui-tui/dist/entry.js``).
    ...
    ink = ws_root / "node_modules" / "@alex" / "ink" / "package.json"
    if not ink.is_file():
        return True
    if not lock.is_file():
        return False
...
_TUI_BUILD_INPUT_FILES = (
    ...
    "scripts/build.mjs",
    "packages/alex-ink/package.json",
    "packages/alex-ink/index.js",
    "packages/alex-ink/text-input.js",
)
...
def _tui_need_rebuild(root: Path) -> bool:
    ...
    or local edits. Set ``ALEX_TUI_FORCE_BUILD=1`` to force the old behaviour.
    """
    force = (os.environ.get("ALEX_TUI_FORCE_BUILD") or "").strip().lower()
...
def _ensure_tui_node() -> None:
    ...
    ``ALEX_SKIP_NODE_BOOTSTRAP=1`` to disable auto-install.
    """
    if shutil.which("node") and shutil.which("npm"):
        return
    if os.environ.get("ALEX_SKIP_NODE_BOOTSTRAP"):
        return
    ...
    alex_home = os.environ.get("ALEX_HOME") or str(Path.home() / ".alex")
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "ALEX_HOME": alex_home},
            capture_output=True,
            text=True,
            encoding="utf-8",
            errors="replace",
    ...
    extras.extend([Path(alex_home) / "node" / "bin", Path.home() / ".local" / "bin"])
...
def _find_bundled_tui(alex_cli_dir: Path | None = None) -> Path | None:
    """Find a pre-built TUI entry.js bundled in the wheel."""
    if alex_cli_dir is None:
        alex_cli_dir = Path(__file__).parent
    bundled = alex_cli_dir / "tui_dist" / "entry.js"
...
def _restore_tui_workspace(tui_dir: Path) -> bool:
    ...
    files deleted in the working tree after ``alex update`` (HEAD stays
...
def _ensure_tui_workspace(tui_dir: Path) -> None:
    ...
    if _restore_tui_workspace(tui_dir):
        if not os.environ.get("ALEX_QUIET"):
            print(f"Restored missing TUI workspace: {tui_dir}")
        return

    print(
        "Error: the TUI workspace is missing from this Alex checkout.\n"
        f"Expected directory: {tui_dir}\n"
        "This usually means `alex update` left tracked ui-tui files deleted.\n"
        "Recovery:\n"
        "  1. From the Alex checkout, run `git restore -- ui-tui`\n"
        "  2. Run `npm install --silent --no-fund --no-audit --progress=false`\n"
        "  3. Retry `alex --tui`\n"
        "If the checkout is still inconsistent, run `alex update --force`.",
        file=sys.stderr,
    )
    sys.exit(1)
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    """TUI: --dev → tsx src; else node dist (ALEX_TUI_DIR prebuilt or esbuild)."""
    _ensure_tui_node()

    def _node_bin(bin: str) -> str:
        if bin == "node":
            env_node = os.environ.get("ALEX_NODE")
            if env_node and os.path.isfile(env_node) and os.access(env_node, os.X_OK):
                return env_node
        path = shutil.which(bin)
        if not path and bin == "node":
            try:
                from alex_cli.dep_ensure import ensure_dependency
    ...
    ext_dir = os.environ.get("ALEX_TUI_DIR")
    if tui_dev and ext_dir:
        print(
            f"Error: --dev is incompatible with ALEX_TUI_DIR={ext_dir}\n"
            f"The prebuilt TUI has no source code to hot-reload.\n"
            f"Unset ALEX_TUI_DIR (e.g. `unset ALEX_TUI_DIR`) to use --dev from a checkout.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if tui_dev:
        # Keep the local @alex/ink package exports in sync with source.
        # --dev runs src/entry.tsx directly, but @alex/ink resolves through
        # packages/alex-ink/dist/entry-exports.js. If that dist bundle is
        ...
        npm = _node_bin("npm")
        ink_dir = tui_dir / "packages" / "alex-ink"
        result = subprocess.run(
            [npm, "run", "build"],
            cwd=str(ink_dir),
            capture_output=True,
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from alex_cli.oneshot import _normalize_toolsets
...
def _launch_tui(
    ...
    env = os.environ.copy()
    try:
        from alex_cli.config import apply_terminal_config_to_env
        apply_terminal_config_to_env(env=env)
    except Exception:
        logger.debug("Failed to apply terminal config bridge for TUI launch", exc_info=True)
    active_session_fd, active_session_file = tempfile.mkstemp(
        prefix="alex-tui-active-session-", suffix=".json"
    )
    os.close(active_session_fd)
    env["ALEX_TUI_ACTIVE_SESSION_FILE"] = active_session_file
    env["ALEX_PYTHON_SRC_ROOT"] = os.environ.get(
        "ALEX_PYTHON_SRC_ROOT", str(PROJECT_ROOT)
    )
    env.setdefault("ALEX_PYTHON", sys.executable)
    env.setdefault("ALEX_CWD", os.getcwd())
    ...
    if worktree:
        ...
        if not wt_info:
            sys.exit(1)
        env["ALEX_CWD"] = wt_info["path"]
        env["TERMINAL_CWD"] = wt_info["path"]

    if model:
        env["ALEX_MODEL"] = model
        env["ALEX_INFERENCE_MODEL"] = model
    if provider:
        env["ALEX_TUI_PROVIDER"] = provider
        env["ALEX_INFERENCE_PROVIDER"] = provider
    tui_toolsets = _normalize_tui_toolsets(toolsets)
    if tui_toolsets:
        env["ALEX_TUI_TOOLSETS"] = ",".join(tui_toolsets)
    if skills:
        if isinstance(skills, (list, tuple)):
            ...
            if flattened:
                env["ALEX_TUI_SKILLS"] = ",".join(flattened)
        else:
            value = str(skills).strip()
            if value:
                env["ALEX_TUI_SKILLS"] = value
    if query:
        env["ALEX_TUI_QUERY"] = query
    if image:
        env["ALEX_TUI_IMAGE"] = image
    if checkpoints:
        env["ALEX_TUI_CHECKPOINTS"] = "1"
    if pass_session_id:
        env["ALEX_TUI_PASS_SESSION_ID"] = "1"
    if max_turns is not None:
        env["ALEX_TUI_MAX_TURNS"] = str(max_turns)
    if verbose:
        env["ALEX_TUI_TOOL_PROGRESS"] = "verbose"
    elif quiet:
        env["ALEX_TUI_TOOL_PROGRESS"] = "off"
    if accept_hooks:
        env["ALEX_ACCEPT_HOOKS"] = "1"
    ...
    # ALEX_TUI_RESUME is an internal hand-off from the Python wrapper to the
    # Ink app.  Because we start from os.environ.copy(), an exported/stale value
    # in the user's shell would otherwise make a plain `alex --tui` try to
    ...
    # still set ALEX_TUI_RESUME themselves.
    env.pop("ALEX_TUI_RESUME", None)
    if resume_session_id:
        env["ALEX_TUI_RESUME"] = resume_session_id
    ...
    # Exit code 42 = TUI requested an update. Relaunch as `alex update` so
    ...
    if code == 42:
        from alex_cli.relaunch import relaunch
...
def _pin_kanban_board_env() -> None:
    """Pin the active kanban board into ``ALEX_KANBAN_BOARD`` for the chat session.

    Without this, in-process tools (``kanban_*``) and shelled-out CLI calls
    (``alex kanban …``) resolve the board on different paths: the env-pin if
    set, otherwise the global ``<root>/kanban/current`` file. A concurrent
    ``alex kanban boards switch`` from another session can flip the file
    ...
    """
    if os.environ.get("ALEX_KANBAN_BOARD"):
        return
    try:
        from alex_cli.kanban_db import get_current_board

        os.environ["ALEX_KANBAN_BOARD"] = get_current_board()
    except Exception:
        pass


def _sync_bundled_skills_quietly() -> None:
    """Seed ``~/.alex/skills/`` with the bundled skill library on first launch.

    Called from any CLI entrypoint that the user might use as their first
    interaction with Alex — chat, dashboard (the desktop GUI's backend),
    ...
    dependency. Alex still functions without them; the user just sees an
...
def _resolve_use_tui(args) -> bool:
    ...
    Precedence (highest first):
      ...
      2. ``--tui`` flag / ``ALEX_TUI=1`` → always TUI
    ...
    if getattr(args, "cli", False):
        return False
    if getattr(args, "tui", False) or os.environ.get("ALEX_TUI") == "1":
        return True
    try:
...
def cmd_chat(args):
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'alex sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    try:
        from alex_cli.xai_retirement import (
            MIGRATION_GUIDE_URL,
            RETIREMENT_DATE,
            find_retired_xai_refs,
            format_issue,
        )
        from alex_cli.config import load_config as _load_config_for_xai_check
        ...
        if _retired_xai_refs:
            ...
            sys.stderr.write("  \033[2mRun 'alex doctor' for details.\033[0m\n\n")
    except Exception:
        pass
    ...
    if not _has_any_provider_configured():
        print()
        print(
            "It looks like Alex isn't configured yet -- no API keys or providers found."
        )
        print()
        print("  Run:  alex setup")
        print()

        from alex_cli.setup import (
        ...
        if reply in {"", "y", "yes"}:
            ...
            return
        print()
        print("You can run 'alex setup' at any time to configure.")
        sys.exit(1)
    ...
    if _termux_should_prefetch_update_check():
        try:
            from alex_cli.banner import prefetch_update_check
    ...
    # plugin/MCP discovery suppression (ALEX_SAFE_MODE is checked by
    # alex_cli/plugins.py and tools/mcp_tool.py).
    if getattr(args, "safe_mode", False):
        os.environ["ALEX_SAFE_MODE"] = "1"
        os.environ["ALEX_IGNORE_USER_CONFIG"] = "1"
        os.environ["ALEX_IGNORE_RULES"] = "1"

    # --ignore-user-config: make load_cli_config() / load_config() skip the
    # user's ~/.alex/config.yaml and return built-in defaults. Set BEFORE
    ...
    if getattr(args, "source", None):
        os.environ["ALEX_SESSION_SOURCE"] = args.source
...
def cmd_gateway(args):
    ...
    from alex_cli.gateway import gateway_command
...
def cmd_proxy(args):
    ...
    from alex_cli.proxy.cli import cmd_proxy as _cmd_proxy
...
def cmd_whatsapp(args):
    ...
    from alex_cli.config import get_env_value, save_env_value
    from alex_constants import find_node_executable, with_alex_node_path
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with Alex?")
        print()
    ...
    # has no creds.json.  Every subsequent `alex gateway` then paid a 30s
    ...
    session_dir = get_alex_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in {"y", "yes"}:
            ...
            print("  Start the gateway with: alex gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        # and `alex gateway` skips it cleanly instead of paying a 30s
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  alex gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with '⚕ Alex Agent'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  alex gateway")
        ...
        print()
        print("  Or install as a service: alex gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'alex whatsapp' to try again.")
...
def cmd_whatsapp_cloud(args):
    ...
    Distinct from ``alex whatsapp`` (the Baileys bridge wizard) — the
    two adapters are complementary, not alternatives. See
    ``alex_cli/setup_whatsapp_cloud.py``.
    """
    _require_tty("whatsapp-cloud")
    from alex_cli.setup_whatsapp_cloud import run_whatsapp_cloud_setup
...
def cmd_setup(args):
    ...
    from alex_cli.setup import run_setup_wizard
...
def cmd_postinstall(args):
    ...
    from alex_cli.config import stamp_install_method
    from alex_cli.dep_ensure import ensure_dependency

    stamp_install_method("pip")

    print("⚕ Alex post-install bootstrap")
    print()
...
def cmd_model(args):
    ...
    if getattr(args, "refresh", False):
        try:
            from alex_cli.models import clear_provider_models_cache
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``alex model``) and the setup wizard
    ...
    from alex_cli.config import (
        get_compatible_custom_providers,
        load_config,
        get_env_value,
    )
    from alex_cli.providers import resolve_provider_full
    ...
    effective_provider = (
        config_provider or os.getenv("ALEX_INFERENCE_PROVIDER") or "auto"
    )
    ...
    if not active and effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            warning = (
                f"Unknown provider '{effective_provider}'. Check 'alex model' for "
                "available providers, or run 'alex doctor' to diagnose config "
                "issues."
            )
    ...
    # PROVIDER_GROUPS in alex_cli/models.py). A multi-member group shows one
    ...
    # a leftover OPENAI_BASE_URL in ~/.alex/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.alex/.env if the active provider is not 'custom'.
    ...
    """
    from alex_cli.config import get_env_value, save_env_value, load_config
...
#
# Alex uses lightweight "auxiliary" models for side tasks (vision analysis,
...
# `alex model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `alex model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
def _all_aux_tasks() -> list[tuple[str, str, str]]:
    ...
    :meth:`alex_cli.plugins.PluginContext.register_auxiliary_task`) appear
    in the same surfaces as built-in ones without core knowing about them.
    """
    tasks = list(_AUX_TASKS)
    try:
        from alex_cli.plugins import get_plugin_auxiliary_tasks
...
def _save_aux_choice(
    ...
    *,
    ...
    """
    from alex_cli.config import load_config, save_config
...
def _aux_config_menu() -> None:
    ...
    while True:
        ...
        print("  Alex only falls back to a lightweight backend (OpenRouter,")
...
def _aux_select_for_task(task: str) -> None:
    ...
    ``alex model`` flow, then route aux tasks to them here.
    """
    from alex_cli.config import load_config
    from alex_cli.model_switch import list_authenticated_providers
...
def _aux_flow_provider_model(
    ...
    from alex_cli.auth import _prompt_model_selection
    from alex_cli.models import get_pricing_for_provider
...
def _aux_flow_custom_endpoint(task: str, task_cfg: dict) -> None:
    ...
    from alex_cli.secret_prompt import masked_secret_prompt
...
def _prompt_provider_choice(choices, *, default=0, title="Select provider:"):
    ...
    """
    try:
        from alex_cli.setup import _curses_prompt_choice
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    """
    from alex_cli.runtime_provider import _detect_api_mode_for_url
    ...
    mode_options = [
        (
            "",
            "Auto-detect",
            "Use Alex URL heuristics; best for standard OpenAI-compatible endpoints.",
        ),
        (
            "chat_completions",
            "Chat Completions",
...
def _remove_custom_provider(config):
    ...
    try:
        from alex_cli.curses_ui import curses_radiolist
...
# downstream call sites read `alex_cli.main._PROVIDER_MODELS` directly,
# so the symbol needs to be reachable as a module attribute. But importing
# the catalog eagerly costs ~55ms on every `alex` invocation — including
# fast paths like `alex --version` and slash-command dispatch that never
...
def __getattr__(name):
    ...
    if name in _LAZY_MODEL_EXPORTS:
        from alex_cli.models import _PROVIDER_MODELS
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "") -> tuple:
    """Shared API-key entry point for ``alex setup`` / ``alex model``.
    ...
    recover from a malformed paste without editing ``~/.alex/.env`` by hand.

    Returns ``(resolved_key, abort)``.  ``abort=True`` means the caller should
    ``return`` immediately — the user cancelled entry, declined to replace, or
    cleared the key and is now unconfigured.
    """
    from alex_cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER
    from alex_cli.config import save_env_value
    from alex_cli.secret_prompt import masked_secret_prompt
    ...
    from alex_cli.env_loader import format_secret_source_suffix
    ...
    if choice.startswith("c"):
        ...
        print(
            f"  API key cleared.  Re-run `alex setup` to configure {pconfig.name} again."
        )
        return "", True
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            print(
                f"    Alex will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env."
            )
            return True
        return False
...
def cmd_login(args):
    """Authenticate Alex CLI with a provider."""
    from alex_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from alex_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from alex_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from alex_cli.status import show_status
...
def cmd_cron(args):
    """Cron job management."""
    from alex_cli.cron import cron_command
...
def cmd_slack(args):
    ...
    Dispatches ``alex slack <subcommand>``. Currently supports:
    ...
    if sub in {None, ""}:
        ...
        print(
            "usage: alex slack <subcommand>\n"
            "\n"
            "subcommands:\n"
            "  manifest   Generate a Slack app manifest with every gateway\n"
            "             command registered as a native slash\n"
            "\n"
            "Run `alex slack manifest -h` for details.",
            file=sys.stderr,
        )
        return 1

    if sub == "manifest":
        from alex_cli.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from alex_cli.kanban import kanban_command
...
def cmd_project(args):
    ...
    from alex_cli.projects_cmd import projects_command
...
def cmd_hooks(args):
    ...
    from alex_cli.hooks import hooks_command
...
def cmd_doctor(args):
    ...
    from alex_cli.doctor import run_doctor

    run_doctor(args)


def cmd_security(args):
    """Dispatch `alex security <subcmd>`."""
    sub = getattr(args, "security_command", None)
    if sub in ("audit", None):
        from alex_cli.security_audit import cmd_security_audit
...
def cmd_dump(args):
    ...
    from alex_cli.dump import run_dump
...
def cmd_debug(args):
    ...
    from alex_cli.debug import run_debug
...
def cmd_config(args):
    ...
    from alex_cli.config import config_command

    config_command(args)


def cmd_backup(args):
    """Back up Alex home directory to a zip file."""
    if getattr(args, "quick", False):
        from alex_cli.backup import run_quick_backup

        run_quick_backup(args)
    else:
        from alex_cli.backup import run_backup

        run_backup(args)


def cmd_import(args):
    """Restore a Alex backup from a zip file."""
    from alex_cli.backup import run_import

    run_import(args)


def _print_version_info(*, check_updates: bool = True) -> None:
    from alex_cli.banner import format_banner_version_label
    ...
    try:
        from alex_cli.banner import check_for_updates
        from alex_cli.config import recommended_update_command
...
def cmd_uninstall(args):
    """Uninstall Alex Agent (or just the Chat GUI with --gui)."""
    ...
    if getattr(args, "gui_summary", False):
        from alex_cli.gui_uninstall import gui_install_summary
    ...
    if getattr(args, "gui", False):
        ...
        from alex_cli.uninstall import run_gui_uninstall
    ...
    from alex_cli.uninstall import run_uninstall
...
# Critical files that every ``alex`` invocation imports at startup. If any
# of these fail to parse after a pull, the CLI is bricked — the user can't
# even run ``alex update`` again to roll forward. The post-pull syntax
# guard validates these and auto-rolls-back on failure.
_UPDATE_CRITICAL_FILES = (
    "alex_cli/main.py",
    "alex_cli/config.py",
    "alex_cli/__init__.py",
    "cli.py",
    "run_agent.py",
    "model_tools.py",
    "toolsets.py",
...
def _validate_critical_files_syntax(root) -> tuple[bool, str | None, str | None]:
    ...
    These are the files imported on every ``alex`` startup; if any of them
    ...
    root = Path(root)
    with tempfile.TemporaryDirectory(prefix="alex-syntax-check-") as tmpdir:
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``alex update --gateway`` so interactive prompts (stash restore,
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    still outputs to ``alex_cli/web_dist/`` (per vite.config.ts
    outDir: "../alex_cli/web_dist"), NOT to ``web/dist/``, so Python
    ...
    dist_dir = project_root / "alex_cli" / "web_dist"
...
def _run_with_idle_timeout(
    ...
    *,
    ...
    ``alex`` launcher present but ``alex_cli`` not importable.
...
def _nixos_build_env() -> dict[str, str] | None:
    ...
    Two-tier resolution:
    1. Fast path — the alex venv's python3 (present in managed installs)
    ...
    # Tier 1: fast path — alex venv python3, no nix-shell overhead
    ...
    # a alex venv (pip / non-managed / bare-git installs).  The resolved
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    the working tree dirty and causes the next ``alex update`` to stash the
    lockfile — repeatedly.
    """
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``alex web``).
    ...
    # build path usable on a stock `py -m alex_cli.main web` invocation.
    ...
    if r1.returncode != 0:
        _say(
            ...
            + ("" if fatal else " (alex web will not be available)")
        )
        _relay(r1)
        if fatal:
...
#   - ``alex update`` can unconditionally call ``alex desktop --build-only``
#     and it will skip if nothing actually changed
#   - ``alex desktop`` (interactive launch) skips the build when the
#     stamp matches, making repeated launches fast
#
# Stamp file: $ALEX_HOME/desktop-build-stamp.json
# Schema:
#   {
...
def _desktop_stamp_path() -> Path:
    """Return the path to the desktop build stamp file under $ALEX_HOME."""
    from alex_constants import get_alex_home
    return get_alex_home() / "desktop-build-stamp.json"
...
def _desktop_build_needed(desktop_dir: Path, project_root: Path, *, source_mode: bool) -> bool:
    ...
    ``alex update`` that pulled new source but hasn't built yet).
    """
...
def _desktop_packaged_executable(desktop_dir: Path) -> Optional[Path]:
    ...
    if sys.platform == "darwin":
        candidates = list(release_dir.glob("mac*/Alex.app/Contents/MacOS/Alex"))
    elif sys.platform == "win32":
        candidates = [
            release_dir / "win-unpacked" / "Alex.exe",
            release_dir / "win-ia32-unpacked" / "Alex.exe",
            release_dir / "win-arm64-unpacked" / "Alex.exe",
        ]
    else:
        candidates = [
            release_dir / "linux-unpacked" / "alex",
            release_dir / "linux-unpacked" / "Alex",
            release_dir / "linux-arm64-unpacked" / "alex",
            release_dir / "linux-arm64-unpacked" / "Alex",
        ]
...
def _purge_electron_build_cache(desktop_dir: Path) -> list[Path]:
    ...
    '…/linux-unpacked/Alex'`` desktop build failure: a corrupt zip in the
...
def _redownload_electron_dist(
    ...
    *,
    ...
    dl_env = with_alex_node_path(env)
    if mirror:
...
def _stop_desktop_processes_locking_build(desktop_dir: Path) -> list[int]:
    ...
    On Windows a running ``Alex.exe`` keeps an exclusive lock on
    ``release/win-unpacked/Alex.exe``. electron-builder's pack then can't
    delete the stale binary and dies with ``remove …\\Alex.exe: Access is
    ...
    an unrelated "Alex" process is never touched. Best-effort: never raises.
...
def _desktop_macos_relaunchable_fixup(desktop_dir: Path) -> None:
    ...
    tampering and macOS reports "Alex is damaged and can't be opened." The
    ...
    if exe is None:
        return
    # exe = .../Alex.app/Contents/MacOS/Alex  ->  app bundle = .../Alex.app
    app = exe.parents[2]
...
def _force_adhoc_macos_signing(env: dict, *, source_mode: bool) -> bool:
    ...
    machine* (``alex desktop --build-only`` → electron-builder ``--dir``).
...
def _desktop_linux_sandbox_fixup(packaged_executable: Path) -> bool:
    ...
    if not sandbox.exists():
        print(f"✗ Alex Desktop is missing Electron's Linux sandbox helper: {sandbox}")
        return False
    ...
    if not sudo:
        print("✗ Alex Desktop requires sudo to configure Electron's Linux sandbox helper.")
        return False
...
def cmd_gui(args: argparse.Namespace):
    ...
    try:
        from alex_logging import setup_logging as _setup_logging_gui
    ...
    # with_alex_node_path() copies os.environ when called with no arg.
    env = with_alex_node_path()
    if getattr(args, "fake_boot", False):
        env["ALEX_DESKTOP_BOOT_FAKE"] = "1"
    if getattr(args, "ignore_existing", False):
        env["ALEX_DESKTOP_IGNORE_EXISTING"] = "1"
    if getattr(args, "alex_root", None):
        env["ALEX_DESKTOP_ALEX_ROOT"] = str(Path(args.alex_root).expanduser().resolve())
    if getattr(args, "cwd", None):
        env["ALEX_DESKTOP_CWD"] = str(Path(args.cwd).expanduser().resolve())
    ...
    if source_mode or not skip_build:
        ...
        if not npm:
            ...
            print("Install Node.js, then run:  alex gui")
            sys.exit(1)
    else:
        npm = None
    ...
    if skip_build:
        ...
        if not build_needed:
            ...
            if not source_mode:
                ...
                # holds Alex.exe locked on Windows, so the pack can't replace
            ...
            if build_result.returncode != 0 and not source_mode:
                ...
                if restored:
                    ...
                    for p in purged:
                        print(f"    - {p}")
                    # The purge can't remove a win-unpacked tree whose Alex.exe
            ...
            if build_result.returncode != 0:
                ...
                if sys.platform == "win32":
                    print("  If this says \"Access is denied\" on Alex.exe, close any")
                    print("  running Alex desktop window and retry.")
                print("  If the log shows Electron download retries, rebuild via a mirror:")
                print("    ELECTRON_MIRROR=<mirror-base-url> alex desktop --force-build")
                sys.exit(build_result.returncode or 1)
            packaged_executable = _desktop_packaged_executable(desktop_dir)
            if not source_mode:
                # Locally-built apps are ad-hoc signed; make them relaunchable after
                # an in-place self-update (otherwise macOS reports "Alex is
    ...
    if source_mode:
        print("→ Launching Alex Desktop from source build...")
    ...
    print(f"→ Launching packaged Alex Desktop: {packaged_executable}")
...
def _find_stale_dashboard_pids(
    *,
    ...
    """Return PIDs of ``alex dashboard`` processes other than ourselves.

    ``alex dashboard`` is a long-lived server process commonly started and
    forgotten.  When ``alex update`` replaces files on disk, the running
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned.
    This is used by the Alex Desktop Electron app to protect its own
    backend child process: when the desktop spawns ``alex dashboard`` as
    ...
    environment variable ``ALEX_DESKTOP_CHILD_PID`` on the spawned
    ...
    patterns = [
        "alex dashboard",
        "alex_cli.main dashboard",
        "alex_cli/main.py dashboard",
    ]
    self_pid = os.getpid()
    ...
    try:
        if sys.platform == "win32":
            ...
            # (rather than `pgrep -f "alex.*dashboard"`) keeps us consistent
            # with `alex_cli.gateway._scan_gateway_pids` and avoids the
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `alex update`.
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``alex update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``alex update`` invocations skip the block until a newer
    ...
    print(
        ...
        "View anytime: alex curator status)"
    )
...
def _kill_stale_dashboard_processes(
    ...
    """Kill running ``alex dashboard`` processes.

    Called at the end of ``alex update`` (default ``reason``) and also
    from ``alex dashboard --stop`` (which overrides ``reason``).  The
    ...
    """
    # When the Alex Desktop Electron app spawns this dashboard as a
    # backend child, it sets ALEX_DESKTOP_CHILD_PID so that the update
    # path can skip killing the desktop-managed process.  (#37532)
    exclude: set[int] | None = None
    raw_pid = os.environ.get("ALEX_DESKTOP_CHILD_PID")
    if raw_pid:
    ...
    if killed:
        ...
        print("    alex dashboard --port <port>")
...
def _atomic_replace_dir(src: str, dst: str) -> None:
    ...
    """
    staging = f"{dst}.alex-update-staging"
    backup = f"{dst}.alex-update-old"
...
def _update_via_zip(args):
    """Update Alex Agent by downloading a ZIP archive.
    ...
    if branch != "main":
        ...
        print(
            ...
            "or NTFS filter holding files open) and rerun `alex update "
            f"--branch {branch}`, or update against main with `alex update`."
        )
        sys.exit(1)
    zip_url = (
    ...
    tmp_dir = tempfile.mkdtemp(prefix="alex-update-")
    try:
        zip_path = os.path.join(tmp_dir, f"alex-agent-{branch}.zip")
        ...
        with zipfile.ZipFile(zip_path, "r") as zf:
            ...
            # symlink members. A GitHub source ZIP for alex-agent itself
        ...
        # GitHub ZIPs extract to alex-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"alex-agent-{branch}")
    ...
    from alex_cli.managed_uv import ensure_uv, update_managed_uv
    ...
    try:
        ...
        if result.get("user_modified"):
            ...
            print(
                "    → see them: alex skills list-modified  "
    ...
    try:
        from alex_cli.model_catalog import seed_cache_from_checkout
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "alex-update-autostash-%Y%m%d-%H%M%S"
    )
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Alex behaves unexpectedly.")
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        # files makes alex completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but Alex couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but Alex couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if Alex behaves unexpectedly.")
    return True
...
def _discard_stashed_changes(
    ...
    if stash_selector is None:
        print(
            ...
            "but Alex couldn't find the stash entry to drop."
        )
        _print_stash_cleanup_guidance(stash_ref)
        return False
    ...
    if drop.returncode != 0:
        print(
            "⚠ Configured to discard local changes, but Alex couldn't drop "
...
# =========================================================================
# Fork detection and upstream management for `alex update`
# =========================================================================
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_alex_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_alex_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _invalidate_update_cache():
    ...
    ``alex update``, every profile is now current.
    """
    homes = []
    # Default profile home (Docker-aware — uses /opt/data in Docker)
    from alex_constants import get_default_alex_root

    default_home = get_default_alex_root()
...
# Install-scoped breadcrumb dropped right before ``alex update`` mutates the
...
# survives and the next ``alex`` launch finishes the install instead of
# limping along on a half-built venv (e.g. pip wiped, a core dep like Pillow
# never landed).  Lives next to the venv (not under $ALEX_HOME) because the
...
def _recover_from_interrupted_install() -> None:
    """Finish a dependency install that a prior ``alex update`` left half-done.
    ...
    protocol stream (``alex acp`` speaks JSON-RPC on stdout) must never get
    ...
    try:
        ...
        print(
            "⚠ A previous `alex update` was interrupted mid-install — "
            "finishing dependency installation now..."
        )

        try:
            from alex_cli.managed_uv import ensure_uv
...
def _run_install_with_heartbeat(
    ...
    *,
    ...
    know ``alex update`` is still progressing even if pip/uv itself is silent.
    """
...
def _alex_exe_shims(scripts_dir: Path) -> list[Path]:
    ...
    if not _is_windows():
        return []
    return [
        scripts_dir / "alex.exe",
        scripts_dir / "alex-gateway.exe",
    ]


def _detect_concurrent_alex_instances(
    ...
    The Alex Desktop Electron app spawns ``alex.EXE`` as a backend child,
    so during ``alex update`` the user-invoked process and the desktop's
    ...
    shims (``alex.exe`` / ``alex-gateway.exe``) and returns ``(pid,
    process_name)`` pairs. The caller's own PID and its entire ancestor
    chain are excluded so the running ``alex update`` invocation never
    reports itself — this matters on Windows where the setuptools .exe
    launcher (``alex.exe``) is a separate process from the Python
    ...
    for shim in _alex_exe_shims(scripts_dir):
        try:
    ...
    # setuptools-generated alex.exe launcher is a separate native process
    ...
    # file lock) is the parent. Without excluding it, every ``alex update``
    ...
    #      alex.exe sitting *under* a non-Alex parent (e.g. a Alex
...
def _format_concurrent_instances_message(
    ...
    shim = scripts_dir / "alex.exe"
    lines = ["✗ Another alex.exe is running:"]
    ...
    lines.append("")
    lines.append("  Close Alex Desktop, exit any open `alex` REPLs, and")
    lines.append("  stop the gateway (`alex gateway stop`) before retrying.")
    lines.append("")
    if matches:
        ...
        lines.append("")
    lines.append("  Override with `alex update --force` if you've already")
    lines.append("  confirmed those processes will not write to the venv.")
    return "\n".join(lines)


def _quarantine_running_alex_exe(
    scripts_dir: Path, *, max_attempts: int = 4
) -> list[tuple[Path, Path]]:
    """Pre-empt Windows file lock on the running ``alex.exe``.
    ...
    when ``alex update`` runs, ``alex.exe`` IS the live process, and uv
    fails with ``Access is denied. (os error 5)``.

    We rename live shims to ``alex.exe.old.<unix-ms>`` first. uv then writes
    fresh shims at the original paths. The ``.old`` files are cleaned up on
    the next alex invocation by ``_cleanup_quarantined_exes``.
    ...
    handles (recovers in <1s), or the Alex Desktop backend child process
    ...
    3. Print a clear warning naming the most likely culprit (running
       Alex Desktop / gateway / REPL) and pointing to ``--force``.
    ...
    for shim in _alex_exe_shims(scripts_dir):
        ...
        print(
            "    Close Alex Desktop, exit other `alex` REPLs, stop the "
            "gateway, or pause AV scanning, then re-run `alex update`."
        )
...
def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    """Roll back ``_quarantine_running_alex_exe`` if uv didn't write replacements."""
...
def _run_quarantined_install(
    ...
    *,
    ...
    """Run an editable install, quarantining the running ``alex.exe`` first.

    Any ``pip install -e .`` (or ``--reinstall``) rewrites the entry-point
    shims, and on Windows the live ``alex.exe`` is the running process —
    pip can neither delete nor overwrite it, so without quarantine the shim
    is left missing and ``alex`` drops off PATH. This wraps
    ...
    if scripts_dir is not None:
        moved = _quarantine_running_alex_exe(scripts_dir)
    try:
...
def _cleanup_quarantined_exes(scripts_dir: Path | None = None) -> None:
    """Sweep ``alex.exe.old.*`` left by prior updates.

    Called early on every alex invocation. The .old files are unlocked once
...
def _refresh_active_lazy_features() -> None:
    ...
    first use. ``alex update`` runs ``uv pip install -e .[all]`` which
    ...
    if failed:
        ...
        print("  `alex update` once the upstream issue is resolved.")
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    On Windows, pre-renames live ``alex.exe`` / ``alex-gateway.exe`` shims
    in the venv Scripts dir before each install attempt so uv can write fresh
    copies (Windows blocks REPLACE on a running .exe but allows RENAME). See
    ``_quarantine_running_alex_exe`` for the rationale.
    """
    ...
    # hours later inside ``alex update``'s desktop-rebuild or skill-sync
...
def _verify_core_dependencies_installed(
    ...
    *,
    ...
    # outer Python that drove ``alex update``, not the venv we just wrote
    ...
    #
    # Quarantine the running ``alex.exe`` first: ``--reinstall -e .``
    # rewrites the entry-point shims, and on Windows pip can't overwrite the
    # live launcher, which would leave ``alex`` off PATH.
    scripts_dir = _venv_scripts_dir() if _is_windows() else None
    ...
    try:
        ...
        print("  ⚠ Repair install failed; check `alex update` output above.")
        return
    ...
    try:
        ...
        print(
            ...
            "Run `alex update --force` after closing other alex processes."
        )
        return
...
def _install_psutil_android_compat(
    ...
    *,
    ...
    from alex_cli.psutil_android import PSUTIL_URL, prepare_patched_psutil_sdist
...
def _ensure_uv_for_termux(pip_cmd: list[str]) -> str | None:
    ...
    standalone uv into ``$ALEX_HOME/bin/uv``, but on Termux the official
    ...
    """
    from alex_cli.managed_uv import resolve_uv
...
def _update_node_dependencies() -> None:
    ...
    # need desktop during `alex update`, so we install root-only first
    ...
    nixos_env = with_alex_node_path(_nixos_build_env())
...
class _UpdateOutputStream:
    """Stream wrapper used during ``alex update`` to survive terminal loss.

    Wraps the process's original stdout/stderr so that:

    * Every write is also mirrored to an append-only log file
      (``~/.alex/logs/update.log``) that users can inspect after the
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``alex update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``alex update`` in an SSH session or a terminal
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.alex/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``alex update --gateway``) the update is already
    ...
    try:
        ...
        # alex_cli.config.get_alex_home to simulate setup failure.
        from alex_cli.config import get_alex_home as _get_alex_home

        logs_dir = _get_alex_home() / "logs"
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    """Implement ``alex update --check``: fetch and report without installing.
    ...
    if method == "docker":
        ...
        # same long-form ``docker pull`` guidance ``alex update`` (apply
        # path) uses — telling the user to "reinstall via curl" or that
        # ".git is missing" would point them at the wrong remediation.
        from alex_cli.config import format_docker_update_message
        print(format_docker_update_message())
        sys.exit(1)
    if method == "pip":
        from alex_cli.config import recommended_update_command
        from alex_cli.banner import check_via_pypi
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``alex update`` without requiring a reinstall.  The
    ...
    ``alex`` prints ``command not found`` even though the symlink lives
    at /usr/local/bin/alex.

    Silent no-op on: non-Linux, non-root, non-FHS installs, and any system
    where ``bash -i -c 'command -v alex'`` already resolves.  Idempotent.
    """
    if sys.platform != "linux":
        return
    try:
    ...
    # /usr/local/bin/alex, code at /usr/local/lib/alex-agent).
    fhs_link = Path("/usr/local/bin/alex")
    ...
    path_comment = (
        "# Alex Agent — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _run_pre_update_backup(args) -> None:
    """Create a full zip backup of ALEX_HOME before running the update.
    ...
    ALEX_HOME directories.  The ``--backup`` flag on ``alex update``
    ...
    # ``alex_cli/config.py``). Fall back to false if the key is missing
    # so the default behaviour matches the shipped config: zipping a large
    # ALEX_HOME can add minutes to every update. Users who want the
    ...
    try:
        from alex_cli.backup import create_pre_update_backup
    except Exception as exc:
        print(
    ...
    # Render path using display_alex_home so the user sees ~/.alex/...
    try:
        from alex_constants import get_alex_home, display_alex_home

        home = get_alex_home()
        try:
            display_path = f"{display_alex_home()}/{out_path.relative_to(home)}"
        except ValueError:
    ...
    print(f"  Restore:  alex import {out_path}")
...
def _pause_windows_gateways_for_update() -> dict | None:
    ...
    alex.exe concurrent-instance guard does not see them. They still import
    ...
    if not running_pids:
        ...
        try:
            from alex_cli import gateway_windows
    ...
    print("→ Stopping Windows gateway process(es) before updating Alex...")
    try:
    ...
    # — e.g. a Windows Scheduled Task running ``pythonw.exe -m alex_cli.main
    ...
    if unmapped_pids:
        ...
        if respawnable < len(unmapped_pids):
            ...
            print("    Restart manually after update: alex gateway run")
...
def _cold_start_windows_gateway_after_update() -> None:
    ...
    ``alex gateway start`` uses (``gateway_windows._spawn_detached``).
    ...
    try:
        from alex_cli import gateway_windows
        from alex_cli.gateway import find_gateway_pids
    except Exception as exc:
...
def _discard_lockfile_churn(git_cmd, repo_root):
    ...
    ``alex update`` sees a clean tree instead of autostashing every run.
...
def cmd_update(args):
    """Update Alex Agent to the latest version.
    ...
    if is_managed():
        managed_error("update Alex Agent")
        return
    ...
    if getattr(args, "check", False):
        ...
        # what a subsequent `alex update --branch=<x>` would actually pull.
        branch = _resolve_update_branch(args)
        _cmd_update_check(
            branch=branch,
...
def _cmd_update_pip(args):
    """Update Alex via pip (for PyPI installs)."""
    from alex_cli import __version__
    from alex_cli.config import is_uv_tool_install
    ...
    if is_uv_tool_install():
        if not uv:
            ...
            sys.exit(1)
        cmd = [uv, "tool", "upgrade", "alex-agent"]
        ...
        cmd = [pipx, "upgrade", "alex-agent"]
    elif uv:
        cmd = [uv, "pip", "install", "--upgrade", "alex-agent"]
        if in_venv:
        ...
        cmd = [sys.executable, "-m", "pip", "install", "--upgrade", "alex-agent"]
    ...
    print("✓ Update complete! Restart alex to use the new version.")
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    print("⚕ Updating Alex Agent...")
    print()

    # On Windows, abort early if another alex.exe is holding the venv shim
    ...
    if _is_windows() and not getattr(args, "force", False):
        scripts_dir = _venv_scripts_dir()
        if scripts_dir is not None:
            concurrent = _detect_concurrent_alex_instances(scripts_dir)
            if concurrent:
    ...
    try:
        ...
        # though `git pull` can't touch $ALEX_HOME, this is cheap
        ...
        try:
            from alex_cli.backup import create_quick_snapshot
        ...
        # orphan merge-conflict markers in alex_cli/config.py bricked
        # every user who ran ``alex update`` for the 7 minutes between
        ...
        try:
            ...
            # so the CLI stays bootable. The user can then retry ``alex
            ...
            if not syntax_ok:
                ...
                if pre_pull_sha:
                    ...
                    if rollback_result.returncode == 0:
                        ...
                        print("  Try ``alex update`` again later once a fix lands.")
                    else:
        ...
        # the old bytecode (e.g. get_alex_home added to alex_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # the marker survives and the next ``alex`` launch finishes the
        ...
        # build.  ``alex desktop --build-only`` uses the content-hash stamp
        ...
        # never run ``alex desktop`` shouldn't be forced into a full
        # Electron build by ``alex update``.
        desktop_dir = PROJECT_ROOT / "apps" / "desktop"
        has_desktop_app = _desktop_packaged_executable(desktop_dir) is not None or _desktop_dist_exists(desktop_dir)
        from alex_constants import find_node_executable

        if (desktop_dir / "package.json").exists() and find_node_executable("npm") and has_desktop_app:
            print("→ Checking if desktop app needs rebuilding...")
            _desktop_build_cmd = [sys.executable, "-m", "alex_cli.main", "desktop", "--build-only"]
            ...
            if build_result.returncode != 0:
                build_result = subprocess.run(_desktop_build_cmd, cwd=PROJECT_ROOT, check=False)
            if build_result.returncode != 0:
                print("  ⚠ Desktop build failed (non-fatal; run `alex desktop` to retry)")
        ...
        # current — so copy it straight over ~/.alex/cache/model_catalog.json
        ...
        # modules in this process.  Reload alex_constants so that any lazy
        # import executed below (skills sync, gateway restart) sees new
        # attributes like display_alex_home() added since the last release.
        try:
            import importlib
            import alex_constants as _hc
        ...
        # seed_profile_skills() uses subprocess with an explicit ALEX_HOME so
        # it is not affected by sync_skills()'s module-level ALEX_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's ALEX_HOME env var points at the default or a named profile.
        try:
            from alex_cli.profiles import (
                list_profiles,
                seed_profile_skills,
            )
        ...
        try:
            from alex_cli.profiles import backfill_profile_envs
        ...
        if version_bump_only:
            ...
            try:
                ...
                print("     Run 'alex config migrate' to retry.")
            ...
            if response in {"", "y", "yes", "auto"}:
                ...
                if (gateway_mode or assume_yes or response == "auto") and missing_env:
                    print("  ℹ API keys require manual entry: alex config migrate")
            else:
                print()
                print("Skipped. Run 'alex config migrate' later to configure.")
        else:
        ...
        # it and warn loudly.
        try:
            from alex_cli.backup import restore_cron_jobs_if_emptied
        ...
        # check `alex curator status`. Self-stamps after printing so it
        ...
        # don't have it. Tying the refresh to ``alex update`` gives users a
        ...
        try:
            if sys.platform in ("darwin", "win32", "linux") and shutil.which("cua-driver"):
                from alex_cli.tools_config import install_cua_driver
        ...
        # When running as ``alex update --gateway`` (spawned by the gateway's
        ...
        if gateway_mode:
            _exit_code_path = get_alex_home() / ".update_exit_code"
            try:
        ...
        try:
            ...
            def _resolve_manage_cmd(scope_: str, scope_cmd_: list, svc_name_: str):
                ...
                ``systemctl ... alex-gateway*`` also qualifies
                ...
                if (
                    ...
                    try:
                        ...
                        if not sudo_ok:
                            ...
                            # (NOPASSWD for systemctl ... alex-gateway*)
            ...
            # Discover all alex-gateway* units (default + profiles)
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            unit = parts[
                                0
                            ]  # e.g. alex-gateway.service or alex-gateway-coder.service
                            ...
                            if _manage_cmd is None:
                                print(
                                    ...
                                    f"    To let `alex update` restart it automatically, allow\n"
                        ...
                        print(
                            ...
                            f"Check the gateway with: alex gateway status"
                        )
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    print("    Restart manually: alex gateway run")
                    if unmapped_count > 1:
                        print(
                            "    (or: alex -p <profile> gateway run  for each profile)"
                        )
        ...
        # Warn if legacy Alex gateway unit files are still installed.
        # When both alex.service (from a pre-rename install) and the
        # current alex-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `alex update` surfaces the issue until the user migrates.
        try:
            from alex_cli.gateway import (
                has_legacy_alex_units,
                _find_legacy_alex_units,
                supports_systemd_services,
            )

            if supports_systemd_services() and has_legacy_alex_units():
                print()
                print("⚠ Legacy Alex gateway unit(s) detected:")
                for name, path, is_sys in _find_legacy_alex_units():
                    scope = "system" if is_sys else "user"
                    print(f"    {path}  ({scope} scope)")
                print()
                print("  These pre-rename units (alex.service) fight the current")
                print("  alex-gateway.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print("    alex gateway migrate-legacy")
                print()
        ...
        print("  alex model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``alex -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    from alex_constants import display_alex_home

    action = getattr(args, "profile_action", None)

    if action is None:
        # Bare `alex profile` — show current profile status
        profile_name = get_active_profile_name()
        dhh = display_alex_home()
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    ...
                    print(f"Alias:          {alias_display} → alex -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print(f"Switched to: default (~/.alex)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        f"  Choose a custom alias:  alex profile alias {name} --name <custom>"
                    )
                    print(f"  Or access via flag:     alex -p {name} chat")
                else:
        ...
        from alex_cli import profiles as _profiles_mod
        ...
        if name and not text_value and not auto_flag:
            try:
                if _profiles_mod.normalize_profile_name(name) == "default":
                    from alex_constants import get_alex_home as _hh
        ...
        from alex_cli import profile_describer as _pd
        ...
        if dist_name:
            ...
            print(f"  (run `alex profile info {name}` for full manifest)")
        if alias_name:
            is_windows = sys.platform == "win32"
            wrapper = _get_wrapper_dir() / (f"{alias_name}.bat" if is_windows else alias_name)
            print(f"Alias:   {alias_name} → alex -p {name}  ({wrapper})")
        print()
        ...
        from alex_cli.profiles import profile_exists
        ...
        from alex_cli.profiles import rename_profile
        ...
        from alex_cli.profiles import export_profile
        ...
        from alex_cli.profiles import import_profile
        ...
        import tempfile
        from alex_cli.profile_distribution import (
            plan_install,
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="alex_dist_preview_") as tmp:
                plan = plan_install(
                    args.source,
                    Path(tmp),
            ...
            if plan.has_cron:
                print(
                    ...
                    f"  Review them with:  alex -p {plan.manifest.name} cron list"
                )
            print(f"\n  Use with:      alex -p {plan.manifest.name} chat")
        ...
        from alex_cli.profiles import get_profile_dir, normalize_profile_name
        ...
        try:
            ...
            if current is None:
                print(
                    ...
                    "Only profiles installed via `alex profile install` can be updated."
                )
                sys.exit(1)
            ...
            if plan.has_cron:
                print(
                    ...
                    f"alex -p {plan.manifest.name} cron list"
                )
        except (DistributionError, ValueError) as e:
            print(f"Error: {e}")
            sys.exit(1)
        ...
        from alex_cli.profile_distribution import describe_distribution, DistributionError
        ...
        if data.get("alex_requires"):
            print(f"Requires:     Alex {data['alex_requires']}")
        if data.get("source"):
...
def _render_distribution_plan(plan) -> None:
    ...
    from alex_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
    ...
    if mf.alex_requires:
        print(f"  Requires: Alex {mf.alex_requires}")
...
def _report_dashboard_status() -> int:
    """Print ``alex dashboard`` PIDs and return the count.

    Uses the same detection logic as ``_find_stale_dashboard_pids`` (the
    current process is excluded, but since ``alex dashboard --status``
    runs in a short-lived CLI process that never matches the pattern,
    the exclusion is irrelevant here).
    """
    pids = _find_stale_dashboard_pids()
    if not pids:
        print("No alex dashboard processes running.")
        return 0

    print(f"{len(pids)} alex dashboard process(es) running:")
    for pid in pids:
...
def _maybe_setup_dashboard_auth_interactively(args) -> None:
    ...
    ``alex dashboard register`` for OAuth.
    ...
    try:
        from alex_cli.web_server import should_require_auth
    ...
    try:
        from alex_cli.dashboard_auth import list_providers
        if list_providers():
    ...
    print("    [2] OAuth via Nous Portal (run `alex dashboard register`)")
    print("    [3] Cancel")
    print()
    ...
    try:
        from alex_cli.plugins import discover_plugins
...
def cmd_dashboard(args):
    ...
    if getattr(args, "stop", False):
        ...
        if not pids:
            print("No alex dashboard processes running.")
            sys.exit(0)
        # Reuse the same SIGTERM-grace-SIGKILL path used after `alex update`.
    ...
    # launches the dashboard (`worker dashboard` → ALEX_HOME points into
    ...
    if (
        ...
        and os.environ.get("ALEX_DESKTOP") != "1"
    ):
        ...
        reexec_argv = [
            sys.executable, "-m", "alex_cli.main",
            "-p", "default",
            "dashboard",
            "--port", str(args.port),
            "--host", args.host,
        ...
        # ALEX_HOME.  We must resolve the root explicitly instead of just
        # dropping ALEX_HOME: in the Docker layout the machine root is
        # /opt/data (set via `ENV ALEX_HOME=/opt/data`), so an unset
        # ALEX_HOME falls back to $HOME/.alex = /opt/data/.alex — an
        ...
        # update-button guard also misfires).  get_default_alex_root()
        # returns the root for both layouts: ~/.alex for a standard install
        ...
        try:
            from alex_constants import get_default_alex_root
            env["ALEX_HOME"] = str(get_default_alex_root())
        except Exception:
            # Best-effort: if root resolution fails, fall back to the prior
            # behaviour (drop ALEX_HOME) rather than block the reroute.
            env.pop("ALEX_HOME", None)
    ...
    # the same logs directory as every other Alex surface.
    try:
        from alex_logging import setup_logging as _setup_logging_gui
        _setup_logging_gui(mode="gui")
    except Exception:
    ...
    if "ALEX_WEB_DIST" not in os.environ and not getattr(args, "skip_build", False):
        ...
        _dist_root = (
            Path(os.environ["ALEX_WEB_DIST"])
            if "ALEX_WEB_DIST" in os.environ
            else PROJECT_ROOT / "alex_cli" / "web_dist"
        )
    ...
    try:
        from alex_cli.mcp_startup import start_background_mcp_discovery
    ...
    from alex_cli.web_server import start_server
...
def cmd_dashboard_register(args):
    ...
    from alex_cli.dashboard_register import cmd_dashboard_register as _impl
...
def cmd_gateway_enroll(args):
    ...
    from alex_cli.gateway_enroll import cmd_gateway_enroll as _impl
...
def cmd_completion(args, parser=None):
    ...
    from alex_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_prompt_size(args):
    ...
    from alex_cli.prompt_size import cmd_prompt_size as _impl

    _impl(args)


def cmd_logs(args):
    """View and filter Alex log files."""
    from alex_cli.logs import tail_log, list_logs
...
def _build_provider_choices() -> list[str]:
    ...
    try:
        from alex_cli.models import CANONICAL_PROVIDERS as _cp
...
# so that in ``alex -m gpt5 chat``, ``gpt5`` is correctly skipped as a
# flag value rather than misclassified as a subcommand. Kept in sync with
# the top-level flags declared in ``alex_cli/_parser.py``.
#
...
def _first_positional_argv() -> str | None:
    ...
    ``alex -m gpt5 --provider openai chat "msg"`` by skipping the
...
def _plugin_cli_discovery_needed() -> bool:
    ...
    if first is None:
        # Bare ``alex`` or only flags → defaults to ``chat``.
        return False
...
def _is_tui_chat_launch(args) -> bool:
    return bool(getattr(args, "tui", False) or os.environ.get("ALEX_TUI") == "1")
...
def _try_termux_fast_cli_launch() -> bool:
    ...
    from alex_cli._parser import build_top_level_parser
    ...
    if getattr(args, "oneshot", None):
        ...
        from alex_cli.oneshot import run_oneshot
    ...
    if args.command in {None, "chat"}:
        ...
        if interactive_prompt:
            ...
            os.environ["ALEX_DEFER_AGENT_STARTUP"] = "1"
            os.environ["ALEX_FAST_STARTUP_BANNER"] = "1"
            if getattr(args, "accept_hooks", False):
                os.environ["ALEX_ACCEPT_HOOKS"] = "1"
        else:
            _prepare_agent_startup(args)
        cmd_chat(args)
        return True
...
def _try_termux_fast_tui_launch() -> bool:
    ...
    `alex --tui` is the hot path on phones. The full parser setup imports
...
def cmd_memory(args):
    ...
    if sub == "off":
        ...
        mem_dir = get_alex_home() / "memories"
        ...
        if not existing:
            print(
                f"\n  Nothing to reset — no memory files found in {display_alex_home()}/memories/\n"
            )
            return
        ...
        print(f"  Files were in: {display_alex_home()}/memories/\n")
    else:
        from alex_cli.memory_setup import memory_command

        memory_command(args)


def cmd_acp(args):
    """Launch Alex Agent as an ACP server."""
    try:
...
def cmd_tools(args):
    ...
    if action in {"list", "disable", "enable"}:
        from alex_cli.tools_config import tools_disable_enable_command

        tools_disable_enable_command(args)
    elif action == "post-setup":
        from alex_cli.tools_config import run_post_setup_command

        sys.exit(run_post_setup_command(args))
    else:
        _require_tty("tools")
        from alex_cli.tools_config import tools_command
...
def cmd_skills(args):
    ...
    if getattr(args, "skills_action", None) == "config":
        ...
        from alex_cli.skills_config import skills_command as skills_config_command

        skills_config_command(args)
    else:
        from alex_cli.skills_hub import skills_command

        skills_command(args)


def cmd_pairing(args):
    from alex_cli.pairing import pairing_command

    pairing_command(args)


def cmd_plugins(args):
    from alex_cli.plugins_cmd import plugins_command

    plugins_command(args)


def cmd_mcp(args):
    from alex_cli.mcp_config import mcp_command

    mcp_command(args)


def cmd_claw(args):
    from alex_cli.claw import claw_command

    claw_command(args)


def main():
    """Main entry point for alex CLI."""
    # Cosmetic: make the process show up as 'alex' instead of 'python3.11'
    # in ps/top/htop.  Non-fatal — just a nicer UX.
    _set_process_title()

    # Force UTF-8 stdio on Windows before anything prints.  No-op elsewhere.
    try:
        from alex_cli.stdio import configure_windows_stdio
        configure_windows_stdio()
    except Exception:
        pass

    # Sweep stale ``alex.exe.old.*`` quarantine files left by previous
    # ``alex update`` runs on Windows. Silent no-op on non-Windows or when
    # there's nothing to clean. See ``_quarantine_running_alex_exe``.
    try:
    ...
    # ``alex skills install update``) merely defers recovery one launch;
    # under-matching (missing ``alex -p work update``) would race a recovery
    ...
    # =========================================================================
    # model command  (parser built in alex_cli/subcommands/model.py)
    # =========================================================================
    build_model_parser(subparsers, cmd_model=cmd_model)

    from alex_cli.moa_cmd import cmd_moa
    ...
    # =========================================================================
    from alex_cli.fallback_cmd import cmd_fallback
    ...
    fallback_subparsers.add_parser(
        "add",
        help="Pick a provider + model (same picker as `alex model`) and append to the chain",
    )
    fallback_subparsers.add_parser(
        "remove",
        aliases=["rm"],
    ...
    from alex_cli import secrets_cli as _secrets_cli
    ...
    # migrate command
    # =========================================================================
    from alex_cli.migrate import cmd_migrate, cmd_migrate_xai
    ...
    # =========================================================================
    # gateway + proxy commands  (parsers built in alex_cli/subcommands/gateway.py)
    # =========================================================================
    ...
    # =========================================================================
    # setup command  (parser built in alex_cli/subcommands/setup.py)
    # =========================================================================
    build_setup_parser(subparsers, cmd_setup=cmd_setup)

    # =========================================================================
    # postinstall command  (parser built in alex_cli/subcommands/postinstall.py)
    # =========================================================================
    build_postinstall_parser(subparsers, cmd_postinstall=cmd_postinstall)

    # =========================================================================
    # whatsapp command  (parser built in alex_cli/subcommands/whatsapp.py)
    # =========================================================================
    ...
    whatsapp_cloud_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "required). Distinct from `alex whatsapp` which sets up "
    ...
    # =========================================================================
    # slack command  (parser built in alex_cli/subcommands/slack.py)
    # =========================================================================
    ...
    # =========================================================================
    from alex_cli.send_cmd import register_send_subparser
    register_send_subparser(subparsers)

    # =========================================================================
    # login command  (parser built in alex_cli/subcommands/login.py)
    # =========================================================================
    build_login_parser(subparsers, cmd_login=cmd_login)

    # =========================================================================
    # logout command  (parser built in alex_cli/subcommands/logout.py)
    # =========================================================================
    build_logout_parser(subparsers, cmd_logout=cmd_logout)

    # =========================================================================
    # auth command  (parser built in alex_cli/subcommands/auth.py)
    # =========================================================================
    build_auth_parser(subparsers, cmd_auth=cmd_auth)

    # =========================================================================
    # status command  (parser built in alex_cli/subcommands/status.py)
    # =========================================================================
    build_status_parser(subparsers, cmd_status=cmd_status)

    # =========================================================================
    # cron command  (parser built in alex_cli/subcommands/cron.py)
    # =========================================================================
    ...
    # =========================================================================
    from alex_cli.portal_cli import add_parser as _add_portal_parser
    _add_portal_parser(subparsers)

    # =========================================================================
    # kanban command — multi-profile collaboration board
    # =========================================================================
    from alex_cli.kanban import build_parser as _build_kanban_parser
    ...
    # =========================================================================
    from alex_cli.projects_cmd import build_parser as _build_project_parser
    ...
    # =========================================================================
    # hooks command  (parser built in alex_cli/subcommands/hooks.py)
    # =========================================================================
    build_hooks_parser(subparsers, cmd_hooks=cmd_hooks)

    # =========================================================================
    # doctor command  (parser built in alex_cli/subcommands/doctor.py)
    # =========================================================================
    ...
    # =========================================================================
    # security command  (parser built in alex_cli/subcommands/security.py)
    # =========================================================================
    build_security_parser(subparsers, cmd_security=cmd_security)

    # =========================================================================
    # dump command  (parser built in alex_cli/subcommands/dump.py)
    # =========================================================================
    build_dump_parser(subparsers, cmd_dump=cmd_dump)

    # =========================================================================
    # debug command  (parser built in alex_cli/subcommands/debug.py)
    # =========================================================================
    build_debug_parser(subparsers, cmd_debug=cmd_debug)

    # =========================================================================
    # backup command  (parser built in alex_cli/subcommands/backup.py)
    # =========================================================================
    ...
    checkpoints_parser = subparsers.add_parser(
        "checkpoints",
        help="Inspect / prune / clear ~/.alex/checkpoints/",
        description="Manage the filesystem checkpoint store — the shadow git "
        "repo alex uses to snapshot working directories before "
        "write_file/patch/terminal calls. Lets you see how much "
        "space checkpoints occupy, force a prune, or wipe the base.",
    )
    from alex_cli.checkpoints import register_cli as _register_checkpoints_cli
    _register_checkpoints_cli(checkpoints_parser)

    # =========================================================================
    # import command  (parser built in alex_cli/subcommands/import_cmd.py)
    # =========================================================================
    build_import_cmd_parser(subparsers, cmd_import=cmd_import)

    # =========================================================================
    # config command  (parser built in alex_cli/subcommands/config.py)
    # =========================================================================
    build_config_parser(subparsers, cmd_config=cmd_config)

    # =========================================================================
    # pairing command  (parser built in alex_cli/subcommands/pairing.py)
    # =========================================================================
    build_pairing_parser(subparsers, cmd_pairing=cmd_pairing)

    # =========================================================================
    # skills command  (parser built in alex_cli/subcommands/skills.py)
    # =========================================================================
    ...
    from alex_cli.bundles import register_cli as _bundles_register, bundles_command
    _bundles_register(bundles_parser)
    bundles_parser.set_defaults(func=bundles_command)

    # =========================================================================
    # plugins command  (parser built in alex_cli/subcommands/plugins.py)
    # =========================================================================
    ...
    # subcommand — ``alex --help``, ``alex version``, ``alex logs``,
    ...
    if _plugin_cli_discovery_needed():
        try:
            ...
            from alex_cli.plugins import discover_plugins, get_plugin_manager
    ...
    try:
        from alex_cli.curator import register_cli as _register_curator_cli
    ...
    pets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "and Alex shows it reacting to agent activity across the CLI, "
            "TUI, and desktop app."
        ),
    )
    try:
        from alex_cli.pets import register_cli as _register_pets_cli
    ...
    # =========================================================================
    # memory command  (parser built in alex_cli/subcommands/memory.py)
    # =========================================================================
    build_memory_parser(subparsers, cmd_memory=cmd_memory)

    # =========================================================================
    # tools command  (parser built in alex_cli/subcommands/tools.py)
    # =========================================================================
    ...
    computer_use_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Linux.\n\n"
            "Use `alex computer-use install` to fetch and run the\n"
            "upstream cua-driver installer. This is equivalent to the\n"
            "post-setup hook that `alex tools` runs when you first\n"
            "enable the Computer Use toolset, and is a stable target\n"
            "for re-running the install if it didn't fire (e.g. when\n"
            "toggling the toolset on a returning-user setup).\n\n"
            "Use `alex computer-use doctor` to run cua-driver's\n"
    ...
    computer_use_perms = computer_use_sub.add_parser(
        ...
        description=(
            ...
            "terminal or the Alex app. `status` reports the driver's grant\n"
    ...
    def cmd_computer_use(args):
        ...
        if action == "status":
            import shutil
            import subprocess
            from alex_cli.tools_config import _cua_driver_cmd
            # Honor ALEX_CUA_DRIVER_CMD for local-build testing — same
            ...
            if path:
                version = ""
                try:
                    from alex_cli.tools_config import _cua_driver_env
                ...
                try:
                    ...
                    if st and st.get("update_available"):
                        ...
                        print("    Run: alex computer-use install --upgrade")
                    elif st:
                        print("  ✓ Up to date.")
                    else:
                        # Older driver (no check-update verb) or offline.
                        print("  Refresh to latest: alex computer-use install --upgrade")
                except Exception:
                    print("  Refresh to latest: alex computer-use install --upgrade")
                return
            print("cua-driver: not installed")
            print("  Run: alex computer-use install")
            return
        if action == "doctor":
        ...
        if action == "permissions":
            ...
            if perms_action == "status":
                ...
                if not st["platform_supported"]:
                    ...
                    sys.exit(1)
                if not st["installed"]:
                    print("cua-driver: not installed. Run: alex computer-use install")
                    sys.exit(1)
                ...
                if st["can_grant"]:  # macOS TCC permissions
                    ...
                    if not st["ready"]:
                        print("  Grant: alex computer-use permissions grant")
    ...
    # =========================================================================
    # mcp command  (parser built in alex_cli/subcommands/mcp.py)
    # =========================================================================
        ...
        if action == "list":
            ...
            if not args.yes:
                if not _confirm_prompt(
                    ...
                    print("Cancelled.")
                    return
            sessions_dir = get_alex_home() / "sessions"
            ...
            # Launch alex --resume <id> by replacing the current process
    ...
    # =========================================================================
    # insights command  (parser built in alex_cli/subcommands/insights.py)
    # =========================================================================
    build_insights_parser(subparsers, cmd_insights=cmd_insights)

    # =========================================================================
    # claw command  (parser built in alex_cli/subcommands/claw.py)
    # =========================================================================
    build_claw_parser(subparsers, cmd_claw=cmd_claw)

    # =========================================================================
    # version command  (parser built in alex_cli/subcommands/version.py)
    # =========================================================================
    build_version_parser(subparsers, cmd_version=cmd_version)

    # =========================================================================
    # update command  (parser built in alex_cli/subcommands/update.py)
    # =========================================================================
    build_update_parser(subparsers, cmd_update=cmd_update)

    # =========================================================================
    # uninstall command  (parser built in alex_cli/subcommands/uninstall.py)
    # =========================================================================
    build_uninstall_parser(subparsers, cmd_uninstall=cmd_uninstall)

    # =========================================================================
    # acp command  (parser built in alex_cli/subcommands/acp.py)
    # =========================================================================
    build_acp_parser(subparsers, cmd_acp=cmd_acp)

    # =========================================================================
    # profile command  (parser built in alex_cli/subcommands/profile.py)
    # =========================================================================
    ...
    # =========================================================================
    # dashboard command  (parser built in alex_cli/subcommands/dashboard.py)
    # =========================================================================
    ...
    # for one release. The Alex-Setup.exe success screen tells users to
    # run `alex desktop` from a terminal, so the canonical name needs
    # to be the one that appears in --help (argparse promotes the primary
    # name; aliases stay hidden).
    # =========================================================================
    # gui command  (parser built in alex_cli/subcommands/gui.py)
    # =========================================================================
    build_gui_parser(subparsers, cmd_gui=cmd_gui)

    # =========================================================================
    # logs command  (parser built in alex_cli/subcommands/logs.py)
    # =========================================================================
    build_logs_parser(subparsers, cmd_logs=cmd_logs)

    # =========================================================================
    # prompt-size command  (parser built in alex_cli/subcommands/prompt_size.py)
    # =========================================================================
    ...
    # e.g. ``alex -c Pokemon Agent Dev`` → ``alex -c 'Pokemon Agent Dev'``
    ...
    from alex_cli.config import get_container_exec_info
    ...
    # fails (e.g. 'alex -c model' where 'model' is consumed as the
    ...
    # so introspection/management commands (alex hooks list, cron
