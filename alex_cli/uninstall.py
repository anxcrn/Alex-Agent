"""
Alex Agent Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.alex/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove Alex PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing alex-agent or alex PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Alex Agent" comment and following line
                if '# Alex Agent' in line or '# alex-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('alex' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing alex
                if 'alex' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the alex wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "alex",
        Path("/usr/local/bin/alex"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains alex_cli reference)
                content = wrapper.read_text()
                if 'alex_cli' in content or 'alex-agent' in content:
                    wrapper.unlink()
...
def remove_node_symlinks(alex_home: Path) -> list:
    ...
    symlinks node/npm/npx into the same directory as the ``alex`` command:
    ...
    the FHS fix).  Only symlinks that resolve into this Alex home's ``node``
    directory are removed — links the user has repointed elsewhere (nvm, fnm,
    etc.) are left untouched.
    """
    node_dir = (alex_home / "node").resolve()
    removed = []
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``alex gateway run`` processes
    ...
    try:
        from alex_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from alex_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``ALEX_HOME`` and ``ALEX_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\alex\git\cmd``, ``%LOCALAPPDATA%\alex\git\bin``,
#      ``%LOCALAPPDATA%\alex\git\usr\bin``, ``%LOCALAPPDATA%\alex\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\alex\git\`` and Node to
#      ``%LOCALAPPDATA%\alex\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``alex dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\alex\gateway-service\`` and sometimes
...
def _alex_path_markers(alex_home: Path) -> list[str]:
    """Path-entry substrings that identify Alex-owned User-PATH entries."""
    root = str(alex_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare alex-agent install dir.
    markers = [root + "\\alex-agent", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if ALEX_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "alex".  We don't want to catch
    # unrelated entries like "calex-foo" or "ephermeral", so we look for
    # backslash-alex as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(alex_home: Path) -> list[str]:
    """Strip Alex-owned entries from User-scope PATH in the registry.
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _alex_path_markers(alex_home)
            kept: list[str] = []
...
def remove_alex_env_vars_windows() -> list[str]:
    """Delete ALEX_HOME and ALEX_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("ALEX_HOME", "ALEX_GIT_BASH_PATH"):
                try:
...
def _is_default_alex_home(alex_home: Path) -> bool:
    """Return True when ``alex_home`` points at the default (non-profile) root."""
    try:
        from alex_constants import get_default_alex_root
        return alex_home.resolve() == get_default_alex_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its ALEX_HOME directory.

    We shell out to ``alex -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current ALEX_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m alex_cli.main` so we don't depend on a `alex`
    #    wrapper that may be half-removed mid-uninstall.
    alex_invocation = [_sys.executable, "-m", "alex_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                alex_invocation + ["gateway", subcmd],
                capture_output=True,
                text=True,
                timeout=60,
                check=False,
    ...
    # 3. Wipe the profile's ALEX_HOME directory.
    try:
...
def run_gui_uninstall(args):
    ...
    Mirrors ``alex uninstall --gui``. Removes the desktop app's built
    artifacts, the packaged app bundle (best-effort), and the Electron
    userData dir — nothing under ``$ALEX_HOME`` config/sessions/.env, and
    never the Python agent or its venv.
    """
    from alex_cli.gui_uninstall import (
        agent_is_installed,
        gui_install_summary,
        uninstall_gui,
    )
    ...
    summary = gui_install_summary(alex_home)
    skip_confirm = bool(getattr(args, "yes", False))

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│         ⚕ Alex Chat GUI Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()

    if not summary["gui_installed"]:
        print("No Alex Chat GUI installation was found.")
        print(f"  Checked: {alex_home}, and the standard app locations for this OS.")
        return

    print(color("This removes the Chat GUI only. The Alex agent stays installed.", Colors.CYAN))
    print()
    ...
    print()
    if agent_is_installed(alex_home):
        print(color("Kept intact:", Colors.GREEN, Colors.BOLD))
        print(f"  • The Alex agent at {alex_home / 'alex-agent'}")
        print(f"  • Your config, sessions, and secrets under {alex_home}")
        print()
    ...
    print(color("└─────────────────────────────────────────────────────────┘", Colors.GREEN, Colors.BOLD))
    print()
    print("The Alex agent is still installed. Run 'alex' to use the CLI,")
    print("or 'alex uninstall' to remove the agent too.")
    print()
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.alex/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.alex/ for future reinstall
    """
    project_root = get_project_root()
    alex_home = get_alex_home()

    # Detect named profiles when uninstalling from the default root —
    # offer to clean them up too instead of leaving zombie ALEX_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_alex_home(alex_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    # Non-interactive fast path (``--yes``): no prompts. ``--full`` selects a
    # full wipe (code + ~/.alex data); otherwise keep-data. Named profiles
    ...
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ Alex Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {alex_home / 'config.yaml'}")
    print(f"  Secrets: {alex_home / '.env'}")
    print(f"  Data:    {alex_home / 'cron/'}, {alex_home / 'sessions/'}, {alex_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their ALEX_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Alex data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Alex code but keep your configuration and data.")
...
def _perform_uninstall(
    *,
    ...
    ``alex`` wrapper + node symlinks → remove the desktop Chat GUI artifacts →
    delete the code checkout → (Windows) remove PortableGit/Node → optionally
    wipe ``$ALEX_HOME`` data and named profiles on full uninstall.
    """
    print()
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in alex_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\alex\git\cmd, not %LOCALAPPDATA%.
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(alex_home))))
        if removed_path_entries:
            for entry in removed_path_entries:
                log_success(f"Removed from User PATH: {entry}")
        else:
            log_info("No Alex-owned PATH entries in User environment")

        log_info("Removing ALEX_HOME / ALEX_GIT_BASH_PATH User env vars...")
        removed_env = remove_alex_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No Alex-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing alex command...")
    ...
    #     (only when they still point into this Alex home's node dir, so we
    #     never clobber an existing nvm / user-managed Node).
    log_info("Removing Alex-managed node/npm/npx symlinks...")
    removed_node_links = remove_node_symlinks(alex_home)
    if removed_node_links:
        for link in removed_node_links:
            log_success(f"Removed {link}")
    else:
        log_info("No Alex-managed node/npm/npx symlinks found")
    ...
    #     step-5 rmtree(alex_home) would sweep the in-tree artifacts anyway,
    #     but the packaged app + Electron userData live OUTSIDE ALEX_HOME and
    #     must be cleaned explicitly here.
    log_info("Removing desktop Chat GUI artifacts...")
    try:
        from alex_cli.gui_uninstall import uninstall_gui
        gui_removed = uninstall_gui(alex_home)
        if not gui_removed:
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.alex/, just remove the alex-agent subdir
            if alex_home in project_root.parents or project_root.parent == alex_home:
    ...
    #     under ALEX_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(alex_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(alex_home)
    ...
    # 5. Optionally remove ~/.alex/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile ALEX_HOME dirs live under
        ...
        try:
            if alex_home.exists():
                shutil.rmtree(alex_home)
                log_success(f"Removed {alex_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {alex_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {alex_home}")
    ...
    if not full_uninstall:
        ...
        if _is_windows():
            print(color("  iex (irm https://alex-agent.charan vankudoth.com/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://alex-agent.charan vankudoth.com/install.sh | bash", Colors.DIM))
        print()
    ...
    print()
    print("Thank you for using Alex Agent! ⚕")
    print()
...
def main(argv=None) -> int:
    """Module entrypoint: ``python -m alex_cli.uninstall --mode <gui|lite|full>``.
    ...
    so ``import alex_cli`` resolves from source while the venv is torn down.

    This module imports only stdlib + ``alex_constants`` + ``alex_cli.colors``
    (and lazily ``alex_cli.gui_uninstall``), so it runs fine under a bare
    system Python with no site-packages from the venv.
    """
    import argparse

    parser = argparse.ArgumentParser(prog="python -m alex_cli.uninstall")
    parser.add_argument(
        "--mode",
