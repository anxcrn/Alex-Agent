"""`alex checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.alex/checkpoints/``.  Actions:

    alex checkpoints               # same as `status`
    alex checkpoints status        # total size, project count, breakdown
    alex checkpoints list          # per-project checkpoint counts + workdir
    alex checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    alex checkpoints clear [-f]    # nuke the entire base (asks first)
    alex checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    alex checkpoints
    alex checkpoints prune --retention-days 3 --max-size-mb 200
    alex checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: alex checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``alex checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `alex checkpoints` → status
