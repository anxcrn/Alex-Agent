...
Profile-aware location: ``$ALEX_HOME/logs/dashboard-auth.log``.
...
from ``alex_constants`` or other alex_cli modules — so it can be
...
def _resolve_log_path() -> Path:
    """``$ALEX_HOME/logs/dashboard-auth.log`` with the standard fallback.

    Mirrors ``alex_constants.get_alex_home`` semantics: env var wins,
    else ``~/.alex``. A local copy avoids an import cycle with the
    middleware which lives below ``alex_cli``.
    """
    home = os.environ.get("ALEX_HOME") or str(Path.home() / ".alex")
