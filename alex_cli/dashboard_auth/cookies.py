...
Three cookies in play:
  ...
  - alex_session_pkce: short-lived PKCE state + CSRF nonce + provider
...
setter wrote ``__Secure-alex_session_at`` would never find the value.
...
SESSION_AT_COOKIE = "alex_session_at"
SESSION_RT_COOKIE = "alex_session_rt"
PKCE_COOKIE = "alex_session_pkce"
...
def _cookie_path(prefix: str) -> str:
    ...
    Under ``X-Forwarded-Prefix: /alex`` we want ``Path=/alex`` so:
...
def set_session_cookies(
    ...
    *,
    ...
    issues a 24h rotating refresh token (alex #37247); a provider that
    ...
    ``prefix`` is the normalised X-Forwarded-Prefix value (e.g. ``/alex``)
