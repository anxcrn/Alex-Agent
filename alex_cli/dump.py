"""
Dump command for alex CLI.

Outputs a compact, plain-text summary of the user's Alex setup
...
from alex_cli.config import get_alex_home, get_env_path, get_project_root, load_config
...
def _get_git_commit(project_root: Path) -> str:
    ...
    to ``<project_root>/.alex_build_sha`` by the Dockerfile's
    ``ALEX_GIT_SHA`` build-arg (see ``alex_cli/build_info.py``).
...
def _redact(value: str) -> str:
    ...
    ``alex dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from alex_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(alex_home: Path) -> int:
...
def _cron_summary(alex_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = alex_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"alex_home:      {display_alex_home()}")
    ...
    for env_var, label in api_keys:
        ...
        # A credential added via `alex auth add openrouter` lives in the
        # credential pool, not as an env var — surface it so the dump doesn't
        # misleadingly read "not set" while `alex auth list` shows it (#42130).
    ...
    toolsets = config.get("toolsets", ["alex-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(alex_home)}")
    lines.append(f"  skills:             {_count_skills(alex_home)}")
