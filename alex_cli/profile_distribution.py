"""Profile distributions — shareable, packaged Alex profiles via git.

A distribution is a Alex profile published as a git repository (or
...
* ``alex profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``alex skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``alex profile``, not a parallel tree):

    alex profile install <source> [--name N] [--alias] [--force] [--yes]
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "backups", "cache",
    # Infrastructure
    "alex-agent", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            alex_requires=str(data.get("alex_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.alex_requires:
            out["alex_requires"] = self.alex_requires
        if self.author:
...
def check_alex_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires Alex {op}{target}, "
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``alex profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a Alex profile distribution."
            )
...
def plan_install(
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a Alex distribution."
        )

    # Version check up-front so we fail fast
    check_alex_requires(manifest.alex_requires, alex_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.alex).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="alex_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `alex profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`alex profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="alex_dist_update_") as tmp:
        plan = plan_install(
