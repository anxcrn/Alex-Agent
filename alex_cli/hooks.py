"""alex hooks — inspect and manage shell-script hooks.

Usage::

    alex hooks list
    alex hooks test <event> [--for-tool X] [--payload-file F]
    alex hooks revoke <command>
    alex hooks doctor

Consent records live under ``~/.alex/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.alex/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``alex hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: alex hooks {list|test|revoke|doctor}")
        print("Run 'alex hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.alex/config.yaml.")
        print("See `alex hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `alex hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `alex hooks test` and `alex hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Alex wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `alex hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `alex hooks doctor` would execute
