"""Shell completion script generation for alex CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_alex_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# Alex Agent bash completion
# Add to ~/.bashrc:
#   eval "$(alex completion bash)"

_alex_profiles() {{
    local profiles_dir="$HOME/.alex/profiles"
...
_alex_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_alex_profiles)" -- "$cur"))
        return
    fi
...
complete -F _alex_completion alex
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef alex
# Alex Agent zsh completion
# Add to ~/.zshrc:
...
_alex_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.alex/profiles" ]]; then
        profiles+=($HOME/.alex/profiles/*(N/:t))
    fi
...
_alex() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_alex_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'alex command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Alex Agent fish completion",
        "# Add to your config:",
        "#   alex completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __alex_profiles",
        "    echo default",
        "    if test -d $HOME/.alex/profiles",
        "        for d in $HOME/.alex/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c alex -f -s p -l profile"
        " -d 'Profile name' -xa '(__alex_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__alex_profiles)' -d 'Profile name'"
                )
