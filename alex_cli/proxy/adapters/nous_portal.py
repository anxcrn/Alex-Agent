...
Reads the user's Nous OAuth state from ``~/.alex/auth.json`` through the
...
# Endpoints inference-api.charan vankudoth.com actually serves. Anything else
...
class NousPortalAdapter(UpstreamAdapter):
    ...
    def _get_credential(
        ...
        *,
        ...
        with self._lock:
            ...
            if state is None:
                raise RuntimeError(
                    "Not logged into Nous Portal. Run `alex auth add nous` first."
                )
            ...
            if not runtime_key:
                raise RuntimeError(
                    ...
                    "Try `alex auth add nous` to re-authenticate."
                )
    ...
    # to alex_cli.auth to avoid expanding that module's public surface.
    # ------------------------------------------------------------------
