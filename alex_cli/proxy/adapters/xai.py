...
# xAI's public API is OpenAI-compatible for the endpoints Alex commonly
# uses. The Responses endpoint is included because Alex' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via Alex-managed OAuth credentials."""

    auth_hint = "alex auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`alex auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`alex auth reset xai-oauth` or re-authenticate with "
                    "`alex auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `alex auth add xai-oauth --type oauth`."
            )
