...
on Japanese locales, etc.).  Alex's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``alex_cli/main.py`` CLI dispatch,
``gateway/run.py`` startup) call :func:`configure_windows_stdio` exactly
once early in startup.

Patterns cribbed from Alex Agent (``src/utils/platform.ts``), OpenCode
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Alex is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``ALEX_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("ALEX_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Alex-managed Git install directories so
    ...
    # already-running shells don't see the change — which means alex
    ...
    # install without Alex-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Alex's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Alex's
    ``alex config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Alex (User env var in Windows Settings, or
    export in a PowerShell profile) and Alex picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Alex-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\alex\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``alex`` from right after install) retain their old PATH.

    Any subprocess Alex spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Alex-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "alex", "git", "cmd"),
        os.path.join(local_appdata, "alex", "git", "bin"),
        os.path.join(local_appdata, "alex", "git", "usr", "bin"),
        # Alex venv Scripts directory — host of the alex.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes alex, but harmless to include.
        os.path.join(local_appdata, "alex", "alex-agent", "venv", "Scripts"),
