...
the tool registry or launching Alex (mirrors the decoupled-helper pattern used
elsewhere in the CLI).

Motivated by #38798: a config migration silently rewrote the valid toolset name
``alex-cli`` to the non-existent ``alex``. ``resolve_toolset('alex')``
...
def validate_platform_toolsets(
    ...
    1. A toolset name that ``is_valid_toolset`` rejects — usually a corrupted or
       renamed entry. When ``alex-<platform>`` would have been valid (the exact
       #38798 shape, where ``cli`` held ``alex`` instead of ``alex-cli``),
    ...
    if valid_count == 0:
        warnings.append(
            ...
            "have no tools. Run `alex tools` to reconfigure."
        )
    return warnings
