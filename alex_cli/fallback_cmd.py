"""
alex fallback — manage the fallback provider chain.
...
Subcommands:
  alex fallback [list]   Show the current fallback chain (default when no subcommand)
  alex fallback add      Pick provider + model via the same picker as `alex model`,
                           then append the selection to the chain
  alex fallback remove   Pick an entry to delete from the chain
  alex fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.alex/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        from alex_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
        ...
        # `alex model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  alex fallback add")
        print()
        return
    ...
    print("  Docs: https://alex-agent.charan vankudoth.com/docs/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `alex model`, then append the selection to the chain."""
    from alex_cli.main import _require_tty, select_provider_and_model
    ...
    print("  `alex model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `alex fallback list` to view, or `alex fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``alex fallback [subcommand]``."""
