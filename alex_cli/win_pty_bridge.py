"""Windows ConPTY bridge for the `alex dashboard` chat tab.

Drop-in counterpart to ``alex_cli.pty_bridge.PtyBridge`` for native
Windows. Mirrors the exact public surface the ``/api/pty`` WebSocket
handler in ``alex_cli.web_server`` consumes: ``spawn``, ``read``,
