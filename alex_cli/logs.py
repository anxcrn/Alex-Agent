"""``alex logs`` — view and filter Alex log files.
...
Usage examples::
    ...
    alex logs                    # last 50 lines of agent.log
    alex logs -f                 # follow agent.log in real time
    alex logs errors             # last 50 lines of errors.log
    alex logs gateway -n 100    # last 100 lines of gateway.log
    alex logs gui -f            # follow gui.log (dashboard/pty/ws)
    alex logs desktop -f        # follow desktop.log (Electron app boot/backend)
    alex logs --level WARNING    # only WARNING+ lines
    alex logs --session abc123   # filter by session ID substring
    alex logs --component tools  # only tool-related lines
    alex logs --since 1h         # lines from the last hour
    alex logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_alex_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print(f"(Logs are created when Alex runs — try 'alex chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_alex_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_alex_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_alex_home()}/logs/")
        return

    print(f"Log files in {display_alex_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'alex chat' to generate logs)")
