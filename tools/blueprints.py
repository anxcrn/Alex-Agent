...
`alex skills publish` for sharing. No new source type, no new store, no new
...
@dataclass
class BlueprintSpec:
    """Parsed ``metadata.alex.blueprint`` automation spec for a skill."""
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    A skill is a blueprint iff ``metadata.alex.blueprint`` is a mapping containing
    ...
    alex = meta.get("alex") if isinstance(meta, dict) else None
    blueprint = alex.get("blueprint") if isinstance(alex, dict) else None
    if blueprint is None:
        return None
    if not isinstance(blueprint, dict):
        raise BlueprintError("metadata.alex.blueprint must be a mapping")
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str] = None) -> str:
    ...
    and emit a SKILL.md (with a ``metadata.alex.blueprint`` block) they can hand
    to ``alex skills publish`` to share. ``body`` is the plain-language
