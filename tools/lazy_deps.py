"""
Lazy dependency installer for opt-in Alex Agent backends.

Many Alex features (Mistral TTS, ElevenLabs TTS, Honcho memory, Bedrock,
...
(``alex-agent[all]``) and install them eagerly at setup time. That has
two problems:
...
remediation hint pointing at ``alex tools`` or the manual pip command.
...
* **Durable-target mode (immutable images).** When the deployment seals the
  agent's own venv (the Docker image sets ``ALEX_DISABLE_LAZY_INSTALLS=1``
  and makes ``/opt/alex`` read-only), setting
  ``ALEX_LAZY_INSTALL_TARGET`` redirects lazy installs to a writable
  ...
  installed package cannot brick Alex, which is what made it safe to seal
...
_LAZY_TARGET_ENV = "ALEX_LAZY_INSTALL_TARGET"
...
def _allow_lazy_installs() -> bool:
    ...
    2. ``ALEX_DISABLE_LAZY_INSTALLS=1`` seals the *agent venv* (set by the
    ...
    if os.environ.get("ALEX_DISABLE_LAZY_INSTALLS") == "1":
...
def _is_satisfied(spec: str) -> bool:
    ...
    ``alex update`` propagate pin bumps in :data:`LAZY_DEPS` to already-
...
def _core_constraints_file() -> Optional[Path]:
    ...
    * A backend that *requires* a version conflicting with core fails loudly
    ...
    try:
        ...
        if not lines:
            return None
        fd, path = tempfile.mkstemp(prefix="alex-core-constraints-", suffix=".txt")
...
def _venv_pip_install(specs: tuple[str, ...], *, timeout: int = 300) -> _InstallResult:
    ...
    * **Durable-target.** When :data:`_LAZY_TARGET_ENV` is set, installs into
    ...
    Mirrors the strategy in ``alex_cli.tools_config._pip_install`` but
...
def active_features() -> list[str]:
    ...
    Used by ``alex update`` to figure out which lazy backends need a
...
def refresh_active_features(*, prompt: bool = False) -> dict[str, str]:
    ...
    Intended for ``alex update``. Never raises; lazy-install failures
