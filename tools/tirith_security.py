...
it is automatically downloaded from GitHub releases to $ALEX_HOME/bin/tirith.
...
def _get_alex_home() -> str:
    """Return the Alex home directory, respecting ALEX_HOME env var."""
    return str(get_alex_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_alex_home(), ".tirith-install-failed")
...
def _alex_bin_dir() -> str:
    """Return $ALEX_HOME/bin, creating it if needed."""
    d = os.path.join(_get_alex_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $ALEX_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_alex_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $ALEX_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $ALEX_HOME/bin/tirith
    ...
    alex_bin = os.path.join(_alex_bin_dir(), "tirith")
    if os.path.isfile(alex_bin) and os.access(alex_bin, os.X_OK):
        _resolved_path = alex_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return alex_bin
