...
`mcp_tool.py`, and `alex_cli/mcp_config.py`. This module is the ONLY place
...
# ---------------------------------------------------------------------------
# AlexMCPOAuthProvider — OAuthClientProvider subclass with disk-watch
# ---------------------------------------------------------------------------


def _make_alex_provider_class() -> Optional[type]:
    ...
    class AlexMCPOAuthProvider(OAuthClientProvider):
        ...
        def __init__(
            ...
            **kwargs: Any,
            ...
            self._alex_server_name = server_name
            ...
            self._alex_preregistered = preregistered
        ...
        async def _initialize(self) -> None:
            ...
            Paired with :class:`AlexTokenStorage` persisting an absolute
            ...
            if (
                isinstance(storage, AlexTokenStorage)
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                for url in build_protected_resource_metadata_discovery_urls(
                    ...
                    try:
                        ...
                        logger.debug(
                            ...
                            self._alex_server_name, url, exc,
                        )
                        continue
                ...
                for url in build_oauth_authorization_server_metadata_discovery_urls(
                    ...
                    if asm:
                        ...
                        storage = self.context.storage
                        from tools.mcp_oauth import AlexTokenStorage
                        if isinstance(storage, AlexTokenStorage):
                            storage.save_oauth_metadata(asm)
                        logger.debug(
                            "MCP OAuth '%s': pre-flight ASM discovered "
                            "token_endpoint=%s",
                            self._alex_server_name, asm.token_endpoint,
                        )
                        break
        ...
        def _persist_oauth_metadata_if_changed(self) -> None:
            ...
            storage = self.context.storage
            from tools.mcp_oauth import AlexTokenStorage
            if not isinstance(storage, AlexTokenStorage):
                return
        ...
        async def _maybe_flag_poisoned_client(self, response: Any) -> None:
            ...
            and is handled by ``alex mcp reauth``.
            ...
            back to ``alex mcp reauth``.
            """
            try:
                if self._alex_preregistered:
                    return
    ...
    return AlexMCPOAuthProvider


# Cached at import time. Tested and used by :class:`MCPOAuthManager`.
_ALEX_PROVIDER_CLS: Optional[type] = _make_alex_provider_class()
...
class MCPOAuthManager:
    ...
    def _build_provider(
        ...
        Constructs :class:`AlexMCPOAuthProvider` directly using the helpers
        ...
        storage = AlexTokenStorage(server_name)

        if not _is_interactive() and not storage.has_cached_tokens():
            raise OAuthNonInteractiveError(
                "MCP OAuth for "
                f"'{server_name}': non-interactive environment and no "
                "cached tokens found. Run `alex mcp login "
    ...
    def remove(self, server_name: str) -> None:
        ...
        Called by ``alex mcp remove <name>`` and (indirectly) by
        ``alex mcp login <name>`` during forced re-auth.
        """
        with self._entries_lock:
