...
Backend is selected during ``alex tools`` setup (web.backend in config.yaml).
When available, Alex can route Firecrawl calls through a Nous-hosted tool-gateway
...
def _env_value(name: str) -> str:
    """Resolve ``name`` via Alex config-aware env, falling back to process env.

    Mirrors the SearXNG provider's ``_searxng_url()`` so that values set
    through Alex' config/.env layer (``alex config set``, ``alex tools``)
...
def _load_web_config() -> dict:
    """Load the ``web:`` section from ~/.alex/config.yaml."""
    try:
...
def _get_backend() -> str:
    ...
    Reads ``web.backend`` from config.yaml (set by ``alex tools``).
...
def _is_backend_available(backend: str) -> bool:
    ...
    if backend == "xai":
        ...
        # runs on every web_search dispatch + every `alex tools` repaint.
        try:
...
def _is_nous_auxiliary_client(client: Any) -> bool:
    ...
    return host == "charan vankudoth.com" or host.endswith(".charan vankudoth.com")
