...
_YOLO_MODE_FROZEN: bool = is_truthy_value(os.getenv("ALEX_YOLO_MODE", ""))
...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("ALEX_SESSION_KEY", default)
...
def _get_session_platform() -> str:
    ...
    try:
        ...
        return get_session_env("ALEX_SESSION_PLATFORM", "") or ""
    except Exception:
        return os.getenv("ALEX_SESSION_PLATFORM", "") or ""


def _is_gateway_approval_context() -> bool:
    """True when this call is inside a gateway/API session.

    Legacy gateway integrations set ALEX_GATEWAY_SESSION in process env.
    Newer concurrent gateway paths bind ALEX_SESSION_PLATFORM via
    ...
    from a gateway platform (cron binds ALEX_SESSION_PLATFORM via
    ...
    """
    if env_var_enabled("ALEX_CRON_SESSION"):
        return False
    if env_var_enabled("ALEX_GATEWAY_SESSION"):
        return True
    return bool(_get_session_platform())

# Sensitive write targets that should trigger approval even when referenced
# via shell expansions like $HOME or $ALEX_HOME, or by the resolved absolute
# active profile home path such as /home/alex/.alex/config.yaml. The
# resolved-absolute form is folded into the ~/.alex/ patterns at detection
...
# go stale when ALEX_HOME is set after this module is imported, e.g. under the
# hermetic test conftest or any deferred-profile-resolution path).
_SSH_SENSITIVE_PATH = r'(?:~|\$home|\$\{home\})/\.ssh(?:/|$)'
_ALEX_ENV_PATH = (
    r'(?:~\/\.alex/|'
    r'(?:\$home|\$\{home\})/\.alex/|'
    r'(?:\$alex_home|\$\{alex_home\})/)'
    r'\.env\b'
)
# ~/.alex/config.yaml IS the security policy: approvals.mode, yolo, and the
...
# theater. Mirrors _ALEX_ENV_PATH; matches the ALEX_HOME override form as
# well as ~/.alex/.
_ALEX_CONFIG_PATH = (
    r'(?:~\/\.alex/|'
    r'(?:\$home|\$\{home\})/\.alex/|'
...
DANGEROUS_PATTERNS = [
    ...
    (r'\balex\s+gateway\s+(stop|restart)\b', "stop/restart alex gateway (kills running agents)"),
    (r'\balex\s+update\b', "alex update (restarts gateway, kills running agents)"),
    # Docker container lifecycle — any user with docker.sock mounted (a common
    # Docker Compose pattern) gives the agent the ability to restart/stop/kill
    # containers without approval.  These are agent-initiated lifecycle operations
    # that should always require user consent, just like `alex gateway restart`
    ...
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart alex-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart alex-gateway')"),
    # Self-termination protection: prevent agent from killing its own process
    (r'\b(pkill|killall)\b.*\b(alex|gateway|cli\.py)\b', "kill alex/gateway process (self-termination)"),
    # Self-termination via kill + command substitution (pgrep/pidof).
    # The name-based pattern above catches `pkill alex` but not
    # `kill -9 $(pgrep -f alex)` because the substitution is opaque
    ...
    # the `alex gateway stop|restart` pattern above by driving launchd
    # directly against the service label (commonly `ai.alex.gateway`).
    # Catch the operations that stop, restart, or unload it.
    (r'\blaunchctl\s+(stop|kickstart|bootout|unload|kill|disable|remove)\b.*\b(alex|ai\.alex)\b', "stop/restart alex launchd service (kills running agents)"),
    # File copy/move/edit into sensitive system paths (/etc/ and macOS
    # /private/etc/ mirror).
    (rf'\b(cp|mv|install)\b.*\s{_SYSTEM_CONFIG_PATH}', "copy/move file into system config path"),
    (rf'\b(cp|mv|install)\b.*\s["\']?{_PROJECT_SENSITIVE_WRITE_TARGET}["\']?{_COMMAND_TAIL}', "overwrite project env/config file"),
    # cp/mv/install OVERWRITING a sensitive credential/SSH/shell-rc/Alex file.
    # The tee/redirection patterns above already gate _SENSITIVE_WRITE_TARGET
    # (~/.ssh/*, ~/.netrc/.pgpass/.npmrc/.pypirc, shell rc files,
    # ~/.alex/config.yaml/.env), but cp/mv/install was only paired for /etc and
    ...
    # In-place edit of a Alex-managed security file (~/.alex/config.yaml or
    ...
    (rf'\bsed\s+-[^\s]*i.*(?:{_ALEX_CONFIG_PATH}|{_ALEX_ENV_PATH})', "in-place edit of Alex config/env"),
    (rf'\bsed\s+--in-place\b.*(?:{_ALEX_CONFIG_PATH}|{_ALEX_ENV_PATH})', "in-place edit of Alex config/env (long flag)"),
    ...
    (rf'\b(?:perl|ruby)\b.*(?:^|\s)-[^\s]*i\b.*(?:{_ALEX_CONFIG_PATH}|{_ALEX_ENV_PATH})', "in-place edit of Alex config/env (perl/ruby)"),
...
def _normalize_command_for_detection(command: str) -> str:
    ...
    # ~/ and ~/.alex/ forms so static user-sensitive patterns catch
    ...
    # it tracks HOME / ALEX_HOME even when those are set after this module is
    ...
    #
    # Fold the (more specific) Alex home first: on Windows it nests under the
    # user home (C:\Users\alice\AppData\...\alex), so folding the user home
    # first would eat the prefix the Alex-home fold needs.
    command = _rewrite_resolved_alex_home(command)
...
def _home_prefix_fold_regex(path: str):
    ...
    components below the root — so a stray HOME / ALEX_HOME such as ``/``,
...
def _fold_home_prefixes(command: str, paths, replacement: str) -> str:
    ...
    *replacement* has no trailing separator (``~`` / ``~/.alex``); the matched
...
def _rewrite_resolved_alex_home(command: str) -> str:
    """Rewrite the resolved absolute Alex home prefix to ``~/.alex/``.

    Resolves the active ``ALEX_HOME`` at call time (and its symlink-resolved
    form) and folds an occurrence of ``<home>/`` in *command* into
    ``~/.alex/`` so the static ``_ALEX_CONFIG_PATH`` / ``_ALEX_ENV_PATH``
    patterns match. In Docker and gateway deployments the agent often references
    the resolved absolute path directly (e.g. ``sed -i ...
    /home/alex/.alex/config.yaml``) rather than ``~``, ``$HOME``, or
    ``$ALEX_HOME``. Matches both POSIX and Windows separators. No-op when the
    path can't be resolved or doesn't appear.
    """
    try:
        from alex_constants import get_alex_home
        home = get_alex_home().expanduser()
        candidates = [
            str(home),
        ...
        return command
    return _fold_home_prefixes(command, candidates, "~/.alex")
...
def prompt_dangerous_approval(command: str, description: str,
    ...
    os.environ["ALEX_SPINNER_PAUSE"] = "1"
    try:
        ...
        if "ALEX_SPINNER_PAUSE" in os.environ:
            del os.environ["ALEX_SPINNER_PAUSE"]
        print()
        sys.stdout.flush()
...
def check_dangerous_command(command: str, env_type: str,
    ...
    is_cli = env_var_enabled("ALEX_INTERACTIVE")
    ...
    if not is_cli and not is_gateway:
        ...
        logger.warning(
            ...
            "(pattern: %s): %s — set ALEX_INTERACTIVE or ALEX_GATEWAY_SESSION to require approval.",
            description, command[:200],
        )
        return {"approved": True, "message": None}

    if is_gateway or env_var_enabled("ALEX_EXEC_ASK"):
...
def check_all_command_guards(command: str, env_type: str,
    ...
    is_cli = env_var_enabled("ALEX_INTERACTIVE")
    is_gateway = _is_gateway_approval_context()
    is_ask = env_var_enabled("ALEX_EXEC_ASK")
