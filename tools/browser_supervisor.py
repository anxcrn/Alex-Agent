...
One ``CDPSupervisor`` runs per Alex ``task_id`` that has a reachable CDP
...
DIALOG_BRIDGE_HOST = "alex-dialog-bridge.invalid"
...
(() => {
  if (window.__alexDialogBridgeInstalled) return;
  window.__alexDialogBridgeInstalled = true;
  const ENDPOINT = "http://alex-dialog-bridge.invalid/";
