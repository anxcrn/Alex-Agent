...
def has_xai_credentials() -> bool:
    ...
    hot-paint paths (``alex tools`` repaint, tool-registration scans,
    ...
    2. ``~/.alex/auth.json`` has a non-empty ``providers.xai-oauth.tokens.access_token``
    ...
    try:
        ...
        auth_path = get_alex_home() / "auth.json"
...
def get_env_value(name: str, default=None):
    """Read ``name`` from ``~/.alex/.env`` first, then ``os.environ``.

    Wraps :func:`alex_cli.config.get_env_value` so tests can patch
    ``tools.xai_http.get_env_value`` to inject dotenv-only secrets into the
    xAI credential resolver.
    """
    try:
        from alex_cli.config import get_env_value as _alex_get_env_value

        value = _alex_get_env_value(name)
...
def alex_xai_user_agent() -> str:
    """Return a stable Alex-specific User-Agent for xAI HTTP calls."""
    try:
        from alex_cli import __version__
    except Exception:
        __version__ = "unknown"
    return f"Alex-Agent/{__version__}"


def resolve_xai_http_credentials(*, force_refresh: bool = False) -> Dict[str, str]:
    """Resolve bearer credentials for direct xAI HTTP endpoints.

    Prefers Alex-managed xAI OAuth credentials when available, then falls back
    to ``XAI_API_KEY`` resolved via ``alex_cli.config.get_env_value`` so keys
    stored in ``~/.alex/.env`` (the standard Alex location) are honored —
