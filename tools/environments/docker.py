...
from tools.environments.local import _ALEX_PROVIDER_ENV_BLOCKLIST
...
def _load_alex_env_vars() -> dict[str, str]:
    """Load ~/.alex/.env values without failing Docker command execution."""
    try:
...
def _get_active_profile_name() -> str:
    """Return the active Alex profile name, or ``"default"`` on any error.
...
def reap_orphan_containers(
    *,
    ...
    """Remove stale alex-tagged containers left behind by prior processes.

    Targets containers that match all of:

    * ``label=alex-agent=1`` (created by this codebase)
    * ``status=exited`` (running containers are NEVER reaped — they may
      belong to a sibling Alex process whose reuse path will pick them
      up; killing them would crash the sibling mid-command)
    * (optional) ``label=alex-profile=<profile_filter>`` (sweep only the
      caller's profile by default; a alex process in profile A must not
    ...
    * ``State.FinishedAt`` older than *max_age_seconds* ago (so a sibling
    ...
    even with the cleanup-fix in the prior commit, a hard-killed Alex
    process leaves its container behind permanently because there's no
    subsequent Alex process scheduled to reuse that exact (task, profile)
    pair.
    """
    docker = docker_exe or find_docker() or "docker"
    filters = ["--filter", "label=alex-agent=1", "--filter", "status=exited"]
    if profile_filter:
        filters.extend(["--filter", f"label=alex-profile={_sanitize_label_value(profile_filter)}"])
...
def find_docker() -> Optional[str]:
    ...
    Resolution order:
    1. ``ALEX_DOCKER_BINARY`` env var — explicit override (e.g. ``/usr/bin/podman``)
    ...
    override = os.getenv("ALEX_DOCKER_BINARY")
    if override and os.path.isfile(override) and os.access(override, os.X_OK):
        _docker_executable = override
        logger.info("Using ALEX_DOCKER_BINARY override: %s", override)
        return override
...
#   SETUID/SETGID - the image's init drops from root to the 'alex'
...
def _image_uses_init_entrypoint(docker_exe: str, image: str) -> bool:
    ...
    ``alex-agent:latest``) already provide their own PID-1 init and execute
...
class DockerEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        # (TERMINAL_SANDBOX_DIR, default ~/.alex/sandboxes/). Non-persistent
        ...
        # s6-overlay images (e.g. alex-agent:latest) already use /init as PID 1
        ...
        container_name = f"alex-{uuid.uuid4().hex[:8]}"
        # Labels make alex-created containers identifiable to:
        #   * the orphan reaper (`alex-agent=1` for the global sweep filter)
        #   * future cross-process reuse (`alex-task-id`, `alex-profile`)
        #   * operators running `docker ps --filter label=alex-agent=1`
        # Values are limited to the safe character set defined by
        # _sanitize_label_value(); the active Alex profile is captured at
        ...
        label_args = [
            "--label", "alex-agent=1",
            "--label", f"alex-task-id={task_label}",
            "--label", f"alex-profile={profile_name}",
        ]
        ...
        self._labels = {
            "alex-agent": "1",
            "alex-task-id": task_label,
            "alex-profile": profile_name,
        }

        # Cross-process container reuse (issue #20561 — docs claim "ONE long-lived
        # container shared across sessions").  If a prior Alex process
    ...
    def _build_init_env_args(self) -> list[str]:
        ...
        # win over the generic Alex secret blocklist. Only implicit passthrough
        # keys are filtered.
        forward_keys = explicit_forward_keys | (passthrough_keys - _ALEX_PROVIDER_ENV_BLOCKLIST)
        alex_env = _load_alex_env_vars() if forward_keys else {}
        for key in sorted(forward_keys):
            value = os.getenv(key)
            if not value:
                value = alex_env.get(key)
            if value:
                exec_env[key] = value
    ...
    def _recreate_container(self) -> bool:
        ...
        task_label = self._labels.get("alex-task-id", "")
        profile_label = self._labels.get("alex-profile", "")
        ...
        if not self._container_id:
            ...
            try:
                ...
                new_name = f"alex-{_uuid.uuid4().hex[:8]}"
    ...
    def _find_reusable_container(self, task_label: str, profile_label: str) -> Optional[tuple[str, str]]:
        ...
        matches containers that happened to be named ``alex-*`` but were
        ...
        try:
            result = subprocess.run(
                [
                    ...
                    "--filter", "label=alex-agent=1",
                    "--filter", f"label=alex-task-id={task_label}",
                    "--filter", f"label=alex-profile={profile_label}",
                    "--format", "{{.ID}}\t{{.State}}",
                ],
                capture_output=True,
                text=True,
        ...
        # container) but can happen if a previous Alex process crashed
    ...
    def cleanup(self, *, force_remove: bool = False):
        ...
        container shared across sessions" and stopping it on every Alex exit
        breaks that promise:
        ...
        * The user-visible difference between "ONE long-lived container" and
        ...
        Alex process touches a labeled container for ``2 × lifetime_seconds``
        it gets ``docker rm -f``'d at the next Alex startup. That covers the
        ...
        # outlives Alex processes, processes inside it stay alive, and
        ...
        import threading
        t = threading.Thread(target=_do_cleanup, daemon=True, name=f"alex-cleanup-{log_id}")
        t.start()
    ...
    def wait_for_cleanup(self, timeout: float = 30.0) -> bool:
        ...
        before the Python process exits — without this, ``alex /quit``
