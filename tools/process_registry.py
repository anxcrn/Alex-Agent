...
CHECKPOINT_PATH = get_alex_home() / "processes.json"
...
class ProcessRegistry:
    ...
    def _daemon_term_grace_seconds() -> float:
        ...
        """
        try:
            from alex_cli.config import read_raw_config, cfg_get, DEFAULT_CONFIG
            cfg = read_raw_config()
    ...
    def spawn_local(
        ...
        Args:
            use_pty: If True, use a pseudo-terminal via ptyprocess for interactive
                     CLI tools (Codex, Alex Agent, Python REPL). Falls back to
    ...
    def spawn_via_env(
        ...
        log_path = f"{temp_dir}/alex_bg_{session.id}.log"
        pid_path = f"{temp_dir}/alex_bg_{session.id}.pid"
        exit_path = f"{temp_dir}/alex_bg_{session.id}.exit"
    ...
    def is_session_waiting(self, session_id: str) -> bool:
        ...
        Used by the goal-loop wait barrier (``alex_cli.goals``) to support
    ...
    def _reconcile_local_exit(self, session: "ProcessSession") -> None:
        ...
        a daemon spawned by `alex update` restarting the gateway) is still
...
def format_process_notification(evt: dict) -> "str | None":
    ...
    if _exit in {-15, 143, "-15", "143"}:
        _signal = ", SIGTERM"
    if _reason == "killed":
        _status = f"terminated by {_source or 'Alex'}"
    elif _reason == "lost":
