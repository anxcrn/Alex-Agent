#!/usr/bin/env python3
"""Google Workspace API CLI for Alex Agent.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing Alex-facing JSON contract and falls back to the Python client
...
from _alex_home import get_alex_home
...
def _gws_binary() -> str | None:
    override = os.getenv("ALEX_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Alex Agent")
