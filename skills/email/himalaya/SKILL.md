...
This skill is separate from the Alex Email gateway adapter. The gateway
adapter lets people email the agent and uses Alex' built-in IMAP/SMTP
...
To reply non-interactively from Alex, read the original message, compose a reply, and pipe it:
...
**Non-interactive (use this from Alex)** — pipe the message via stdin:
