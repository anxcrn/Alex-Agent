...
ALEX_HOME_DIR="${ALEX_HOME:-$HOME/.alex}"
ALEX_CFG="${ALEX_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure Alex config has twozero_td MCP entry ──
if [[ ! -f "$ALEX_CFG" ]]; then
    echo -e " ${FAIL} Alex config not found at ${ALEX_CFG}"
    manual_steps+=("Create ${ALEX_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$ALEX_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in Alex config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to Alex config..."
    python3 -c "
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${ALEX_CFG} manually"); }
    manual_steps+=("Restart Alex session to pick up config change")
fi
