...
## 5. Alex Configuration

### Config location

`$ALEX_HOME/config.yaml` (defaults to `~/.alex/config.yaml` when `ALEX_HOME` is unset)
...
Restart the Alex session for changes to take effect. The MCP connection is
