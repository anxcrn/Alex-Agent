#!/usr/bin/env bash
# GitHub environment detection helper for Alex Agent skills.
#
...
if command -v gh &>/dev/null && gh auth status &>/dev/null 2>&1; then
    ...
    GH_AUTH_METHOD="curl"
elif _alex_env="${ALEX_HOME:-$HOME/.alex}/.env"; [ -f "$_alex_env" ] && grep -q "^GITHUB_TOKEN=" "$_alex_env" 2>/dev/null; then
