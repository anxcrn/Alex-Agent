...
_ALEX_CORE_TOOLS = [
    ...
    "terminal", "process",
    # Read the desktop GUI's embedded terminal pane (gated on ALEX_DESKTOP
    ...
    # spawned as a kanban worker (ALEX_KANBAN_TASK env set) or the current
    ...
    "computer_use",
    # Rich Artifacts
    "create_artifact", "update_artifact", "view_artifact",
...
TOOLSETS = {
    ...
    "x_search": {
        "description": (
            ...
            "enable in `alex tools` → X (Twitter) Search."
        ),
        "tools": ["x_search"],
        "includes": []
    },
    ...
    "dialogs": {
        "description": "Interactive GUI modal dialogs (multiple choice questions, permission approvals)",
        "tools": ["ask_question", "ask_permission"],
        "includes": []
    },
    ...
    "nexus": {
        "description": "Control and query the autonomous self-evolution Nexus engine",
        "tools": ["nexus"],
        "includes": []
    },
    ...
    "kanban": {
        "description": (
            ...
            "is spawned by the kanban dispatcher (ALEX_KANBAN_TASK env "
    ...
    # Coding posture (base Alex — CLI/TUI/desktop/ACP). Auto-selected in a
    ...
    "coding": {
        ...
        # non-configurable-toolset recovery loop in alex_cli/tools_config.py).
        "posture": True,
    },
    
    # ==========================================================================
    # Full Alex toolsets (CLI + messaging platforms)
    #
    ...
    # the `alex send` CLI), not by the model deciding to send on its own.
    # ==========================================================================
    ...
    "alex-cli": {
        ...
        "tools": _ALEX_CORE_TOOLS,
        "includes": []
    },

    "alex-cron": {
        # Mirrors alex-cli so cron's "default" toolset is the same set of
        # core tools users see interactively — then `alex tools` filters
        # them down per the platform config. _DEFAULT_OFF_TOOLSETS (moa,
        # homeassistant) are excluded by _get_platform_tools() unless
        # the user explicitly enables them.
        "description": "Default cron toolset - same core tools as alex-cli; gated by `alex tools`",
        "tools": _ALEX_CORE_TOOLS,
        "includes": []
    },
    ...
    "alex-email": {
        "description": "Email bot toolset - interact with Alex via email (IMAP/SMTP)",
        "tools": _ALEX_CORE_TOOLS,
        "includes": []
    },
    ...
    "alex-sms": {
        "description": "SMS bot toolset - interact with Alex via SMS (Twilio)",
        "tools": _ALEX_CORE_TOOLS,
        "includes": []
    },
    ...
    "alex-webhook": {
        ...
        "tools": _ALEX_WEBHOOK_SAFE_TOOLS,
        "includes": []
    },
    ...
    "alex-gateway": {
        ...
        "tools": [],
        "includes": ["alex-telegram", "alex-discord", "alex-whatsapp", "alex-slack", "alex-signal", "alex-bluebubbles", "alex-homeassistant", "alex-email", "alex-sms", "alex-mattermost", "alex-matrix", "alex-dingtalk", "alex-feishu", "alex-wecom", "alex-wecom-callback", "alex-weixin", "alex-qqbot", "alex-webhook", "alex-yuanbao"]
    }
}
...
def bundle_non_core_tools(toolset_name: str) -> Set[str]:
    """Return a ``alex-*`` bundle's platform-specific tools, excluding core.

    Platform bundles are defined as ``_ALEX_CORE_TOOLS + [platform extras]``.
    ...
    Bundle nesting is one level deep in practice (only ``alex-gateway``
...
def resolve_toolset(name: str, visited: Set[str] = None) -> List[str]:
    ...
    if not toolset:
        # Auto-generate a toolset for plugin platforms (alex-<name>).
        # Gives them _ALEX_CORE_TOOLS plus any tools the plugin registered
        # into a toolset matching the platform name.
        if name.startswith("alex-"):
            platform_name = name[len("alex-"):]
            try:
                from gateway.platform_registry import platform_registry
                if platform_registry.is_registered(platform_name):
                    plugin_tools = set(_ALEX_CORE_TOOLS)
                    try:
                        from tools.registry import registry
                        plugin_tools.update(
