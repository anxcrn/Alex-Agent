...
# Python-driven tool session (e.g. Alex terminal tool). A pre-set PYTHONPATH
...
if [ -n "${ALEX_INSTALL_DIR:-}" ]; then
    INSTALL_DIR="$ALEX_INSTALL_DIR"
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   write \$ALEX_HOME/.no-bundled-skills so future"
            echo "                   'alex update' runs never inject bundled skills either"
            ...
            echo "  --include-desktop  Also build the desktop app (apps/desktop -> Alex.app)"
            echo "  --dir PATH     Installation directory"
            echo "                   default (non-root):  ~/.alex/alex-agent"
            echo "                   default (root, Linux): /usr/local/lib/alex-agent"
            echo "  --alex-home PATH  Data directory (default: ~/.alex, or \$ALEX_HOME)"
            ...
            echo "  Existing installs at \$ALEX_HOME/alex-agent are preserved in-place."
            echo "  --ensure DEPS  Install only specified deps (comma-separated)"
            echo "                   Supported: node, browser, ripgrep, ffmpeg"
            echo "                   Does NOT clone repo or create venv"
            echo "  --postinstall  Run post-install setup only (for pip users)"
            echo "                   Installs optional deps + runs alex setup"
            echo "                   Does NOT clone repo or create venv"
            exit 0
            ;;
        *)
...
# intentional, but they leave the checkout dirty — which forces `alex update`
...
emit_manifest() {
    ...
    # install.ps1: the signed bootstrap installer (Alex-Setup) passes it so
    ...
    printf '%s' '{"protocol_version":1,"stages":[{"name":"prerequisites","title":"System prerequisites","category":"runtime","needs_user_input":false},{"name":"repository","title":"Download Alex Agent","category":"runtime","needs_user_input":false},{"name":"venv","title":"Create Python virtual environment","category":"runtime","needs_user_input":false},{"name":"python-deps","title":"Install Python dependencies","category":"runtime","needs_user_input":false},{"name":"node-deps","title":"Install browser-tool dependencies","category":"runtime","needs_user_input":false},{"name":"path","title":"Install alex command","category":"runtime","needs_user_input":false},{"name":"config","title":"Prepare config and skills","category":"configuration","needs_user_input":false},{"name":"setup","title":"Configure API keys and settings","category":"configuration","needs_user_input":true},{"name":"gateway","title":"Configure gateway service","category":"configuration","needs_user_input":true},'"$desktop_stage"'{"name":"complete","title":"Finish install","category":"runtime","needs_user_input":false}]}'
    printf '\n'
}
...
# Decide where the repo checkout + venv live, and where the `alex` command
# symlink goes.  Called after detect_os so $OS/$DISTRO are known.
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $ALEX_HOME/alex-agent
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $ALEX_HOME/alex-agent
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/alex-agent
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $ALEX_HOME/alex-agent — then preserve it)
#
# Always no-op when the user set --dir or $ALEX_INSTALL_DIR.
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in ALEX_HOME.
    if is_termux; then
        INSTALL_DIR="$ALEX_HOME/alex-agent"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$ALEX_HOME/alex-agent/.git" ]; then
            INSTALL_DIR="$ALEX_HOME/alex-agent"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/alex-agent)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/alex-agent"
        ...
        # /usr/local/bin/alex wrapper unable to exec the bad-interpreter venv
        # python.  See #21457.
        ...
        log_info "  Command: /usr/local/bin/alex"
        log_info "  Data:    $ALEX_HOME (unchanged)"
...
# Point a Alex-managed Node's `npm install -g` at a directory that is on
# PATH. npm's default global prefix for a bundled Node is the Node dir itself,
# so global package binaries land in $ALEX_HOME/node/bin — which is NOT on
...
# Alex's own global installs pass an explicit --prefix and are unaffected.
# Idempotent and a no-op when there is no Alex-managed npm, so calling it on
# every install run repairs pre-existing installs, not just fresh ones.
configure_managed_node_npm_prefix() {
    [ -x "$ALEX_HOME/node/bin/npm" ] || return 0
    local link_dir
    link_dir="$(get_command_link_dir)"
    mkdir -p "$ALEX_HOME/node/etc"
    printf 'prefix=%s\n' "$(dirname "$link_dir")" > "$ALEX_HOME/node/etc/npmrc"
}

get_alex_command_path() {
    local link_dir
    link_dir="$(get_command_link_dir)"
    if [ -x "$link_dir/alex" ]; then
        echo "$link_dir/alex"
    else
        echo "alex"
    fi
...
install_uv() {
    ...
    # Alex owns its own uv at $ALEX_HOME/bin/uv.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (alex_cli/managed_uv.py) looks in the same
    # place, so install.sh and `alex update` stay in sync.
    local _managed_uv="$ALEX_HOME/bin/uv"
    ...
    log_info "Installing managed uv into $ALEX_HOME/bin ..."
    mkdir -p "$ALEX_HOME/bin"

    # Two-stage: download the installer, then run it.  Piping
    # `curl | sh` masks curl failures (sh exits 0 on empty stdin)
    # and conflates network errors with installer errors.
    local _uv_install_log _uv_installer
    _uv_install_log="$(mktemp 2>/dev/null || echo "/tmp/alex-uv-install.$$.log")"
    _uv_installer="$(mktemp 2>/dev/null || echo "/tmp/alex-uv-installer.$$.sh")"
    ...
    # directly into $ALEX_HOME/bin instead of ~/.local/bin.
    if UV_UNMANAGED_INSTALL="$ALEX_HOME/bin" sh "$_uv_installer" >>"$_uv_install_log" 2>&1; then
        rm -f "$_uv_installer"
        if [ -x "$_managed_uv" ]; then
            UV_CMD="$_managed_uv"
        else
...
# string clears that floor; anything below it is replaced with the Alex-
...
check_node() {
    ...
    # Repair pre-existing Alex-managed installs where `npm install -g` lands
    ...
    # Prefer a Alex-managed Node from a previous run over a too-old system one.
    if [ -x "$ALEX_HOME/node/bin/node" ] && node_satisfies_build "$("$ALEX_HOME/node/bin/node" --version)"; then
        export PATH="$ALEX_HOME/node/bin:$PATH"
        log_success "Node.js $("$ALEX_HOME/node/bin/node" --version) found (Alex-managed)"
        HAS_NODE=true
        return 0
    fi

    if command -v node &> /dev/null; then
        log_warn "Node.js $(node --version) is too old for the desktop build (need ^20.19 or >=22.12) — installing Alex-managed Node $NODE_VERSION LTS..."
...
install_node() {
    ...
    log_info "Extracting to ~/.alex/node/..."
    ...
    # Place into ~/.alex/node/ and symlink binaries into the same bin dir
    # the alex command uses (get_command_link_dir): /usr/local/bin for root
    # FHS installs, $PREFIX/bin on Termux, ~/.local/bin otherwise.
    rm -rf "$ALEX_HOME/node"
    mkdir -p "$ALEX_HOME"
    mv "$extracted_dir" "$ALEX_HOME/node"
    rm -rf "$tmp_dir"

    local node_link_dir
    node_link_dir="$(get_command_link_dir)"
    mkdir -p "$node_link_dir"
    ln -sf "$ALEX_HOME/node/bin/node" "$node_link_dir/node"
    ln -sf "$ALEX_HOME/node/bin/npm"  "$node_link_dir/npm"
    ln -sf "$ALEX_HOME/node/bin/npx"  "$node_link_dir/npx"
    ...
    local installed_ver
    installed_ver=$("$ALEX_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.alex/node/"
    HAS_NODE=true
}
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. Alex install may complete, but web search and dependency downloads can fail."
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "Alex Agent itself does not require or retain root access."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                ...
                # index-level conflict state. Mirrors the `alex update` path
                # (#4735).
                ...
                local stash_name
                stash_name="alex-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if git stash apply "$autostash_ref"; then
                        ...
                        log_warn "Review git diff / git status if Alex behaves unexpectedly."
                    else
...
setup_path() {
    log_info "Setting up alex command..."

    if [ "$USE_VENV" = true ]; then
        ALEX_BIN="$INSTALL_DIR/venv/bin/alex"
    else
        ALEX_BIN="$(which alex 2>/dev/null || echo "")"
        if [ -z "$ALEX_BIN" ]; then
            log_warn "alex not found on PATH after install"
            return 0
        fi
    fi
    ...
    if [ ! -x "$ALEX_BIN" ]; then
        log_warn "alex entry point not found at $ALEX_BIN"
    ...
    # Create a user-facing shim for the alex command.
    # We intentionally clear PYTHONPATH/PYTHONHOME here so inherited env vars
    # can't make this launcher import modules from another checkout.
    mkdir -p "$command_link_dir"
    # Older installs created this path as a symlink to $ALEX_BIN. Without
    # the rm, `cat >` follows the symlink and overwrites the venv pip entry
    # point with this shim — making `exec "$ALEX_BIN"` self-recurse. (#21454)
    rm -f "$command_link_dir/alex"
    cat > "$command_link_dir/alex" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$ALEX_BIN" "\$@"
EOF
    chmod +x "$command_link_dir/alex"
    log_success "Installed alex launcher → $command_link_display_dir/alex"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "alex command ready"
        return 0
    fi
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        if env -i HOME="$HOME" TERM="${TERM:-dumb}" bash -i -c 'command -v alex' \
                >/dev/null 2>&1; then
        ...
        log_info "alex not on PATH in non-login shells (common on RHEL-family)"
        PATH_LINE='export PATH="/usr/local/bin:$PATH"'
        PATH_COMMENT='# Alex Agent — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# Alex Agent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# Alex Agent — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so alex works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.alex directory structure (config at top level, code in subdir)
    mkdir -p "$ALEX_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.alex/.env (top level, easy to find)
    if [ ! -f "$ALEX_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$ALEX_HOME/.env"
            log_success "Created ~/.alex/.env from template"
        else
            touch "$ALEX_HOME/.env"
            log_success "Created ~/.alex/.env"
        fi
    else
        log_info "~/.alex/.env already exists, keeping it"
    fi
    ...
    chmod 600 "$ALEX_HOME/.env"
    configure_browser_env_from_system_browser

    # Create config.yaml at ~/.alex/config.yaml (top level, easy to find)
    if [ ! -f "$ALEX_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$ALEX_HOME/config.yaml"
            log_success "Created ~/.alex/config.yaml from template"
        fi
    else
        log_info "~/.alex/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file).
    # This MUST match DEFAULT_SOUL_MD in alex_cli/default_soul.py — the
...
SOUL_EOF
    ...
    log_success "Configuration directory ready: ~/.alex/"

    # Seed bundled skills into ~/.alex/skills/ (manifest-based, one-time per skill)
    if [ "$NO_SKILLS" = true ]; then
        # Blank-slate install: write the opt-out marker and skip seeding.
        # skills_sync.py and `alex update` both honor this marker, so the
        ...
        printf '%s\n' \
            ...
            "Delete this file to re-enable sync on the next 'alex update'." \
            > "$ALEX_HOME/.no-bundled-skills" 2>/dev/null || true
        log_info "Skipping bundled skills (--no-skills). Wrote $ALEX_HOME/.no-bundled-skills"
        log_info "  Future 'alex update' runs will not inject bundled skills. Delete the marker to opt back in."
    else
        log_info "Syncing bundled skills to ~/.alex/skills/ ..."
        if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
            log_success "Skills synced to ~/.alex/skills/"
        else
            # Fallback: simple directory copy if Python sync fails
            if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$ALEX_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
                cp -r "$INSTALL_DIR/skills/"* "$ALEX_HOME/skills/" 2>/dev/null || true
                log_success "Skills copied to ~/.alex/skills/"
            fi
        fi
    fi
}
...
strip_snap_browser_override() {
    ...
    local tmp
    tmp="$(mktemp)" || return 0
    if grep -Ev '^AGENT_BROWSER_EXECUTABLE_PATH=/snap/|^# Alex Agent browser tools' "$env_file" > "$tmp"; then
        mv "$tmp" "$env_file"
        log_warn "Removed stale Snap browser override (AGENT_BROWSER_EXECUTABLE_PATH=/snap/...) from $env_file"
        log_info "Alex will use the bundled Chromium instead."
...
configure_browser_env_from_system_browser() {
    ...
    {
        echo ""
        echo "# Alex Agent browser tools — explicit browser override."
...
install_node_deps() {
    ...
    if [ -f "$INSTALL_DIR/ui-tui/package.json" ]; then
        ...
        npm install --silent 2>/dev/null || {
            log_warn "TUI npm install failed (alex --tui may not work)"
        }
        log_success "TUI dependencies installed"
    fi

    # Keep the checkout clean so `alex update` doesn't autostash every run.
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'alex setup' after install."
        return 0
    fi
    ...
    # Run alex setup using the venv Python directly (no activation needed).
    # Redirect stdin from /dev/tty so interactive prompts work when piped from curl.
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m alex_cli.main setup < /dev/tty
    else
        python -m alex_cli.main setup < /dev/tty
    fi
}

maybe_start_gateway() {
    # Check if any messaging platform tokens were configured
    ENV_FILE="$ALEX_HOME/.env"
    if [ ! -f "$ENV_FILE" ]; then
        return 0
    fi
    ...
    log_info "The gateway needs to be running for Alex to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$ALEX_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'alex whatsapp' to pair via QR code..."
            echo ""
            if prompt_yes_no "Pair WhatsApp now?" "yes"; then
                ALEX_CMD="$(get_alex_command_path)"
                $ALEX_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'alex whatsapp' to pair."
        fi
    fi
    ...
    # but opening fails with ENXIO. See #16746.
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Gateway setup skipped (no terminal available). Run 'alex gateway install' later."
        return 0
    fi
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $ALEX_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $ALEX_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: alex gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: alex gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            nohup $ALEX_CMD gateway > "$ALEX_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.alex/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: alex gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: alex gateway"
    fi
}
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $ALEX_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $ALEX_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $ALEX_HOME/cron/, sessions/, logs/"
    ...
    echo ""
    echo -e "   ${GREEN}alex${NC}              Start chatting"
    echo -e "   ${GREEN}alex setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}alex config${NC}       View/edit configuration"
    echo -e "   ${GREEN}alex config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}alex gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}alex update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'alex' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ 'alex' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'alex' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
...
ensure_browser() {
    if ! command -v node >/dev/null 2>&1; then
        local node_bin="$ALEX_HOME/node/bin/node"
        if [ -x "$node_bin" ]; then
            export PATH="$ALEX_HOME/node/bin:$PATH"
        else
    ...
    local npm_bin
    npm_bin="$(command -v npm 2>/dev/null || echo "$ALEX_HOME/node/bin/npm")"
    if [ ! -x "$npm_bin" ]; then
    ...
    local log_file
    log_file="$(mktemp)"
    if ! "$npm_bin" install -g --prefix "$ALEX_HOME/node" --silent --ignore-scripts \
    ...
    local ab_bin="$ALEX_HOME/node/bin/agent-browser"
    if [ -x "$ab_bin" ]; then
...
postinstall_mode() {
    ...
    log_info "Post-install mode: setting up Alex for pip install"
    ...
    ALEX_CMD="$(command -v alex 2>/dev/null || echo "")"
    if [ -n "$ALEX_CMD" ]; then
        log_info "Running alex setup..."
        "$ALEX_CMD" setup
    else
        log_warn "alex command not found on PATH"
        log_info "Try: python -m alex_cli.main setup"
    fi
}
...
# extract a tree MISSING the electron binary, so the `electron`->`Alex` rename
...
# _purge_electron_build_cache() used by `alex desktop`; install.sh was the only
...
install_desktop() {
    ...
    # floor (^20.19 || >=22.12) and prepends the Alex-managed Node to PATH, so
    ...
    #         retry (matches install.ps1 / `alex desktop`),
    ...
    local app=""
    if [ "$OS" = "linux" ]; then
        if [ -x "$desktop_dir/release/linux-unpacked/Alex" ]; then
            app="$desktop_dir/release/linux-unpacked/Alex"
        elif [ -x "$desktop_dir/release/linux-unpacked/alex" ]; then
            app="$desktop_dir/release/linux-unpacked/alex"
        fi
    else
        local cand
        for cand in \
            "$desktop_dir/release/mac-arm64/Alex.app" \
            "$desktop_dir/release/mac/Alex.app"; do
            if [ -d "$cand" ]; then
                app="$cand"
                break
            fi
    ...
    # trips Gatekeeper's tamper check ("Alex is damaged and can't be opened").
    ...
    # checkout stays clean for the next `alex update`.
    restore_dirty_lockfiles "$INSTALL_DIR"
}
...
run_stage_body() {
    ...
    case "$stage" in
        ...
        desktop)
            ...
            resolve_install_layout
            require_install_dir
            # Each stage runs in its own process, so the Alex-managed Node
            # provisioned during prerequisites/node-deps (at $ALEX_HOME/node/bin)
...
main() {
    ...
    # Code-scoped stamp: write next to the install tree, not into $ALEX_HOME.
    # $ALEX_HOME is a shared data dir (it can be bind-mounted into a Docker
    # gateway too), so a stamp there gets clobbered by the container's 'docker'
    # stamp and wrongly blocks 'alex update' on this host install.
