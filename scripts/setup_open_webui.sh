...
# Bootstrap Open WebUI against Alex Agent's OpenAI-compatible API server.
#
# Idempotent by design:
# - ensures ~/.alex/.env has API server settings
# - installs Open WebUI into ~/.local/open-webui-venv
# - writes a reusable launcher at ~/.local/bin/start-open-webui-alex.sh
...
#   ALEX_API_PORT=8642
#   ALEX_API_HOST=127.0.0.1
#   ALEX_API_MODEL_NAME='Alex Agent'

OPEN_WEBUI_PORT="${OPEN_WEBUI_PORT:-8080}"
OPEN_WEBUI_HOST="${OPEN_WEBUI_HOST:-127.0.0.1}"
OPEN_WEBUI_NAME="${OPEN_WEBUI_NAME:-Alex Agent WebUI}"
...
ALEX_ENV_FILE="${ALEX_ENV_FILE:-$HOME/.alex/.env}"
ALEX_API_PORT="${ALEX_API_PORT:-8642}"
ALEX_API_HOST="${ALEX_API_HOST:-127.0.0.1}"
ALEX_API_CONNECT_HOST="${ALEX_API_CONNECT_HOST:-127.0.0.1}"
ALEX_API_MODEL_NAME="${ALEX_API_MODEL_NAME:-Alex Agent}"
ALEX_API_BASE_URL="http://${ALEX_API_CONNECT_HOST}:${ALEX_API_PORT}/v1"
LAUNCHER_PATH="$HOME/.local/bin/start-open-webui-alex.sh"
...
write_launcher() {
  ...
  quoted_base_url="$(shell_quote "$ALEX_API_BASE_URL")"
...
ensure_env_permissions() {
  chmod 600 "$ALEX_ENV_FILE" 2>/dev/null || true
}

install_launchd_service() {
  local plist="$HOME/Library/LaunchAgents/ai.openwebui.alex.plist"
...
<dict>
  <key>Label</key>
  <string>ai.openwebui.alex</string>
...
EOF
  ...
  launchctl enable "gui/$(id -u)/ai.openwebui.alex"
  launchctl kickstart -k "gui/$(id -u)/ai.openwebui.alex"
}

install_systemd_user_service() {
  require_cmd systemctl
  local unit_dir="$HOME/.config/systemd/user"
  local unit="$unit_dir/openwebui-alex.service"
  mkdir -p "$unit_dir"
  cat > "$unit" <<EOF
[Unit]
Description=Open WebUI connected to Alex Agent
After=default.target

[Service]
Type=simple
ExecStart=/bin/bash %h/.local/bin/start-open-webui-alex.sh
Restart=always
RestartSec=3
WorkingDirectory=%h
StandardOutput=append:%h/.alex/logs/openwebui.log
StandardError=append:%h/.alex/logs/openwebui.error.log

[Install]
WantedBy=default.target
EOF
  systemctl --user daemon-reload
  systemctl --user enable --now openwebui-alex.service
}
...
main() {
  ...
  local api_key
  api_key="$(get_env_value API_SERVER_KEY "$ALEX_ENV_FILE")"
  if [[ -z "$api_key" ]]; then
    api_key="$(generate_secret)"
  fi

  log 'Ensuring Alex API server is configured...'
  upsert_env API_SERVER_ENABLED true "$ALEX_ENV_FILE"
  upsert_env API_SERVER_HOST "$ALEX_API_HOST" "$ALEX_ENV_FILE"
  upsert_env API_SERVER_PORT "$ALEX_API_PORT" "$ALEX_ENV_FILE"
  upsert_env API_SERVER_MODEL_NAME "$ALEX_API_MODEL_NAME" "$ALEX_ENV_FILE"
  upsert_env API_SERVER_KEY "$api_key" "$ALEX_ENV_FILE"
  ensure_env_permissions

  log 'Restarting Alex gateway so API server settings take effect...'
  alex gateway restart >/dev/null 2>&1 || true
  sleep 4
  if ! curl -fsS "http://${ALEX_API_CONNECT_HOST}:${ALEX_API_PORT}/health" >/dev/null; then
    log 'Alex API server did not answer on the first check. Trying to start gateway in the background...'
    nohup alex gateway run >/dev/null 2>&1 &
    sleep 6
  fi
  curl -fsS "http://${ALEX_API_CONNECT_HOST}:${ALEX_API_PORT}/health" >/dev/null
  ...
  log "Alex API endpoint: ${ALEX_API_BASE_URL}"
