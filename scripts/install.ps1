# ============================================================================
# Alex Agent Installer for Windows
# ============================================================================
...
param(
    ...
    [string]$Commit = "",
    [string]$Tag = "",
    [string]$AlexHome = $(if ($env:ALEX_HOME) { $env:ALEX_HOME } else { "$env:LOCALAPPDATA\alex" }),
    [string]$InstallDir = $(if ($env:ALEX_HOME) { "$env:ALEX_HOME\alex-agent" } else { "$env:LOCALAPPDATA\alex\alex-agent" }),
    ...
    # builds apps/desktop into a launchable Alex.exe.
    #
    # Why opt-in:
    #   * Alex-Setup.exe (the signed Tauri bootstrap installer) passes
    ...
    #     from inside an already-launched Alex.exe; if THAT recursively
    #     built apps/desktop it would try to overwrite the live Alex.exe
    ...
    #     `alex desktop` already builds on demand.
...
$RepoUrlSsh = "git@github.com:charan vankudoth/alex-agent.git"
$RepoUrlHttps = "https://github.com/charan vankudoth/alex-agent.git"
$PythonVersion = "3.11"
...
function Write-Banner {
    ...
    Write-Host "|             * Alex Agent Installer                    |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host "|  An open source AI agent by charan vankudoth.              |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host ""
}
...
function Write-BrowserEnv {
    ...
    if (-not (Test-Path $AlexHome)) {
        New-Item -ItemType Directory -Force -Path $AlexHome | Out-Null
    }
    $envFile = Join-Path $AlexHome ".env"
...
function Install-AgentBrowser {
    ...
    $prefixDir = Join-Path $AlexHome "node"
...
function Install-Uv {
    # Alex owns its own uv at $AlexHome\bin\uv.exe.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (alex_cli/managed_uv.py) looks in the same
    # place, so install.ps1 and `alex update` stay in sync.
    $managedUv = Join-Path $AlexHome "bin\uv.exe"
    ...
    Write-Info "Installing managed uv into $AlexHome\bin ..."
    New-Item -ItemType Directory -Path (Join-Path $AlexHome "bin") -Force | Out-Null

    # UV_INSTALL_DIR tells the astral installer to place the binary
    # directly into $AlexHome\bin instead of ~/.local/bin.
    $prevEAP = $ErrorActionPreference
    try {
        $ErrorActionPreference = "Continue"
        $env:UV_INSTALL_DIR = Join-Path $AlexHome "bin"
...
function Resolve-UvCmd {
    ...
    # sibling process and ALEX_HOME wasn't propagated).
...
function Resolve-AvailablePythonVersion {
    ...
    # ``$script:PythonVersion = $fallbackVer`` mutation.  Under Alex-Setup.exe
...
function Install-Git {
    ...
    which Alex uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\alex\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    Alex needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the Alex copy of Git ourselves is predictable and
    recoverable: if it ever breaks, ``Remove-Item %LOCALAPPDATA%\alex\git``
    and re-running this installer fully recovers.

    After install we locate ``bash.exe`` and persist the path in
    ``ALEX_GIT_BASH_PATH`` (User scope) so Alex can find it in a fresh
    ...
    # Download PortableGit into $AlexHome\git.  Always works as long as
    # we can reach github.com -- no admin, no winget, no reliance on the
    # user's possibly-broken system Git install.
    Write-Info "Git not found -- downloading PortableGit to $AlexHome\git\ ..."
    ...
    try {
        ...
        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected -- PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent Alex features (terminal tool, agent-browser) will not work on this machine."
...
function Set-GitBashEnvVar {
    ...
    ``ALEX_GIT_BASH_PATH`` (User env scope) so Alex can find it even before
    ...
    #
    # Layouts:
    #   PortableGit (our default): $AlexHome\git\bin\bash.exe
    #   MinGit (32-bit fallback):  $AlexHome\git\usr\bin\bash.exe
    $candidates += "$AlexHome\git\bin\bash.exe"       # PortableGit layout (primary)
    $candidates += "$AlexHome\git\usr\bin\bash.exe"   # MinGit / PortableGit usr\bin fallback
    ...
    foreach ($candidate in $candidates) {
        if ($candidate -and (Test-Path $candidate)) {
            [Environment]::SetEnvironmentVariable("ALEX_GIT_BASH_PATH", $candidate, "User")
            $env:ALEX_GIT_BASH_PATH = $candidate
            Write-Info "Set ALEX_GIT_BASH_PATH=$candidate"
            return
        }
    }

    Write-Warn "Could not locate bash.exe -- Alex may not find Git Bash."
    Write-Info "If needed, set ALEX_GIT_BASH_PATH manually to your bash.exe path."
}
...
function Test-Node {
    ...
    $managedNode = "$AlexHome\node\node.exe"
    if ((Test-Path $managedNode) -and (Test-NodeVersionOk (& $managedNode --version))) {
        $version = & $managedNode --version
        $env:Path = "$AlexHome\node;$env:Path"
        Write-Success "Node.js $version found (Alex-managed)"
        $script:HasNode = $true
        return $true
    }

    Write-Info "Installing Alex-managed Node.js $NodeVersion LTS..."
    ...
    # The portable zip path drops node.exe + npm into $AlexHome\node\
    ...
    Write-Info "Downloading portable Node.js $NodeVersion to $AlexHome\node\ ..."
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\alex-node-extract"
            ...
            if ($extractedDir) {
                if (Test-Path "$AlexHome\node") { Remove-Item -Recurse -Force "$AlexHome\node" }
                Move-Item $extractedDir.FullName "$AlexHome\node"
                ...
                $version = & "$AlexHome\node\node.exe" --version
                Write-Success "Node.js $version installed to $AlexHome\node\ (portable, user-scoped)"
                $script:HasNode = $true
...
function Install-SystemPackages {
    ...
    if ($hasWinget) {
        ...
        foreach ($pkg in $wingetPkgs) {
            $log = "$env:TEMP\alex-winget-$($pkg -replace '[^A-Za-z0-9]','_')-$(Get-Random).log"
            $pkgLogs[$pkg] = $log
...
function Install-Repository {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                if (-not [string]::IsNullOrWhiteSpace(($statusOut -join "`n"))) {
                    ...
                    # the index conflict state is dropped. Mirrors the `alex
                    # update` path (#4735).
                    ...
                    $stashName = "alex-install-autostash-" + (Get-Date -Format "yyyyMMdd-HHmmss")
                ...
                if ($autostashRef) {
                    ...
                    if ($restoreNow) {
                        ...
                        if ($LASTEXITCODE -eq 0) {
                            ...
                            Write-Warn "Review git diff / git status if Alex behaves unexpectedly."
                        } else {
            ...
            try {
                ...
                Write-Info "terminals, running alex processes) and try again."
                throw
            }
        }
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($Commit) {
                    $zipUrl = "https://github.com/charan vankudoth/alex-agent/archive/$Commit.zip"
                    $zipLabel = $Commit
                } elseif ($Tag) {
                    $zipUrl = "https://github.com/charan vankudoth/alex-agent/archive/refs/tags/$Tag.zip"
                    $zipLabel = $Tag
                } else {
                    $zipUrl = "https://github.com/charan vankudoth/alex-agent/archive/refs/heads/$Branch.zip"
                    $zipLabel = $Branch
                }
                $zipPath = "$env:TEMP\alex-agent-$zipLabel.zip"
                $extractPath = "$env:TEMP\alex-agent-extract"
    ...
    # next `alex update` checkout aborts on a "dirty" tree the user never
...
function Install-Venv {
    ...
    # Re-resolve the interpreter before creating the venv.  Under Alex-Setup.exe
    ...
    if (Test-Path "venv") {
        ...
        # speedups.pyd) are loaded as DLLs by any running alex process.
        ...
        if ($env:OS -eq "Windows_NT") {
            $myPid = $PID
            Write-Info "Stopping any running alex processes before recreating venv..."
            # The launcher CLI (alex.exe) plus its child tree.
            & taskkill /F /T /IM alex.exe /FI "PID ne $myPid" 2>$null | Out-Null
            # taskkill /IM alex.exe is NOT enough: the gateway/agent that a
            # scheduled task or watchdog autostarts runs as
            # `pythonw.exe -m alex_cli.main gateway run` straight out of
            # venv\Scripts\, so its image name is python/pythonw, not alex.exe.
            ...
            # user (see alex_cli/gateway_windows.py), so the installer always
...
function Install-Dependencies {
    ...
    if (Test-Path "uv.lock") {
        ...
        # empty and producing the broken state where `alex.exe` exists
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install alex-agent package even with no extras. Inspect the uv pip install output above."
    }
    ...
    # isn't set, leaving venv\ empty and alex.exe broken with
    ...
    # here, not 30 seconds later when the user runs `alex`.
    if (-not $NoVenv) {
    ...
    # users hit and lazy-import errors from `alex dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        $ErrorActionPreference = $prevEAP
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable -- `alex dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `alex dashboard` should now work."
            } else {
...
function Set-PathVariable {
    Write-Info "Setting up alex command..."
    
    if ($NoVenv) {
        $alexBin = "$InstallDir"
    } else {
        $alexBin = "$InstallDir\venv\Scripts"
    }
    
    # Add the venv Scripts dir to user PATH so alex is globally available
    # On Windows, the alex.exe in venv\Scripts\ has the venv Python baked in
    $currentPath = [Environment]::GetEnvironmentVariable("Path", "User")
    
    if ($currentPath -notlike "*$alexBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$alexBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $alexBin"
    } else {
        Write-Info "PATH already configured"
    }
    
    # Set ALEX_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\alex instead
    # of the Unix default ~/.alex
    $currentAlexHome = [Environment]::GetEnvironmentVariable("ALEX_HOME", "User")
    if (-not $currentAlexHome -or $currentAlexHome -ne $AlexHome) {
        [Environment]::SetEnvironmentVariable("ALEX_HOME", $AlexHome, "User")
        Write-Success "Set ALEX_HOME=$AlexHome"
    }
    $env:ALEX_HOME = $AlexHome
    
    # Update current session
    $env:Path = "$alexBin;$env:Path"
    
    Write-Success "alex command ready"
}

function Write-BootstrapMarker {
    # Writes $InstallDir\.alex-bootstrap-complete which tells the Alex
    ...
    $markerPath = Join-Path $InstallDir ".alex-bootstrap-complete"
    $marker = [ordered]@{
        schemaVersion = 1
...
function Copy-ConfigTemplates {
    ...
    # Create the ALEX_HOME directory structure ($AlexHome, default %LOCALAPPDATA%\alex)
    New-Item -ItemType Directory -Force -Path "$AlexHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$AlexHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$AlexHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$AlexHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$AlexHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$AlexHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$AlexHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$AlexHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$AlexHome\skills" | Out-Null

    
    # Create .env
    $envPath = "$AlexHome\.env"
    ...
    # Create config.yaml
    $configPath = "$AlexHome\config.yaml"
    ...
    # (the default PS5 behaviour), and Alex's prompt-injection scanner
    ...
    # PowerShell version.
    $soulPath = "$AlexHome\SOUL.md"
    if (-not (Test-Path $soulPath)) {
        # MUST match DEFAULT_SOUL_MD in alex_cli/default_soul.py. The runtime
...
"@
    ...
    Write-Success "Configuration directory ready: $AlexHome"
    
    # Seed bundled skills into $AlexHome\skills (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to $AlexHome\skills ..."
    $pythonExe = "$InstallDir\venv\Scripts\python.exe"
    if (Test-Path $pythonExe) {
        try {
            & $pythonExe "$InstallDir\tools\skills_sync.py" 2>$null
            Write-Success "Skills synced to $AlexHome\skills"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$AlexHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to $AlexHome\skills"
            }
        }
    }
}
...
function Install-NodeDeps {
    if (-not $HasNode) {
        # Cross-process driver mode (Alex-Setup.exe runs each -Stage NAME
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'alex setup tools' later."
        return
    }
    $npmExe = $npmCmd.Source
    if ($npmExe -like "*.ps1") {
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\alex-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\alex-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        $tuiLog = "$env:TEMP\alex-npm-tui-$(Get-Random).log"
...
# the final `electron` -> `Alex` rename dies with ENOENT and every re-run
...
function Install-Desktop {
    # Build apps/desktop into a launchable Alex.exe. Only called from
    ...
    #
    # The Tauri bootstrap installer's launch_alex_desktop command
    # resolves apps/desktop/release/win-unpacked/Alex.exe directly,
    ...
    # enforces the build floor (^20.19 || >=22.12) and prepends the Alex-managed
    ...
    # The --dir mode produces an unpacked Alex.exe in
    ...
    # are NOT trying to work around). The Alex icon + product name are
    # stamped onto Alex.exe by our own rcedit step (Set-DesktopExeIdentity)
    ...
    $buildLog = "$env:TEMP\alex-desktop-build-$(Get-Random).log"
    ...
    $exeCandidates = @(
        "$desktopDir\release\win-unpacked\Alex.exe",
        "$desktopDir\release\win-arm64-unpacked\Alex.exe"
    )
    $found = $false
    $desktopExe = $null
    ...
    if (-not $found) {
        throw "Desktop build completed but no Alex.exe was found under $desktopDir\release\*-unpacked\"
    }

    # 3b. The Alex icon + identity are stamped onto Alex.exe by the
    ...
    #    Alex.exe. We deliberately do NOT point them at `alex desktop`: that
...
function New-DesktopShortcuts {
    ...
    try {
        ...
        $targets = @(
            (Join-Path ([Environment]::GetFolderPath('Programs')) 'Alex.lnk'),
            (Join-Path ([Environment]::GetFolderPath('Desktop')) 'Alex.lnk')
        )
        ...
        # with the Alex icon, but without this the shortcut can keep drawing
...
function Install-PlatformSdks {
    ...
    # ~/.alex/.env are importable.  Two problems this solves:
    #
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'alex setup'."
        return
    }
    ...
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m alex_cli.main setup
    } else {
        python -m alex_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $alexCmd = "$InstallDir\venv\Scripts\alex.exe"
    if (-not (Test-Path $alexCmd)) {
        $alexCmd = "alex"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$AlexHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'alex whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: alex gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$AlexHome\logs\gateway.log"
            Start-Process -FilePath $alexCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$AlexHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: alex gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: alex gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$AlexHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$AlexHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$AlexHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$AlexHome\alex-agent\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   alex              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   alex setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   alex config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   alex config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   alex gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   alex update       " -NoNewline -ForegroundColor Green
...
$InstallStages = @(
    ...
    @{ Name = "repository";       Title = "Cloning Alex repository";            Category = "install";      NeedsUserInput = $false; Worker = "Stage-Repository" }
...
if ($IncludeDesktop) {
    ...
    # (Alex-Setup.exe), never via the irm|iex CLI one-liner.
    $InstallStages += @{ Name = "desktop"; Title = "Building desktop app"; Category = "install"; NeedsUserInput = $false; Worker = "Stage-Desktop" }
}
$InstallStages += @(
    @{ Name = "path";             Title = "Adding Alex to PATH";                Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-Path" }
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://alex-agent.charan vankudoth.com/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
