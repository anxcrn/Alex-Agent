...
VERSION_FILE = REPO_ROOT / "alex_cli" / "__init__.py"
...
AUTHOR_MAP = {
    ...
    "274096618+alex-agent-dhabibi@users.noreply.github.com": "dhabibi",
    ...
    "alex@example.com": "shellybotmoyer",  # PR #18915 (bot-committed)
    ...
    "github@nadyaalex.anonaddy.com": "ruangraung",  # PR #42308
    ...
    "sylw3st3rr@gmail.com": "Sylw3ster",  # salvage of #23252 (ALEX_KANBAN_BOARD restore)
    ...
    "58116817+TurgutKural@users.noreply.github.com": "TurgutKural",  # salvage of #23356 (ALEX_HOME inject)
    ...
    "sauravsejal40@gmail.com": "Saurav0989",  # PR #27071 (docs: alex-eval community link)
    ...
    "AlexAgent26@gmail.com": "AlexAgent26",  # PR #26438 (kimi model-name reasoning pad)
    "276067471+AlexAgent26@users.noreply.github.com": "AlexAgent26",
    ...
    "vigo@alex": "hawknewton",  # PR #26294 (bedrock boto3 lazy_deps)
    ...
    "kdkcfp@gmail.com": "slowtokki0409",  # PR #29025 (ignore local Alex runtime files)
    ...
    "2663402852@qq.com": "x1am1",  # PR #35098 (chown root-owned top-level ALEX_HOME state files)
...
def update_version_files(semver: str, calver_date: str):
    ...
    # Python package version. The desktop About panel reads the live Alex
...
def _update_acp_registry_versions(semver: str) -> None:
    ...
    if ACP_REGISTRY_MANIFEST.exists():
        ...
        if "package" in uvx:
            uvx["package"] = f"alex-agent[acp]=={semver}"
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/charan vankudoth/alex-agent",
    ...
    lines.append(f"# Alex Agent v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for Alex Agent. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="Alex Agent Release Tool")
    ...
    print(f"{'='*60}")
    print(f"  Alex Agent Release Preview")
    print(f"{'='*60}")
    ...
    if args.publish:
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"Alex Agent v{new_version} ({calver_date})\n\nWeekly release"
        )
        ...
        gh_cmd = [
            ...
            "--title", f"Alex Agent v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            print(
                f"    gh release create {tag_name} --title 'Alex Agent v{new_version} ({calver_date})' "
