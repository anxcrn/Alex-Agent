#!/usr/bin/env python3
"""Live test harness for Alex Agent's Tool Search feature.
...
# Force-isolate the test environment BEFORE any alex imports.
ORIGINAL_HOME = os.environ.get("ALEX_HOME")
ORIGINAL_AUTH = Path.home() / ".alex" / "auth.json"
...
def setup_isolated_home(enabled: bool) -> Path:
    """Create a fresh ~/.alex/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.alex/.env`` so
    the agent can authenticate against OpenRouter inside the isolated home.
    """
    home_dir = Path(tempfile.mkdtemp(prefix="alex_ts_live_"))
    alex_home = home_dir / ".alex"
    alex_home.mkdir(parents=True)

    if ORIGINAL_AUTH.exists():
        shutil.copy(ORIGINAL_AUTH, alex_home / "auth.json")

    # Copy .env so OPENROUTER_API_KEY (or others) are visible to the agent
    # running inside the isolated home.
    real_env_file = Path.home() / ".alex" / ".env"
    if real_env_file.exists():
        shutil.copy(real_env_file, alex_home / ".env")
        ...
        load_alex_dotenv(alex_home=str(Path.home() / ".alex"))
    ...
    (alex_home / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
    return alex_home
...
def reset_module_state():
    """Drop cached modules so the new ALEX_HOME takes effect."""
    keys = [k for k in sys.modules.keys()
            if k.startswith(("tools.", "model_tools", "toolsets",
                             "alex_cli", "agent.", "run_agent"))]
    for k in keys:
        del sys.modules[k]
...
def main():
    ...
    # Restore original ALEX_HOME
    if ORIGINAL_HOME is not None:
        os.environ["ALEX_HOME"] = ORIGINAL_HOME
    else:
