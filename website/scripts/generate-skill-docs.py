...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/charan vankudoth/alex-agent/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    alex_meta = (fm.get("metadata") or {}).get("alex") or {}
    tags = alex_meta.get("tags") or []
    related = alex_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `alex skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with Alex Agent"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Alex ships with a large built-in skill library copied into `~/.alex/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Alex also syncs bundled skills on `alex update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.alex/skills/` tree, it is still shipped with Alex; restore it with `alex skills reset <name> --restore`.",
        "",
