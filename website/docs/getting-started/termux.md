...
description: "Run Alex Agent directly on an Android phone with Termux"
---

# Alex on Android with Termux
...
Alex Agent can run directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop Alex from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.
...
- links `alex` into `$PREFIX/bin` so it stays on your Termux PATH
...
### 5. Put `alex` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/alex" "$PREFIX/bin/alex"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `alex` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.alex/.env`.
...
### `alex doctor` says ripgrep or Node is missing
