...
Type `@` followed by a reference to inject content directly into your message. Alex expands the reference inline and appends the content under an `--- Attached Context ---` section.
...
- Alex env: `$ALEX_HOME/.env`

These directories are fully blocked (any file inside):
- `~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.kube/`, `$ALEX_HOME/skills/.hub/`
