...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of Alex's built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple Alex instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
alex memory setup    # select "honcho" from the provider list
```
...
- `per-session` — each `alex` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — Alex syncs them back at session init.
...
The `alex honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). On a fresh install, configure Honcho directly with `alex memory setup honcho` (or run `alex memory setup` and pick it from the list); the `alex honcho` subcommand then appears on the next invocation.

```bash
alex memory setup honcho    # Configure Honcho directly (works before activation)
alex honcho status          # Connection status, config, and key settings
alex honcho setup           # Redirects to `alex memory setup` (post-activation alias)
alex honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
alex honcho peer            # Show or update peer names + dialectic reasoning level
alex honcho mode            # Show or set recall mode (hybrid/context/tools)
alex honcho tokens          # Show or set token budget for context and dialectic
alex honcho identity        # Seed or show the AI peer's Honcho identity
alex honcho sync            # Sync Honcho config to all existing profiles
alex honcho peers           # Show peer identities across all profiles
alex honcho sessions        # List known Honcho session mappings
alex honcho map             # Map current directory to a Honcho session name
alex honcho enable          # Enable Honcho for the active profile
alex honcho disable         # Disable Honcho for the active profile
alex honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `alex honcho`

If you previously used the standalone `alex honcho setup`:
...
No re-login or re-setup needed. Run `alex memory setup` and select "honcho" — the wizard detects your existing config.
