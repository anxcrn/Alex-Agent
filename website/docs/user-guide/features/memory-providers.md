...
Alex Agent ships with 8 external memory provider plugins that give the agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. Only **one** external provider can be active at a time — the built-in memory is always active alongside it.

## Quick Start

```bash
alex memory setup      # interactive picker + configuration
alex memory status     # check what's active
alex memory off        # disable external provider
```

You can also select the active memory provider via `alex plugins` → Provider Plugins → Memory Provider.

Or set manually in `~/.alex/config.yaml`:
...
When a memory provider is active, Alex automatically:
...
**Setup Wizard:**
```bash
alex memory setup        # select "honcho" — runs the Honcho-specific post-setup
```
...
:::tip Migrating from `alex honcho`
If you previously used `alex honcho setup`, your config and all server-side data are intact. Just re-enable through the setup wizard again or manually set `memory.provider: honcho` to reactivate via the new system.
:::

**Multi-peer setup:**

Honcho models conversations as peers exchanging messages — one user peer plus one AI peer per Alex profile, all sharing a workspace. The workspace is the shared environment: the user peer is global across profiles, each AI peer is its own identity. Every AI peer builds an independent representation / card from its own observations, so a `coder` profile stays code-oriented while a `writer` profile stays editorial against the same user.

The mapping:

| Concept | What it is |
|---------|-----------|
| **Workspace** | Shared environment. All Alex profiles under one workspace see the same user identity. |
| **User peer** (`peerName`) | The human. Shared across profiles in the workspace. |
| **AI peer** (`aiPeer`) | One per Alex profile. Host key `alex` → default; `alex.<profile>` for others. |
...
```bash
alex profile create coder --clone
```

`--clone` creates a `alex.coder` host block in `honcho.json` with `aiPeer: "coder"`, shared `workspace`, inherited `peerName`, `recallMode`, `writeFrequency`, `observation`, etc. The AI peer is eagerly created in Honcho so it exists before the first message.
...
Scans every Alex profile, creates host blocks for any profile without one, inherits settings from the default `alex` block, and creates the new AI peers eagerly. Idempotent — skips profiles that already have a host block.
...
Off-gateway these keys do nothing. `alex memory setup` only prompts for them when it detects a connected gateway platform. See the [Honcho page](./honcho.md#gateway-identity-mapping) for the resolver ladder and the setup flow.
...
# Then configure Alex
alex memory setup    # select "openviking"
# Or manually:
alex config set memory.provider openviking
echo "OPENVIKING_ENDPOINT=http://localhost:1933" >> ~/.alex/.env
# Authenticated servers should use a user/admin API key:
echo "OPENVIKING_API_KEY=..." >> ~/.alex/.env
```
...
`OPENVIKING_AGENT` is Alex' peer ID in OpenViking for peer-scoped memories.
...
**Setup (Platform):**
```bash
alex memory setup    # select "mem0" → "Platform"
# Or manually:
alex config set memory.provider mem0
echo "MEM0_API_KEY=your-key" >> ~/.alex/.env
```

**Setup (OSS):**
```bash
alex memory setup    # select "mem0" → "Open Source (self-hosted)"
# Or via flags:
alex memory setup mem0 --mode oss --oss-llm openai --oss-llm-key sk-... --oss-vector qdrant
```

Preview without writing files:
```bash
alex memory setup mem0 --mode oss --oss-llm-key sk-... --dry-run
```

**Config:** `$ALEX_HOME/mem0.json` (behavioral settings). Only the secret `MEM0_API_KEY` belongs in `~/.alex/.env`.

| Key | Default | Description |
|-----|---------|-------------|
| `mode` | `platform` | `platform` (Mem0 Cloud) or `oss` (self-hosted) |
| `user_id` | `alex-user` | User identifier |
| `agent_id` | `alex` | Agent identifier |
...
**Switching modes:** Re-run `alex memory setup mem0 --mode <platform|oss>` or edit `mem0.json` directly.
...
**Setup:**
```bash
alex memory setup    # select "hindsight"
# Or manually:
alex config set memory.provider hindsight
echo "HINDSIGHT_API_KEY=your-key" >> ~/.alex/.env
```

The setup wizard installs dependencies automatically and only installs what's needed for the selected mode (`hindsight-client` for cloud, `hindsight-all` for local). Requires `hindsight-client >= 0.4.22` (auto-upgraded on session start if outdated).

**Local mode UI:** `hindsight-embed -p alex ui start`

**Config:** `$ALEX_HOME/hindsight/config.json`

| Key | Default | Description |
|-----|---------|-------------|
| `mode` | `cloud` | `cloud` or `local` |
| `bank_id` | `alex` | Memory bank identifier |
| `recall_budget` | `mid` | Recall thoroughness: `low` / `mid` / `high` |
| `memory_mode` | `hybrid` | `hybrid` (context + tools), `context` (auto-inject only), `tools` (tools only) |
| `auto_retain` | `true` | Automatically retain conversation turns |
| `auto_recall` | `true` | Automatically recall memories before each turn |
| `retain_async` | `true` | Process retain asynchronously on the server |
| `retain_context` | `conversation between Alex Agent and the User` | Context label for retained memories |
...
**Setup:**
```bash
alex memory setup    # select "holographic"
# Or manually:
alex config set memory.provider holographic
```

**Config:** `config.yaml` under `plugins.alex-memory-store`

| Key | Default | Description |
|-----|---------|-------------|
| `db_path` | `$ALEX_HOME/memory_store.db` | SQLite database path |
...
**Setup:**
```bash
alex memory setup    # select "retaindb"
# Or manually:
alex config set memory.provider retaindb
echo "RETAINDB_API_KEY=your-key" >> ~/.alex/.env
```
...
# Then configure Alex
alex memory setup    # select "byterover"
# Or manually:
alex config set memory.provider byterover
```

**Key features:**
- Automatic pre-compression extraction (saves insights before context compression discards them)
- Knowledge tree stored at `$ALEX_HOME/byterover/` (profile-scoped)
...
| **Requires** | `pip install supermemory` + [API key](http://app.supermemory.ai/integrations?connect=alex) |
...
**Setup:**
```bash
alex memory setup    # select "supermemory"
# Or manually:
alex config set memory.provider supermemory
echo 'SUPERMEMORY_API_KEY=***' >> ~/.alex/.env
```

**Config:** `$ALEX_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `alex` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags. |
...
- **Profile-scoped containers** — use `{identity}` in `container_tag` (e.g. `alex-{identity}` → `alex-coder`) to isolate memories per Alex profile
...
| **Requires** | `pip install alex-memori` + `alex-memori install` + [Memori API key](https://app.memorilabs.ai/signup) |
...
**Setup:**
```bash
pip install alex-memori
alex-memori install
alex config set memory.provider memori
alex memory setup
```
...
| **Memori** | Cloud | Free/Paid | 5 | `alex-memori` | Tool-aware memory + structured recall |

## Profile Isolation

Each provider's data is isolated per [profile](/user-guide/profiles):

- **Local storage providers** (Holographic, ByteRover) use `$ALEX_HOME/` paths which differ per profile
- **Config file providers** (Honcho, Mem0, Hindsight, Supermemory) store config in `$ALEX_HOME/` so each profile has its own credentials
