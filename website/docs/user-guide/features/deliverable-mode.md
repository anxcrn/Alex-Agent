...
When Alex Agent runs inside a messaging gateway (Slack, Discord, Telegram,
...
persona in `~/.alex/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.alex/config.yaml` (switchable per session
via `/personality`).
...
If you use Alex' kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.alex/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. Alex Agent's deliverable
