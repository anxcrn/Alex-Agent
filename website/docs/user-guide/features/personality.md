...
description: "Customize Alex Agent's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

Alex Agent's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.

- `SOUL.md` — a durable persona file that lives in `ALEX_HOME` and serves as the agent's identity (slot #1 in the system prompt)
- built-in or custom `/personality` presets — session-level system-prompt overlays

If you want to change who Alex is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

Alex now seeds a default `SOUL.md` automatically in:

```text
~/.alex/SOUL.md
```

More precisely, it uses the current instance's `ALEX_HOME`, so if you run Alex with a custom home directory, it will use:
...
- Alex creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- Alex loads `SOUL.md` only from `ALEX_HOME`
- Alex does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, Alex falls back to a built-in default identity
...
If Alex loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `ALEX_HOME`, the personality belongs to the Alex instance itself.

That also makes it easier to teach users:
- "Edit `~/.alex/SOUL.md` to change Alex' default personality."
...
- how Alex should handle uncertainty, disagreement, or ambiguity
...
## What Alex injects into the prompt
...
If the file is empty, whitespace-only, or cannot be read, Alex falls back to a built-in default identity ("You are Alex Agent, an intelligent AI assistant created by charan vankudoth..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
...
Alex ships with built-in personalities you can switch to with `/personality`.
...
| **pirate** | Captain Alex, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives Alex its persistent default personality unless the overlay meaningfully changes it.

## Custom personalities in config

You can also define named custom personalities in `~/.alex/config.yaml` under `agent.personalities`.
...
1. Keep a thoughtful global `SOUL.md` in `~/.alex/SOUL.md`
...
- [SOUL.md Guide](/guides/use-soul-with-alex)

## CLI appearance vs conversational personality

Conversational personality and CLI appearance are separate:

- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how Alex speaks
- `display.skin` and `/skin` affect how Alex looks in the terminal
