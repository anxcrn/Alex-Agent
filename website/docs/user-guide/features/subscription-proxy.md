...
chat completions — use your Alex-managed provider subscription as their
...
| Use case | "Use Alex as a chat backend" | "Use my Portal sub from another app" |
...
This opens your browser for the Nous Portal OAuth flow. Alex stores
the refresh token in `~/.alex/auth.json` — the same place all Alex
...
Model:      Alex-4-70B    # or Alex-4.3-36B, Alex-4-405B
```
...
interface in `alex_cli/proxy/adapters/`.
...
If you see `not logged in`, run `alex portal`. If you see
...
supports; check `portal.charan vankudoth.com/models`.
...
[portal.charan vankudoth.com](https://portal.charan vankudoth.com).
...
`alex_cli/proxy/adapters/<provider>.py` and registering it in
