...
- Give it a name like "alex-agent"
- Select scopes:
...
- Give it a title like "alex-agent-&lt;machine-name>"
...
elif _alex_env="${ALEX_HOME:-$HOME/.alex}/.env"; [ -f "$_alex_env" ] && grep -q "^GITHUB_TOKEN=" "$_alex_env"; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_alex_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
