---
title: "Alex Agent — Configure, extend, or contribute to Alex Agent"
...
Configure, extend, or contribute to Alex Agent.
...
| Path | `skills/autonomous-ai-agents/alex-agent` |
| Version | `2.1.0` |
...
alex auth add PROVIDER    Add OAuth or API-key credential (e.g. nous, openai-codex, qwen-oauth)
alex auth list            List stored credentials
alex auth remove PROVIDER Remove a stored credential
...
without BOM. `alex config edit` writes without BOM; manual edits in
...
Alex's own env scrubber dropping `SYSTEMROOT` / `WINDIR` / `COMSPEC`
...
POSIX venv layouts (`.venv/bin/activate`). The Alex-installed venv at
...
If Alex reports a smaller context window than your local model supports
(e.g., 128k when llama-server has `-c 262144`):

**Check if `model.context_length` is explicitly set.** Alex uses a
...
**2. Add to `toolsets.py`** → `_ALEX_CORE_TOOLS` list.

Auto-discovery: any `tools/*.py` file with a top-level `registry.register()` call is imported automatically — no manual list needed.

All handlers must return JSON strings. Use `get_alex_home()` for paths, never hardcode `~/.alex`.
...
**Windows contributors:** `scripts/run_tests.sh` currently looks for POSIX venvs (`.venv/bin/activate` / `venv/bin/activate`) and will error out on Windows where the layout is `venv/Scripts/activate` + `python.exe`. The Alex-installed venv at `venv/Scripts/` also has no `pip` or `pytest` — it's stripped for end-user install size. Workaround: install pytest + pytest-xdist + pyyaml into a system Python 3.11 user site (`/c/Program Files/Python311/python -m pip install --user pytest pytest-xdist pyyaml`), then run tests directly:
