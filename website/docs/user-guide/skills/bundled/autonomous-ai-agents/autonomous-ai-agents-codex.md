...
| Related skills | [`claude-code`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-claude-code), [`alex-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-alex-agent) |
...
Delegate coding tasks to [Codex](https://github.com/openai/codex) via the Alex terminal. Codex is OpenAI's autonomous coding agent CLI.
...
For Alex itself, `model.provider: openai-codex` uses Alex-managed Codex
OAuth from `~/.alex/auth.json` after `alex auth add openai-codex`. For the
...
When invoking the Codex CLI from a Alex gateway/service context (for example,
