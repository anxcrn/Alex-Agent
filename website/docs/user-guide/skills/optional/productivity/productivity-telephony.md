---
title: "Telephony — Give Alex phone capabilities without core tool changes"
sidebar_label: "Telephony"
description: "Give Alex phone capabilities without core tool changes"
---
...
Give Alex phone capabilities without core tool changes. Provision and persist a Twilio number, send and receive SMS/MMS, make direct calls, and place AI-driven outbound calls through Bland.ai or Vapi.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `alex skills install official/productivity/telephony` |
| Path | `optional-skills/productivity/telephony` |
| Version | `1.0.0` |
| Author | charan vankudoth |
| License | MIT |
...
This optional skill gives Alex practical phone capabilities while keeping telephony out of the core tool list.

It ships with a helper script, `scripts/telephony.py`, that can:
- save provider credentials into `${ALEX_HOME:-~/.alex}/.env`
...
It does **not** turn Alex into a real-time inbound phone gateway. Inbound SMS is handled by polling the Twilio REST API. That is enough for many workflows, including notifications and some one-time-code retrieval, without adding core webhook infrastructure.
...
4. Treat third-party phone numbers as sensitive operational data:
   - do not save them to Alex memory
...
### 1) "I want Alex to own a real phone number"
Use **Twilio**.
...
- pairs well with Alex `text_to_speech` plus a public file host or tunnel
...
```bash
SCRIPT="$(find ~/.alex/skills -path '*/telephony/scripts/telephony.py' -print -quit)"
```
...
```bash
alex skills search telephony
alex skills install official/productivity/telephony
```
...
Then save credentials into Alex:
...
3. Buy it and save it into `${ALEX_HOME:-~/.alex}/.env` + state:
```bash
python3 "$SCRIPT" twilio-buy "+17025551234" --save-env
```
...
```bash
python3 "$SCRIPT" twilio-call "+15551230000" --message "Hello! This is Alex calling with your status update." --voice Polly.Joanna
```

### E. Call with a prerecorded / custom voice message

This is the main path for reusing Alex's existing `text_to_speech` support.

Use this when:
- you want the call to use Alex's configured TTS voice rather than Twilio `<Say>`
...
Recommended Alex TTS -> Twilio Play workflow:

1. Generate the audio with Alex `text_to_speech`.
2. Make the resulting MP3 publicly reachable.
3. Place the Twilio call with `--audio-url`.

Example agent flow:
- Ask Alex to create the message audio with `text_to_speech`
...
Important note:
- Alex TTS is great for prerecorded outbound messages
...
- Alex STT/TTS alone is not being used here as a full duplex phone conversation engine; that would require a much heavier streaming/webhook integration than this skill is trying to introduce
...
7. Summarize the outcome without persisting third-party numbers to Alex memory.
...
- Do not store arbitrary third-party phone numbers in Alex memory.
...
3. persist that number to `${ALEX_HOME:-~/.alex}/.env`
