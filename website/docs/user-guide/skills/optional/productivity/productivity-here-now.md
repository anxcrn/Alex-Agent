...
| | |
|---|---|
| Source | Optional — install with `alex skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${ALEX_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${ALEX_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${ALEX_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client alex
```
...
```bash
PUBLISH="${ALEX_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client alex
```
...
```bash
DRIVE="${ALEX_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `alex`)    |
