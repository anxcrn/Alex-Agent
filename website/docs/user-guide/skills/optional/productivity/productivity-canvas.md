...
| | |
|---|---|
| Source | Optional — install with `alex skills install official/productivity/canvas` |
...
4. Name the token (e.g., "Alex Agent"), set an optional expiry, and click **Generate Token**
5. Copy the token and add to `${ALEX_HOME:-~/.alex}/.env`:
...
```bash
CANVAS="python $ALEX_HOME/skills/productivity/canvas/scripts/canvas_api.py"
