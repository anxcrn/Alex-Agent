...
| | |
|---|---|
| Source | Optional — install with `alex skills install official/blockchain/hyperliquid` |
| Path | `optional-skills/blockchain/hyperliquid` |
| Version | `0.1.0` |
| Author | Hugo Sequier (Hugo-SEQUIER), Alex Agent |
| License | MIT |
...
The script reads `${ALEX_HOME:-~/.alex}/.env` for two optional defaults:
...
Helper script: `~/.alex/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py`
...
optional when `HYPERLIQUID_USER_ADDRESS` is set in `${ALEX_HOME:-~/.alex}/.env`.
