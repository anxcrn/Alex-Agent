...
| | |
|---|---|
| Source | Optional — install with `alex skills install official/security/1password` |
| Path | `optional-skills/security/1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by Alex Agent |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during Alex terminal calls (desktop app flow only)
...
### Service Account (recommended for Alex)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `${ALEX_HOME:-~/.alex}/.env` (the skill will prompt for this on first load).
...
## Alex Execution Pattern (desktop app flow)

Alex terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/alex-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/alex-op.sock"
