...
description: "Configure and use Honcho memory with Alex -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session su..."
---
...
Configure and use Honcho memory with Alex -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation, recall, and dialectic settings.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `alex skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for Alex

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Alex profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/alex#running-honcho-locally-with-alex

### Verify

```bash
alex honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Alex profile's AI peer
...
Honcho models conversations as interactions between **peers**. Alex creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Alex instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Alex run |
| `global` | Single session across all directories |

Manual override: `alex honcho map my-project-name`
...
Each Alex profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block alex.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `alex.coder` host block in `honcho.json`
...
```bash
alex honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Alex when Honcho memory is active.
...
Config file: `$ALEX_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `alex honcho setup`. Ensure `memory.provider: honcho` is in `~/.alex/config.yaml`.

### Memory not persisting across sessions
Check `alex honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `alex profile create <name> --clone`. For existing profiles: `alex honcho sync`.
...
| Command | Description |
|---------|-------------|
| `alex honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `alex honcho status` | Show resolved config, connection test, peer info for active profile |
| `alex honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `alex honcho disable` | Disable Honcho for the active profile |
| `alex honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `alex honcho peers` | Show peer identities across all profiles |
| `alex honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `alex honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `alex honcho sessions` | List known directory-to-session-name mappings |
| `alex honcho map <name>` | Map current working directory to a Honcho session name |
| `alex honcho identity` | Seed AI peer identity or show both peer representations |
| `alex honcho sync` | Create host blocks for all Alex profiles that don't have one yet |
| `alex honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Alex + Honcho |
| `alex memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `alex memory status` | Show active memory provider and config |
| `alex memory off` | Disable external memory provider |
