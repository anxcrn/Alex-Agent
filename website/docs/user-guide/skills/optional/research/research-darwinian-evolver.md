...
| | |
|---|---|
| Source | Optional — install with `alex skills install official/research/darwinian-evolver` |
| Path | `optional-skills/research/darwinian-evolver` |
| Version | `0.1.0` |
| Author | Bihruze (Asahi0x), Alex Agent |
| License | MIT |
...
import upstream classes into Alex itself.
...
```bash
mkdir -p ~/.alex/cache/darwinian-evolver && cd ~/.alex/cache/darwinian-evolver
...
```bash
cd ~/.alex/cache/darwinian-evolver/darwinian_evolver \
...
Open `~/.alex/cache/darwinian-evolver/darwinian_evolver/darwinian_evolver/lineage_visualizer.html`
...
SKILL_DIR=~/.alex/skills/research/darwinian-evolver
DE_DIR=~/.alex/cache/darwinian-evolver/darwinian_evolver
...
7. **AGPL.** Never `from darwinian_evolver import ...` inside Alex core.
   Custom driver scripts under `~/.alex/skills/...` are user-side and fine.
