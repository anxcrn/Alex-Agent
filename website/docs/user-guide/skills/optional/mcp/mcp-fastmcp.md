...
Build, test, inspect, install, and deploy MCP servers with FastMCP in Python. Use when creating a new MCP server, wrapping an API or database as MCP tools, exposing resources or prompts, or preparing a FastMCP server for Alex Agent, Cursor, or HTTP deployment.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `alex skills install official/mcp/fastmcp` |
...
```bash
python ~/.alex/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.alex/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Alex integration, either:

- configure the server in `~/.alex/config.yaml` using the `native-mcp` skill, or
...
### Alex cannot see the deployed server

The server-building part may be correct while the Alex config is not. Load the `native-mcp` skill and configure the server in `~/.alex/config.yaml`, then restart Alex.
