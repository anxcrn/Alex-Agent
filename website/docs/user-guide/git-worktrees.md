...
title: "Git Worktrees"
description: "Run multiple Alex agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Alex Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Alex so each session has a clean, isolated working directory.

## Why Use Worktrees with Alex?

Alex treats the **current working directory** as the project root:

- CLI: the directory where you run `alex` or `alex chat`
- Messaging gateways: the directory set by `terminal.cwd` in `~/.alex/config.yaml`
...
git worktree add ../repo-feature feature/alex-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/alex-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Alex there:

```bash
cd ../repo-feature

# Start Alex in the worktree
alex
```
...
git worktree add ../repo-experiment-a feature/alex-a
git worktree add ../repo-experiment-b feature/alex-b
```
...
- Works on its own branch (`feature/alex-a` vs `feature/alex-b`).
...
- Alex checkpoint data under `~/.alex/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Alex experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/alex-checkpoints-docs`, `feature/alex-refactor-tests`.
- **Commit frequently**
...
- **Avoid running Alex from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `alex -w` (Automatic Worktree Mode)

Alex has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `alex/alex-<hash>`).
...
```bash
alex -w -z "Fix issue #123"
```

For parallel agents, open multiple terminals and run `alex -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Alex session its own clean checkout.
