...
If you only run one Alex agent, you don't need this page — see
[Profiles](./profiles.md) for the basics.

## When to use this

You want this setup when you have two or more Alex agents that should all
...
(`ai.alex.gateway-<name>.plist`) or systemd user service
(`alex-gateway-<name>.service`). This guide adds the patterns for managing
them collectively.
...
alex profile create coder
alex profile create personal-bot
alex profile create research
...
```bash
alex config set gateway.multiplex_profiles true
alex gateway restart
```

Equivalently, in the default profile's `~/.alex/config.yaml`:
...
You do **not** run `alex gateway start` for the secondary profiles — the
...
With a multiplexer running, a named-profile `alex gateway start` / `run` is a
...
home). `alex status` reports the multiplexer and the profiles it serves;
`alex status -p <name>` slices to one profile. Each profile still writes its
...
`~/.local/bin/alex-gateways` and `chmod +x` it:
...
usage() {
  echo "Usage: alex-gateways {start|stop|restart|status|list}"
}
...
```bash
alex-gateways start      # start every configured profile
alex-gateways stop       # stop every configured profile
alex-gateways restart    # restart all
alex-gateways status     # status across all
alex-gateways list       # delegates to `alex gateway list`
```

:::tip
The `default` profile is targeted with `alex gateway <action>` (no `-p`),
not `alex -p default gateway <action>`. The wrapper above handles both forms.
:::
...
These are equivalent to `alex -p coder gateway <action>` — useful if a
...
| Platform | Path                                                              |
| -------- | ----------------------------------------------------------------- |
| macOS    | `~/Library/LaunchAgents/ai.alex.gateway-<profile>.plist`        |
| Linux    | `~/.config/systemd/user/alex-gateway-<profile>.service`         |

The default profile keeps the historical names: `ai.alex.gateway.plist` /
`alex-gateway.service`.
...
tail -f ~/.alex/logs/gateway.error.log

# Named profile
tail -f ~/.alex/profiles/<name>/logs/gateway.log
tail -f ~/.alex/profiles/<name>/logs/gateway.error.log
```

Stream every profile's log simultaneously:

```bash
tail -f ~/.alex/logs/gateway.log ~/.alex/profiles/*/logs/gateway.log
```

The CLI also has a structured log viewer:

```bash
alex logs -f                  # follow default profile
alex -p coder logs -f         # follow one profile
alex logs --help              # filters, levels, JSON output
```

## Identify what's actually running

```bash
alex profile list             # profiles + model + gateway state
alex-gateways status          # full status across every profile
launchctl list | grep alex    # macOS — PIDs and labels
systemctl --user list-units 'alex-gateway-*'   # Linux — units
```
...
The default profile uses `~/.alex/` directly with the same three files.

Edit them with any editor or via the CLI:

```bash
alex config set model.model anthropic/claude-sonnet-4    # default profile
...
caffeinate -i -w $(cat ~/.alex/gateway.pid) &   # awake while default gateway runs
...
systemd-inhibit --what=idle:sleep --who=alex --why="gateways running" \
  sleep infinity &
...
`alex-gateway-<profile>.service`) continue running across SSH disconnects
and reboots.
...
grep -H 'TELEGRAM_BOT_TOKEN\|DISCORD_BOT_TOKEN' \
     ~/.alex/.env ~/.alex/profiles/*/.env
```

## Updating the code

`alex update` pulls the latest code once and syncs new bundled skills into
every profile:
...
You ran `alex gateway start` after a previous `alex gateway stop`. The
...
```bash
ps -ef | grep "alex_cli.*-p <profile>"
cat ~/.alex/profiles/<profile>/gateway.pid
...
```bash
# macOS
launchctl unload ~/Library/LaunchAgents/ai.alex.gateway-<profile>.plist
launchctl load   ~/Library/LaunchAgents/ai.alex.gateway-<profile>.plist

# Linux
systemctl --user restart alex-gateway-<profile>.service
```

### Health check

```bash
alex doctor                  # default profile
alex -p <profile> doctor     # one profile
```
