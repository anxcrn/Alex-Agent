...
`~/.alex/config.yaml`, `~/.alex/.env`, and even the shell environment — for
...
Managed scope is read from a system-level directory, default `/etc/alex`:

```text
/etc/alex/
├── config.yaml     # managed config layer (wins over ~/.alex/config.yaml)
└── .env            # managed env layer (wins over ~/.alex/.env + shell)
```
...
The location can be relocated with the `ALEX_MANAGED_DIR` environment variable
(for containers or non-`/etc` deployments). This is a deployment/bootstrap path
knob — like `ALEX_HOME` — set by the same administrator who owns the managed
files. It is **never persisted** to any `.env` by Alex.

```bash
# Point managed scope at a custom directory (set by IT / the deployment, not the user)
export ALEX_MANAGED_DIR=/opt/org/alex-policy
```

:::warning
A user who can set `ALEX_MANAGED_DIR` can repoint managed scope at a directory
...
left user-settable. `alex doctor` reports the *resolved* managed directory so
...
|---|---|---|
| 1 | `/etc/alex/config.yaml` (managed) | `/etc/alex/.env` (managed) |
| 2 | `~/.alex/config.yaml` (user) | `~/.alex/.env` (user) |
...
```bash
alex config        # shows a header naming the managed source + the pinned keys
alex doctor        # reports the resolved managed dir + pinned key counts
```

If you try to change a managed value, Alex refuses and names the source:

```bash
$ alex config set model.default my/model
Cannot set 'model.default': it is managed by your administrator
(/etc/alex/config.yaml) and cannot be changed.
```

The same applies to managed secrets — `alex config set` / setup will not write
...
sudo tee /etc/alex/config.yaml >/dev/null <<'YAML'
model:
  provider: nous
security:
  redact_secrets: true
...
sudo tee /etc/alex/.env >/dev/null <<'ENV'
OPENAI_API_BASE=https://inference.example.com/v1
ENV

sudo chmod 0755 /etc/alex
sudo chmod 0644 /etc/alex/config.yaml /etc/alex/.env
```

Changes take effect on the next Alex start (a malformed managed file is logged
loudly and ignored — it never blocks startup, but the admin should check
`alex doctor` to confirm the policy is being applied).

## Security model and limitations (v1)

- **Enforcement is filesystem permissions only.** If a user has write access to
  the managed directory (or runs Alex as `root`), managed scope is advisory.
