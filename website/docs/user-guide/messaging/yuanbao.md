...
sidebar_position: 16
title: "Yuanbao"
description: "Connect Alex Agent to the Yuanbao enterprise messaging platform via WebSocket gateway"
---

# Yuanbao

Connect Alex to [Yuanbao](https://yuanbao.tencent.com/), Tencent's enterprise messaging platform. The adapter uses a WebSocket gateway for real-time message delivery and supports both direct (C2C) and group conversations.
...
After initial setup, verify these variables in `~/.alex/.env`:
...
All standard Alex commands work on Yuanbao:
...
4. Review gateway logs: `tail -f ~/.alex/logs/gateway.log`
...
4. Consider enabling verbose logging: `ALEX_LOG_LEVEL=debug`
...
Yuanbao has a maximum message size. Alex automatically chunks large responses with Markdown-aware splitting (respects code fences, tables, and paragraph boundaries).
...
```bash
ALEX_LOG_LEVEL=debug alex gateway
```
...
```bash
alex chat -q "Send 'Hello from CLI' to yuanbao:group:group_code"
```
