...
Connect Alex to **iMessage** through [Photon][photon], a managed
...
your first iMessage from Alex — just a phone number we can bind to
your account.
:::
...
both directions. Because the SDK is TypeScript-only, Alex runs it in a
...
alex photon setup --phone +15551234567
```
...
2. **Finds or creates** the `Alex Agent` project on your account.
...
Runtime credentials are written to `~/.alex/.env`
...
(device token, dashboard project id) lives in `~/.alex/auth.json` under
`credential_pool.photon` / `credential_pool.photon_project`.

## Authorizing users

Photon uses the same authorization model as every other Alex
channel. Choose one approach:

**DM pairing (default).** When an unknown number messages your Photon
line, Alex replies with a pairing code. Approve it with:

```bash
alex pairing approve photon <CODE>
```

Use `alex pairing list` to see pending codes and approved users.

**Pre-authorize specific numbers** (in `~/.alex/.env`):

```bash
PHOTON_ALLOWED_USERS=+15551234567,+15559876543
```

**Open access** (dev only, in `~/.alex/.env`):
...
By default Alex responds to every authorized DM and group message.
...
they match a wake-word pattern. The defaults match `Alex` and
`@Alex agent` variants. For a custom agent name, set regex patterns:
...
Send an iMessage to your assigned number and Alex will reply.
...
assigned iMessage line Alex uses. When a Photon token and dashboard project
...
- **`sidecar deps : ✗ run alex photon install-sidecar`** — Node is
  installed but `spectrum-ts` isn't. Run the suggested command.
- **`device token : ✗ missing`** — run `alex photon setup` to log in.
- **`No iMessage line assigned yet`** — Spectrum is enabled but no line
  has been provisioned; re-run `alex photon setup` or check the
  [dashboard][app].
- **Sidecar won't start** — confirm `node --version` is 18.17+ and that
  `alex photon install-sidecar` completed without errors.
...
- **Outbound attachments are supported.** Alex sends images, voice
...
| `PHOTON_MENTION_PATTERNS` | Alex wake words  | JSON list / comma / newline regex patterns for group mentions |
