...
Connect Alex to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Alex supports two WeCom integration modes:
...
> Run `alex gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
3. Alex decrypts the message, queues it for the agent
...
(Use `alex gateway start` only after `alex gateway install` has registered the systemd/launchd service.)
...
console. A mismatch between the token configured in Alex and the token the
...
in `~/.alex/.env` values around `=` will also break signature checks. After
fixing, restart `alex gateway run`.
...
Check `alex gateway run` logs for the bound host/port. If the adapter bound to
