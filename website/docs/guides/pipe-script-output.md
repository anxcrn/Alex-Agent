...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `alex send`."
---

# Pipe Script Output to Messaging Platforms

`alex send` is a small, scriptable CLI that pushes a message to any
messaging platform Alex is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | alex send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `alex
...
alex send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
alex send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
alex send --list

# Filter by platform
alex send --list telegram
```
...
Any platform Alex ships adapters for works as a target:
...
`alex send` resolves the message body in this order:

1. **Positional argument** — `alex send --to telegram "hi"`
2. **`--file PATH`** — `alex send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | alex send --to telegram`

When stdin is a TTY (no pipe), Alex does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  alex send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `alex send` reuses your Alex config, the same script works on
any host where Alex is installed — no need to export bot tokens into
...
`alex send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  alex send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | alex send \
...
```bash
./train.py --epochs 200 && \
  alex send --to telegram "training done" || \
  alex send --to telegram "training failed (exit $?)"
```
...
alex send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(alex send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `alex send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `alex send` calls
the platform's REST endpoint directly using credentials from
`~/.alex/.env` and `~/.alex/config.yaml`. It's a standalone subprocess
...
pointing at the gateway; start it with `alex gateway start` and retry.
...
The listing is built from `~/.alex/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`alex gateway
...
| Approach | Multi-platform | Reuses Alex creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `alex send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`alex send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `alex send`.
...
- [Gateway Internals](/developer-guide/gateway-internals) —
  the delivery router that `alex send` shares with cron delivery.
