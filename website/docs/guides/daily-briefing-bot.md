...
:::tip
This recipe hits web search, summarization, and optional TTS — all bundled in a Portal subscription. The fastest setup is `alex setup --portal`. See [Nous Portal](/integrations/nous-portal).
:::
...
2. **Alex spins up** a fresh agent session with your prompt
...
You can still follow this tutorial using `deliver: "local"`. Briefings will be saved to `~/.alex/cron/output/` and you can read them anytime.
:::
...
Alex will search the web, read through results, and produce something like:
...
---
3 stories • Sources searched: 8 • Generated by Alex Agent
```
...
Before creating cron jobs, ensure Alex has a default model and provider configured globally. If you want a specific job to use different values, set explicit per-job model/provider overrides when creating it.

### Option A: Natural Language (in chat)

Just tell Alex what you want:
...
Alex will create the cron job for you using the unified `cronjob` tool.
...
For faster briefings, tell Alex to delegate each topic to a sub-agent:
...
Alex will use `cronjob(action="list")` to find it and `cronjob(action="remove")` to delete it.
