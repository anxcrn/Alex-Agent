...
```bash
alex teams-pipeline token-health
alex teams-pipeline token-health --force-refresh
```
...
```bash
alex teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
alex teams-pipeline maintain-subscriptions
alex teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: Alex cron (recommended if you already run the Alex gateway)

Alex ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.alex/scripts/`. First create the script:

```bash
mkdir -p ~/.alex/scripts
cat > ~/.alex/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec alex teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.alex/scripts/maintain-teams-subscriptions.sh
```

Then register a script-only cron job that runs every 12 hours (gives 6x headroom against the 72h expiry window):

```bash
alex cron create "0 */12 * * *" \
...
Create `/etc/systemd/system/alex-teams-pipeline-maintain.service`:

```ini
[Unit]
Description=Alex Teams pipeline subscription maintenance
After=network-online.target

[Service]
Type=oneshot
User=alex
EnvironmentFile=/etc/alex/env
ExecStart=/usr/local/bin/alex teams-pipeline maintain-subscriptions
```

And `/etc/systemd/system/alex-teams-pipeline-maintain.timer`:

```ini
[Unit]
Description=Run Alex Teams pipeline subscription maintenance every 12 hours
...
sudo systemctl enable --now alex-teams-pipeline-maintain.timer
systemctl list-timers alex-teams-pipeline-maintain.timer
```

#### Option 3: Plain crontab

```cron
0 */12 * * * /usr/local/bin/alex teams-pipeline maintain-subscriptions >> /var/log/alex/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.alex/.env` at the top of a wrapper script that crontab calls.
...
```bash
alex teams-pipeline subscriptions   # should show expirationDateTime advanced
alex teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
alex teams-pipeline list
alex teams-pipeline list --status failed
alex teams-pipeline show <job-id>
```

### Replay a stored job

```bash
alex teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
alex teams-pipeline fetch --meeting-id <meeting-id>
alex teams-pipeline fetch --join-web-url "<join-url>"
```
...
- run `alex teams-pipeline maintain-subscriptions --dry-run`
- inspect `alex teams-pipeline list --status failed`
...
- run `alex teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `alex teams-pipeline run`
...
- [ ] `alex teams-pipeline validate` returns an OK snapshot
- [ ] `alex teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (Alex cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
