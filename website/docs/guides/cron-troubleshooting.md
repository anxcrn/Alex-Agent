...
description: "Diagnose and fix common Alex cron issues — jobs not firing, delivery failures, skill loading errors, and performance problems"
---
...
If you're expecting jobs to fire automatically, you need a running gateway (`alex gateway` for foreground, or `alex gateway start` for the installed service). For one-off debugging, you can manually trigger a tick with `alex cron tick`.
...
```bash
date
alex cron list   # Compare next_run times with local time
```
...
| Target | Requires |
|--------|----------|
| `telegram` | `TELEGRAM_BOT_TOKEN` in `~/.alex/.env` |
| `discord` | `DISCORD_BOT_TOKEN` in `~/.alex/.env` |
| `slack` | `SLACK_BOT_TOKEN` in `~/.alex/.env` |
...
| `local` | Write access to `~/.alex/cron/output/` |
...
If delivery fails, the job still runs — it just won't send anywhere. Check `alex cron list` for updated `last_error` field (if available).
...
Skills must be installed before they can be attached to cron jobs. If a skill is missing, install it first with `alex skills install <skill-name>` or via `/skills` in the CLI.

### Check 2: Check skill name vs. skill folder name

Skill names are case-sensitive and must match the installed skill's folder name. If your job specifies `ai-funding-daily-report` but the skill folder is `ai-funding-daily-report`, confirm the exact name from `alex skills list`.
...
2. `~/.alex/logs/agent.log` for scheduler messages (or `errors.log` for warnings)
3. The job's `last_run` metadata via `alex cron list`

### Check 2: Common error patterns

**"No such file or directory" for scripts**
The `script` path must be an absolute path (or relative to the Alex config directory). Verify:
```bash
ls ~/.alex/scripts/your-script.py   # Must exist
alex cron edit <job_id> --script ~/.alex/scripts/your-script.py
```

**"Skill not found" at job execution**
The skill must be installed on the machine running the scheduler. If you move between machines, skills don't automatically sync — reinstall them with `alex skills install <skill-name>`.
...
Jobs are stored in `~/.alex/cron/jobs.json`. If this file is not readable/writable by your user, the scheduler will fail silently:

```bash
ls -la ~/.alex/cron/jobs.json
chmod 600 ~/.alex/cron/jobs.json   # Your user should own it
```
...
```bash
alex cron list                    # Show all jobs, states, next_run times
alex cron run <job_id>            # Schedule for next tick (for testing)
alex cron edit <job_id>           # Fix configuration issues
alex logs                         # View recent Alex logs
alex skills list                  # Verify installed skills
```
...
1. Run the job with `alex cron run <job_id>` (fires on next gateway tick) and watch for errors in the chat output
2. Check `~/.alex/logs/agent.log` for scheduler messages and `~/.alex/logs/errors.log` for warnings
3. Open an issue at [github.com/charan vankudoth/alex-agent](https://github.com/charan vankudoth/alex-agent) with:
