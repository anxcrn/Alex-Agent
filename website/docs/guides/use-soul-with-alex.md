---
sidebar_position: 7
title: "Use SOUL.md with Alex"
description: "How to use SOUL.md to shape Alex Agent's default voice, what belongs there, and how it differs from AGENTS.md and /personality"
---

# Use SOUL.md with Alex

`SOUL.md` is the **primary identity** for your Alex instance. It's the first thing in the system prompt — it defines who the agent is, how it speaks, and what it avoids.

If you want Alex to feel like the same assistant every time you talk to it — or if you want to replace the Alex persona entirely with your own — this is the file to use.
...
- personality
- communication style
- how direct or warm Alex should be
- what Alex should avoid stylistically
- how Alex should relate to uncertainty, disagreement, and ambiguity

In short:
- `SOUL.md` is about who Alex is and how Alex speaks
...
Alex now uses only the global SOUL file for the current instance:

```text
~/.alex/SOUL.md
```

If you run Alex with a custom home directory, it becomes:
...
Alex automatically seeds a starter `SOUL.md` for you if one does not already exist.

That means most users now begin with a real file they can read and edit immediately.

Important:
- if you already have a `SOUL.md`, Alex does not overwrite it
- if the file exists but is empty, Alex adds nothing from it to the prompt

## How Alex uses it

When Alex starts a session, it reads `SOUL.md` from `ALEX_HOME`, scans it for prompt-injection patterns, truncates it if needed, and uses it as the **agent identity** — slot #1 in the system prompt. This means SOUL.md completely replaces the built-in default identity text.
...
That alone can noticeably change how Alex feels.
...
Alex already tries to be helpful and clear. `SOUL.md` should add real personality and style, not restate obvious defaults.
...
# Style
How Alex should sound.

# Avoid
What Alex should not do.

# Defaults
How Alex should behave when ambiguity appears.
```
...
Then restart Alex or start a new session.
...
### I edited SOUL.md but Alex still sounds the same

Check:
- you edited `~/.alex/SOUL.md` or `$ALEX_HOME/SOUL.md`
...
### Alex is ignoring parts of my SOUL.md
