---
sidebar_position: 8
title: "Use Voice Mode with Alex"
description: "A practical guide to setting up and using Alex voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with Alex
...
- you want Alex sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in Alex.
...
## Step 1: make sure normal Alex works first
...
Alex supports both local and cloud speech stacks.
...
### If you use `alex setup`

If you choose NeuTTS in the setup wizard, Alex checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. Alex transcribes and responds
...
- asking Alex to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full Alex loop.

## Tuning CLI behavior

### Silence threshold

If Alex starts/stops too aggressively, tune:
...
Alex stays a normal chat bot, but can speak replies.
...
- you want Alex to function like a portable research or ops assistant
...
Alex joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- Alex detects speech boundaries
- transcripts are posted in the associated text channel
- Alex responds in text and audio
...
1. get text Alex working
2. install `alex-agent[voice]`
