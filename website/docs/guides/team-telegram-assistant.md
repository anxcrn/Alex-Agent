...
This tutorial walks you through setting up a Telegram bot powered by Alex Agent that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **Alex Agent installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.alex/.env`

:::tip
A $5/month VPS is plenty for running the gateway. Alex itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team Alex Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_alex_bot`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by Alex Agent. DM me for help with code, research, debugging, and more.
   ```
...
# View live logs
journalctl --user -u alex-gateway -f
...
:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `alex gateway install` to update the plist.
:::
...
```bash
alex gateway stop && alex gateway start
```
...
alex pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.alex/.env`:
...
Control how much detail the bot shows when using tools. In `~/.alex/config.yaml`:
...
Customize how the bot communicates by editing `~/.alex/SOUL.md`:

For a full guide, see [Use SOUL.md with Alex](/guides/use-soul-with-alex).
...
```bash
# From the CLI
alex cron list          # View all scheduled jobs
alex cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u alex-gateway` (Linux) or `~/.alex/logs/gateway.log` (macOS) |
| Cron job output | `~/.alex/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.alex/cron/jobs.json` |
| Pairing data | `~/.alex/pairing/` |
| Session history | `~/.alex/sessions/` |
