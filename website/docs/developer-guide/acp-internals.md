...
The ACP adapter wraps Alex' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
alex acp / alex-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> parse --version / --check / --setup before server startup
  -> load ~/.alex/.env
  -> configure stderr logging
  -> construct AlexACPAgent
  -> acp.run_agent(agent, use_unstable_protocol=True)
```

The Zed ACP Registry path launches the same adapter through `uvx --from 'alex-agent[acp]==<version>' alex-acp`, pointed at the `alex-agent` PyPI release.
...
- `allow_once` -> Alex `once`
- `allow_always` -> Alex `always`
- reject options -> Alex `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps Alex tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["alex-acp"])
...
Instead it reuses Alex' runtime resolver:

- `acp_adapter/auth.py`
- `alex_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Alex provider/credentials. It also always advertises a terminal setup auth method (`alex-setup`, args `--setup`) so first-run registry clients can open Alex' interactive model/provider configuration before starting a normal ACP session.
...
- ACP sessions are persisted to the shared `~/.alex/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `alex-acp` toolset definition
- `alex_cli/main.py` — `alex acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `alex-acp` script
