...
description: "How Alex stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `alex_cli/cron.py` | CLI `alex cron` subcommands |
...
Jobs are stored in `~/.alex/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
In CLI mode, cron jobs only fire when `alex cron` commands are run or during active CLI sessions.
...
```python
# ~/.alex/scripts/check_competitors.py
...
2. **Environment variable** — `ALEX_CRON_SCRIPT_TIMEOUT`
...
| Local file | `local` | Save to `~/.alex/cron/output/` |
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `alex cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.

## CLI Interface

The `alex cron` CLI provides direct job management:

```bash
alex cron list                    # Show all jobs
alex cron create                  # Interactive job creation (alias: add)
alex cron edit <job_id>           # Edit job configuration
alex cron pause <job_id>          # Pause a running job
alex cron resume <job_id>         # Resume a paused job
alex cron run <job_id>            # Trigger immediate execution
alex cron remove <job_id>         # Delete a job
```
