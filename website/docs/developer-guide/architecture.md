...
sidebar_position: 1
title: "Architecture"
description: "Alex Agent internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of Alex Agent internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
│ alex_state.py   │              │ Browser (5 backends)  │
...
├── cli.py                    # AlexCLI — interactive terminal UI (large file)
├── model_tools.py            # Tool discovery, schema collection, dispatch
├── toolsets.py               # Tool groupings and platform presets
├── alex_state.py           # SQLite session/state database with FTS5
├── alex_constants.py       # ALEX_HOME, profile-aware paths
...
│
├── alex_cli/               # CLI subcommands and setup
│   ├── main.py               # Entry point — all `alex` subcommands (large file)
...
│   ├── skills_config.py      # alex skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # alex tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # alex gateway start/stop
│
...
```text
User input → AlexCLI.process_input()
...
Three discovery sources: `~/.alex/plugins/` (user), `.alex/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Two specialized plugin types exist: memory providers (`plugins/memory/`) and context engines (`plugins/context_engine/`). Both are single-select — only one of each can be active at a time, configured via `alex plugins` or `config.yaml`.

→ [Plugin Guide](/guides/build-a-alex-plugin), [Memory Provider Plugin](./memory-provider-plugin.md)
...
Exposes Alex as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`alex -p <name>`) gets its own ALEX_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
