...
Alex Agent uses a SQLite database (`~/.alex/state.db`) to persist session
metadata, full message history, and model configuration across CLI and gateway
sessions. This replaces the earlier per-session JSONL file approach.

Source file: `alex_state.py`


## Architecture Overview

```
~/.alex/state.db (SQLite, WAL mode)
...
Multiple alex processes (gateway + CLI sessions + worktree agents) share one
...
db = SessionDB()                           # Default: ~/.alex/state.db
...
Default path: `~/.alex/state.db`

This is derived from `alex_constants.get_alex_home()` which resolves to
`~/.alex/` by default, or the value of `ALEX_HOME` environment variable.
