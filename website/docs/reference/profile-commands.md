...
This page covers all commands related to [Alex profiles](../user-guide/profiles.md). For general CLI commands, see [CLI Commands Reference](./cli-commands.md).
...
Top-level command for managing profiles. Running `alex profile` without a subcommand shows help.
...
```bash
alex profile use <name>
```

Sets `<name>` as the active profile. All subsequent `alex` commands (without `-p`) will use this profile.
...
alex profile use default
```

## `alex profile create`

```bash
alex profile create <name> [options]
```
...
| `--description "<text>"` | One- or two-sentence description of what this profile is good at. Used by the kanban orchestrator to route tasks based on role instead of profile name alone. Skip and add later via `alex profile describe`. Persisted in `<profile_dir>/profile.yaml`. |
| `--no-skills` | Create an **empty** profile with zero bundled skills enabled. Writes a `.no-bundled-skills` marker into the profile so future `alex update` runs won't re-seed the bundled set, and refuses to combine with `--clone`, `--clone-from`, or `--clone-all` (which would copy skills in anyway). Useful for narrow orchestrator profiles or sandbox profiles that should not inherit the full skill catalog. To toggle this on an already-created profile (including the default `~/.alex`), use `alex skills opt-out` / `alex skills opt-in`. |
...
alex profile create work2 --clone-from work

# Clone everything from a specific profile
alex profile create work2-backup --clone-from work --clone-all
```

## `alex profile describe`

```bash
alex profile describe [<name>] [options]
```
...
alex profile describe researcher

# Set it explicitly
alex profile describe researcher --text "Reads source code and writes findings."

# Let the LLM generate one
alex profile describe researcher --auto

# Fill in descriptions for every profile that doesn't have one
alex profile describe --all --auto
```

## `alex profile delete`

```bash
alex profile delete <name> [options]
```
...
```bash
alex profile delete mybot
alex profile delete mybot --yes
```
...
```bash
alex profile show <name>
```

Displays details about a profile including its home directory, configured model, gateway status, skills count, and configuration file status.

This shows the profile's Alex home directory, not the terminal working directory. Terminal commands start from `terminal.cwd` (or the launch directory on the local backend when `cwd: "."`).
...
```bash
$ alex profile show work
Profile: work
Path:    ~/.alex/profiles/work
...
```bash
alex profile alias <name> [options]
```

Regenerates the shell alias script at `~/.local/bin/<name>`. Useful if the alias was accidentally deleted or if you need to update it after moving your Alex installation.
...
```bash
alex profile alias work
# Creates/updates ~/.local/bin/work

alex profile alias work --name mywork
# Creates ~/.local/bin/mywork

alex profile alias work --remove
...
```bash
alex profile rename <old-name> <new-name>
```
...
```bash
alex profile rename mybot assistant
# ~/.alex/profiles/mybot → ~/.alex/profiles/assistant
...
```bash
alex profile export <name> [options]
```
...
```bash
alex profile export work
# Creates work.tar.gz in the current directory

alex profile export work -o ./work-2026-03-29.tar.gz
```

## `alex profile import`

```bash
alex profile import <archive> [options]
```
...
```bash
alex profile import ./work-2026-03-29.tar.gz
# Infers profile name from the archive

alex profile import ./work-2026-03-29.tar.gz --name work-restored
```
...
:::info
`alex profile export` / `import` are still the right commands for
...
### `alex profile install`

```bash
alex profile install <source> [--name <name>] [--alias] [--force] [--yes]
```
...
| `--alias` | Also create a shell wrapper (e.g. `telemetry` → `alex -p telemetry`). |
...
alex profile install github.com/kyle/telemetry-distribution --alias

# Install from a full HTTPS git URL
alex profile install https://github.com/kyle/telemetry-distribution.git

# Install from SSH
alex profile install git@github.com:kyle/telemetry-distribution.git

# Install from a local directory during development
alex profile install ./telemetry/
```

### `alex profile update`

```bash
alex profile update <name> [--force-config] [--yes]
```
...
```bash
alex profile info <name>
```

Prints the profile's distribution manifest — name, version, required
Alex version, author, env var requirements, the source URL/path, and
...
`alex profile list` also shows the distribution name and version in a
`Distribution` column, and `alex profile show <name>` / `delete <name>`
...
alex profile install git@github.com:your-org/internal-assistant.git

# Uses your git credential helper
alex profile install https://github.com/your-org/internal-assistant.git
```
...
`alex_requires` supports `>=`, `<=`, `==`, `!=`, `>`, `<`, or a bare
version (treated as `>=`). Install fails with a clear error if the current
Alex version doesn't satisfy the spec.
...
2. Initialize a git repo (or use an existing one) and push to GitHub /
   GitLab / any host Alex can clone from.
3. Tell recipients to run `alex profile install <your-repo-url>`.
...
```bash
alex -p <name> <command> [options]
alex --profile <name> <command> [options]
```

Global flag to run any Alex command under a specific profile without changing the sticky default. This overrides the active profile for the duration of the command.
...
```bash
alex -p work chat -q "Check the server status"
alex --profile dev gateway start
alex -p personal skills list
alex -p work config edit
```

## `alex completion`

```bash
alex completion <shell>
```
...
- `alex profile <TAB>` — subcommands (list, use, create, etc.)
- `alex profile use <TAB>` — profile names
- `alex -p <TAB>` — profile names
