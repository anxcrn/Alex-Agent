"""ACP auth helpers — detect and advertise Alex authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "alex-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active Alex runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if Alex can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Alex.
    ...
    not have Alex provider credentials configured yet, so Alex always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate Alex using the currently configured "
