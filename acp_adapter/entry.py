"""CLI entry point for the alex-agent ACP adapter.

Loads environment variables from ``~/.alex/.env``, configures logging
...
try:
    ...
    # shadowing Alex's own modules — ``alex acp`` can be started from any
    # cwd, including a project that has same-named packages on its path.
    alex_bootstrap.harden_import_path()
...
def _load_env() -> None:
    """Load .env from ALEX_HOME (default ``~/.alex``)."""
    from alex_cli.env_loader import load_alex_dotenv

    alex_home = get_alex_home()
    loaded = load_alex_dotenv(alex_home=alex_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", alex_home / ".env"
        )
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="alex-acp",
        description="Run Alex Agent as an ACP stdio server.",
    )
    parser.add_argument("--version", action="store_true", help="Print Alex version and exit")
    parser.add_argument(
        "--check",
        action="store_true",
    ...
    parser.add_argument(
        "--setup",
        action="store_true",
        help="Run interactive Alex provider/model setup for ACP terminal auth",
    )
    parser.add_argument(
        "--setup-browser",
        action="store_true",
        help="Install agent-browser + Playwright Chromium into ~/.alex/node/ "
...
def _run_check() -> None:
    ...
    from acp_adapter.server import AlexACPAgent  # noqa: F401

    print("Alex ACP check OK")


def _run_setup() -> None:
    from alex_cli.main import main as alex_main

    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "alex", "model"]
        alex_main()
    finally:
        sys.argv = old_argv
...
def _run_setup_browser(assume_yes: bool = False) -> int:
    ...
    with ``alex postinstall`` and the runtime lazy installer.
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting alex-agent ACP adapter")
    ...
    import acp
    from .server import AlexACPAgent
