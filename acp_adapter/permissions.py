"""ACP permission bridging for Alex dangerous-command approvals."""
...
# Maps ACP permission option ids to Alex approval result strings.
...
def _build_permission_options(*, allow_permanent: bool) -> list[PermissionOption]:
    """Return ACP options that match Alex approval semantics."""
    options = [
        PermissionOption(option_id="allow_once", kind="allow_once", name="Allow once"),
        PermissionOption(
            option_id="allow_session",
            # ACP has no session-scoped kind, so use the closest persistent
            # hint while keeping Alex semantics in the option id.
            kind="allow_always",
...
def _map_outcome_to_alex(outcome: object, *, allowed_option_ids: set[str]) -> str:
    """Map an ACP permission outcome into Alex approval strings."""
    ...
    if option_id not in allowed_option_ids:
        ...
        return "deny"
    return _OPTION_ID_TO_ALEX.get(option_id, "deny")
...
def make_approval_callback(
    ...
    """
    Return a Alex-compatible approval callback that bridges to ACP.
