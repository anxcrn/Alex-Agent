...
def test_session_search_lazily_opens_db_when_entrypoint_did_not_pass_one(monkeypatch):
    ...
    alex_state = ModuleType("alex_state")
    alex_state.SessionDB = FakeSessionDB
    monkeypatch.setitem(sys.modules, "alex_state", alex_state)
    ...
    result = json.loads(agent._invoke_tool("session_search", {"query": "Alex"}, "task-id"))

    assert result["success"] is True
    assert captured["db"] is sentinel_db
    assert captured["query"] == "Alex"
