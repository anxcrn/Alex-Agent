...
binary download are exercised manually by `alex secrets bitwarden
...
def alex_home(tmp_path, monkeypatch):
    """Point Alex at an isolated home directory."""
    home = tmp_path / ".alex"
    home.mkdir()
    monkeypatch.setenv("ALEX_HOME", str(home))
    # Some modules cache get_alex_home; clear if needed.
    import alex_constants
    if hasattr(alex_constants, "_ALEX_HOME_CACHE"):
        alex_constants._ALEX_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_install_bws_rejects_malicious_member(alex_home, monkeypatch):
...
def test_install_bws_happy_path(alex_home, monkeypatch):
...
def test_install_bws_checksum_mismatch(alex_home, monkeypatch):
...
def test_install_bws_missing_checksum_entry(alex_home, monkeypatch):
...
def test_env_loader_skips_when_disabled(tmp_path, monkeypatch):
    ...
    from alex_cli.env_loader import _apply_external_secret_sources
...
def test_disk_cache_written_after_first_fetch(monkeypatch, tmp_path):
    """First fetch hits bws AND writes a 0600 file under alex_home/cache/."""
    home = tmp_path / ".alex"
    home.mkdir()
    fake_binary = tmp_path / "bws"
    fake_binary.write_text("")
