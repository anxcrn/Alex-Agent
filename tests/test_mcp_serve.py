"""
Tests for mcp_serve — Alex MCP server.
...
def _isolate_alex_home(tmp_path, monkeypatch):
    """Redirect ALEX_HOME to a temp directory."""
    monkeypatch.setenv("ALEX_HOME", str(tmp_path))
    try:
        import alex_constants
        monkeypatch.setattr(alex_constants, "get_alex_home", lambda: tmp_path)
...
def _create_test_db(db_path, session_id, messages):
    """Create a minimal SQLite DB mimicking alex_state schema."""
