"""Shared fixtures for the alex-agent test suite.
...
2. **Isolated ALEX_HOME.** ALEX_HOME points to a per-test tempdir so
   code reading ``~/.alex/*`` via ``get_alex_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".alex"`` instead of the canonical
   ``get_alex_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No ALEX_SESSION_* inheritance** — the agent's current gateway
...
# ALEX_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_ALEX_BEHAVIORAL_VARS = frozenset({
    "ALEX_YOLO_MODE",
    "ALEX_INTERACTIVE",
    "ALEX_QUIET",
    "ALEX_TOOL_PROGRESS",
    "ALEX_TOOL_PROGRESS_MODE",
    "ALEX_MAX_ITERATIONS",
    "ALEX_SESSION_PLATFORM",
    "ALEX_SESSION_CHAT_ID",
    "ALEX_SESSION_CHAT_NAME",
    "ALEX_SESSION_THREAD_ID",
    "ALEX_SESSION_SOURCE",
    "ALEX_SESSION_KEY",
    "ALEX_GATEWAY_SESSION",
    "ALEX_CRON_SESSION",
    "_ALEX_GATEWAY",
    "ALEX_PLATFORM",
    "ALEX_MODEL",
    "ALEX_INFERENCE_MODEL",
    "ALEX_INFERENCE_PROVIDER",
    "ALEX_TUI_PROVIDER",
    "ALEX_MANAGED",
    "ALEX_MANAGED_DIR",
    "ALEX_DEV",
    ...
    "ALEX_TIMEZONE",
    "ALEX_REDACT_SECRETS",
    "ALEX_BACKGROUND_NOTIFICATIONS",
    "ALEX_EXEC_ASK",
    "ALEX_HOME_MODE",
    "ALEX_AGENT_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.alex/kanban.db instead of the per-test ALEX_HOME.
    "ALEX_KANBAN_DB",
    "ALEX_KANBAN_BOARD",
    "ALEX_KANBAN_HOME",
    "ALEX_KANBAN_WORKSPACES_ROOT",
    "ALEX_KANBAN_LOGS_ROOT",
    "ALEX_KANBAN_TASK",
    "ALEX_KANBAN_WORKSPACE",
    "ALEX_KANBAN_RUN_ID",
    "ALEX_KANBAN_CLAIM_LOCK",
    "ALEX_KANBAN_DISPATCH_IN_GATEWAY",
    "ALEX_TENANT",
    ...
    "ALEX_DASHBOARD_OAUTH_CLIENT_ID",
    "ALEX_DASHBOARD_PORTAL_URL",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and ALEX_HOME to per-test tempdirs so code that
    reads ``~/.alex/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral ALEX_* vars that could change test semantics.
    for name in _ALEX_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)

    # 3. Redirect ALEX_HOME to a per-test tempdir. Code that reads
    #    ``~/.alex/*`` via ``get_alex_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.alex/*`` via
    #    ``Path.home() / ".alex"`` instead of ``get_alex_home()``
    #    is a bug to fix at the callsite.
    fake_alex_home = tmp_path / "alex_test"
    fake_alex_home.mkdir()
    (fake_alex_home / "sessions").mkdir()
    (fake_alex_home / "cron").mkdir()
    (fake_alex_home / "memories").mkdir()
    (fake_alex_home / "skills").mkdir()
    monkeypatch.setenv("ALEX_HOME", str(fake_alex_home))
    ...
    #    ~/.alex/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import alex_cli.plugins as _plugins_mod
...
@pytest.fixture(autouse=True)
def _isolate_alex_home(_hermetic_environment):
...
@pytest.fixture()
def mock_config():
    """Return a minimal alex config dict suitable for unit tests."""
    return {
...
# environment and finds the developer's live ``alex-gateway`` process
...
# correlated with a ``tests/alex_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> alex-gateway``
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart alex-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting alex/python patterns are also blocked.
    """
    ...
    def _matches_alex_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _ALEX_TOKENS)

    def _is_blocked_systemctl(cmd) -> bool:
        cmd_str = _cmd_to_string(cmd)
        if "systemctl" not in cmd_str:
            return False
        if not _matches_alex_gateway(cmd_str):
            return False
        try:
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch alex-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m alex_cli.main".
                if (
                    "alex" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live alex-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting alex/python could hit the live gateway. "
                "Mark with @pytest.mark.live_system_guard_bypass if "
                "intentional."
            )
        # Block any subprocess that would run `alex update` (or the
        # equivalent `python -m alex_cli.main update`).  These commands
        ...
        if "update" in low and (
            # alex update / alex update --gateway / setsid bash -c ... alex update
            ("alex" in low and "update" in low.split())
            or
            # python -m alex_cli.main update --gateway
            ("alex_cli" in low and "update" in low.split())
            or
            # venv/bin/alex update  (absolute path variant used in tests)
            (".venv/bin/alex" in low and "update" in low)
        ):
            raise RuntimeError(
                f"tests/conftest.py live-system guard: blocked "
                f"subprocess.{name}({cmd!r}) — this command would run "
                "`alex update` against the real checkout, fetching "
