...
closure the PR changed, against a real temp ``ALEX_HOME``.
"""
...
def _fake_switch_result():
    ...
    from alex_cli.model_switch import ModelSwitchResult
...
def _setup_isolated_home(tmp_path, monkeypatch, model_yaml_value):
    ...
    monkeypatch.setattr(
        "alex_cli.model_switch.resolve_display_context_length",
        lambda *a, **k: 272000,
    )
    # save_config writes to ``get_alex_home() / config.yaml`` — point it here.
    monkeypatch.setattr("alex_constants.get_alex_home", lambda: alex_home)
    monkeypatch.setattr("alex_cli.config.get_alex_home", lambda: alex_home)
    return cfg_path
