...
class TestToolsetConsistency:
    ...
    def test_alex_platforms_share_core_tools(self):
        """All alex-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on alex-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["alex-cli", "alex-telegram", "alex-discord", "alex-whatsapp", "alex-slack", "alex-signal", "alex-homeassistant"]
...
class TestDefaultPlatformWebSearchCoverage:
    def test_alex_whatsapp_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("alex-whatsapp")

    def test_alex_api_server_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("alex-api-server")
