"""Tests for alex_constants module."""
...
from alex_constants import (
    ...
    find_alex_node_executable,
    find_node_executable,
    find_node_executable_on_path,
    get_default_alex_root,
    get_alex_dir,
    get_alex_home,
    heal_alex_managed_node,
    alex_managed_node_tree_present,
    iter_alex_node_dirs,
    is_container,
    node_tool_runnable,
...
class TestGetDefaultAlexRoot:
    """Tests for get_default_alex_root() — Docker/custom deployment awareness."""

    def test_no_alex_home_returns_native(self, tmp_path, monkeypatch):
        """When ALEX_HOME is not set, returns ~/.alex."""
        monkeypatch.delenv("ALEX_HOME", raising=False)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        assert get_default_alex_root() == tmp_path / ".alex"

    def test_alex_home_is_native(self, tmp_path, monkeypatch):
        """When ALEX_HOME = ~/.alex, returns ~/.alex."""
        native = tmp_path / ".alex"
        native.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ALEX_HOME", str(native))
        assert get_default_alex_root() == native

    def test_alex_home_is_profile(self, tmp_path, monkeypatch):
        """When ALEX_HOME is a profile under ~/.alex, returns ~/.alex."""
        native = tmp_path / ".alex"
    ...
    def test_alex_home_is_docker(self, tmp_path, monkeypatch):
        """When ALEX_HOME points outside ~/.alex (Docker), returns ALEX_HOME."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ALEX_HOME", str(docker_home))
        assert get_default_alex_root() == docker_home

    def test_alex_home_is_custom_path(self, tmp_path, monkeypatch):
        """Any ALEX_HOME outside ~/.alex is treated as the root."""
        custom = tmp_path / "my-alex-data"
        custom.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ALEX_HOME", str(custom))
        assert get_default_alex_root() == custom

    def test_docker_profile_active(self, tmp_path, monkeypatch):
        """When a Docker profile is active (ALEX_HOME=<root>/profiles/<name>),
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ALEX_HOME", str(profile))
        assert get_default_alex_root() == docker_root

    def test_no_alex_home_returns_localappdata_root_on_windows(self, tmp_path, monkeypatch):
        """Native Windows falls back to %LOCALAPPDATA%\\alex, not ~/.alex."""
        local_appdata = tmp_path / "LocalAppData"
        monkeypatch.delenv("ALEX_HOME", raising=False)
        monkeypatch.setenv("LOCALAPPDATA", str(local_appdata))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "Home")
        monkeypatch.setattr(alex_constants.sys, "platform", "win32")

        assert get_default_alex_root() == local_appdata / "alex"

    def test_no_alex_home_uses_windows_path_when_localappdata_missing(self, tmp_path, monkeypatch):
        """Windows fallback still uses AppData/Local/alex without LOCALAPPDATA."""
        home = tmp_path / "Home"
        monkeypatch.delenv("ALEX_HOME", raising=False)
        ...
        assert get_default_alex_root() == home / "AppData" / "Local" / "alex"


class TestGetAlexHome:
    """Tests for get_alex_home() platform-aware fallback."""

    def test_windows_fallback_uses_localappdata(self, tmp_path, monkeypatch):
        """When ALEX_HOME is unset on Windows, use %LOCALAPPDATA%\\alex."""
        local_appdata = tmp_path / "LocalAppData"
        monkeypatch.delenv("ALEX_HOME", raising=False)
        monkeypatch.setenv("LOCALAPPDATA", str(local_appdata))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "Home")
        monkeypatch.setattr(alex_constants.sys, "platform", "win32")
        monkeypatch.setattr(alex_constants, "_profile_fallback_warned", False)

        assert get_alex_home() == local_appdata / "alex"


class TestAlexManagedNode:
    def test_windows_node_dir_prefers_portable_root(self, tmp_path, monkeypatch):
        ...
        assert iter_alex_node_dirs() == [node_dir, bin_dir]
    ...
    def test_windows_finds_npm_cmd_before_path(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(alex_constants.sys, "platform", "win32")
        monkeypatch.setenv("ALEX_HOME", str(home))
        monkeypatch.setattr(alex_constants, "node_tool_runnable", lambda path: True)

        assert find_alex_node_executable("npm") == str(npm_cmd)
    ...
    def test_windows_skips_broken_managed_npm_without_path_fallback(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("ALEX_HOME", str(home))
        monkeypatch.setenv("PATH", str(bin_dir))
        monkeypatch.setattr(alex_constants, "_managed_node_heal_attempted", False)
        monkeypatch.setattr(alex_constants, "heal_alex_managed_node", lambda: False)
        monkeypatch.setattr(
            alex_constants,
            "node_tool_runnable",
            lambda path: False,
        ...
        assert alex_managed_node_tree_present() is True
        assert find_node_executable("npm") is None
        assert find_node_executable("npm") != str(path_npm)

    def test_with_alex_node_path_prepends_existing_managed_dirs(self, tmp_path, monkeypatch):
        home = tmp_path / "alex"
        node_dir = home / "node"
        bin_dir = node_dir / "bin"
        node_dir.mkdir(parents=True)
        ...
        env = with_alex_node_path({"PATH": "system-node"})
...
class TestNodeToolRunnable:
    """node_tool_runnable() rejects broken Alex-managed npm/node wrappers."""
    ...
    def test_broken_managed_npm_heals_when_node_still_runs(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(alex_constants, "heal_alex_managed_node", _heal)
    ...
    def test_broken_managed_npm_heals_instead_of_path_fallback(self, tmp_path, monkeypatch):
        ...
        assert find_alex_node_executable("npm") == str(healed_npm)
...
class TestIsContainer:
    ...
    def _reset_cache(self, monkeypatch):
        ...
        monkeypatch.setattr(alex_constants, "_container_detected", None)
    ...
    def test_caches_result(self, monkeypatch):
        ...
        monkeypatch.setattr(alex_constants, "_container_detected", True)
        assert is_container() is True
...
class TestSecureParentDir:
    ...
    def test_safe_path_calls_chmod(self, tmp_path, monkeypatch):
        ...
        safe_dir = tmp_path / "home" / "user" / ".alex"
...
class TestGetAlexDir:
    """Tests for ``get_alex_dir(new_subpath, old_name)``.
    ...
    def test_neither_exists_returns_new(self, tmp_path, monkeypatch):
        ...
        result = get_alex_dir("platforms/pairing", "pairing")
    ...
    def test_legacy_populated_returns_legacy(self, tmp_path, monkeypatch):
        ...
        result = get_alex_dir("cache/images", "image_cache")
    ...
    def test_legacy_populated_with_subdir_returns_legacy(self, tmp_path, monkeypatch):
        ...
        result = get_alex_dir("platforms/matrix/store", "matrix/store")
        assert result == legacy
    ...
    def test_legacy_empty_and_new_missing_returns_new(self, tmp_path, monkeypatch):
        ...
        legacy.mkdir()
        result = get_alex_dir("cache/audio", "audio_cache")
