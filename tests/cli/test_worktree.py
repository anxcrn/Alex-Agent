...
class TestWorktreeCreation:
    ...
    def test_creates_worktree(self, git_repo):
        ...
        assert info["branch"].startswith("alex/alex-")
...
class TestOrphanedBranchPruning:
    """Test cleanup of orphaned alex/* and pr-* branches."""

    def test_prunes_orphaned_alex_branch(self, git_repo):
        """alex/alex-* branches with no worktree should be deleted."""
        # Create a branch that looks like a worktree branch but has no worktree
        subprocess.run(
            ["git", "branch", "alex/alex-deadbeef", "HEAD"],
            cwd=str(git_repo), capture_output=True,
        )

        # Verify it exists
        result = subprocess.run(
            ["git", "branch", "--list", "alex/alex-deadbeef"],
            capture_output=True, text=True, cwd=str(git_repo),
        )
        assert "alex/alex-deadbeef" in result.stdout
        ...
        assert "alex/alex-deadbeef" in orphaned
        ...
        assert "alex/alex-deadbeef" not in result.stdout
