...
def _isolate(tmp_path, monkeypatch):
    """Isolate ALEX_HOME so tests don't touch real config."""
    home = tmp_path / ".alex"
    home.mkdir()
...
def cli_obj(_isolate):
    """Create a minimal AlexCLI instance for banner testing."""
    with patch("cli.load_cli_config", return_value={
        ...
        obj = AlexCLI.__new__(AlexCLI)
        obj.model = "test-model"
        obj.enabled_toolsets = ["alex-core"]
        obj.compact = False
...
class TestLowContextWarning:
    ...
    def test_warning_for_below_minimum_context(self, cli_obj):
        """Warning shown when context is below Alex' minimum."""
    ...
    def test_no_warning_at_boundary(self, cli_obj):
        """No warning at exactly Alex' minimum context length."""
    ...
    def test_no_warning_above_boundary(self, cli_obj):
        """No warning above Alex' minimum context length."""
