...
def server():
    with patch.dict(
        "sys.modules",
        {
            "alex_constants": MagicMock(
                get_alex_home=MagicMock(return_value="/tmp/alex_test_review_summary")
            ),
...
def test_init_session_attaches_background_review_callback(server, monkeypatch):
    ...
    cb("💾 Self-improvement review: Skill 'alex-release' patched")
    ...
    assert sid == "sid-abc"
    assert payload == {
        "text": "💾 Self-improvement review: Skill 'alex-release' patched"
    }
