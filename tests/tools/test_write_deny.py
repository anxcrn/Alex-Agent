...
class TestWriteDenyExactPaths:
    ...
    def test_alex_env(self):
        # ``.env`` under the active ALEX_HOME (profile-aware, not just
        # ``~/.alex``) must be write-denied. The hermetic test conftest
        # points ALEX_HOME at a tempdir — resolve via get_alex_home()
        # to match the denylist.
        from alex_constants import get_alex_home
        path = str(get_alex_home() / ".env")
        assert _is_write_denied(path) is True

    def test_alex_root_env_when_running_under_profile(self, tmp_path, monkeypatch):
        ...
        ``~/.alex/.env`` (whose credentials are inherited by every profile)
        could be silently overwritten by ``write_file`` while a profile was
        active.
        """
        root = tmp_path / "alex_root"
        ...
        # Sanity check: ALEX_HOME does point to the profile dir, not the root.
        from alex_constants import get_alex_home, get_default_alex_root
        assert get_alex_home() == profile_home
        assert get_default_alex_root() == root
...
class TestWriteAllowed:
    ...
    def test_alex_control_files_requested_writable(self):
        from alex_constants import get_alex_home
