...
Older images made /opt/alex/ui-tui and /opt/alex/node_modules writable so a
...
def test_dockerfile_uses_prebuilt_tui_instead_of_writable_node_modules() -> None:
    ...
    assert "ENV ALEX_TUI_DIR=/opt/alex/ui-tui" in text
    assert "cd ../ui-tui && npm run build" in text
    assert "chown -R alex:alex /opt/alex/ui-tui" not in text
    assert "chown -R alex:alex /opt/alex/node_modules" not in text
