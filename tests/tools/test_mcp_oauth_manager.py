...
def test_alex_provider_subclass_exists():
    """AlexMCPOAuthProvider is defined and subclasses OAuthClientProvider."""
    from tools.mcp_oauth_manager import _ALEX_PROVIDER_CLS
    from mcp.client.auth.oauth2 import OAuthClientProvider

    assert _ALEX_PROVIDER_CLS is not None
    assert issubclass(_ALEX_PROVIDER_CLS, OAuthClientProvider)
...
def test_manager_builds_alex_provider_subclass(tmp_path, monkeypatch):
    """get_or_build_provider returns AlexMCPOAuthProvider, not plain OAuthClientProvider."""
    from tools.mcp_oauth_manager import (
        MCPOAuthManager, _ALEX_PROVIDER_CLS, reset_manager_for_tests,
    )
    reset_manager_for_tests()
    monkeypatch.setenv("ALEX_HOME", str(tmp_path))
    _set_interactive_stdin(monkeypatch)
    ...
    assert _ALEX_PROVIDER_CLS is not None
    assert isinstance(provider, _ALEX_PROVIDER_CLS)
    assert provider._alex_server_name == "srv"
