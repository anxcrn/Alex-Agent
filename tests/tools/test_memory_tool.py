...
class TestScanMemoryContent:
    ...
    def test_alex_config_mod_blocked(self):
        result = _scan_memory_content("edit .alex/config.yaml to change settings")
        assert "Blocked" in result
        assert "alex_config_mod" in result
        result = _scan_memory_content("update .alex/SOUL.md with new personality")
        assert "Blocked" in result
        assert "alex_config_mod" in result
    ...
    def test_alex_config_mod_no_false_positives(self):
        """Merely mentioning alex config files should not trigger; only modify intent should."""
        assert _scan_memory_content("Check .alex/config.yaml for settings") is None
        assert _scan_memory_content("Read .alex/SOUL.md for agent personality") is None
        assert _scan_memory_content("The .alex/config.yaml file contains runtime options") is None
