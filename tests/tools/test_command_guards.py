...
def _clean_state():
    ...
    saved = {}
    for k in ("ALEX_INTERACTIVE", "ALEX_GATEWAY_SESSION", "ALEX_EXEC_ASK", "ALEX_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["ALEX_INTERACTIVE"] = "1"
        session_key = os.getenv("ALEX_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No ALEX_INTERACTIVE or ALEX_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
