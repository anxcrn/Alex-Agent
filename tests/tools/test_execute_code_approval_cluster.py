...
Covers the canonical fix for issues #4146, #27303, #30882, #33057:
  ...
  4. tools.code_execution_tool._scrub_child_env — broad ALEX_ prefix dropped,
...
def gw_session(monkeypatch):
    """A clean gateway session: ALEX_GATEWAY_SESSION set, a bound session
    ...
    monkeypatch.delenv("ALEX_CRON_SESSION", raising=False)
...
def test_guard_cron_deny_blocks(monkeypatch):
    monkeypatch.setenv("ALEX_CRON_SESSION", "1")
    monkeypatch.delenv("ALEX_GATEWAY_SESSION", raising=False)
    monkeypatch.setattr(A, "_get_approval_mode", lambda: "manual")
...
def test_env_scrub_alex_allowlist_and_secret_blocks():
    from tools.code_execution_tool import _scrub_child_env

    env = {
        # operational allowlist → kept
        "ALEX_HOME": "/h", "ALEX_PROFILE": "p",
        "ALEX_CONFIG": "/c.yaml", "ALEX_ENV": "/e",
        # other ALEX_* → dropped (broad prefix removed)
        "ALEX_BASE_URL": "https://x", "ALEX_INTERACTIVE": "1",
        "ALEX_KANBAN_DB": "postgres://u:p@h/db",
    ...
    for kept in ("ALEX_HOME", "ALEX_PROFILE", "ALEX_CONFIG", "ALEX_ENV", "PATH"):
        assert kept in out, f"{kept} should be kept"
    for dropped in (
        "ALEX_BASE_URL", "ALEX_INTERACTIVE", "ALEX_KANBAN_DB",
...
def test_env_scrub_logs_dropped_alex_vars(caplog):
    """Dropping a non-allowlisted, non-secret ALEX_* var must be diagnosable:
    ...
    leave users guessing why a sandbox script sees an unset ALEX_* var."""
    import logging

    from tools.code_execution_tool import _scrub_child_env

    env = {
        "ALEX_HOME": "/h",          # allowlisted → kept, not logged
        "ALEX_BASE_URL": "https://x",   # dropped → logged
        "ALEX_KANBAN_DB": "postgres://u:p@h/db",  # dropped → logged
        "ALEX_API_KEY": "sk",       # secret → dropped silently (not logged)
        "PATH": "/usr/bin",           # safe prefix → kept
    }
    with caplog.at_level(logging.DEBUG, logger="tools.code_execution_tool"):
        out = _scrub_child_env(env, is_passthrough=lambda _: False, is_windows=False)

    assert "ALEX_HOME" in out and "PATH" in out
    assert "ALEX_BASE_URL" not in out and "ALEX_KANBAN_DB" not in out

    msgs = "\n".join(r.getMessage() for r in caplog.records)
    assert "ALEX_BASE_URL" in msgs and "ALEX_KANBAN_DB" in msgs
    assert "env_passthrough" in msgs
    # Secret vars are dropped but must NOT be named in the diagnostic log.
    assert "ALEX_API_KEY" not in msgs


def test_env_scrub_no_log_when_nothing_dropped(caplog):
    """No diagnostic noise when there are no dropped ALEX_* vars."""
    import logging
