...
def test_root_session_no_compression(db):
    ...
    assert prov["currentAlexSessionId"] == "root1"
    assert prov["rootAlexSessionId"] == "root1"
    assert prov["parentAlexSessionId"] is None
...
def test_compression_split_continuation(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "new", previous_alex_session_id="old"
    )
    assert prov["sessionKind"] == "continuation"
    assert prov["parentAlexSessionId"] == "old"
    assert prov["rootAlexSessionId"] == "old"
    assert prov["compressionDepth"] == 1
    assert prov["previousAlexSessionId"] == "old"
...
def test_multi_depth_chain(db):
    ...
    assert prov["rootAlexSessionId"] == "s0"
...
def test_non_compression_parent_is_root_not_continuation(db):
    ...
    assert prov["sessionKind"] == "root"
    assert prov["compressionDepth"] == 0
    assert prov["rootAlexSessionId"] == "p"  # lineage root still walked
...
def test_no_false_rotation_when_head_unchanged(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "s", previous_alex_session_id="s"
    )
    assert "reason" not in prov
    assert "creatorKind" not in prov
    assert prov["previousAlexSessionId"] == "s"
...
def test_meta_wrapper_shape(db):
    ...
    assert set(meta.keys()) == {"alex"}
    assert "sessionProvenance" in meta["alex"]
    assert meta["alex"]["sessionProvenance"]["currentAlexSessionId"] == "root1"
