"""Tests for alex_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every Alex entry point
(alex, alex-agent, alex-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Alex entry point imports alex_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported alex_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("alex_bootstrap", None)
    import alex_bootstrap  # noqa: WPS433
    return alex_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Alex CLI
...
class TestEntryPointsImportBootstrap:
    """Every Alex entry point must import alex_bootstrap as its
    ...
    # Entry points that invoke Alex as a process.  Each one must
    # import alex_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "alex_cli/main.py",   # alex CLI (console_script)
        "run_agent.py",          # alex-agent (console_script)
        "acp_adapter/entry.py",  # alex-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import alex_bootstrap' and that
        ...
        half-finished ``alex update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``alex_bootstrap`` as a top-level module) leaves alex
        ...
        """
        # Resolve relative to the alex-agent repo root.  Tests live
        # at tests/test_alex_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # alex start even when alex_bootstrap hasn't been
        ...
        assert first_import_name == "alex_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'alex_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import alex_bootstrap' line to be the first import."
        )


class TestHardenImportPath:
    """harden_import_path() must keep a same-named package in the launch
    directory from shadowing Alex's own top-level modules — covering both
    the relative ('' / '.') and absolute-path forms the cwd can take on
    sys.path (issue #51286)."""

    def _run(self, hb, path_seed, env=None):
        original = sys.path[:]
        original_env = os.environ.get("ALEX_PYTHON_SRC_ROOT")
        try:
            sys.path[:] = path_seed
            if env is not None:
                os.environ["ALEX_PYTHON_SRC_ROOT"] = env
            elif "ALEX_PYTHON_SRC_ROOT" in os.environ:
                del os.environ["ALEX_PYTHON_SRC_ROOT"]
            hb.harden_import_path(src_root="/opt/alex")
            return sys.path[:]
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("ALEX_PYTHON_SRC_ROOT", None)
            else:
                os.environ["ALEX_PYTHON_SRC_ROOT"] = original_env

    def test_relative_cwd_forms_removed(self):
        hb = _fresh_import()
        result = self._run(hb, ["", ".", "/opt/alex", "/usr/lib/python"])
    ...
    def test_absolute_cwd_path_loses_to_src_root(self):
        ...
        # Alex root.  The guard must relocate Alex to the front.
        hb = _fresh_import()
        result = self._run(hb, ["/home/user/tg-ws-proxy", "/opt/alex"])
        assert result[0] == "/opt/alex"
        # The cwd absolute path may still appear (it can hold legit deps),
        # but only AFTER the Alex root.
        assert result.index("/opt/alex") < result.index("/home/user/tg-ws-proxy")

    def test_src_root_not_duplicated(self):
        hb = _fresh_import()
        result = self._run(hb, ["/opt/alex", "/opt/alex", ""])
        assert result.count("/opt/alex") == 1
    ...
    def test_env_var_used_when_no_arg(self):
        ...
        try:
            ...
            os.environ["ALEX_PYTHON_SRC_ROOT"] = "/env/alex"
            hb.harden_import_path()
            assert sys.path[0] == "/env/alex"
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("ALEX_PYTHON_SRC_ROOT", None)
