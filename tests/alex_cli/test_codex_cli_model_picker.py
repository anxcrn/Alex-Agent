...
existing Codex CLI tokens via `alex auth openai-codex`. The old
...
@pytest.fixture()
def alex_auth_only_env(tmp_path, monkeypatch):
    """Tokens already in Alex auth store (no Codex CLI needed)."""
...
def test_normal_path_still_works(alex_auth_only_env):
    """openai-codex appears when tokens are already in Alex auth store."""
...
def test_codex_picker_uses_live_codex_catalog(alex_auth_only_env, tmp_path, monkeypatch):
