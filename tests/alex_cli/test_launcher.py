"""Tests for the top-level `./alex` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./alex` should use `alex_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "alex"
    called = []

    fake_main_module = types.ModuleType("alex_cli.main")

    def fake_main():
        called.append("alex_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "alex_cli.main", fake_main_module)
