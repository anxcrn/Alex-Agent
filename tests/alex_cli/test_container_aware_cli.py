...
def container_env(tmp_path, monkeypatch):
    """Set up a fake ALEX_HOME with .container-mode file."""
    alex_home = tmp_path / ".alex"
    alex_home.mkdir()
    monkeypatch.setenv("ALEX_HOME", str(alex_home))
    monkeypatch.delenv("ALEX_DEV", raising=False)

    container_mode = alex_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=alex-agent\n"
        "exec_user=alex\n"
        "alex_bin=/data/current-package/bin/alex\n"
    )
    return alex_home
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("alex_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "alex-agent"
    assert info["exec_user"] == "alex"
    assert info["alex_bin"] == "/data/current-package/bin/alex"


def test_get_container_exec_info_none_inside_container(container_env):
    """Returns None when we're already inside a container."""
    with patch("alex_constants.is_container", return_value=True):
        info = get_container_exec_info()
...
def test_get_container_exec_info_skipped_when_alex_dev(container_env, monkeypatch):
    """Returns None when ALEX_DEV=1 is set (dev mode bypass)."""
    monkeypatch.setenv("ALEX_DEV", "1")
...
def test_get_container_exec_info_not_skipped_when_alex_dev_zero(container_env, monkeypatch):
    """ALEX_DEV=0 does NOT trigger bypass — only '1' does."""
    monkeypatch.setenv("ALEX_DEV", "0")
...
def test_get_container_exec_info_defaults():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        alex_home = Path(tmpdir) / ".alex"
        alex_home.mkdir()
        (alex_home / ".container-mode").write_text(
            "# minimal file with no keys\n"
        )

        with patch("alex_constants.is_container", return_value=False), \
             patch.dict(get_container_exec_info.__globals__, {"get_alex_home": lambda: alex_home}), \
             patch.dict(os.environ, {}, clear=False):
            os.environ.pop("ALEX_DEV", None)
            info = get_container_exec_info()
...
def test_get_container_exec_info_docker_backend(container_env):
    ...
    (container_env / ".container-mode").write_text(
        "backend=docker\n"
        "container_name=alex-custom\n"
        "exec_user=myuser\n"
        "alex_bin=/opt/alex/bin/alex\n"
    )
    ...
    assert info["backend"] == "docker"
    assert info["container_name"] == "alex-custom"
    assert info["exec_user"] == "myuser"
    assert info["alex_bin"] == "/opt/alex/bin/alex"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from alex_cli.main import _exec_in_container
