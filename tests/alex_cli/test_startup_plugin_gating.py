...
``alex_cli.main`` skips eager plugin discovery at argparse-setup time
when the invocation is clearly targeting a known built-in subcommand.
This saves 500-650ms on ``alex --help``, ``alex version``,
``alex logs``, etc., by not importing ``google.cloud.pubsub_v1``,
...
def _live_subcommand_names() -> set[str]:
    """Run ``alex --help`` in-process and parse the subcommand block.
    ...
    built-in-only set.
    """
    from alex_cli import main as _main
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (["alex", "--tui", "chat"], "chat"),
        (["alex", "-w", "logs"], "logs"),
        (["alex", "chat", "hello world"], "chat"),
        (["alex", "gateway", "run"], "gateway"),
        # Top-level value-taking flags: the value should be skipped.
        (["alex", "-m", "gpt5", "chat"], "chat"),
        (["alex", "--model", "gpt5", "chat", "hi"], "chat"),
        (["alex", "-m", "gpt5", "--provider", "openai", "chat"], "chat"),
        (["alex", "-z", "hello world"], None),
        (["alex", "-z", "hello", "chat"], "chat"),
        (["alex", "--model=gpt5", "chat"], "chat"),     # inline form
        (["alex", "--", "chat"], "chat"),               # -- terminator
        (["alex", "-w", "--"], None),
        # Unknown positional after skipped flags → plugin-cmd candidate.
        (["alex", "some-plugin-cmd"], "some-plugin-cmd"),
        (["alex", "-m", "gpt5", "some-plugin-cmd"], "some-plugin-cmd"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ["alex"],                          # bare → chat
        ["alex", "--help"],                # top-level help
        ["alex", "-h"],
        ["alex", "version"],               # known built-in
        ["alex", "logs"],
        ["alex", "gateway", "run"],
        ["alex", "--tui"],
        ["alex", "-w", "--tui"],
        ["alex", "chat", "hi"],
        ["alex", "help"],                  # accepted built-in-ish
        ["alex", "-m", "gpt5", "chat"],    # flag-value-skipping
    ],
)
...
@pytest.mark.parametrize(
    "argv",
    [
        ["alex", "meet", "join"],          # potential google_meet plugin
        ["alex", "honcho", "status"],      # potential memory plugin
        ["alex", "unknown-subcmd"],
    ],
)
...
def test_builtin_set_covers_every_registered_subcommand():
    ...
    # in the set so ``alex help <cmd>`` skips discovery; it won't show
    ...
    assert not missing_from_declaration, (
        ...
        f"alex_cli/main.py::_BUILTIN_SUBCOMMANDS so plugin discovery "
