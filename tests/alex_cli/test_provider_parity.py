...
the SAME provider universe as ``alex model`` (the CLI/TUI picker).
...
def test_every_alex_model_provider_is_configurable_in_desktop():
    """PARITY CONTRACT: GUI (keys ∪ accounts) ⊇ `alex model` universe."""
    ...
    assert not missing, (
        "providers shown in `alex model` but not configurable in the desktop "
