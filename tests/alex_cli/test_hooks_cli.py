"""Tests for the ``alex hooks`` CLI subcommand."""
...
from alex_cli import hooks as hooks_cli


@pytest.fixture(autouse=True)
def _isolated_home(tmp_path, monkeypatch):
    monkeypatch.setenv("ALEX_HOME", str(tmp_path / "home"))
    monkeypatch.delenv("ALEX_ACCEPT_HOOKS", raising=False)
...
class TestHooksDoctor:
    ...
    def test_unallowlisted_script_is_not_executed(self, tmp_path):
        """Regression for M4: `alex hooks doctor` used to run every
