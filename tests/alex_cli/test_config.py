"""Tests for alex_cli configuration management."""
...
class TestGetAlexHome:
    def test_default_path(self):
        with patch.dict(os.environ, {}, clear=False):
            ...
            home = get_alex_home()
            assert home == Path.home() / ".alex"

    def test_env_override(self):
        with patch.dict(os.environ, {"ALEX_HOME": "/custom/path"}):
            home = get_alex_home()
            assert home == Path("/custom/path")


class TestEnsureAlexHome:
    def test_creates_subdirs(self, tmp_path):
        with patch.dict(os.environ, {"ALEX_HOME": str(tmp_path)}):
            ensure_alex_home()
    ...
    def test_upgrades_legacy_template_soul_md(self, tmp_path):
        ...
        from alex_cli.default_soul import DEFAULT_SOUL_MD, _LEGACY_TEMPLATE_SOULS
    ...
    def test_preserves_legacy_template_with_user_persona(self, tmp_path):
        ...
        from alex_cli.default_soul import _LEGACY_TEMPLATE_SOULS
...
class TestLoadConfigParseFailure:
    ...
    their config.yaml was being ignored. The helper must:
      * log at WARNING (so ``alex logs`` surfaces it)
    ...
    def test_logs_and_warns_on_parse_failure(self, tmp_path, caplog, capsys):
        ...
        from alex_cli import config as cfg_mod
        ...
        with patch.dict(os.environ, {"ALEX_HOME": str(tmp_path)}):
            ...
            import logging
            with caplog.at_level(logging.WARNING, logger="alex_cli.config"):
                config = load_config()
            ...
            # stands out at alex startup before logging is configured)
            captured = capsys.readouterr()
            assert "alex config:" in captured.err
    ...
    def test_rewarns_after_file_edit(self, tmp_path, capsys):
        ...
        with patch.dict(os.environ, {"ALEX_HOME": str(tmp_path)}):
            ...
            assert "alex config:" in after_edit, "edited file should re-warn"
...
class TestSaveEnvValueSecure:
    ...
    def test_save_env_value_updates_process_environment(self, tmp_path):
        with patch.dict(os.environ, {"ALEX_HOME": str(tmp_path)}, clear=False):
...
class TestRemoveEnvValue:
    def test_removes_key_from_env_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"ALEX_HOME": str(tmp_path), "KEY_B": "value_b"}):
    ...
    def test_clears_os_environ(self, tmp_path):
        ...
        with patch.dict(os.environ, {"ALEX_HOME": str(tmp_path), "MY_KEY": "my_value"}):
    ...
    def test_handles_missing_env_file(self, tmp_path):
        with patch.dict(os.environ, {"ALEX_HOME": str(tmp_path), "GHOST_KEY": "ghost"}):
    ...
    def test_clears_os_environ_even_when_not_in_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"ALEX_HOME": str(tmp_path), "ORPHAN_KEY": "orphan"}):
    ...
    def test_remove_env_value_preserves_existing_file_mode_on_posix(self, tmp_path):
        ...
        with patch.dict(os.environ, {"ALEX_HOME": str(tmp_path), "DROP": "gone"}):
...
class TestOptionalEnvVarsRegistry:
    ...
    def test_tavily_in_env_vars_by_version(self):
        ...
        from alex_cli.config import ENV_VARS_BY_VERSION
        all_vars = []
    ...
    def test_max_iterations_not_offered_as_env_var(self):
        """ALEX_MAX_ITERATIONS must NOT be in OPTIONAL_ENV_VARS (issue #17534).

        Offering it as an editable env var (dashboard, `alex setup`) lets a
        ...
        via config.yaml; ALEX_MAX_ITERATIONS remains a read-only backward-compat
        fallback in the gateway/CLI, never a promoted write target.
        """
        from alex_cli.config import OPTIONAL_ENV_VARS
        assert "ALEX_MAX_ITERATIONS" not in OPTIONAL_ENV_VARS
...
class TestEnvWriteDenylist:
    ...
    ``PATH``, ``EDITOR``, etc. — or any ``ALEX_*`` runtime flag.
    ...
    ``LD_PRELOAD=/tmp/evil.so`` in ``.env`` and own the next Alex
    process on next startup via the dotenv → ``os.environ`` chain in
    ``alex_cli/env_loader.py``.
    ...
    def _alex_home(self, tmp_path, monkeypatch):
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            "ALEX_SPOTIFY_CLIENT_ID",
            "ALEX_QWEN_BASE_URL",
            "ALEX_MAX_ITERATIONS",
        ],
    )
    def test_alex_integration_keys_still_writable(self, allowed_key):
        """``ALEX_*`` overall is NOT blocked — only the four runtime
        location names (HOME/PROFILE/CONFIG/ENV) are. Integration
        credentials following the ``ALEX_*`` convention must keep
    ...
    def test_arbitrary_user_key_still_works(self):
        ...
        denylist and outside ``ALEX_*``) keep working. The denylist
