"""Tests for IRC gateway configuration via `alex setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Alex install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Alex install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#alex")
            monkeypatch.setenv("IRC_NICKNAME", "alex-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `alex setup gateway` experience with IRC present."""
