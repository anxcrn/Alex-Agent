...
Before the fix, ``alex whatsapp`` wrote ``WHATSAPP_ENABLED=true`` at
...
``alex gateway`` then paid a 30s bridge-bootstrap timeout and queued
...
def isolated_home(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("ALEX_HOME", str(alex))
...
def _env_value(alex_home: Path, key: str) -> str | None:
...
def test_aborted_setup_does_not_enable_whatsapp(isolated_home, monkeypatch):
    ...
    """
    from alex_cli.main import cmd_whatsapp
    ...
    monkeypatch.setattr("alex_cli.main._require_tty", lambda *_a, **_kw: None)
...
def test_existing_pairing_skip_branch_enables_whatsapp(isolated_home, monkeypatch):
    """User runs ``alex whatsapp`` with an existing paired session and
