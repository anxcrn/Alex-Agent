...
* skips plugin discovery entirely (``alex_cli.plugins``)
...
_VARS = ("ALEX_SAFE_MODE", "ALEX_IGNORE_USER_CONFIG", "ALEX_IGNORE_RULES")
...
class TestSafeModeEnvWiring:
    ...
    def test_safe_mode_sets_all_gates(self):
        # Mirrors the cmd_chat logic in alex_cli/main.py.
        class Args:
            safe_mode = True
        ...
        assert os.environ.get("ALEX_SAFE_MODE") == "1"
...
class TestSafeModePluginDiscovery:
    """Plugin discovery must be a no-op under ALEX_SAFE_MODE=1."""

    def test_discovery_skipped(self, monkeypatch):
        monkeypatch.setenv("ALEX_SAFE_MODE", "1")
    ...
    def test_discovery_runs_without_safe_mode(self, monkeypatch):
        monkeypatch.delenv("ALEX_SAFE_MODE", raising=False)
        from alex_cli.plugins import PluginManager
...
class TestSafeModeMCP:
    """_load_mcp_config must return no servers under ALEX_SAFE_MODE=1."""
...
class TestSafeModeParser:
    """--safe-mode must parse on both the root parser and `alex chat`."""
