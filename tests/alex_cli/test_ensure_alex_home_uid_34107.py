"""Regression tests for #34107 — Docker UID/GID handling in ensure_alex_home.

When Alex runs in Docker with ``ALEX_UID=1000`` / ``ALEX_GID=911``,
the entrypoint chowns the top-level ``ALEX_HOME`` once at startup. But
...
The fix is a ``_chown_to_alex_uid`` helper that reads the env vars and
...
# ---------------------------------------------------------------------------
# _resolve_alex_uid_gid
# ---------------------------------------------------------------------------


class TestResolveAlexUidGid:
    def test_returns_parsed_values_when_both_set(self, monkeypatch):
        monkeypatch.setenv("ALEX_UID", "1000")
        monkeypatch.setenv("ALEX_GID", "911")
        from alex_cli.config import _resolve_alex_uid_gid
        uid, gid = _resolve_alex_uid_gid()
        assert uid == 1000
        assert gid == 911

    def test_returns_none_when_unset(self, monkeypatch):
        monkeypatch.delenv("ALEX_UID", raising=False)
        monkeypatch.delenv("ALEX_GID", raising=False)
        from alex_cli.config import _resolve_alex_uid_gid
        uid, gid = _resolve_alex_uid_gid()
        assert uid is None
        assert gid is None
    ...
    def test_invalid_uid_returns_none_for_that_field(self, monkeypatch):
        monkeypatch.setenv("ALEX_UID", "not-a-number")
        monkeypatch.setenv("ALEX_GID", "911")
        from alex_cli.config import _resolve_alex_uid_gid
        uid, gid = _resolve_alex_uid_gid()
        assert uid is None
    ...
    def test_empty_string_treated_as_unset(self, monkeypatch):
        monkeypatch.setenv("ALEX_UID", "")
        monkeypatch.setenv("ALEX_GID", "")
        from alex_cli.config import _resolve_alex_uid_gid
        uid, gid = _resolve_alex_uid_gid()
        assert uid is None
        assert gid is None
...
class TestChownToAlexUid:
    ...
    def test_attributeerror_swallowed_for_windows_compat(self, tmp_path, monkeypatch):
        ...
        with patch.object(cfg.os, "chown", side_effect=AttributeError("no chown on this platform")):
            cfg._chown_to_alex_uid(d)  # must not raise
