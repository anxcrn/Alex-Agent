...
alex_cli.config._load_config_impl, so the managed-scope merge has to be
applied in BOTH places or the interactive CLI/TUI surface (skin, display prefs)
silently ignores administrator-pinned values while `alex config`/`doctor`
...
def _load_cli_config(home):
    ...
    cli.py binds ``_alex_home = get_alex_home()`` at import time (module
    singleton), so monkeypatching ALEX_HOME after import doesn't move it.
    ...
    this only matters for tests that swap ALEX_HOME.)
    """
    import cli
