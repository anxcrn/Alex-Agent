"""Tests for the `alex memory reset` CLI command.
...
- Profile-scoped reset (uses ALEX_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake ALEX_HOME with memory files."""
    alex_home = tmp_path / ".alex"
    memories = alex_home / "memories"
    memories.mkdir(parents=True)
    monkeypatch.setenv("ALEX_HOME", str(alex_home))

    # Create sample memory files
    (memories / "MEMORY.md").write_text(
        "§\nAlex repo is at ~/.alex/alex-agent\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `alex memory reset` is run.
    """
...
class TestMemoryReset:
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        alex_home = tmp_path / ".alex"
        (alex_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("ALEX_HOME", str(alex_home))
    ...
    def test_reset_profile_scoped(self, tmp_path, monkeypatch):
        """Reset should work on the active profile's ALEX_HOME."""
    ...
    def test_reset_empty_memories_dir(self, tmp_path, monkeypatch):
        ...
        # The memories dir won't exist; get_alex_home() / "memories" won't have files
        result = _run_memory_reset(target="all", yes=True)
        assert result == "nothing"
