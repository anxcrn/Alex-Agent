...
def alex_home(tmp_path, monkeypatch):
    ...
    # /etc/alex on the dev/CI box can't influence the test.
    monkeypatch.setenv("ALEX_MANAGED_DIR", str(tmp_path / "no_such_managed_dir"))
...
def test_user_config_overrides_default(alex_home, monkeypatch):
    from alex_cli.config import load_config, cfg_get
...
def test_env_expansion_in_user_config(alex_home, monkeypatch):
    from alex_cli.config import load_config, cfg_get
...
def test_no_managed_dir_means_user_value_wins(alex_home):
...
def test_user_env_overrides_shell(tmp_path, monkeypatch):
    ...
    load_alex_dotenv(alex_home=str(home))
