...
The Python launcher (`alex --tui …`) has a ~100–300ms cold-start window
where stdin is still in cooked + echo mode. If a previous Alex session
...
from alex_cli.main import _suppress_mouse_residue_early
...
class TestEarlyMouseDisable:
    def test_writes_disable_sequence_when_tui_flag_in_argv(self, monkeypatch):
        monkeypatch.setattr(sys, "argv", ["alex", "--tui", "-c", "abc"])
        monkeypatch.delenv("ALEX_TUI", raising=False)
        monkeypatch.delenv("ALEX_TUI_NO_EARLY_DISABLE", raising=False)
    ...
    def test_writes_disable_sequence_when_alex_tui_env_set(self, monkeypatch):
        monkeypatch.setattr(sys, "argv", ["alex"])
        monkeypatch.setenv("ALEX_TUI", "1")
        monkeypatch.delenv("ALEX_TUI_NO_EARLY_DISABLE", raising=False)
    ...
    def test_respects_diagnostic_escape_hatch(self, monkeypatch):
        ...
        monkeypatch.delenv("ALEX_TUI", raising=False)
        monkeypatch.setenv("ALEX_TUI_NO_EARLY_DISABLE", "1")
    ...
    def test_skips_when_stdout_is_not_a_tty(self, monkeypatch):
        # `alex --tui … >log` or CI capture: pipe is fd 1, not a TTY. The
