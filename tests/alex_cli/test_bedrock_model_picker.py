...
Covers the three paths changed by fix/bedrock-provider-model-ids-live-discovery:
  ...
  2. list_authenticated_providers() Section 2 (ALEX_OVERLAYS) — bedrock
...
# ---------------------------------------------------------------------------
# 2. list_authenticated_providers() — bedrock via ALEX_OVERLAYS (Section 2)
# ---------------------------------------------------------------------------
...
class TestBedrockOverlayRegistration:
    """bedrock entry in ALEX_OVERLAYS is correctly configured."""

    def test_bedrock_overlay_exists(self):
        from alex_cli.providers import ALEX_OVERLAYS
        assert "bedrock" in ALEX_OVERLAYS

    def test_bedrock_overlay_transport(self):
        from alex_cli.providers import ALEX_OVERLAYS
        assert ALEX_OVERLAYS["bedrock"].transport == "bedrock_converse"

    def test_bedrock_overlay_auth_type(self):
        from alex_cli.providers import ALEX_OVERLAYS
        assert ALEX_OVERLAYS["bedrock"].auth_type == "aws_sdk"
