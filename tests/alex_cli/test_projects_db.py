"""Tests for the per-profile Projects store (alex_cli/projects_db)."""
...
def test_create_get_list(conn):
    pid = pdb.create_project(conn, name="Alex Agent", folders=["/tmp/alex"])
    proj = pdb.get_project(conn, pid)

    assert proj is not None
    assert proj.slug == "alex-agent"
    assert proj.name == "Alex Agent"
    # First folder becomes primary.
    assert proj.primary_path == "/tmp/alex"
    assert [f.path for f in proj.folders] == ["/tmp/alex"]
    assert proj.folders[0].is_primary is True

    # Lookup by slug too.
    assert pdb.get_project(conn, "alex-agent").id == pid
    assert len(pdb.list_projects(conn)) == 1


def test_slug_collision_disambiguates(conn):
    pdb.create_project(conn, name="Alex Agent")
    pdb.create_project(conn, name="Alex Agent")
    slugs = sorted(p.slug for p in pdb.list_projects(conn))

    assert slugs == ["alex-agent", "alex-agent-2"]
...
def test_per_profile_isolation(tmp_path):
    # Two distinct DB paths stand in for two profiles' ALEX_HOME.
...
def test_db_path_under_alex_home():
    # Resolves under ALEX_HOME (set by the autouse isolation fixture).
