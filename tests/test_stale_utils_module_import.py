...
Real incident (gateway session 1518671026962174144)::
    ...
    cannot import name 'env_float' from 'utils' (~/.alex/alex-agent/utils.py)
...
2. ``alex update`` ran ``git pull``, updating ``utils.py`` (now defining
...
``alex_cli/main.py`` (the ``alex update`` flow, ~line 9326) already
acknowledges this exact hazard -- "source files on disk are newer than cached
Python modules in this process" -- and reloads ``alex_constants`` after the
