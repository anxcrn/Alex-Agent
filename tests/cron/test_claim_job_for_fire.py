...
These exercise the real store against a temp ALEX_HOME (no mocks) per the
...
def temp_home(tmp_path, monkeypatch):
    """Isolated ALEX_HOME so jobs.json doesn't touch the real store."""
    monkeypatch.setenv("ALEX_HOME", str(tmp_path))
    # cron.jobs caches no home at import; get_alex_home() reads the env live.
    yield tmp_path
