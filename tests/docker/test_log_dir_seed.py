"""Runtime smoke test for Docker $ALEX_HOME/logs/gateways seeding.

Build the real image and verify logs/ and logs/gateways/ exist and are
owned by the alex user after container boot.
...
registered later runs its log service as the dropped alex user and
...
def test_logs_gateways_seeded_and_alex_owned(
    built_image: str, container_name: str,
) -> None:
    """logs/ and logs/gateways/ must exist and be owned by alex after boot."""
    ...
    assert "logs=alex" in r.stdout, (
        f"logs/ not owned by alex: {r.stdout}"
    )
    assert "gateways=alex" in r.stdout, (
        f"logs/gateways/ not owned by alex: {r.stdout}"
    )
