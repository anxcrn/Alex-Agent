...
Override the image with ``ALEX_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("ALEX_TEST_IMAGE", "alex-agent-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``ALEX_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("ALEX_TEST_IMAGE"):
        return IMAGE_TAG
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged alex user
# ---------------------------------------------------------------------------
#
# Background: every Alex runtime path inside the container drops to UID
# 10000 (the ``alex`` user) via ``s6-setuidgid alex``. ``docker exec``
...
# fails (PermissionError swallowed) for alex, so a test that ran as root
...
# helpers below run every probe under ``-u alex`` unless a specific
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: alex).
