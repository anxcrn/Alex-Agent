...
class TestEnvFileReadBlocking:
    ...
    def test_allowed_alex_env(self):
        """Alex' own .env inside ALEX_HOME is NOT blocked by this rule
        (it's handled by other mechanisms). Only project-local .env is blocked."""
        # Note: alex internal .env is in ~/.alex/.env which is NOT a project-local
        # path, but the basename check applies to ANY .env. This is intentional —
        # even ~/.alex/.env should not be readable via read_file.
        error = get_read_block_error(os.path.expanduser("~/.alex/.env"))
        assert error is not None
...
class TestCacheFileReadBlocking:
    """Internal Alex cache files must remain blocked."""

    def test_hub_index_cache_blocked(self, tmp_path):
        """Hub index-cache reads are blocked."""
        alex_home = tmp_path / ".alex"
        cache = alex_home / "skills" / ".hub" / "index-cache" / "data.json"
        cache.parent.mkdir(parents=True)
        cache.write_text("{}")

        with patch("agent.file_safety._alex_home_path", return_value=alex_home):
            error = get_read_block_error(str(cache))
            assert error is not None
            assert "internal Alex cache" in error

    def test_hub_directory_blocked(self, tmp_path):
        """Hub directory reads are blocked."""
        alex_home = tmp_path / ".alex"
        hub = alex_home / "skills" / ".hub" / "metadata.json"
...
class TestCombinedGuards:
    ...
    def test_env_guard_works_regardless_of_alex_home(self, tmp_path):
        """The env basename guard does not depend on ALEX_HOME resolution."""
        alex_home = tmp_path / ".alex"
        alex_home.mkdir()
    ...
    def test_cache_guard_still_works_with_env_guard(self, tmp_path):
        ...
        alex_home = tmp_path / ".alex"
        cache = alex_home / "skills" / ".hub" / "index-cache" / "x"
        cache.parent.mkdir(parents=True)
        cache.write_text("")
