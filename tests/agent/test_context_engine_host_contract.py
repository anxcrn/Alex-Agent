...
engine plugins (e.g. alex-lcm) rely on:
...
def test_discover_context_engines_includes_plugin_registered_engines(monkeypatch):
    """Plugin-registered context engines appear in the ``alex plugins`` picker."""
    from alex_cli import plugins_cmd
    ...
    monkeypatch.setattr(
        "alex_cli.plugins.discover_plugins",
        lambda *_a, **_kw: None,
    )
    monkeypatch.setattr(
        "alex_cli.plugins.get_plugin_context_engine",
