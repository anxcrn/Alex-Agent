...
class TestSpawnEnvIsolation:
    ...
    def test_kanban_worker_adds_only_kanban_writable_root(self, monkeypatch):
        ...
        danger-full-access. Alex passes a narrow app-server config override
        ...
        monkeypatch.setattr(subprocess, "Popen", FakePopen)
        monkeypatch.setenv("HOME", "/users/alice")
        monkeypatch.setenv("ALEX_HOME", "/users/alice/.alex/profiles/backend-worker")
        monkeypatch.setenv("ALEX_KANBAN_TASK", "t_smoke")
        monkeypatch.setenv(
            "ALEX_KANBAN_DB",
            "/users/alice/.alex/kanban/boards/smoke/kanban.db",
        )
        ...
        assert (
            'sandbox_workspace_write.writable_roots=["/users/alice/.alex/kanban/boards/smoke"]'
            in cmd
        )
