...
def http_server(tmp_path, monkeypatch):
    """Spin up a localhost HTTP server and isolate ALEX_HOME under tmp_path."""
    monkeypatch.setenv("ALEX_HOME", str(tmp_path / ".alex"))
    (tmp_path / ".alex").mkdir()

    # Force the constants/image cache helpers to re-read ALEX_HOME.
    import sys
    for mod in list(sys.modules):
        if mod.startswith("alex_constants") or mod.startswith("agent.image_gen_provider"):
...
class TestSaveUrlImage:
    def test_writes_real_bytes_to_alex_home_cache(self, http_server):
        base, _ = http_server
        ...
        # The cache directory must be under ALEX_HOME — gateway cleanup
