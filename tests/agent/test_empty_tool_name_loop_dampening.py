...
@pytest.fixture()
def agent_env():
    """Spin up the mock provider + an isolated ALEX_HOME, yield (agent, helpers)."""
    ...
    test_home = tempfile.mkdtemp(prefix="alex_e2e_47967_")
    os.makedirs(os.path.join(test_home, ".alex"))
    prev_home = os.environ.get("ALEX_HOME")
    os.environ["ALEX_HOME"] = os.path.join(test_home, ".alex")
    ...
    for mod in list(sys.modules):
        if mod == "run_agent" or mod.startswith("agent.") or mod.startswith("tools.") or mod.startswith("alex_"):
            del sys.modules[mod]
    ...
    try:
        ...
        if prev_home is None:
            ...
            os.environ["ALEX_HOME"] = prev_home
