...
def isolated_home(monkeypatch):
    """Temp ALEX_HOME with config + clean credential env vars."""
    test_home = tempfile.mkdtemp(prefix="alex_test_31179_")
    alex_home = os.path.join(test_home, ".alex")
    os.makedirs(alex_home)
...
def _fresh_modules():
    """Drop cached alex modules so each test reloads against current env."""
