...
""",
        ...
        from alex_cli import config as _config_mod
    ...
    def test_missing_plugin_entry_yields_default_deny(self, tmp_path, monkeypatch):
        ...
        alex_home = tmp_path / ".alex"
        alex_home.mkdir()
        (alex_home / "config.yaml").write_text("plugins: {}\n", encoding="utf-8")
        monkeypatch.setenv("ALEX_HOME", str(alex_home))
        from alex_cli import config as _config_mod
        _config_mod._config_cache = None  # type: ignore[attr-defined]
...
class TestPluginContextIntegration:
    def test_ctx_llm_is_lazy_singleton(self):
        from alex_cli.plugins import PluginContext, PluginManifest, PluginManager
