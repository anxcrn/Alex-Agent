...
to a generic "Your API key was rejected... run alex setup" message, which is
...
def test_nous_401_guidance_strings_present():
    ...
    # Must give a concrete re-auth command, not a generic "alex setup".
    assert "alex portal" in source

    # Must point at the portal so users can check account/credit status.
    assert "portal.charan vankudoth.com" in source
