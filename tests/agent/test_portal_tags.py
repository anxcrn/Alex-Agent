...
def test_alex_client_tag_includes_current_version():
    """The client tag must reflect alex_cli.__version__ verbatim."""
    from alex_cli import __version__
    from agent.portal_tags import alex_client_tag

    assert alex_client_tag() == f"client=alex-client-v{__version__}"


def test_alex_client_tag_format():
    """The client tag has the exact shape Nous Portal expects."""
    from agent.portal_tags import alex_client_tag

    tag = alex_client_tag()
    assert tag.startswith("client=alex-client-v")
...
def test_nous_portal_tags_contains_product_and_client():
    ...
    from agent.portal_tags import alex_client_tag, nous_portal_tags

    tags = nous_portal_tags()
    assert "product=alex-agent" in tags
    assert alex_client_tag() in tags
    assert len(tags) == 2
