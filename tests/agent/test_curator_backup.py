...
def backup_env(monkeypatch, tmp_path):
    """Isolate ALEX_HOME + reload modules so every test starts clean."""
    home = tmp_path / ".alex"
    home.mkdir()
    (home / "skills").mkdir()
    ...
    # Reload so get_alex_home picks up the env var fresh.
...
def test_real_run_takes_pre_snapshot(backup_env, monkeypatch):
    ...
    # Reload curator module against the freshly-env'd alex_constants
...
def _write_cron_jobs(home: Path, jobs: list) -> Path:
    """Write a synthetic cron/jobs.json under ALEX_HOME. Returns the path.
...
def _reload_cron_jobs(home: Path):
    """Reload cron.jobs so its module-level ALEX_DIR picks up the tmp HOME."""
    import alex_constants
    importlib.reload(alex_constants)
...
def test_snapshot_without_cron_jobs_file_still_succeeds(backup_env):
    ...
    cb = backup_env["cb"]
    _write_skill(backup_env["skills"], "alpha")
    # Deliberately do not create ~/.alex/cron/jobs.json
