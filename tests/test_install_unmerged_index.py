...
The ``alex update`` Python path already clears the conflict with ``git reset``
...
def test_install_ps1_stops_venv_resident_processes_before_removing_venv() -> None:
    ...
    ``venv\\Scripts\\pythonw.exe -m alex_cli.main gateway run`` — image name
    ``pythonw``, not ``alex.exe`` — so the ``taskkill /IM alex.exe`` guard
    ...
    # The alex.exe tree-kill is preserved (kills spawned child processes too).
    assert 'taskkill /F /T /IM alex.exe' in text
