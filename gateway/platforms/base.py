...
# Audio file extensions Alex recognizes for native audio delivery.
...
def _thread_metadata_for_source(source, reply_to_message_id: str | None = None) -> dict | None:
    ...
    value. Telegram private-chat topics created through Alex' DM-topic helper
...
def _reply_anchor_for_event(event) -> str | None:
    ...
    replying to the triggering message. Alex-created Telegram private-chat
...
from alex_constants import get_default_alex_root, get_alex_dir, get_alex_home


GATEWAY_SECRET_CAPTURE_UNSUPPORTED_MESSAGE = (
    "Secure secret entry is not supported over messaging. "
    "Load this skill in the local CLI to be prompted, or add the key to ~/.alex/.env manually."
)
...
# Default location: {ALEX_HOME}/cache/images/ (legacy: image_cache/)
IMAGE_CACHE_DIR = get_alex_dir("cache/images", "image_cache")
        ...
        for attempt in range(retries + 1):
            try:
                async with client.stream(
                    ...
                    url,
                    headers={
                        "User-Agent": "Mozilla/5.0 (compatible; AlexAgent/1.0)",
                        "Accept": "image/*,*/*;q=0.8",
                    },
                ) as response:
...
AUDIO_CACHE_DIR = get_alex_dir("cache/audio", "audio_cache")
...
VIDEO_CACHE_DIR = get_alex_dir("cache/videos", "video_cache")
...
DOCUMENT_CACHE_DIR = get_alex_dir("cache/documents", "document_cache")
SCREENSHOT_CACHE_DIR = get_alex_dir("cache/screenshots", "browser_screenshots")
_ALEX_HOME = get_alex_home()
_ALEX_ROOT = get_default_alex_root()
MEDIA_DELIVERY_ALLOW_DIRS_ENV = "ALEX_MEDIA_ALLOW_DIRS"
MEDIA_DELIVERY_TRUST_RECENT_ENV = "ALEX_MEDIA_TRUST_RECENT_FILES"
MEDIA_DELIVERY_TRUST_RECENT_SECONDS_ENV = "ALEX_MEDIA_TRUST_RECENT_SECONDS"
...
MEDIA_DELIVERY_STRICT_ENV = "ALEX_MEDIA_DELIVERY_STRICT"
MEDIA_DELIVERY_SAFE_ROOTS = (
    ...
    _ALEX_HOME / "image_cache",
    _ALEX_HOME / "audio_cache",
    _ALEX_HOME / "video_cache",
    _ALEX_HOME / "document_cache",
    _ALEX_HOME / "browser_screenshots",
    ...
    _ALEX_HOME / "cache" / "images",
    _ALEX_HOME / "cache" / "audio",
    _ALEX_HOME / "cache" / "videos",
    _ALEX_HOME / "cache" / "documents",
    _ALEX_HOME / "cache" / "screenshots",
)
...
def _media_delivery_denied_paths() -> List[Path]:
    ...
    # The active Alex profile and shared Alex root both contain control
    ...
    # ALEX_HOME root. The set mirrors the canonical read guard in
    ...
    # tree, so skills/, logs/, and ad-hoc agent-written files under ~/.alex
    ...
    for alex_root in (_ALEX_HOME, _ALEX_ROOT):
        for rel in _ROOT_CREDENTIAL_FILES:
            denied.append(alex_root / rel)
...
def _path_under_denied_prefix(resolved: Path) -> bool:
    ...
    sub-directories inside home (``~/.ssh``, ``~/.aws``, ...) and Alex
    secrets (``~/.alex/.env``, ``auth.json``) are *separate, more-specific*
...
def validate_media_delivery_path(path: str) -> Optional[str]:
    ...
    ``ALEX_MEDIA_DELIVERY_STRICT=1``): the file MUST live under a
    Alex-managed cache, under an operator-allowlisted root
    (``ALEX_MEDIA_ALLOW_DIRS``), or be freshly produced inside the
    ...
    # credential/secret stores under the Alex root (~/.alex/.env,
    ...
    # ``MEDIA:~/.alex/google_token.json``) remain rejected.
...
_PLAINTEXT_GATEWAY_RESTART_PATTERNS: tuple[re.Pattern[str], ...] = (
    ...
    re.compile(r"^(?:please\s+)?restart\s+(?:the\s+)?alex\s+gateway[.!?\s]*$", re.IGNORECASE),
    re.compile(r"^(?:please\s+)?restart\s+alex[.!?\s]*$", re.IGNORECASE),
)
...
class BasePlatformAdapter(ABC):
    ...
    # into the ``ALEX_SESSION_ASYNC_DELIVERY`` contextvar at session-bind
    ...
    # Alex commands.  Default "/" (most platforms deliver "/approve" etc.
    ...
    def __init__(self, config: PlatformConfig, platform: Platform):
        ...
        self._busy_text_mode: str = (
            os.environ.get("ALEX_GATEWAY_BUSY_TEXT_MODE", "interrupt").strip().lower()
            or "interrupt"
        )
        self._busy_text_debounce_seconds: float = _float_env(
            "ALEX_GATEWAY_BUSY_TEXT_DEBOUNCE_SECONDS", 0.35
        )
        self._busy_text_hard_cap_seconds: float = _float_env(
            "ALEX_GATEWAY_BUSY_TEXT_HARD_CAP_SECONDS", 1.0
        )
    ...
    def prefers_fresh_final_streaming(
        ...
        implementation supports. Telegram is the motivating case: Alex sends
    ...
    def _mask_json_string_media(content: str) -> str:
        ...
        Serialized tool results frequently embed a previous reply's text, e.g.::
            ...
            {"result": "MEDIA:/Users/x/.alex/media/generated/stale.png"}
    ...
    async def handle_message(self, event: MessageEvent) -> None:
        ...
        if session_key in self._active_sessions:
            ...
            from alex_cli.commands import should_bypass_active_session
    ...
    def _get_human_delay() -> float:
        ...
        Reads from env vars:
          ALEX_HUMAN_DELAY_MODE: "off" (default) | "natural" | "custom"
          ALEX_HUMAN_DELAY_MIN_MS: minimum delay in ms (default 800, custom mode)
          ALEX_HUMAN_DELAY_MAX_MS: maximum delay in ms (default 2500, custom mode)
        """
        mode = os.getenv("ALEX_HUMAN_DELAY_MODE", "off").lower()
        if mode == "off":
            return 0.0
        if mode == "natural":
        ...
        try:
            min_ms = int(os.getenv("ALEX_HUMAN_DELAY_MIN_MS", "800"))
        except (TypeError, ValueError):
            min_ms = 800
        try:
            max_ms = int(os.getenv("ALEX_HUMAN_DELAY_MAX_MS", "2500"))
        except (TypeError, ValueError):
            max_ms = 2500
        return random.uniform(min_ms / 1000.0, max_ms / 1000.0)
    ...
    async def _process_message_background(self, event: MessageEvent, session_key: str) -> None:
        ...
        try:
            ...
            # not at the top of this task.  _alex_run_generation is set on
