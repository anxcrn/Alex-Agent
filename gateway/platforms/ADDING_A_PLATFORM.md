...
There are two ways to add a platform to the Alex gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.alex/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Alex code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `alex gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `alex_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Alex core.
...
And add it to the `alex-gateway` composite:

```python
"alex-gateway": {
    "includes": [..., "alex-your-platform"]
}
```
...
## 12. Status Display (`alex_cli/status.py`)
...
## 13. Gateway Setup Wizard (`alex_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ alex_cli/ toolsets.py \
  --include="*.py" -l | sort -u
