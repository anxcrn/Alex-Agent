...
class WhatsAppBehaviorMixin:
    ...
    DEFAULT_REPLY_PREFIX: str = "⚕ *Alex Agent*\n────────────\n"
...
def resolve_whatsapp_bridge_dir() -> Path:
    """Resolve the WhatsApp bridge directory, mirroring to ALEX_HOME if needed.

    When the install tree is read-only (e.g., Docker /opt/alex), this function
    mirrors the bridge source to a writable ALEX_HOME location and returns that
    ...
    # Try ALEX_HOME location first
    alex_home = get_alex_home()
    alex_home_bridge = alex_home / "scripts" / "whatsapp-bridge"
    ...
    # Install dir is read-only, mirror to ALEX_HOME if needed
    if alex_home_bridge.exists():
        return alex_home_bridge

    # Mirror the bridge source to ALEX_HOME
    try:
        alex_home_bridge.parent.mkdir(parents=True, exist_ok=True)
        shutil.copytree(
            install_bridge,
            alex_home_bridge,
            dirs_exist_ok=False,
