"""Windows UTF-8 bootstrap for Alex entry points.
...
should be imported at the very top of every Alex entry point
(``alex``, ``alex-agent``, ``alex-acp``, ``python -m gateway.run``,
...
def harden_import_path(src_root: str | None = None) -> None:
    """Stop a package in the current directory from shadowing Alex modules.

    Alex ships top-level modules with common names (``utils``, ``proxy``,
    ...
    We drop the relative forms outright, then force the real Alex source root
    ...
    """
    root = src_root or os.environ.get("ALEX_PYTHON_SRC_ROOT") or os.path.dirname(
...
def activate_durable_lazy_target() -> None:
    ...
    (``ALEX_LAZY_INSTALL_TARGET``, e.g. ``/opt/data/lazy-packages``).
    ...
    """
    if not os.environ.get("ALEX_LAZY_INSTALL_TARGET", "").strip():
        return
    try:
...
# Apply on import — entry points just need ``import alex_bootstrap``
# (or ``from alex_bootstrap import apply_windows_utf8_bootstrap``) at
