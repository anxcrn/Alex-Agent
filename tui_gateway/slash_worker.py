"""Persistent slash-command worker — one AlexCLI per TUI session.
...
def _env_float(name: str, default: float) -> float:
    ...
    import time on a typo (e.g. ``ALEX_SLASH_WATCHDOG_POLL_S=2s``) and kill
...
_WATCHDOG_POLL_S = max(0.05, _env_float("ALEX_SLASH_WATCHDOG_POLL_S", 2.0))
_ORPHAN_GRACE_S = max(0.0, _env_float("ALEX_SLASH_WATCHDOG_GRACE_S", 5.0))
...
def _run(cli: AlexCLI, command: str) -> str:
...
def main():
    ...
    os.environ["ALEX_SESSION_KEY"] = args.session_key
    os.environ["ALEX_INTERACTIVE"] = "1"

    # Start before the (hundreds-of-ms) AlexCLI build — that window is itself
    ...
    with contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(io.StringIO()):
        cli = AlexCLI(model=args.model or None, compact=True, resume=args.session_key, verbose=False)
