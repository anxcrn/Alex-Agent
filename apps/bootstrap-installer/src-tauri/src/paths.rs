...
//!
//! Mirrors `alex_constants.get_alex_home()` from the Python CLI:
//!   Windows: %LOCALAPPDATA%\alex
//!   macOS:   ~/.alex
//!   Linux:   ~/.alex  (override via $ALEX_HOME)
//!
//! NOTE (macOS): Python's get_alex_home(), scripts/install.sh, and the
//! Electron desktop's resolveAlexHome() ALL use ~/.alex on macOS — there
...
/// Returns the canonical Alex home directory, respecting $ALEX_HOME if set.
pub fn alex_home() -> PathBuf {
    if let Ok(override_path) = std::env::var("ALEX_HOME") {
    ...
    {
        // %LOCALAPPDATA%\alex — matches scripts/install.ps1's $AlexHome.
        if let Some(local_app_data) = dirs::data_local_dir() {
            return local_app_data.join("alex");
        }
    }

    // macOS + Linux + fallback: ~/.alex (matches Python get_alex_home(),
    // install.sh, and the Electron desktop's resolveAlexHome()).
    if let Some(home) = dirs::home_dir() {
        return home.join(".alex");
    }
...
pub fn bootstrap_cache_dir() -> PathBuf {
    alex_home().join("bootstrap-cache")
}
...
/// ALEX_HOME so it survives repo checkout deletion (unlike anything under
/// alex-agent/).
///
/// On Windows this is `%LOCALAPPDATA%\alex\alex-setup.exe`; on other
...
/// mid-update re-locks the venv shim and triggers `force_kill_other_alex`,
/// which then kills that legitimate backend in a respawn loop (#50238).
///
/// Lives directly under ALEX_HOME (same rationale as `installer_dest`) so the
/// Electron desktop — which resolves ALEX_HOME identically and pins it into
/// the updater's env — agrees on the exact path.
pub fn update_in_progress_marker() -> PathBuf {
    alex_home().join(".alex-update-in-progress")
}
...
pub fn copy_self_to_alex_home() -> std::io::Result<()> {
    ...
    tracing::info!(?src, ?dest, "copied installer to ALEX_HOME");
    Ok(())
}
...
/// We don't always know ACTIVE_ALEX_ROOT until install.ps1 reports it, so
/// this is a probe helper, not a definitive path.
pub fn likely_bootstrap_marker(install_root: &Path) -> PathBuf {
    install_root.join(".alex-bootstrap-complete")
}

/// Initializes tracing to bootstrap-installer.log under ALEX_HOME/logs/.
...
pub fn init_logging() -> Option<WorkerGuard> {
    ...
    if let Err(err) = std::fs::create_dir_all(&dir) {
        ...
        eprintln!("[alex-setup] could not create log dir {dir:?}: {err}");
        return None;
    }
    ...
    let env_filter = tracing_subscriber::EnvFilter::try_from_env("ALEX_BOOTSTRAP_LOG")
...
#[tauri::command]
pub fn get_alex_home() -> String {
    alex_home().to_string_lossy().into_owned()
}
