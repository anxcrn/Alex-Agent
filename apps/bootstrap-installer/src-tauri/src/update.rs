...
//!
//! Driven when the installer is launched as `Alex-Setup.exe --update` (see
//! `AppMode` in lib.rs). The desktop app hands off to us — it exits, then we:
//!
//!   1. wait for the old Alex desktop process to fully exit (so both the
//!      venv shim and packaged app.asar are free; otherwise `alex update`
//!      or repair bootstrap can race locked files),
//!   2. run `alex update --yes --gateway` (Python/repo update; this does NOT
//!      rebuild apps/desktop by design — see cmd_update in alex_cli/main.py),
//!   3. run `alex desktop --build-only` (the rebuild step update skips),
...
//!
//! Cross-platform note: `alex update` already handles macOS/Linux (git/pip).
//! The only OS-specific bits here are the venv shim path (resolve_alex) and
...
/// `alex update` exit code meaning "another alex process is holding the
/// venv shim open / dirty precondition" — see _cmd_update_impl in
/// alex_cli/main.py (sys.exit(2)). We surface a targeted message for this.
const UPDATE_EXIT_CONCURRENT: i32 = 2;

/// How long to wait for the old desktop process to release files under the
/// install tree before giving up and letting `alex update`'s own guard decide.
...
pub async fn start_update(app: AppHandle) -> Result<(), String> {
        ...
        .is_err()
    {
        ...
        let mut stages = vec![
            stage_info("update", "Updating Alex"),
...
async fn run_update(app: AppHandle) -> Result<()> {
    let alex_home = crate::paths::alex_home();
    let install_root = alex_home.join("alex-agent");
    ...
    // it, that backend re-locks the venv shim, our `force_kill_other_alex`
    ...
    let alex = resolve_alex(&install_root).ok_or_else(|| {
        let msg = format!(
            "Could not find the alex CLI under {}. Is Alex installed? \
    ...
    // async on Windows. If it still holds the venv shim, `alex update`
    ...
    // ---- stage 1: alex update -----------------------------------------
    // Pass --branch so `alex update` targets the branch this installer was
    ...
    // without --branch, `alex update` switches the checkout to `main` (a
    ...
    // --force skips `alex update`'s Windows running-exe guard (which would
    ...
    // time `alex update` runs there is no legitimate alex.exe to protect,
    // and the guard would only produce a false "Alex is still running" stop.
    ...
    // Retry-once for the update-boundary crash. `alex update` lazily imports
    ...
    // second `alex update` runs clean because the now-current module is loaded
    ...
    // for the concurrent-instance guard (exit 2) — that's a "close Alex" state
    // a retry can't fix.
    ...
    match update.exit_code {
        ...
        Some(code) if code == UPDATE_EXIT_CONCURRENT => {
            let msg = "Alex is still running. Close all Alex windows and try \
                       the update again."
                .to_string();
            emit_stage(
                &app,
        ...
        other => {
            let msg = format!(
                "alex update failed (exit {:?}). See {} for details.",
                other,
                crate::paths::alex_home()
                    .join("logs")
                    .join("update.log")
                    .display()
            );
    ...
    // ---- stage 2: alex desktop --build-only ----------------------------
    // `alex update` deliberately does NOT build apps/desktop (it installs
    ...
    // retry-once `alex update` already does above, and `alex update`'s own
    ...
    if rebuild.exit_code != Some(0) {
        let msg = format!(
            "Rebuilding the desktop app failed (exit {:?}). The update was \
             applied but the app could not be rebuilt; run `alex desktop` \
    ...
    if let Some(target_app) = launch_target {
        if let Err(err) = launch_macos_app_and_exit(&app, &target_app).await {
            emit_log(
                ...
                None,
                LogStream::Stderr,
                &format!("[update] could not auto-launch desktop: {err}. Launch Alex manually."),
            );
        }
    } else if let Err(err) =
        crate::bootstrap::launch_alex_desktop(app.clone(), install_root.to_string_lossy().into_owned()).await
    {
...
pub(crate) async fn wait_for_install_locks_free(install_root: &Path, app: &AppHandle, stage: &str) {
    ...
    emit_log(app, Some(stage), LogStream::Stdout, "[handoff] waiting for Alex to exit…");
    ...
    loop {
        ...
        if Instant::now() >= deadline {
            // Last resort: a backend alex.exe (or the desktop Alex.exe
            ...
            // force-kill every Alex.exe except ourselves, then give the OS a
            ...
            emit_log(
                ...
                LogStream::Stdout,
                &format!(
                    "[handoff] Alex still holding install files ({}); force-killing stragglers…",
                    format_locked_paths(&locked)
                ),
            );
            force_kill_other_alex();
...
fn install_lock_probe_paths(install_root: &Path) -> Vec<PathBuf> {
    let mut paths = vec![venv_alex(install_root)];
...
fn desktop_app_payload_paths(install_root: &Path) -> Vec<PathBuf> {
    ...
    if cfg!(target_os = "windows") {
        ...
        vec![
            release.join("mac").join("Alex.app").join("Contents").join("Resources").join("app.asar"),
            release.join("mac-arm64").join("Alex.app").join("Contents").join("Resources").join("app.asar"),
        ]
    } else {
...
/// Force-kill any `alex.exe` other than this process. Windows-only; a no-op
...
/// knew its children — so we kill the whole `alex.exe` image tree via
/// taskkill, excluding our own PID.
///
/// Safe w.r.t. our own update child: this runs inside the install-lock wait,
/// which completes BEFORE we spawn `venv\Scripts\alex.exe update`. And a
/// desktop the user relaunches mid-update will NOT have spawned a backend —
/// `startAlex()` in the desktop gates local-backend startup on our
/// update-in-progress marker and parks until we finish (#50238). So the only
/// alex.exe images here are stragglers from the old desktop — exactly what
/// we want gone. (`/FI PID ne <self>` also spares this Tauri process, though it
/// isn't named alex.exe.)
fn force_kill_other_alex() {
    if !cfg!(target_os = "windows") {
        return;
    }
...
/// Spawn `alex <args>` from `cwd`, stream stdout/stderr as Log events on the
...
/// Path to the venv alex shim under an install root, regardless of existence.
fn venv_alex(install_root: &Path) -> PathBuf {
    if cfg!(target_os = "windows") {
        install_root.join("venv").join("Scripts").join("alex.exe")
    } else {
        install_root.join("venv").join("bin").join("alex")
    }
}

/// Resolve the alex CLI to drive. Prefer the venv shim in the install we
/// just updated; fall back to `alex` on PATH.
fn resolve_alex(install_root: &Path) -> Option<PathBuf> {
    let shim = venv_alex(install_root);
    if shim.exists() {
        return Some(shim);
    }
    // PATH fallback. which-style probe via env, kept dependency-free.
    let exe = if cfg!(target_os = "windows") { "alex.exe" } else { "alex" };
...
fn update_child_env(install_root: &Path) -> Vec<(String, OsString)> {
    ...
    let mut envs = vec![(
        "ALEX_HOME".to_string(),
        alex_home.as_os_str().to_os_string(),
    )];
    if let Some(path) = path_with_prepended_entries(&[
        alex_home.join("node").join("bin"),
...
async fn install_macos_app_update(
    ...
    let rebuilt_app = crate::bootstrap::resolve_alex_desktop_app(install_root).ok_or_else(|| {
        anyhow!(
            "desktop rebuild succeeded but no Alex.app was found under {}",
    ...
    let tmp = PathBuf::from(format!("{}.alex-update-new", target_app.display()));
    let old = PathBuf::from(format!("{}.alex-update-old", target_app.display()));
    ...
    let ditto = Command::new("/usr/bin/ditto")
        .arg(&rebuilt_app)
        .arg(&tmp)
        .current_dir(crate::paths::alex_home())
        .status()
        .await
...
mod tests {
    ...
    #[test]
    fn venv_alex_is_under_install_root() {
        let root = Path::new("/x/alex-agent");
        let shim = venv_alex(root);
    ...
    fn lock_probe_paths_include_desktop_app_payload() {
        ...
        assert!(
            probes.iter().any(|p| p == &venv_alex(root)),
    ...
    fn locked_paths_ignores_missing_payloads() {
        let root = Path::new("/nonexistent/alex-agent");
        let probes = install_lock_probe_paths(root);
    ...
    fn update_marker_guard_writes_then_removes_on_drop() {
        ...
        let marker = dir.join(".alex-update-in-progress");
    ...
    fn parses_only_app_targets() {
        assert_eq!(
            target_app_from_args(["--update", "--target-app", "/Applications/Alex.app"]),
            Some(PathBuf::from("/Applications/Alex.app"))
        );
    ...
    async fn swap_installs_new_bundle_and_cleans_up() {
        ...
        let target = base.join("Alex.app");
        let tmp = base.join("Alex.app.alex-update-new");
        let old = base.join("Alex.app.alex-update-old");
    ...
    async fn swap_failure_never_leaves_target_missing() {
        ...
        let target = base.join("Alex.app");
        let tmp = base.join("Alex.app.alex-update-new"); // intentionally absent
        let old = base.join("Alex.app.alex-update-old");
        write_marker(&target, "OLD");
    ...
    async fn swap_rolls_back_when_install_step_fails() {
        ...
        let target = base.join("Alex.app");
        let tmp = base.join("Alex.app.alex-update-new"); // absent
        let old = base.join("Alex.app.alex-update-old");
        write_marker(&target, "OLD");
