...
const SKIN_KEY = 'alex-desktop-theme-v2'
const MODE_KEY = 'alex-desktop-mode-v1'
...
const PROFILE_SKINS_KEY = 'alex-desktop-profile-themes-v1'
const PROFILE_MODES_KEY = 'alex-desktop-profile-modes-v1'
// Last active profile, recorded so the boot-time paint can pick that profile's
// theme before the gateway reports which profile actually launched.
const LAST_PROFILE_KEY = 'alex-desktop-active-profile-v1'
...
function applyTheme(theme: DesktopTheme, mode: 'light' | 'dark') {
  ...
  root.dataset.alexTheme = skinName
  root.dataset.alexMode = rendered
  ...
  window.alexDesktop?.setTitleBarTheme?.({
    background: chromeBg,
  ...
  try {
    window.localStorage.setItem('alex-boot-background', chromeBg)
    window.localStorage.setItem('alex-boot-color-scheme', rendered)
  } catch {
  ...
  if (typo.fontUrl && !INJECTED_FONT_URLS.has(typo.fontUrl)) {
    ...
    link.dataset.alexThemeFont = 'true'
...
const syncNativeTheme = (pref: ThemeMode, rendered: 'light' | 'dark') =>
  window.alexDesktop?.setNativeTheme?.(pref === 'system' ? 'system' : rendered)
...
/** Sync the desktop skin with the active Alex backend theme on connect. */
