...
import type { AlexConnection } from '@/global'
import type { ChatMessage } from '@/lib/chat-messages'
import { persistBoolean, persistString, storedBoolean, storedString } from '@/lib/storage'
import type { SessionInfo, UsageStats } from '@/types/alex'

type Updater<T> = T | ((current: T) => T)

const WORKSPACE_CWD_KEY = 'alex.desktop.workspace-cwd'
...
const COMPOSER_MODEL_KEY = 'alex.desktop.composer.model'
const COMPOSER_PROVIDER_KEY = 'alex.desktop.composer.provider'
const COMPOSER_EFFORT_KEY = 'alex.desktop.composer.reasoning-effort'
const COMPOSER_FAST_KEY = 'alex.desktop.composer.fast'
...
const LAST_SESSION_KEY = 'alex.desktop.lastSessionId'
...
function workspaceCwdKey(connection: AlexConnection | null = $connection.get()): string {
...
export async function syncConfiguredDefaultProjectDir(): Promise<string> {
  const settings = window.alexDesktop?.settings?.getDefaultProjectDir
...
export async function ensureDefaultWorkspaceCwd(): Promise<void> {
  const sanitize = window.alexDesktop?.sanitizeWorkspaceCwd
...
export const $connection = atom<AlexConnection | null>(null)
...
export const setConnection = (next: Updater<AlexConnection | null>) => updateAtom($connection, next)
