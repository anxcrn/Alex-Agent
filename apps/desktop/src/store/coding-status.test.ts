...
import type { AlexRepoStatus } from '@/global'

import { $repoStatus, refreshRepoStatus } from './coding-status'
import { $currentCwd } from './session'

const sampleStatus: AlexRepoStatus = {
...
function stubProbe(impl: (cwd: string) => Promise<AlexRepoStatus | null>) {
  ;(window as unknown as { alexDesktop?: unknown }).alexDesktop = { git: { repoStatus: impl } }
}
