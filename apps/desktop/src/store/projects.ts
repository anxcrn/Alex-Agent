...
import type { AlexGitBranch } from '@/global'
...
import type { ProjectInfo, ProjectsPayload } from '@/types/alex'
...
const PROJECT_SCOPE_KEY = 'alex.desktop.projectScope'
...
async function gatewayRequest<T>(method: string, params: Record<string, unknown> = {}): Promise<T> {
  ...
  if (!gateway) {
    throw new Error('Alex gateway is not connected')
  }
...
export async function scanAndRecordRepos(force = false): Promise<void> {
  const scan = window.alexDesktop?.git?.scanRepos
...
async function writeProjectIdea(folder: null | string | undefined, idea: string): Promise<void> {
  ...
  const write = window.alexDesktop?.writeTextFile
...
// repo, returning where Alex should start working. Git is the source of
...
export async function startWorkInRepo(
  ...
  const git = window.alexDesktop?.git
...
export async function listRepoBranches(repoPath: string): Promise<AlexGitBranch[]> {
  const git = window.alexDesktop?.git
...
export async function revealPath(path: null | string): Promise<void> {
  if (path) {
    await window.alexDesktop?.revealPath?.(path)
  }
}
...
export async function copyPath(path: null | string): Promise<void> {
  if (path) {
    await window.alexDesktop?.writeClipboard?.(path)
  }
}
...
export async function pickProjectFolder(): Promise<null | string> {
  const pick = window.alexDesktop?.settings?.pickDefaultProjectDir
