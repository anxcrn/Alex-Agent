...
import type { AlexGitWorktree, AlexRepoStatus } from '@/global'
...
export const $repoStatus = atom<AlexRepoStatus | null>(null)
...
export const $repoWorktrees = atom<AlexGitWorktree[]>([])
...
async function loadWorktrees(target: string): Promise<void> {
  const list = window.alexDesktop?.git?.worktreeList
...
export async function refreshRepoStatus(cwd?: null | string): Promise<void> {
  ...
  const probe = window.alexDesktop?.git?.repoStatus
