...
const STORAGE_KEY = 'alex:native-notifications'
...
// "Backgrounded" = the user isn't on Alex. `document.hidden` only flips when
...
export function dispatchNativeNotification(input: NativeNotificationInput): void {
  ...
  void window.alexDesktop?.notify({
...
export async function sendTestNativeNotification(title: string, body: string): Promise<boolean> {
  const bridge = window.alexDesktop
