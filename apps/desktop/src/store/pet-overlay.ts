 ...
 * app's bounds and stays visible while Alex is minimized. That window carries
...
const OVERLAY_ACTIVE_KEY = 'alex.desktop.pet-overlay-active.v1'
const OVERLAY_BOUNDS_KEY = 'alex.desktop.pet-overlay-bounds.v1'
...
function pushNow(): void {
  window.alexDesktop?.petOverlay?.pushState(currentPayload())
}
...
function openOverlay(request: PetOverlayOpenRequest): void {
  const api = window.alexDesktop?.petOverlay
...
export function restorePetOverlay(): void {
  if (!window.alexDesktop?.petOverlay || !$petOverlayActive.get() || stateUnsubs.length) {
    return
  }
...
export function popInPet(): void {
  ...
  stateUnsubs = []
  $petOverlayActive.set(false)
  void window.alexDesktop?.petOverlay?.close()
}
