...
const remoteConn = (over: Partial<AlexConnection> = {}): AlexConnection =>
  ({ baseUrl: 'https://alex-roy.tail.ts.net', mode: 'remote', profile: 'vps-remote', ...over }) as AlexConnection

const localConn = (over: Partial<AlexConnection> = {}): AlexConnection =>
  ({ baseUrl: '', mode: 'local', profile: 'default', ...over }) as AlexConnection

const getConnection = vi.fn<(profile?: string | null) => Promise<AlexConnection>>()
...
beforeEach(() => {
  ...
  vi.stubGlobal('window', { alexDesktop: { getConnection } })
})
