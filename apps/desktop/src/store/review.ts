...
import type { AlexReviewFile, AlexReviewShipInfo } from '@/global'
...
//
// Scope is always "uncommitted" — Alex' flow is agent edits you review BEFORE
...
const OPEN_KEY = 'alex.desktop.reviewOpen'
const COMMIT_DEFAULT_KEY = 'alex.desktop.reviewCommitDefault'
const TREE_MODE_KEY = 'alex.desktop.reviewTreeMode'
const SELECTED_KEY = 'alex.desktop.reviewSelectedPath'
...
export const $reviewFiles = atom<AlexReviewFile[]>([])
...
export const $reviewShipInfo = atom<AlexReviewShipInfo>({ ghReady: false, pr: null })
...
type ReviewBridge = NonNullable<NonNullable<NonNullable<Window['alexDesktop']>['git']>['review']>
...
function reviewCtx(): { cwd: string; review: ReviewBridge } | null {
  const cwd = repoCwd()
  const review = window.alexDesktop?.git?.review
...
export async function selectReviewFile(file: AlexReviewFile): Promise<void> {
...
export async function stageReviewFile(path: null | string): Promise<void> {
  await window.alexDesktop?.git?.review?.stage(repoCwd() ?? '', path)
  await afterMutation()
}

export async function unstageReviewFile(path: null | string): Promise<void> {
  await window.alexDesktop?.git?.review?.unstage(repoCwd() ?? '', path)
  await afterMutation()
}

export async function revertReviewFile(path: null | string): Promise<void> {
  await window.alexDesktop?.git?.review?.revert(repoCwd() ?? '', path)
  await afterMutation()
}
...
export async function createOrOpenPr(): Promise<void> {
  ...
  if (existing?.url) {
    void window.alexDesktop?.openExternal?.(existing.url)
  ...
  await runShip(async () => {
    ...
    if (url) {
      void window.alexDesktop?.openExternal?.(url)
    }
