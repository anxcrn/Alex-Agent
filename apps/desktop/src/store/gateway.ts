import type { ConnectionState, GatewayEvent } from '@alex/shared'
...
const isOpen = (gateway: AlexGateway | null): boolean => gateway?.connectionState === 'open'
...
export const $gateway = atom<AlexGateway | null>(null)
...
let primaryGateway: AlexGateway | null = null
let primaryProfile = 'default'

export function setPrimaryGateway(gateway: AlexGateway | null, profile = 'default'): void {
...
export function activeGateway(): AlexGateway | null {
...
export async function ensureActiveGatewayOpen(): Promise<AlexGateway | null> {
  if (activeKey === primaryProfile) {
    return primaryGateway
  }
