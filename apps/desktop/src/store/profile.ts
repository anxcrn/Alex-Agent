...
import { getProfiles, setApiRequestProfile } from '@/alex'
...
import type { ProfileInfo } from '@/types/alex'
...
// /api/profiles/active `current`). "default" is the root ~/.alex. This is the
...
const PROFILE_ORDER_STORAGE_KEY = 'alex.desktop.profileOrder'
...
const PROFILE_COLORS_STORAGE_KEY = 'alex.desktop.profileColors'
...
export async function refreshActiveProfile(): Promise<void> {
  try {
    const res = await window.alexDesktop.api<ActiveProfileResponse>({ path: '/api/profiles/active' })
...
// Persist the choice and relaunch the backend under the new ALEX_HOME. The
...
export async function switchProfile(name: string): Promise<void> {
  ...
  await window.alexDesktop.profile.set(name)
}
...
async function syncConnectionToActiveProfile(profile: string): Promise<void> {
  const getConnection = window.alexDesktop?.getConnection
...
const SHOW_ALL_PROFILES_STORAGE_KEY = 'alex.desktop.showAllProfiles'
...
// Switch to the default (root ~/.alex) profile — bound to ⌘1.
...
export function touchActiveGatewayBackend(): void {
  ...
  void window.alexDesktop?.touchBackend?.(target).catch(() => undefined)
}
