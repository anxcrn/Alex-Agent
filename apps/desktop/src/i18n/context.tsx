...
import { getAlexConfigRecord, type AlexConfigRecord, saveAlexConfig } from '@/alex'
...
export interface I18nConfigClient {
  getConfig: () => Promise<AlexConfigRecord>
  saveConfig: (config: AlexConfigRecord) => Promise<{ ok: boolean }>
}

const defaultConfigClient: I18nConfigClient = {
  getConfig: () => {
    if (typeof window === 'undefined' || !window.alexDesktop?.api) {
      return Promise.resolve({})
    }

    return getAlexConfigRecord()
  },
  saveConfig: config => {
    if (typeof window === 'undefined' || !window.alexDesktop?.api) {
...
export function getConfigDisplayLanguage(config: AlexConfigRecord): unknown {
  return isRecord(config.display) ? config.display.language : undefined
}

export function withConfigDisplayLanguage(config: AlexConfigRecord, locale: Locale): AlexConfigRecord {
