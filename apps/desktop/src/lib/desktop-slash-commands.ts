...
/** Why a known Alex command has no desktop UI surface. */
...
const DESKTOP_COMMAND_SPECS: readonly DesktopCommandSpec[] = [
  ...
  { name: '/profile', description: 'Switch the active Alex profile', surface: action('profile') },
  ...
  { name: '/version', description: 'Show Alex Agent version', surface: exec() },
...
function isKnownAlexSlashCommand(command: string): boolean {
  const normalized = normalizeCommand(command)
 ...
 * Alex' built-in slash commands — i.e. skill commands (`/gif-search`,
...
export function isDesktopSlashExtensionCommand(command: string): boolean {
  ...
  return !isKnownAlexSlashCommand(normalized)
}
