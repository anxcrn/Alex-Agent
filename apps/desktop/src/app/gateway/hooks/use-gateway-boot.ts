...
import type { RpcEvent } from '@/types/alex'
...
interface GatewayBootOptions {
  ...
  onConnectionReady: (
    connection: Awaited<ReturnType<NonNullable<typeof window.alexDesktop>['getConnection']>> | null
  ) => void
  onGatewayReady: (gateway: AlexGateway | null) => void
  refreshAlexConfig: () => Promise<void>
...
export function useGatewayBoot({
  ...
  useEffect(() => {
    ...
    const publish = (next: AlexConnection | null) => {
    ...
    // composer stayed disabled forever on "Starting Alex...". Once the
    ...
    const attemptReconnect = async () => {
      ...
      try {
        ...
        // "Starting Alex…". The probe is a no-op for a healthy or local backend.
        ...
        // as an opaque "Could not connect to Alex gateway". resolveGatewayWsUrl
        ...
        await callbacksRef.current.refreshAlexConfig().catch(() => undefined)
    ...
    async function boot() {
      try {
        ...
        await callbacksRef.current.refreshAlexConfig()
