...
// bug that drives the REAL useGatewayBoot hook + REAL AlexGateway through a
...
function Harness() {
  useGatewayBoot({
    ...
    refreshAlexConfig: async () => undefined,
...
beforeEach(() => {
  ...
  ;(window as { alexDesktop?: unknown }).alexDesktop = fakeDesktop()
...
describe('useGatewayBoot remote reconnect loop (real hook, fake socket)', () => {
  it('INITIAL boot against a dead VPS: getConnection hangs (waitForAlex) → app sits in the connecting combo, then fails', async () => {
    // The report's actual path: a fresh launch pointed at an unreachable VPS.
    // startAlex()'s remote branch awaits waitForAlex() for 45s before it
    ...
    ;(window as { alexDesktop?: unknown }).alexDesktop = desktop
    ...
    // After ~45s waitForAlex gives up and getConnection rejects → boot()
    // catch → failDesktopBoot → the BootFailureOverlay recovery surface.
    await act(async () => {
      rejectConn(new Error('Alex backend did not become ready: timeout'))
      await vi.advanceTimersByTimeAsync(0)
    })
