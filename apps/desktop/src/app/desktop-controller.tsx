...
import { getCronJobs, getSessionMessages, listAllProfileSessions, type SessionInfo, triggerCronJob } from '../alex'
...
import { useAlexConfig } from './session/hooks/use-alex-config'
...
export function DesktopController() {
  ...
  useEffect(() => {
    window.alexDesktop?.setPreviewShortcutActive?.(Boolean(chatOpen && (filePreviewTarget || previewTarget)))
  }, [chatOpen, filePreviewTarget, previewTarget])

  useEffect(() => {
    startUpdatePoller()
    const unsubscribe = window.alexDesktop?.onOpenUpdatesRequested?.(() => openUpdatesWindow())
  ...
  useEffect(() => {
    const unsubscribe = window.alexDesktop?.onFocusSession?.(sessionId => {
      if (sessionId) {
  ...
  useEffect(() => {
    const unsubscribe = window.alexDesktop?.onNotificationAction?.(({ actionId, sessionId }) => {
  ...
  // alex:// deep links (e.g. a docs "Send to App" button for an automation blueprint).
  ...
  useEffect(() => {
    const unsubscribe = window.alexDesktop?.onDeepLink?.(payload => {
    ...
    void window.alexDesktop?.signalDeepLinkReady?.()
  ...
  useEffect(() => {
    ...
    const unsubscribe = window.alexDesktop?.onClosePreviewRequested?.(closeActiveRightRailTab)
  ...
  const { refreshAlexConfig, sttEnabled, voiceMaxRecordingSeconds } = useAlexConfig({
    activeSessionIdRef,
  ...
  useEffect(() => {
    if (gatewayState === 'open' && !activeSessionId && freshDraftReady) {
      void refreshCurrentModel()
      void refreshAlexConfig()
    }
  }, [activeSessionId, freshDraftReady, gatewayState, refreshCurrentModel, refreshAlexConfig])
