...
import type { ConfigFieldSchema, AlexConfigRecord } from '@/types/alex'
...
export function voiceFieldVisible(key: string, config: AlexConfigRecord): boolean {
...
export function ConfigSettings({
  ...
  const [_defaults, setDefaults] = useState<AlexConfigRecord | null>(null)
  ...
  useEffect(() => {
    let cancelled = false
    Promise.all([getAlexConfigRecord(), getAlexConfigDefaults(), getAlexConfigSchema()])
      .then(([c, d, s]) => {
        if (cancelled) {
          return
        }
  ...
  const updateConfig = (next: AlexConfigRecord) => {
