...
import { disconnectOAuthProvider, listOAuthProviders } from '@/alex'
import { useI18n } from '@/i18n'
import { Check, ChevronDown, ChevronRight, KeyRound, Loader2, Terminal, Trash2 } from '@/lib/icons'
import { cn } from '@/lib/utils'
import { notify, notifyError } from '@/store/notifications'
import { $desktopOnboarding, startManualProviderOAuth } from '@/store/onboarding'
import type { EnvVarInfo, OAuthProvider } from '@/types/alex'
...
const canRunInTerminal = () => typeof window !== 'undefined' && Boolean(window.alexDesktop?.terminal)
...
//      in alex_cli/provider_catalog.py) — the SAME provider identity
//      `alex model` uses. This is authoritative: a provider tagged by the
...
function ConnectedProviderRow({
  ...
  // Alex can clear this provider's creds via the API.
  const canDisconnect = provider.disconnectable ?? provider.flow !== 'external'
  // External (CLI-managed) provider Alex can't clear via the API, but ships a
...
export function ProvidersSettings({ onClose, onViewChange, view }: ProvidersSettingsProps) {
  ...
  // Alex never deletes creds another tool owns behind a silent API call.
