        ...
        <>
          <a
            ...
            onClick={event => {
              ...
              void window.alexDesktop?.openExternal(error.url)
            }}
          >
...
export function PreviewPane({
  ...
  const restartServer = useCallback(async () => {
    ...
    // "Ask Alex to restart the server" looked like it did nothing —
  ...
  useEffect(() => {
    if (
      ...
      !window.alexDesktop?.watchPreviewFile ||
      !window.alexDesktop?.onPreviewFileChanged
    ) {
      return
    }
    ...
    const unsubscribe = window.alexDesktop.onPreviewFileChanged(payload => {
    ...
    void window.alexDesktop
      .watchPreviewFile(target.url)
      .then(watch => {
        if (!active) {
          void window.alexDesktop?.stopPreviewFileWatch?.(watch.id)
    ...
    return () => {
      ...
      if (watchId) {
        void window.alexDesktop?.stopPreviewFileWatch?.(watchId)
      }
    }
  ...
  useEffect(() => {
    ...
    webview.setAttribute('partition', 'persist:alex-preview')
