...
const branchActionLabel = (branch: AlexGitBranch, copy: BranchActionCopy) => {
...
// inside it. Naming is explicit — no auto-generated `alex/work-<ts>` trees.
export function StartWorkButton({ repoPath, onStarted }: { repoPath: string; onStarted: (path: string) => void }) {
  ...
  const [branches, setBranches] = useState<AlexGitBranch[]>([])
  ...
  const convert = async (branch: AlexGitBranch) => {
    if (pending || !repoPath || !branch) {
      return
    }
