...
import { getProfiles, transcribeAudio } from '@/alex'
...
async function readFileDataUrlForAttach(filePath: string): Promise<string | null> {
  const reader = window.alexDesktop?.readFileDataUrl
...
export function usePromptActions({
  ...
  // separate `alex gateway` process performs the actual transfer, so we
