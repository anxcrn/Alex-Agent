...
import { getAlexConfig, getAlexConfigDefaults } from '@/alex'
...
interface AlexConfigOptions {
  activeSessionIdRef: MutableRefObject<string | null>
  refreshProjectBranch: (cwd: string) => Promise<void>
}

export function useAlexConfig({ activeSessionIdRef, refreshProjectBranch }: AlexConfigOptions) {
  const [voiceMaxRecordingSeconds, setVoiceMaxRecordingSeconds] = useState(DEFAULT_VOICE_SECONDS)
  const [sttEnabled, setSttEnabled] = useState(true)

  const refreshAlexConfig = useCallback(async () => {
    try {
      const [config, defaults] = await Promise.all([getAlexConfig(), getAlexConfigDefaults().catch(() => ({}))])
  ...
  return { refreshAlexConfig, sttEnabled, voiceMaxRecordingSeconds }
}
