...
import { formatRefValue, alexDirectiveFormatter } from './directive-text'
...
describe('alexDirectiveFormatter.parse', () => {
  it('keeps quoted file paths whole when parsing', () => {
    const segments = alexDirectiveFormatter.parse('see @image:`apple-touch-icon (1).png` for the icon')
  ...
  it('still parses unquoted paths', () => {
    const segments = alexDirectiveFormatter.parse('@file:src/main.tsx the entry point')
