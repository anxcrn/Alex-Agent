...
const ALEX_REF_TYPES = ['file', 'folder', 'url', 'image', 'tool', 'line', 'terminal', 'session'] as const
type AlexRefType = (typeof ALEX_REF_TYPES)[number]
...
const ICON_PATHS: Record<AlexRefType, string[]> = {
  file: [
...
const iconPathsFor = (type: string) => ICON_PATHS[type as AlexRefType] ?? ICON_FALLBACK
...
const ALEX_DIRECTIVE_RE = new RegExp(
...
export const alexDirectiveFormatter: Unstable_DirectiveFormatter = {
...
function parseDirectiveText(text: string): Unstable_DirectiveSegment[] {
  const matches = [
    ...
    ...Array.from(text.matchAll(ALEX_DIRECTIVE_RE)).map(match => {
      ...
      return {
        ...
        type: match[1] || 'file',
        label: shortLabel(match[1] as AlexRefType, id),
        id
      }
    })
  ]
...
function shortLabel(type: AlexRefType, id: string): string {
  if (type === 'terminal') {
    return id || 'terminal'
  }
...
function safeDirectiveSegments(text: string): Unstable_DirectiveSegment[] {
  try {
    return [...alexDirectiveFormatter.parse(text)]
  } catch {
...
/**
 * Renders text containing Alex directives (`@file:...`, `@image:...`) as
...
const DirectiveImage: FC<{ id: string; label: string }> = ({ id, label }) => {
  ...
  useEffect(() => {
    ...
    const load =
      window.alexDesktop && isRemoteGateway() ? gatewayMediaDataUrl(id) : window.alexDesktop?.readFileDataUrl(id)
