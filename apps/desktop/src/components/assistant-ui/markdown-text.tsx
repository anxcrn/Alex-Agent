...
async function mediaSrc(path: string): Promise<string> {
  ...
  if (window.alexDesktop && ['audio', 'video'].includes(mediaKind(path))) {
  ...
  if (window.alexDesktop && isRemoteGateway()) {
    return gatewayMediaDataUrl(path)
  }

  if (!window.alexDesktop?.readFileDataUrl) {
    return mediaExternalUrl(path)
  }

  return window.alexDesktop.readFileDataUrl(filePathFromMediaPath(path))
}
...
function OpenMediaButton({ kind, path }: { kind: 'audio' | 'video'; path: string }) {
  return (
    <button
      ...
      onClick={() => void window.alexDesktop?.openExternal(mediaExternalUrl(path))}
      type="button"
    >
      Open {kind} file
    </button>
...
function MarkdownLink({ children, className, href, ...props }: ComponentProps<'a'>) {
  ...
  if (window.alexDesktop && text && normalizeExternalUrl(text) === target) {
