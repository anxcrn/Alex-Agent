...
declare global {
  interface Window {
    alexDesktop: {
      ...
      getConnection: (profile?: string | null) => Promise<AlexConnection>
      ...
      profile: {
        ...
        // backend under the new ALEX_HOME (reloads the window). Pass null to
        // clear the preference.
        set: (name: string | null) => Promise<DesktopActiveProfile>
      }
      api: <T>(request: AlexApiRequest) => Promise<T>
      notify: (payload: AlexNotification) => Promise<boolean>
      requestMicrophoneAccess: () => Promise<boolean>
      readFileDataUrl: (filePath: string) => Promise<string>
      readFileText: (filePath: string) => Promise<AlexReadFileTextResult>
      ...
      normalizePreviewTarget: (target: string, baseDir?: string) => Promise<AlexPreviewTarget | null>
      watchPreviewFile: (url: string) => Promise<AlexPreviewWatch>
      stopPreviewFileWatch: (id: string) => Promise<boolean>
      setTitleBarTheme?: (payload: AlexTitleBarTheme) => void
      ...
      readDir: (path: string) => Promise<AlexReadDirResult>
      ...
      git?: {
        worktreeList: (repoPath: string) => Promise<AlexGitWorktree[]>
        worktreeAdd: (
          repoPath: string,
        ...
        branchList: (repoPath: string) => Promise<AlexGitBranch[]>
        ...
        repoStatus: (repoPath: string) => Promise<AlexRepoStatus | null>
        ...
        review: {
          list: (repoPath: string, scope: AlexReviewScope, baseRef?: null | string) => Promise<AlexReviewList>
          diff: (
            repoPath: string,
            filePath: string,
            scope: AlexReviewScope,
            baseRef?: null | string,
            staged?: boolean
          ) => Promise<string>
          ...
          shipInfo: (repoPath: string) => Promise<AlexReviewShipInfo>
      ...
      terminal: {
        ...
        onExit: (id: string, callback: (payload: AlexTerminalExit) => void) => () => void
        resize: (id: string, size: { cols: number; rows: number }) => Promise<boolean>
        start: (options?: { cols?: number; cwd?: string; rows?: number }) => Promise<AlexTerminalSession>
      ...
      onWindowStateChanged?: (callback: (payload: AlexWindowState) => void) => () => void
      onFocusSession?: (callback: (sessionId: string) => void) => () => void
      onNotificationAction?: (callback: (payload: { actionId: string; sessionId?: string }) => void) => () => void
      onPreviewFileChanged: (callback: (payload: AlexPreviewFileChanged) => void) => () => void
...
export interface AlexTerminalSession {
  cwd: string
  id: string
  shell: string
}

export interface AlexTerminalExit {
  code: number | null
  signal: string | null
}
...
export interface DesktopUpdateApplyResult {
   ...
   *  `alex update` themselves. `command` is the exact line to run. */
  manual?: boolean
  command?: string
  alexRoot?: string
...
export interface AlexConnection {
  baseUrl: string
...
export interface AlexTitleBarTheme {
  background: string
  foreground: string
}

export interface AlexWindowState {
  isFullscreen: boolean
  nativeOverlayWidth: number
  windowButtonPosition: { x: number; y: number } | null
}
...
export interface AlexApiRequest {
  path: string
  method?: string
  body?: unknown
  timeoutMs?: number
...
export interface AlexNotification {
  title?: string
  body?: string
  silent?: boolean
  kind?: string
...
export interface AlexPreviewTarget {
  binary?: boolean
  byteSize?: number
  kind: 'file' | 'url'
  label: string
...
export interface AlexReadFileTextResult {
  binary?: boolean
  byteSize?: number
  language?: string
  mimeType?: string
...
export interface AlexPreviewWatch {
  id: string
  path: string
}
...
export interface AlexGitWorktree {
  path: string
  branch: null | string
  isMain: boolean
  detached: boolean
...
export interface AlexGitBranch {
  name: string
  checkedOut: boolean
  isDefault: boolean
...
export interface AlexRepoStatusFile {
  path: string
  staged: boolean
  unstaged: boolean
  untracked: boolean
...
export interface AlexRepoStatus {
  branch: null | string
...
// turn began.
export type AlexReviewScope = 'branch' | 'lastTurn' | 'uncommitted'

// One changed file in the review pane (status letter, +/- lines, staged flag).
export interface AlexReviewFile {
  path: string
  added: number
  removed: number
...
export interface AlexReviewList {
  files: AlexReviewFile[]
...
export interface AlexReviewShipInfo {
  ghReady: boolean
  pr: AlexReviewPr | null
}

export interface AlexReadDirEntry {
  name: string
  path: string
  isDirectory: boolean
}

export interface AlexReadDirResult {
  entries: AlexReadDirEntry[]
  error?: string
}

export interface AlexPreviewFileChanged {
  id: string
  path: string
  url: string
}

export interface AlexSelectPathsOptions {
  title?: string
  defaultPath?: string
  directories?: boolean
  multiple?: boolean
