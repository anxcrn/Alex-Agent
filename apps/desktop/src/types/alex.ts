...
export interface OAuthProvider {
   ...
   *  embedded terminal. Null when Alex doesn't know how to remove it. */
...
export interface EnvVarInfo {
  ...
  // in alex_cli/provider_catalog.py). When present, the Keys tab groups by
  // this provider identity — the SAME one `alex model` uses — instead of
...
export type AlexConfigRecord = Record<string, unknown>
...
export interface ModelOptionProvider {
   ...
   *  the `alex model` CLI / onboarding OAuth flow. */
  auth_type?: string
...
// folders. Mirrors alex_cli/projects_db.Project.to_dict().
...
export interface ToolProvider {
   ...
   *  CLI `alex tools` active-provider detection). */
  is_active: boolean
}
 ...
 *  not Alex); elsewhere, driver health from `cua-driver doctor`. `null`
...
/** Shape of `GET /api/alex/update/check` — the backend's own update state.
