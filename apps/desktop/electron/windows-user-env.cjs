...
// even though a fresh shell — and the Alex CLI — sees it immediately. The
// desktop's ALEX_HOME resolution relies on process.env, so that stale-snapshot
// gap silently sends the backend to the default %LOCALAPPDATA%\alex. Reading
