 ...
 *     `X-Alex-Session-Token` header; WS uses `?token=`.
...
// alex_cli/dashboard_auth/cookies.py.
//
// Two cookies are in play (see that module):
//   - alex_session_at: the OAuth access token. Short-lived (~15 min); its
//     Max-Age tracks the access-token TTL, so the cookie jar drops it the
//     instant the AT expires.
//   - alex_session_rt: the OAuth refresh token. Long-lived (24h rotating,
//     reuse-detected — Portal NAS #293 / alex #37247). When the AT cookie
...
const AT_COOKIE_VARIANTS = ['__Host-alex_session_at', '__Secure-alex_session_at', 'alex_session_at']
const RT_COOKIE_VARIANTS = ['__Host-alex_session_rt', '__Secure-alex_session_rt', 'alex_session_rt']
...
function pathWithGlobalRemoteProfile(path, profile, opts = {}) {
  ...
  let parsed
  try {
    parsed = new URL(rawPath, 'http://alex.local')
  } catch {
    return path
  }
...
/**
 * True if any cookie in `cookies` is a alex session ACCESS-token cookie
