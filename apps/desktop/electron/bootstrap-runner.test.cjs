...
function mkTmpHome() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'alex-bootstrap-test-'))
}
...
test('runBootstrap bails immediately when the signal is already aborted', async () => {
  ...
  const result = await runBootstrap({
    installStamp: null,
    activeRoot: '/tmp/alex-runner-test',
    sourceRepoRoot: null,
    alexHome: '/tmp/alex-runner-test',
    logRoot: '/tmp/alex-runner-test',
...
test('installedAgentInstallScript resolves the installer in the agent checkout', () => {
  ...
  try {
    ...
    const scriptsDir = path.join(home, 'alex-agent', 'scripts')
