...
function markerPath(alexHome) {
  return path.join(alexHome, '.alex-update-in-progress')
}
 ...
 * clock for tests.
 */
function readLiveUpdateMarker(alexHome, { kill, now = Date.now, maxAgeMs = UPDATE_MARKER_MAX_AGE_MS } = {}) {
  const file = markerPath(alexHome)
  let raw
  try {
