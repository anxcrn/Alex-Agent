 ...
 * Background
 * ----------
 * After `alex update` + `alex desktop --build-only`, the freshly-rebuilt
...
// backend/profile/root it talks to). Anything ALEX_DESKTOP_* is preserved
// plus ALEX_HOME. We snapshot the values, not the live env, so the new
...
const PRESERVED_ENV_KEYS = ['ALEX_HOME', 'ELECTRON_DISABLE_SANDBOX']
const PRESERVED_ENV_PREFIXES = ['ALEX_DESKTOP_']
