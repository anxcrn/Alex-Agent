 ...
 * the three user-facing uninstall modes to the `alex uninstall` CLI flags,
...
/**
 * Map an uninstall mode to the `python -m alex_cli.uninstall` argv (after the
 * python executable). Uses the dedicated lightweight module entrypoint (not
 * `alex_cli.main`) so it can run under a system Python OUTSIDE the venv that
...
function uninstallArgsForMode(mode) {
  ...
  return ['-m', 'alex_cli.uninstall', '--mode', mode]
}
 ...
 *
 *   macOS:   …/Alex.app/Contents/MacOS/Alex  → …/Alex.app
 *   Windows: …\Alex\Alex.exe                 → …\Alex  (install dir)
...
function resolveRemovableAppPath(execPath, platform, env = {}) {
  ...
  if (platform === 'darwin') {
    // …/Alex.app/Contents/MacOS/Alex → strip 3 segments to the .app
    const macOsDir = p.dirname(exe) // …/Contents/MacOS
    const contents = p.dirname(macOsDir) // …/Contents
    const appBundle = p.dirname(contents) // …/Alex.app
    if (appBundle.endsWith('.app')) return appBundle
    return null
  }

  if (platform === 'win32') {
    // NSIS per-user installs Alex.exe directly in the install dir.
    const dir = p.dirname(exe)
    if (/[\\/]Alex$/i.test(dir) || /[\\/]alex-desktop$/i.test(dir)) return dir
    return null
  }

  // Linux: an AppImage exposes its own path via the APPIMAGE env var.
  if (env.APPIMAGE) return env.APPIMAGE
  // Unpacked electron-builder tree: …/linux-unpacked/alex
  const dir = p.dirname(exe)
 ...
 * being deleted); `pythonPath` is prepended to PYTHONPATH so `import alex_cli`
 ...
 */
function buildPosixCleanupScript({ desktopPid, pythonExe, pythonPath, agentRoot, uninstallArgs, appPath, alexHome }) {
 ...
 * modes + `pythonPath`=agentRoot so `import alex_cli` resolves from source
...
function buildWindowsCleanupScript({
  ...
  // a problem, but Alex install paths don't use them.
  const q = s => `"${String(s).replace(/"/g, '')}"`
  const lines = [
    '@echo off',
    'setlocal enableextensions',
    `set "ALEX_HOME=${String(alexHome).replace(/"/g, '')}"`,
    `set "PID=${pid}"`
  ]
  if (pythonPath) {
