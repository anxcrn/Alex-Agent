...
test('desktop background child processes opt into hidden Windows consoles', () => {
  ...
  requireHiddenChildOptions(source, /alexProcess = spawn\(\s*backend\.command,\s*backend\.args/)
  requireHiddenChildOptions(source, /spawn\(\s*py,\s*\['-m', 'alex_cli\.main', 'uninstall', '--gui-summary'\]/)

  assert.match(source, /function unwrapWindowsVenvAlexCommand\(command, dashboardArgs\)/)
  assert.match(source, /existing Alex no-console Python at/)
  ...
  assert.match(source, /ALEX_DESKTOP_READY_FILE/)
  ...
  assert.match(source, /args: \['-m', 'alex_cli\.main', \.\.\.dashboardArgs\]/)
})
