...
const { buildDesktopBackendEnv, normalizeAlexHomeRoot } = require('./backend-env.cjs')
...
const USER_DATA_OVERRIDE = process.env.ALEX_DESKTOP_USER_DATA_DIR
...
const DEV_SERVER = process.env.ALEX_DESKTOP_DEV_SERVER
...
// switches only apply pre-launch. Override with ALEX_DESKTOP_DISABLE_GPU
...
if (REMOTE_DISPLAY_REASON) {
  ...
  console.log(
    `[alex] remote display detected (${REMOTE_DISPLAY_REASON}); disabling GPU hardware acceleration to prevent flicker`
  )
}
...
if (IS_WSL && !REMOTE_DISPLAY_REASON && fs.existsSync('/dev/dxg')) {
  ...
  console.log('[alex] WSL GPU passthrough (/dev/dxg) detected; enabling GPU acceleration')
}

ipcMain.handle('alex:get-remote-display-reason', () => REMOTE_DISPLAY_REASON)
    ...
    try {
      ...
      if (parsed && typeof parsed === 'object' && typeof parsed.commit === 'string' && parsed.commit.length >= 7) {
        if (parsed.schemaVersion !== INSTALL_STAMP_SCHEMA_VERSION) {
          console.warn(
            `[alex] install-stamp.json schemaVersion ${parsed.schemaVersion} != expected ${INSTALL_STAMP_SCHEMA_VERSION}; ignoring`
          )
          continue
        }
...
if (INSTALL_STAMP) {
  console.log(
    `[alex] install stamp: ${INSTALL_STAMP.commit.slice(0, 12)}${INSTALL_STAMP.branch ? ` (${INSTALL_STAMP.branch})` : ''}${INSTALL_STAMP.dirty ? ' [DIRTY]' : ''} from ${INSTALL_STAMP.source || 'unknown'}`
  )
} else if (IS_PACKAGED) {
  ...
  console.error(
    '[alex] WARNING: no install-stamp.json found in packaged build. First-launch bootstrap will not have a pinned ref to install.'
  )
}

// ALEX_HOME — the user-facing root for everything Alex-related. Mirrors
// scripts/install.ps1's $AlexHome and scripts/install.sh's $ALEX_HOME.
//
// Defaults:
//   Windows: %LOCALAPPDATA%\alex (matches install.ps1)
//   macOS / Linux: ~/.alex (matches install.sh)
//
// Special case for Windows: if the user has a legacy ~/.alex directory
// (e.g., from a prior pip install or a manual setup) AND no
// %LOCALAPPDATA%\alex yet, prefer the legacy path so we don't orphan their
// existing config / sessions / .env. New installs go to %LOCALAPPDATA%.
//
// ALEX_DESKTOP_USER_DATA_DIR (used by test:desktop:fresh) puts the sandbox
// ALEX_HOME beneath the throwaway userData dir so a fresh-install run never
// touches the user's real ~/.alex / %LOCALAPPDATA%\alex.
function resolveAlexHome() {
  if (process.env.ALEX_HOME) return normalizeAlexHomeRoot(process.env.ALEX_HOME)
  if (USER_DATA_OVERRIDE) return path.join(path.resolve(USER_DATA_OVERRIDE), 'alex-home')
  if (IS_WINDOWS) {
    // A GUI app launched from Explorer inherits the environment block captured
    // at login, so a ALEX_HOME set via `setx` AFTER login is invisible in
    // process.env even though the CLI (a fresh shell) sees it. Without this the
    // backend silently falls back to %LOCALAPPDATA%\alex and reports "No
    // inference provider configured" despite a valid configured home (#45471).
    // Consult the live User-scoped registry value before the default below.
    const fromRegistry = readWindowsUserEnvVar('ALEX_HOME')
    if (fromRegistry) return normalizeAlexHomeRoot(fromRegistry)
  }
  if (IS_WINDOWS && process.env.LOCALAPPDATA) {
    const localappdata = path.join(process.env.LOCALAPPDATA, 'alex')
    const legacy = path.join(app.getPath('home'), '.alex')
    // Migrate transparently to LOCALAPPDATA, but honour an existing legacy
    // ~/.alex setup (no LOCALAPPDATA install yet) so users don't lose state.
    if (!directoryExists(localappdata) && directoryExists(legacy)) return legacy
    return localappdata
  }
  return path.join(app.getPath('home'), '.alex')
}

const ALEX_HOME = resolveAlexHome()

function alexManagedNodePathEntries() {
  // NOTE: keep this ordering in sync with iter_alex_node_dirs() in
  // alex_constants.py — this Node main process cannot import the Python
  // module, so the platform-ordering rule is mirrored here.
  const root = path.join(ALEX_HOME, 'node')
  const bin = path.join(root, 'bin')
  const entries = IS_WINDOWS ? [root, bin] : [bin, root]
  return entries.filter(directoryExists)
}

function pathWithAlexManagedNode(...entries) {
  return [...alexManagedNodePathEntries(), ...entries, process.env.PATH].filter(Boolean).join(path.delimiter)
}

// ACTIVE_ALEX_ROOT — the canonical mutable Alex install. Same path
// install.ps1 / install.sh use, so a desktop-only user and a CLI-only user end
// up with identical layouts and can share one install.
const ACTIVE_ALEX_ROOT = path.join(ALEX_HOME, 'alex-agent')
// VENV_ROOT — venv lives inside the repo, exactly like install.ps1 does it.
const VENV_ROOT = path.join(ACTIVE_ALEX_ROOT, 'venv')
...
// boots, going straight to `resolveAlexBackend()`. Missing or stale marker
...
//
// We deliberately put the marker INSIDE ACTIVE_ALEX_ROOT (not alongside)
...
const BOOTSTRAP_COMPLETE_MARKER = path.join(ACTIVE_ALEX_ROOT, '.alex-bootstrap-complete')
const BOOTSTRAP_MARKER_SCHEMA_VERSION = 1

const DESKTOP_CONNECTION_CONFIG_PATH = path.join(app.getPath('userData'), 'connection.json')
const DESKTOP_UPDATE_CONFIG_PATH = path.join(app.getPath('userData'), 'updates.json')
const DESKTOP_WINDOW_STATE_PATH = path.join(app.getPath('userData'), 'window-state.json')
// active-profile.json records which Alex profile the desktop launches its
// local backend as. When set, startAlex() passes `alex --profile <name>
// dashboard …`, which deterministically pins ALEX_HOME (see
// _apply_profile_override in alex_cli/main.py) and bypasses the sticky
// ~/.alex/active_profile file. Unset (null) preserves the legacy behavior:
// no --profile flag, so the backend honors active_profile / default.
const DESKTOP_PROFILE_CONFIG_PATH = path.join(app.getPath('userData'), 'active-profile.json')
// Mirrors alex_cli.profiles._PROFILE_ID_RE so we never hand the backend a
// value its profile resolver would reject and exit on.
const PROFILE_NAME_RE = /^[a-z0-9][a-z0-9_-]{0,63}$/
// Branch we track for self-update. The GUI work has merged to main, so this
// tracks main. User can also override at runtime via
// alexDesktop.updates.setBranch().
const DEFAULT_UPDATE_BRANCH = 'main'
// desktop.log lives under ALEX_HOME/logs/ so it sits next to agent.log,
// errors.log, gateway.log produced by alex_logging.setup_logging — one log
// directory per user, regardless of which UI surface produced the line.
const DESKTOP_LOG_PATH = path.join(ALEX_HOME, 'logs', 'desktop.log')
...
//
// Mirror the Python logs (alex_logging.py RotatingFileHandler, maxBytes x
...
const BOOT_FAKE_MODE = process.env.ALEX_DESKTOP_BOOT_FAKE === '1'
const BOOT_FAKE_STEP_MS = (() => {
  const raw = Number.parseInt(String(process.env.ALEX_DESKTOP_BOOT_FAKE_STEP_MS || ''), 10)
...
// 'alex:native-theme' ('dark' | 'light' | 'system'); we pin
...
// an in-place `alex update` mid-session is reflected without an app restart;
...
const MEDIA_PROTOCOL = 'alex-media'
// Only audio/video may be streamed. Without this the handler would read any
// non-blocklisted local file (no size cap) for any `fetch(alex-media://…)`.
...
let mainWindow = null
let alexProcess = null
let connectionPromise = null
// Additional per-profile backends, keyed by profile name. The PRIMARY backend
// (the desktop's launch profile) stays managed by alexProcess +
// connectionPromise + startAlex(); this pool only holds EXTRA profile
...
const POOL_MAX_BACKENDS = Math.max(1, Number(process.env.ALEX_DESKTOP_POOL_MAX) || 3)
const POOL_IDLE_MS = Math.max(60_000, Number(process.env.ALEX_DESKTOP_POOL_IDLE_MS) || 10 * 60_000)
...
// onto the error so subsequent startAlex() calls (e.g. the renderer's
// ensureGatewayOpen retrying after the WS won't open) return the same error
// instead of re-running install.ps1 in a hot loop. Cleared explicitly by
// the renderer's "Reload and retry" path or by quitting the app.
let bootstrapFailure = null
// Latched non-bootstrap backend spawn failure — stops getConnection() from
// respawning alex dashboard children in a tight loop while boot is broken.
...
let bootProgressState = {
  ...
  message: 'Waiting to start Alex backend',
  phase: 'idle',
  progress: 0,
  running: false,
  timestamp: Date.now()
...
function rememberLog(chunk) {
  ...
  alexLog.push(...lines)
  if (alexLog.length > 300) {
    alexLog.splice(0, alexLog.length - 300)
  }
...
function ensureWslWindowsFonts() {
  ...
  try {
    ...
    const confPath = path.join(confDir, '99-alex-wsl-windows-fonts.conf')
    let existing = ''
    try {
...
function broadcastBootProgress() {
  ...
  webContents.send('alex:boot-progress', bootProgressState)
}
...
//
// The snapshot is queryable via the alex:bootstrap:get IPC handler so a
...
function broadcastBootstrapEvent(ev) {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('alex:bootstrap:event', ev)
}
...
// The Tauri updater writes ALEX_HOME/.alex-update-in-progress for the whole
...
// (`force_kill_other_alex`, taskkill /IM alex.exe) kills it, the launch
...
async function waitForUpdateToFinish() {
  let marker = readLiveUpdateMarker(ALEX_HOME)
  ...
  while (marker && Date.now() < deadline) {
    await advanceBootProgress(
      'backend.update-wait',
      'An update is finishing — Alex will start automatically when it completes…',
      12
    )
    await new Promise(r => setTimeout(r, UPDATE_WAIT_POLL_MS))
    marker = readLiveUpdateMarker(ALEX_HOME)
  }
  if (marker) {
...
function unwrapWindowsVenvAlexCommand(command, dashboardArgs) {
  if (!IS_WINDOWS || !command || isCommandScript(command)) return null

  const resolved = path.resolve(String(command))
  if (!/^alex(?:\.exe)?$/i.test(path.basename(resolved))) return null
  ...
  return {
    label: `existing Alex no-console Python at ${python}`,
    command: python,
    args: ['-m', 'alex_cli.main', ...dashboardArgs],
    bootstrap: false,
...
function isAlexSourceRoot(root) {
  return directoryExists(root) && fileExists(path.join(root, 'alex_cli', 'main.py'))
}

function findPythonForRoot(root) {
  const override = process.env.ALEX_DESKTOP_PYTHON
...
function findSystemPython() {
  ...
  // CPython but several Alex deps (notably pywinpty's Rust-built
...
// findGitBash — locate bash.exe on Windows. Alex' terminal tool requires
...
function findGitBash() {
  ...
  // install.ps1 drops PortableGit at %LOCALAPPDATA%\alex\git\... — checked
  // first so users who installed via install.ps1 are detected before we
  // start probing system-wide locations.
  const localAppData = process.env.LOCALAPPDATA || ''
  const candidates = []
  if (localAppData) {
    candidates.push(path.join(localAppData, 'alex', 'git', 'bin', 'bash.exe'))
    candidates.push(path.join(localAppData, 'alex', 'git', 'usr', 'bin', 'bash.exe'))
  }
...
function getNoConsoleVenvPython(venvRoot) {
  ...
  // and breaks imports (yaml, alex_cli, …) even when PYTHONPATH is patched.
...
function applyWindowsNoConsoleSpawnHints(backend) {
  ...
  const usesAlexModule =
    backend.kind === 'python' ||
    (Array.isArray(backend.args) && backend.args[0] === '-m' && backend.args[1] === 'alex_cli.main')

  if (!usesAlexModule) return backend
...
// install only has PortableGit under %LOCALAPPDATA%\alex\git (never on
...
function resolveGitBinary() {
  ...
  const candidates = []
  if (localAppData) {
    candidates.push(path.join(localAppData, 'alex', 'git', 'cmd', 'git.exe'))
    candidates.push(path.join(localAppData, 'alex', 'git', 'bin', 'git.exe'))
  }
...
function recentAlexLog() {
  return alexLog.slice(-20).join('\n')
}

// ─── Self-update (git-pull against the running backend's alex root) ──────
...
// Dev → SOURCE_REPO_ROOT. Packaged/CLI install → ACTIVE_ALEX_ROOT.
// ALEX_DESKTOP_ALEX_ROOT always wins so devs can pin a worktree.
function resolveUpdateRoot() {
  const candidates = [
    process.env.ALEX_DESKTOP_ALEX_ROOT && path.resolve(process.env.ALEX_DESKTOP_ALEX_ROOT),
    !IS_PACKAGED && isAlexSourceRoot(SOURCE_REPO_ROOT) ? SOURCE_REPO_ROOT : null,
    isAlexSourceRoot(ACTIVE_ALEX_ROOT) ? ACTIVE_ALEX_ROOT : null
  ].filter(Boolean)

  return candidates.find(c => directoryExists(path.join(c, '.git'))) || candidates[0] || ACTIVE_ALEX_ROOT
}
...
function emitUpdateProgress(payload) {
  ...
  for (const window of BrowserWindow.getAllWindows()) {
    window.webContents.send('alex:updates:progress', merged)
  }
}
...
// ALEX_HOME/alex-setup.exe on a successful install (see
// apps/bootstrap-installer paths::copy_self_to_alex_home). That binary owns
// ALL repo mutation — running `alex update` + rebuilding the desktop — so
...
function resolveUpdaterBinary() {
  const name = IS_WINDOWS ? 'alex-setup.exe' : 'alex-setup'
  const candidate = path.join(ALEX_HOME, name)
...
// Path to the venv shim whose lock decides whether `alex update` can write
// fresh entry points. On Windows this is the file the running backend
// `alex.exe` holds open; on POSIX it's never mandatory-locked.
function venvAlexShimPath(updateRoot) {
  return IS_WINDOWS
    ? path.join(updateRoot, 'venv', 'Scripts', 'alex.exe')
    : path.join(updateRoot, 'venv', 'bin', 'alex')
}
...
// only signals the direct child, so on Windows a backend `alex.exe` that
// spawned its own grandchildren (a `alex` REPL, a pty terminal session, the
...
// `alexProcess.kill('SIGTERM')` + `app.quit()` fire-and-forget: SIGTERM on
// Windows doesn't reap the backend's grandchildren, and quit didn't wait for
// teardown, so the updater raced a still-locked `alex.exe`, the quarantine
...
// problem: the desktop's backend (and the grandchildren IT spawned — a alex
// REPL, a pty terminal, the gateway) keep `alex.exe` and other files in the
...
async function releaseBackendLock(updateRoot, tag) {
  ...
  const pids = []
  if (alexProcess && Number.isInteger(alexProcess.pid)) pids.push(alexProcess.pid)
  ...
  if (alexProcess && !alexProcess.killed) {
    try {
      alexProcess.kill('SIGTERM')
    } catch {
      void 0
    }
  }
  ...
  const shim = venvAlexShimPath(updateRoot)
...
// `alex update`). Instead we spawn the staged Alex-Setup binary with
// --update and quit, so it can run `alex update` (which refuses while we
...
async function applyUpdates(opts = {}) {
  ...
  try {
    ...
    if (!updater && !IS_WINDOWS) {
      // macOS/Linux drag-install: no staged Tauri alex-setup. Unlike Windows
      ...
      // whole update itself: `alex update` (backend) + `alex desktop
    ...
    if (!updater) {
      ...
      // `alex desktop`, never the Tauri installer that self-copies
      // alex-setup.exe into ALEX_HOME). They DO have a working `alex`
      ...
      // checkout they're on — bare `alex update` defaults to main and would
      ...
      const updateRoot = resolveUpdateRoot()
      let command = 'alex update'
      try {
        const head = await runGit(['rev-parse', '--abbrev-ref', 'HEAD'], { cwd: updateRoot })
        const current = (head.stdout || '').trim()
        if (head.code === 0 && current && current !== 'HEAD') {
          const branch = await resolveHealedBranch(updateRoot, current)
          if (branch !== 'main') command = `alex update --branch ${branch}`
        }
      } catch {
        // Best-effort: fall back to bare `alex update` if branch detection fails.
      }
      rememberLog(`[updates] no staged updater; surfacing manual \`${command}\` for CLI install at ${updateRoot}`)
      emitUpdateProgress({ stage: 'manual', message: command, percent: null })
      return { ok: true, manual: true, command, alexRoot: updateRoot }
    }

    emitUpdateProgress({
      stage: 'restart',
      message:
        'Updating Alex — this window will close and the updater will open. Don’t reopen Alex yourself; it restarts automatically when the update finishes.',
      percent: 100
    })
    ...
    // alex.exe (held by the backend child / its grandchildren) and the update
    // bricks. See releaseBackendLockForUpdate for the full failure analysis.
    await releaseBackendLockForUpdate(updateRoot)

    // Detached so the updater outlives this process — it needs us GONE before
    // `alex update` will run (the venv shim is locked while we live).
    const child = spawn(updater, updaterArgs, {
      cwd: ALEX_HOME,
      env: {
        ...process.env,
        ALEX_HOME,
        PATH: pathWithAlexManagedNode(venvBin)
      },
      detached: true,
      stdio: 'ignore',
      windowsHide: false
...
async function handOffWindowsBootstrapRecovery(reason) {
  ...
  const venvBin = path.join(updateRoot, 'venv', IS_WINDOWS ? 'Scripts' : 'bin')
  const venvAlex = path.join(venvBin, IS_WINDOWS ? 'alex.exe' : 'alex')
  const updaterArgs = fileExists(venvAlex) ? ['--update', '--branch', branch] : ['--repair', '--branch', branch]
...
// Resolve the alex CLI to drive an in-app update: prefer the venv shim in
// the install we're updating, fall back to `alex` on PATH.
function resolveAlexCliBinary(updateRoot) {
  const venvAlex = path.join(updateRoot, 'venv', 'bin', 'alex')
  if (fileExists(venvAlex)) return venvAlex
  return findOnPath('alex') || null
}
...
// macOS/Linux in-app update: backend (`alex update`) + OS-aware GUI rebuild
// (`alex desktop --build-only`), then atomically swap the running .app bundle
// with the freshly built one and relaunch. Degrades to "backend updated,
// restart to load the new GUI" if the swap can't be performed.
async function applyUpdatesPosixInApp() {
  const updateRoot = resolveUpdateRoot()
  const alex = resolveAlexCliBinary(updateRoot)
  if (!alex) {
    emitUpdateProgress({ stage: 'manual', message: 'alex update', percent: null })
    return { ok: true, manual: true, command: 'alex update', alexRoot: updateRoot }
  }

  // Put the Alex-managed Node and the venv on PATH so `alex desktop`'s
  // npm build can find them on a machine with no system Node. Windows portable
  // Node lives directly under %LOCALAPPDATA%\alex\node, not node\bin.
  const env = {
    ALEX_HOME,
    PATH: pathWithAlexManagedNode(path.join(updateRoot, 'venv', 'bin'))
  }

  // `alex update` reaps stale `alex dashboard` backends (a code update
  ...
  // _kill_stale_dashboard_processes reads ALEX_DESKTOP_CHILD_PID and excludes
  ...
  if (alexProcess && Number.isInteger(alexProcess.pid)) {
    desktopChildPids.push(alexProcess.pid)
  }
  for (const entry of backendPool.values()) {
  ...
  if (desktopChildPids.length) {
    env.ALEX_DESKTOP_CHILD_PID = desktopChildPids.join(',')
  }
  ...
  emitUpdateProgress({ stage: 'update', message: 'Updating Alex (git + dependencies)…', percent: 10 })
  const updated = await runStreamedUpdate(alex, ['update', '--yes', ...branchArgs], {
    cwd: updateRoot,
    env,
    stage: 'update'
  })
  if (updated.code !== 0) {
    emitUpdateProgress({ stage: 'error', message: 'alex update failed.', error: updated.error || 'update-failed' })
    return { ok: false, error: 'alex update failed' }
  }
  ...
  const rebuilt = await runRebuildWithRetry(attempt => {
    ...
    return runStreamedUpdate(alex, ['desktop', '--build-only'], { cwd: updateRoot, env, stage: 'rebuild' })
  })
  if (rebuilt.code !== 0) {
    emitUpdateProgress({
      stage: 'error',
      message: 'Backend updated, but the desktop rebuild failed. Restart Alex to retry.',
      error: rebuilt.error || 'rebuild-failed'
    })
    return { ok: false, backendUpdated: true, error: 'desktop rebuild failed' }
  }

  // Linux in-app update terminal state (#45205). `alex desktop --build-only`
  ...
  if (!IS_MAC) {
    ...
    if (outcome === 'relaunch') {
      emitUpdateProgress({ stage: 'restart', message: 'Restarting Alex…', percent: 100 })
      ...
      const scriptPath = path.join(app.getPath('temp'), `alex-desktop-update-${Date.now()}.sh`)
      try {
        ...
        return {
          ...
          guiUpdated: false,
          manualRestart: true,
          message: 'Backend updated. Quit and reopen Alex to load the new version.'
        }
      }
    }
    ...
    if (outcome === 'guiSkew') {
      emitUpdateProgress({
        ...
        message:
          ...
          'Update or reinstall the Alex desktop app to match.',
        percent: 100
      })
      rememberLog(
    ...
    return {
      ...
      message:
        ...
        '(sandbox helper needs root). Quit and reopen Alex to finish.'
    }
  }

  const rebuiltApp = [
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac-arm64', 'Alex.app'),
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac', 'Alex.app')
  ...
  if (!rebuiltApp || !targetApp) {
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend updated. Restart Alex to load the new version.',
      percent: 100
    })
...
if [ "$SRC" != "$DST" ]; then
  if /usr/bin/ditto "$SRC" "$DST.alex-update-new"; then
    rm -rf "$DST.alex-update-old" 2>/dev/null || true
    mv "$DST" "$DST.alex-update-old" 2>/dev/null || rm -rf "$DST"
    mv "$DST.alex-update-new" "$DST"
    rm -rf "$DST.alex-update-old" 2>/dev/null || true
  fi
fi
...
`
  ...
  try {
    ...
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend + app updated. Restart Alex to load the new version.',
      percent: 100
    })
...
function isBootstrapComplete() {
  ...
  // pinned commit -- users update via the in-app update path or `alex
  ...
  return isAlexSourceRoot(ACTIVE_ALEX_ROOT) && fileExists(getVenvPython(VENV_ROOT))
}
...
function resolveWebDist() {
  const override = process.env.ALEX_DESKTOP_WEB_DIST
  ...
  if (IS_PACKAGED && /app\.asar(?=$|[\\/])/.test(fallback) && !directoryExists(fallback)) {
    rememberLog(
      `[web-dist] dashboard frontend dir resolved to an asar-internal path that ` +
        ...
        `Ensure dist/** is unpacked (asarUnpack) or set ALEX_DESKTOP_WEB_DIST.`
    )
  }
  return fallback
}
...
function resolveRendererIndex() {
  ...
  rememberLog(
    `[renderer] index.html not found — the desktop app was packaged without a ` +
      ...
      `Rebuild with: alex desktop --force-build`
  )
  return candidates[0]
}
...
function resolveAlexCwd() {
  // In a packaged build, `process.cwd()` resolves to the install root (e.g.
  // `…/win-unpacked` on Windows or `/Applications/Alex.app/Contents/...`
...
function sanitizeWorkspaceCwd(cwd) {
  ...
  if (!trimmed || isPackagedInstallPath(trimmed)) {
    return { cwd: resolveAlexCwd(), sanitized: Boolean(trimmed) }
  }
...
// createActiveBackend — build a backend pointing at ACTIVE_ALEX_ROOT, the
...
function createActiveBackend(dashboardArgs) {
  ...
  return applyWindowsNoConsoleSpawnHints({
    kind: 'python',
    label: `Alex at ${ACTIVE_ALEX_ROOT}`,
    command,
    args: ['-m', 'alex_cli.main', ...dashboardArgs],
    env: buildDesktopBackendEnv({
      alexHome: ALEX_HOME,
      pythonPathEntries: [ACTIVE_ALEX_ROOT],
      venvRoot: VENV_ROOT
    }),
    root: ACTIVE_ALEX_ROOT,
    bootstrap: true,
    shell: false
  })
}

function resolveAlexBackend(dashboardArgs) {
  // 1. Explicit override -- ALEX_DESKTOP_ALEX_ROOT points at a developer
  //    checkout. Honour it as-is (no bootstrap; the user is driving).
  const overrideRoot = process.env.ALEX_DESKTOP_ALEX_ROOT && path.resolve(process.env.ALEX_DESKTOP_ALEX_ROOT)
  if (overrideRoot && isAlexSourceRoot(overrideRoot)) {
    const backend = createPythonBackend(overrideRoot, `Alex source at ${overrideRoot}`, dashboardArgs)
  ...
  //    installed `alex` on PATH so local Python edits are actually exercised.
  //    (In dev with no checkout, SOURCE_REPO_ROOT won't pass isAlexSourceRoot.)
  if (!IS_PACKAGED && isAlexSourceRoot(SOURCE_REPO_ROOT)) {
    const backend = createPythonBackend(SOURCE_REPO_ROOT, `Alex source at ${SOURCE_REPO_ROOT}`, dashboardArgs)
    if (backend) return backend
  }

  // 3. Bootstrap-complete ACTIVE_ALEX_ROOT -- the canonical install at
  //    %LOCALAPPDATA%\alex\alex-agent (Windows) or ~/.alex/alex-agent.
  ...
  //    to spawning alex. Updates flow through the in-app update path
  //    (applyUpdates -> git pull) or `alex update` from the CLI.
  if (isBootstrapComplete()) {
    return createActiveBackend(dashboardArgs)
  }

  // 4. Existing `alex` on PATH -- installed via install.ps1 / install.sh from
  //    a previous tool-only setup, or pip-installed system-wide. Use it but
  //    do NOT write a bootstrap marker; the user did this themselves and we
  //    don't want to take ownership of an install we didn't perform.
  //    ALEX_DESKTOP_IGNORE_EXISTING=1 forces the bootstrap path for testing.
  if (process.env.ALEX_DESKTOP_IGNORE_EXISTING !== '1') {
    let alexCommand = null
    const alexOverride = process.env.ALEX_DESKTOP_ALEX

    if (alexOverride) {
      const resolvedOverride = findOnPath(alexOverride)
      if (resolvedOverride) {
        alexCommand = resolvedOverride
      } else if (!isWindowsBinaryPathInWsl(alexOverride, { isWsl: IS_WSL })) {
        alexCommand = alexOverride
      } else {
        rememberLog(`Ignoring Windows Alex override under WSL: ${alexOverride}`)
      }
    } else {
      alexCommand = findOnPath('alex')
    }

    if (alexCommand) {
      if (looksLikeDesktopAppBinary(alexCommand)) {
        rememberLog(`Ignoring desktop app executable on PATH while resolving Alex CLI: ${alexCommand}`)
        alexCommand = null
      }
    }

    if (alexCommand) {
      const unwrapped = unwrapWindowsVenvAlexCommand(alexCommand, dashboardArgs)
      if (unwrapped) {
        return unwrapped
      }

      // Smoke-test the candidate before trusting it. A `alex` shim
      ...
      const shellForProbe = isCommandScript(alexCommand)
      if (verifyAlexCli(alexCommand, { shell: shellForProbe })) {
        return (
          unwrapWindowsVenvAlexCommand(alexCommand, dashboardArgs) || {
            label: `existing Alex CLI at ${alexCommand}`,
            command: alexCommand,
            args: dashboardArgs,
            bootstrap: false,
            env: {},
      ...
      rememberLog(
        `Ignoring existing Alex CLI at ${alexCommand}: --version probe failed; falling through to bootstrap.`
      )
    }
  }

  // 5. Last-ditch: pip-installed alex_cli module via system Python.
  ...
  if (python) {
    ...
    // having alex_cli installed -- common on dev boxes that have
    ...
    // a uv-managed 3.11 into %LOCALAPPDATA%\alex\alex-agent\venv.
    if (canImportAlexCli(python)) {
      return applyWindowsNoConsoleSpawnHints({
        kind: 'python',
        label: `installed alex_cli module via ${python}`,
    ...
    rememberLog(`Ignoring system Python ${python}: alex_cli is not importable; falling through to bootstrap.`)
  }
  ...
  //    resolveAlexBackend was the old "no payload" path and forced the
  //    user into a dead end. With the bootstrap protocol, "no install yet"
  //    is a recoverable state the GUI can drive through.
  return {
    kind: 'bootstrap-needed',
    label: 'Alex Agent not installed yet; bootstrap required',
    command: null,
    args: dashboardArgs,
    bootstrap: true,
    env: {},
...
async function ensureRuntime(backend) {
  ...
  // backend.kind === 'bootstrap-needed' means resolveAlexBackend couldn't
  ...
  if (backend.kind === 'bootstrap-needed') {
    rememberLog('[bootstrap] no Alex install found; starting first-launch bootstrap')

    if (await handOffWindowsBootstrapRecovery('bootstrap-needed')) {
      const handoffError = new Error(
        'Alex recovery was handed off to Alex Setup. The desktop will restart when recovery completes.'
      )
    ...
    const bootstrapResult = await runBootstrap({
      ...
      alexHome: ALEX_HOME,
      logRoot: path.join(ALEX_HOME, 'logs'),
    ...
    if (bootstrapResult.cancelled) {
      const cancelledError = new Error('Alex install was cancelled.')
    ...
    if (!bootstrapResult.ok) {
      const bootstrapError = new Error(
        `Alex bootstrap failed${bootstrapResult.failedStage ? ` at stage '${bootstrapResult.failedStage}'` : ''}: ` +
          `${bootstrapResult.error || 'unknown error'}. ` +
          `Check ${path.join(ALEX_HOME, 'logs', 'desktop.log')} for the full transcript.`
      )
      bootstrapError.isBootstrapFailure = true
      bootstrapError.failedStage = bootstrapResult.failedStage || null
      // Latch the failure so subsequent startAlex() calls return this
      // same error without re-running install.ps1.  Cleared by the
      // alex:bootstrap:reset IPC (renderer's "Reload and retry").
    ...
    return ensureRuntime(resolveAlexBackend(backend.args))
  }
  ...
  if (!isAlexSourceRoot(ACTIVE_ALEX_ROOT)) {
    throw new Error(
      `Alex install at ${ACTIVE_ALEX_ROOT} is missing or incomplete. ` +
        'Reinstall via the desktop installer or scripts/install.ps1.'
    )
  }

  // On Windows, preflight Git Bash. Alex' terminal tool calls bash.exe
  ...
  // %LOCALAPPDATA%\alex\git\, which findGitBash() picks up, so for any
  // user who completed the bootstrap this is a no-op. For users who got
  // here via an external `alex` on PATH, this check still helps.
  if (IS_WINDOWS && !findGitBash()) {
    throw new Error(
      'Git for Windows is required for Alex on Windows (provides Git Bash, ' +
  ...
  if (!fileExists(venvPython)) {
    ...
    // isAlexSourceRoot() and the bootstrap writes the marker only after
    // install.ps1 succeeds. If we hit this, the user (or a deleted venv)
    // broke the invariant; tell them to re-run the install.
    throw new Error(
      `Alex venv missing at ${VENV_ROOT}. Re-run the desktop installer or ` + '`scripts/install.ps1` to rebuild it.'
    )
  }

  backend.command = getNoConsoleVenvPython(VENV_ROOT)
  backend.label = `Alex at ${ACTIVE_ALEX_ROOT} (venv: ${VENV_ROOT})`
  updateBootProgress({
    phase: 'runtime.ready',
    message: 'Alex runtime is ready',
    progress: 82,
    running: true,
    error: null
  })
...
function fetchJson(url, token, options = {}) {
  return new Promise((resolve, reject) => {
    ...
    if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
      reject(new Error(`Unsupported Alex backend URL protocol: ${parsed.protocol}`))
      return
    }
            ...
            reject(
              new Error(
                `Expected JSON from ${url} but got HTML (status ${res.statusCode}). ` +
                  'The endpoint is likely missing on the Alex backend.'
              )
            )
            return
          }
    ...
    req.setTimeout(timeoutMs, () => {
      req.destroy(new Error(`Timed out connecting to Alex backend after ${timeoutMs}ms`))
    })
...
function fetchPublicJson(url, options = {}) {
  ...
  // NO ``X-Alex-Session-Token`` header — used by the auth-mode probe before
...
function getLinkTitleSession() {
  ...
  linkTitleSession = session.fromPartition('alex:link-titles', { cache: false })
...
async function previewFileTarget(rawTarget, baseDir) {
  ...
  const base = baseDir ? path.resolve(expandUserPath(baseDir)) : resolveAlexCwd()
...
function sendPreviewFileChanged(payload) {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('alex:preview-file-changed', payload)
}
...
async function waitForAlex(baseUrl, token) {
  ...
  throw new Error(`Alex backend did not become ready: ${lastError?.message || 'timeout'}`)
}
...
function sendBackendExit(payload) {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('alex:backend-exit', payload)
}
...
function sendClosePreviewRequested() {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('alex:close-preview-requested')
}

// Tell the renderer the machine just woke. Sleep silently drops the
// renderer's WebSocket to the local backend; the renderer reconnects on this
// signal so the chat composer doesn't stay stuck on "Starting Alex...".
function sendPowerResume() {
  if (!mainWindow || mainWindow.isDestroyed()) return
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('alex:power-resume')
}
...
function sendOpenUpdatesRequested() {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('alex:open-updates')
...
function sendWindowStateChanged(nextIsFullscreen) {
  ...
  webContents.send('alex:window-state-changed', state)
}
...
const ZOOM_STORAGE_KEY = 'alex:desktop:zoomLevel'
...
//     alongside the ~15-min access token (Portal NAS #293 / alex #37247).
//     Both are set as HttpOnly cookies (``alex_session_at`` ~15 min,
//     ``alex_session_rt`` 24h). When the AT cookie lapses but the RT cookie
...
const OAUTH_SESSION_PARTITION = 'persist:alex-remote-oauth'
...
function fetchJsonViaOauthSession(url, options = {}) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      reject(new Error(`Timed out connecting to Alex backend after ${timeoutMs}ms`))
    }, timeoutMs)
...
// a valid stored value (pins the root ALEX_HOME explicitly); null means "no
...
async function sanitizeDesktopConnectionConfig(config = readDesktopConnectionConfig(), profile = null) {
  ...
  const envOverride = key ? false : Boolean(process.env.ALEX_DESKTOP_REMOTE_URL)

  const remoteToken = decryptDesktopSecret(block.token)
  const authMode = normAuthMode(block.authMode)
  const remoteUrl = envOverride ? String(process.env.ALEX_DESKTOP_REMOTE_URL || '') : String(block.url || '')
  ...
  return {
    ...
    // scope is never overridden by ALEX_DESKTOP_REMOTE_URL.
    envOverride
  }
}
...
async function buildRemoteConnection(rawUrl, authMode, token, source) {
  ...
  if (authMode === 'oauth') {
    ...
    // Portal issues a 24h rotating refresh token (alex #37247), and the
    ...
    if (!(await hasLiveOauthSession(baseUrl))) {
      const err = new Error(
        'Remote Alex gateway uses OAuth, but you are not signed in. ' +
  ...
  if (!token) {
    throw new Error(
      'Remote Alex gateway is selected, but no session token is saved. ' +
...
//   2. env override (ALEX_DESKTOP_REMOTE_URL/_TOKEN) — applies app-wide
...
async function resolveRemoteBackend(profile) {
  ...
  const rawEnvUrl = process.env.ALEX_DESKTOP_REMOTE_URL
  const rawEnvToken = process.env.ALEX_DESKTOP_REMOTE_TOKEN
  if (rawEnvUrl) {
    if (!rawEnvToken) {
      throw new Error(
        'ALEX_DESKTOP_REMOTE_URL is set but ALEX_DESKTOP_REMOTE_TOKEN is not. ' +
          'Both must be provided to connect to a remote Alex backend.'
      )
    }
...
function globalRemoteActive() {
  if (process.env.ALEX_DESKTOP_REMOTE_URL) {
    return true
  }
...
async function probeRemoteAuthMode(rawUrl) {
  ...
  // sending any credentials. ``/api/status`` is public on every Alex
...
async function testDesktopConnectionConfig(input = {}) {
  ...
  if (wantRemote && block?.url) {
    ...
    const remote = (await resolveRemoteBackend(key)) || (await startAlex())
  ...
  // connect to Alex gateway". Mirror the renderer's connect here so the test
...
// startAlex() spawns fresh instead of racing the dying one. Shared by the
// connection-config and profile switch flows.
async function teardownPrimaryBackendAndWait() {
  // Capture the reference before resetAlexConnection() nulls alexProcess.
  const dying = alexProcess && !alexProcess.killed ? alexProcess : null
  resetAlexConnection()
...
// profile to startAlex() (the window backend: boot UI, bootstrap, remote
...
// local-spawn portion of startAlex() but without the boot-progress UI,
...
async function spawnPoolBackend(profile, entry) {
  ...
  if (remote) {
    await waitForAlex(remote.baseUrl, remote.token)
    return {
      ...remote,
      profile,
      logs: alexLog.slice(-80),
      ...getWindowState()
    }
  }

  const token = crypto.randomBytes(32).toString('base64url')
  // --profile wins over the inherited ALEX_HOME env (see _apply_profile_override
  // step 3 in alex_cli/main.py), so the child re-homes to this profile.
  // --port 0: the OS assigns an ephemeral port; the child announces it on stdout.
  const dashboardArgs = ['--profile', profile, 'dashboard', '--no-open', '--host', '127.0.0.1', '--port', '0']
  const backend = await ensureRuntime(resolveAlexBackend(dashboardArgs))
  const alexCwd = resolveAlexCwd()
  const webDist = resolveWebDist()
  const readyFile = backend.readyFile ? makeDashboardReadyFile() : null

  rememberLog(`Starting Alex backend for profile "${profile}" via ${backend.label}`)
  ...
  const child = spawn(
    ...
    hiddenWindowsChildOptions({
      ...
      env: {
        ...
        TERMINAL_CWD: alexCwd,
        ALEX_DASHBOARD_SESSION_TOKEN: token,
        // Marks this dashboard backend as desktop-spawned so it runs the cron
        // scheduler tick loop (the gateway isn't running under the app).
        ALEX_DESKTOP: '1',
        ALEX_WEB_DIST: webDist,
        ...(readyFile ? { ALEX_DESKTOP_READY_FILE: readyFile } : {})
      },
      shell: backend.shell,
      stdio: ['ignore', 'pipe', 'pipe']
    })
  ...
  child.once('error', error => {
    rememberLog(`Alex backend for profile "${profile}" failed to start: ${error.message}`)
    backendPool.delete(profile)
    rejectStart?.(error)
  })
  child.once('exit', (code, signal) => {
    rememberLog(`Alex backend for profile "${profile}" exited (${signal || code})`)
    backendPool.delete(profile)
    if (!ready) {
      rejectStart?.(
        new Error(`Alex backend for profile "${profile}" exited before it became ready (${signal || code}).`)
      )
    }
  })
  ...
  await Promise.race([waitForAlex(baseUrl, token), startFailed])
  ready = true
  const authToken = await adoptServedDashboardToken(baseUrl, token, {
    childAlive: () => child.exitCode === null && !child.killed,
    label: `Alex backend for profile "${profile}"`,
    rememberLog
  })
...
async function startAlex() {
  ...
  // process, every subsequent startAlex() call re-throws the same error
  ...
  connectionPromise = (async () => {
    await advanceBootProgress('backend.resolve', 'Resolving Alex backend', 8)
    // Resolve for the desktop's primary profile so a per-profile remote
    // override on the active profile is honored (falls back to env / global).
    const remote = await resolveRemoteBackend(primaryProfileKey())
    if (remote) {
      await advanceBootProgress('backend.remote', `Connecting to remote Alex backend at ${remote.baseUrl}`, 24)
      await waitForAlex(remote.baseUrl, remote.token)
      updateBootProgress({
        phase: 'backend.ready',
        message: 'Remote Alex backend is ready',
        progress: 94,
        running: true,
        error: null
      })
    ...
    // deterministic (it wins over the sticky ~/.alex/active_profile file) and
    // resolves ALEX_HOME the same way `alex -p <name>` does on the CLI. An
    ...
    await advanceBootProgress('backend.runtime', 'Resolving Alex runtime', 28)
    const backend = await ensureRuntime(resolveAlexBackend(dashboardArgs))
    const alexCwd = resolveAlexCwd()
    const webDist = resolveWebDist()
    const readyFile = backend.readyFile ? makeDashboardReadyFile() : null

    await advanceBootProgress('backend.spawn', `Starting Alex backend via ${backend.label}`, 84)
    rememberLog(`Starting Alex backend via ${backend.label}`)
    ...
    alexProcess = spawn(
      ...
      hiddenWindowsChildOptions({
        ...
        env: {
          ...process.env,
          // Explicitly pin ALEX_HOME for the child so Python's get_alex_home()
          // resolves to the SAME location our resolveAlexHome() picked. Without
          // this pin, Python falls back to ~/.alex on every platform — fine on
          // mac/linux (where our default matches), but on Windows our default is
          // %LOCALAPPDATA%\alex, which differs from C:\Users\<u>\.alex.
          // Mismatch would split config / sessions / .env / logs across two
          // directories. install.ps1 sets ALEX_HOME via setx; the desktop
    ...
    alexProcess.stdout.on('data', rememberLog)
    alexProcess.stderr.on('data', rememberLog)
    let backendReady = false
    let rejectBackendStart = null
    const backendStartFailed = new Promise((_resolve, reject) => {
      rejectBackendStart = reject
    })
    alexProcess.once('error', error => {
      rememberLog(`Alex backend failed to start: ${error.message}`)
      updateBootProgress(
        {
          error: error.message,
          message: `Alex backend failed to start: ${error.message}`,
          phase: 'backend.error',
          running: false
        },
        { allowDecrease: true }
    ...
    alexProcess.once('exit', (code, signal) => {
      rememberLog(`Alex backend exited (${signal || code})`)
      alexProcess = null
      connectionPromise = null
      sendBackendExit({ code, signal })
      if (!backendReady) {
        const message = `Alex backend exited before it became ready (${signal || code}).`
        updateBootProgress(
          {
            error: message,
            message,
        ...
        rejectBackendStart?.(
          new Error(
            `Alex backend exited before it became ready (${signal || code}). Log: ${DESKTOP_LOG_PATH}\n${recentAlexLog()}`
          )
        )
      }
    })

    await advanceBootProgress('backend.port', 'Waiting for Alex backend to launch', 86)
    // Discover the ephemeral port the child bound to
    const port = await Promise.race([
      waitForDashboardPortAnnouncement(alexProcess, { readyFile }),
      backendStartFailed
    ])
    if (readyFile) {
      fs.unlink(readyFile, () => {})
    ...
    const baseUrl = `http://127.0.0.1:${port}`
    await advanceBootProgress('backend.wait', 'Waiting for Alex backend to become ready', 90)
    await Promise.race([waitForAlex(baseUrl, token), backendStartFailed])
    backendReady = true
    backendStartFailure = null
    const authToken = await adoptServedDashboardToken(baseUrl, token, {
      // The exit/error handlers null alexProcess when the child dies.
      childAlive: () => alexProcess !== null && alexProcess.exitCode === null && !alexProcess.killed,
      rememberLog
    })
    updateBootProgress({
      phase: 'backend.ready',
      message: 'Alex backend is ready. Finalizing desktop startup',
      progress: 94,
      running: true,
      error: null
    })
...
// here so it can leave the app's bounds and stay visible while Alex is
...
// pushes pet state over IPC (alex:pet-overlay:state); the overlay just renders
// it. Control flows back (pop-in, composer submit) via alex:pet-overlay:control.
...
function spawnPetOverlayWindow(bounds) {
  const win = new BrowserWindow({
    ...
    // instead, leaving the main Alex app as the Dock/cmd-tab anchor.
    skipTaskbar: !IS_MAC,
    hasShadow: false,
    alwaysOnTop: true,
    ...
    // anchor and the Alex icon drops out of cmd/alt-tab — especially when the
    // main window is minimized. We flip this on only while the composer needs
    // the keyboard (see alex:pet-overlay:set-focusable).
    focusable: false,
    show: false,
  ...
  try {
    ...
    // which briefly hides the Dock/cmd-tab presence. Keep Alex in the normal
  ...
  win.on('closed', () => {
    ...
    if (mainWindow && !mainWindow.isDestroyed()) {
      mainWindow.webContents.send('alex:pet-overlay:control', { type: 'pop-in' })
    }
  })
...
function createWindow() {
  ...
  mainWindow.webContents.once('did-finish-load', () => {
    ...
    startAlex().catch(error => rememberLog(error.stack || error.message))
  })
}

ipcMain.handle('alex:connection', async (_event, profile) => ensureBackend(profile))
...
// "Starting Alex…". Before the renderer's backoff loop reconnects, it asks us
// to confirm the cached PRIMARY backend is still reachable; if a remote one is
// not, we drop the cache so the next getConnection() rebuilds it. Local backends
// self-heal via their child 'exit' handler, so we never touch them here.
ipcMain.handle('alex:connection:revalidate', async () => {
  ...
  try {
    ...
    // tick rebuilds a fresh, reachable descriptor. resetAlexConnection only
    // nulls connectionPromise for a remote (no child to SIGTERM).
    rememberLog('Cached remote Alex backend failed liveness probe; dropping stale connection.')
    resetAlexConnection()
    return { ok: true, rebuilt: true }
  }
})
ipcMain.handle('alex:backend:touch', async (_event, profile) => {
  touchPoolBackend(profile)
  return { ok: true }
})
ipcMain.handle('alex:gateway:ws-url', async (_event, profile) => freshGatewayWsUrl(profile))
ipcMain.handle('alex:window:openSession', async (_event, sessionId, opts) => {
...
ipcMain.handle('alex:window:openNewSession', async () => {
...
ipcMain.handle('alex:pet-overlay:open', async (_event, request) => {
...
ipcMain.handle('alex:pet-overlay:close', async () => {
  closePetOverlay()
...
ipcMain.on('alex:pet-overlay:set-bounds', (_event, bounds) => {
...
ipcMain.on('alex:pet-overlay:ignore-mouse', (_event, ignore) => {
  if (petOverlayWindow && !petOverlayWindow.isDestroyed()) {
...
ipcMain.on('alex:pet-overlay:set-focusable', (_event, focusable) => {
  if (!petOverlayWindow || petOverlayWindow.isDestroyed()) {
    return
  }
...
ipcMain.on('alex:pet-overlay:state', (_event, payload) => {
  if (petOverlayWindow && !petOverlayWindow.isDestroyed()) {
    petOverlayWindow.webContents.send('alex:pet-overlay:state', payload)
  }
})
// Overlay → main renderer: control messages (pop back in, composer submit).
ipcMain.on('alex:pet-overlay:control', (_event, payload) => {
  if (!mainWindow || mainWindow.isDestroyed()) {
    return
  }
  ...
  mainWindow.webContents.send('alex:pet-overlay:control', payload)
})
ipcMain.handle('alex:bootstrap:reset', async () => {
  // Renderer's "Reload and retry" path. Clear the latched failure and
  // reset connection state so the next startAlex() call restarts the
...
ipcMain.handle('alex:bootstrap:repair', async () => {
  // Forceful repair: drop the bootstrap-complete marker so the next
  // startAlex() re-runs the full installer (refreshing a broken/partial
...
ipcMain.handle('alex:bootstrap:cancel', async () => {
...
ipcMain.handle('alex:boot-progress:get', async () => bootProgressState)
ipcMain.handle('alex:bootstrap:get', async () => getBootstrapState())
ipcMain.handle('alex:connection-config:get', async (_event, profile) =>
  sanitizeDesktopConnectionConfig(readDesktopConnectionConfig(), profile)
)
ipcMain.handle('alex:connection-config:test', async (_event, payload) => testDesktopConnectionConfig(payload))
ipcMain.handle('alex:connection-config:probe', async (_event, rawUrl) => probeRemoteAuthMode(rawUrl))
ipcMain.handle('alex:connection-config:oauth-login', async (_event, rawUrl) => {
...
ipcMain.handle('alex:connection-config:oauth-logout', async (_event, rawUrl) => {
...
ipcMain.handle('alex:connection-config:save', async (_event, payload) => {
  const config = coerceDesktopConnectionConfig(payload)
  writeDesktopConnectionConfig(config)

  return sanitizeDesktopConnectionConfig(config, payload?.profile)
})
ipcMain.handle('alex:connection-config:apply', async (_event, payload) => {
  const config = coerceDesktopConnectionConfig(payload)
  writeDesktopConnectionConfig(config)
...
ipcMain.handle('alex:profile:get', async () => ({ profile: readActiveDesktopProfile() }))
ipcMain.handle('alex:profile:set', async (_event, name) => {
  const next = writeActiveDesktopProfile(name)

  // Switching profiles is a backend re-home: relaunch the dashboard under the
  // new ALEX_HOME. Pool backends keep their own homes, so only the primary
  // is torn down.
  await teardownPrimaryBackendAndWait()
  mainWindow?.reload()
...
ipcMain.on('alex:previewShortcutActive', (_event, active) => {
  previewShortcutActive = Boolean(active)
})

ipcMain.handle('alex:requestMicrophoneAccess', async () => {
...
ipcMain.handle('alex:api', async (_event, request) => {
...
ipcMain.handle('alex:notify', (_event, payload) => {
  ...
  notification.on('click', () => {
    ...
    if (payload?.sessionId) {
      mainWindow.webContents.send('alex:focus-session', payload.sessionId)
    }
  })
  notification.on('action', (_actionEvent, index) => {
    ...
    if (action?.id) {
      mainWindow.webContents.send('alex:notification-action', { sessionId: payload?.sessionId, actionId: action.id })
    }
  })
  notification.show()
  return true
...
ipcMain.handle('alex:readFileDataUrl', async (_event, filePath) => {
...
ipcMain.handle('alex:readFileText', async (_event, filePath) => {
...
ipcMain.handle('alex:selectPaths', async (_event, options = {}) => {
...
ipcMain.handle('alex:writeClipboard', (_event, text) => {
  clipboard.writeText(String(text || ''))
  return true
})

ipcMain.handle('alex:saveImageFromUrl', (_event, url) => saveImageFromUrl(String(url || '')))

ipcMain.handle('alex:saveImageBuffer', async (_event, payload) => {
...
ipcMain.handle('alex:saveClipboardImage', async () => {
...
ipcMain.handle('alex:normalizePreviewTarget', (_event, target, baseDir) =>
  normalizePreviewTarget(String(target || ''), baseDir ? String(baseDir) : '')
)

ipcMain.handle('alex:watchPreviewFile', (_event, url) => watchPreviewFile(String(url || '')))

ipcMain.handle('alex:stopPreviewFileWatch', (_event, id) => stopPreviewFileWatch(String(id || '')))

ipcMain.on('alex:titlebar-theme', (_event, payload) => {
...
ipcMain.on('alex:native-theme', (_event, mode) => {
...
ipcMain.on('alex:translucency', (_event, payload) => {
...
ipcMain.handle('alex:openExternal', (_event, url) => {
  if (!openExternalUrl(url)) {
    throw new Error('Invalid external URL')
  }
})

ipcMain.handle('alex:openPreviewInBrowser', async (_event, url) => {
...
// it via writeDefaultProjectDir so resolveAlexCwd picks it up on the next
// session spawn (no app restart needed).
ipcMain.handle('alex:setting:defaultProjectDir:get', async () => ({
  dir: readDefaultProjectDir(),
  defaultLabel: app.getPath('home'),
  resolvedCwd: resolveAlexCwd()
}))

ipcMain.handle('alex:workspace:sanitize', async (_event, cwd) => sanitizeWorkspaceCwd(cwd))

ipcMain.handle('alex:setting:defaultProjectDir:set', async (_event, dir) => {
...
ipcMain.handle('alex:setting:defaultProjectDir:pick', async () => {
...
ipcMain.handle('alex:fetchLinkTitle', (_event, url) => fetchLinkTitle(url))

ipcMain.handle('alex:logs:reveal', async () => {
  try {
    await fs.promises.mkdir(path.dirname(DESKTOP_LOG_PATH), { recursive: true })
...
ipcMain.handle('alex:logs:recent', async () => ({ path: DESKTOP_LOG_PATH, lines: alexLog.slice(-200) }))
...
function terminalShellCommand() {
  // ALEX_DESKTOP_SHELL is the cross-platform escape hatch (a path or a bare
  ...
  const override = (process.env.ALEX_DESKTOP_SHELL || (IS_WINDOWS ? '' : process.env.SHELL) || '').trim()
...
function terminalShellEnv() {
  ...
  // /TERM=dumb; some terminals set COLORFGBG which would flip Alex' TUI into
  ...
  env.TERM_PROGRAM = 'Alex'
  env.TERM_PROGRAM_VERSION = app.getVersion()

  // Let a alex/--tui launched in this pane know it's embedded in the desktop
  // GUI (build_environment_hints surfaces this). Distinct from ALEX_DESKTOP,
  // which marks the agent *backend* and gates cron/gateway behavior.
  env.ALEX_DESKTOP_TERMINAL = '1'

  return env
}

function terminalChannel(id, suffix) {
  return `alex:terminal:${id}:${suffix}`
}
...
ipcMain.handle('alex:fs:readDir', async (_event, dirPath) => readDirForIpc(dirPath))

ipcMain.handle('alex:fs:gitRoot', async (_event, startPath) => gitRootForIpc(startPath))

// Reveal a path in the OS file manager (Finder / Explorer / Files).
ipcMain.handle('alex:fs:reveal', async (_event, targetPath) => {
...
ipcMain.handle('alex:fs:rename', async (_event, targetPath, newName) => {
...
ipcMain.handle('alex:fs:writeText', async (_event, filePath, content) => {
...
ipcMain.handle('alex:fs:trash', async (_event, targetPath) => {
  const target = String(targetPath || '').trim()
...
ipcMain.handle('alex:git:worktreeList', async (_event, repoPath) => listWorktrees(repoPath, resolveGitBinary()))

ipcMain.handle('alex:git:worktreeAdd', async (_event, repoPath, options) =>
  addWorktree(repoPath, options || {}, resolveGitBinary())
)

ipcMain.handle('alex:git:worktreeRemove', async (_event, repoPath, worktreePath, options) =>
  removeWorktree(repoPath, worktreePath, options || {}, resolveGitBinary())
)

ipcMain.handle('alex:git:branchSwitch', async (_event, repoPath, branch) =>
  switchBranch(repoPath, branch, resolveGitBinary())
)

ipcMain.handle('alex:git:branchList', async (_event, repoPath) => listBranches(repoPath, resolveGitBinary()))
...
ipcMain.handle('alex:git:repoStatus', async (_event, repoPath) => repoStatus(repoPath, resolveGitBinary()))
...
ipcMain.handle('alex:git:review:list', async (_event, repoPath, scope, baseRef) =>
  reviewList(repoPath, scope, baseRef, resolveGitBinary())
)
ipcMain.handle('alex:git:review:diff', async (_event, repoPath, filePath, scope, baseRef, staged) =>
...
ipcMain.handle('alex:git:fileDiff', async (_event, repoPath, filePath) =>
  fileDiffVsHead(repoPath, filePath, resolveGitBinary())
)
ipcMain.handle('alex:git:review:stage', async (_event, repoPath, filePath) =>
  reviewStage(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('alex:git:review:unstage', async (_event, repoPath, filePath) =>
  reviewUnstage(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('alex:git:review:revert', async (_event, repoPath, filePath) =>
  reviewRevert(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('alex:git:review:revParse', async (_event, repoPath, ref) =>
  reviewRevParse(repoPath, ref, resolveGitBinary())
)
ipcMain.handle('alex:git:review:commit', async (_event, repoPath, message, push) =>
  reviewCommit(repoPath, message, Boolean(push), resolveGitBinary())
)
ipcMain.handle('alex:git:review:commitContext', async (_event, repoPath) =>
  reviewCommitContext(repoPath, resolveGitBinary())
)
ipcMain.handle('alex:git:review:push', async (_event, repoPath) => reviewPush(repoPath, resolveGitBinary()))
ipcMain.handle('alex:git:review:shipInfo', async (_event, repoPath) => reviewShipInfo(repoPath, resolveGhBinary()))
ipcMain.handle('alex:git:review:createPr', async (_event, repoPath) =>
...
ipcMain.handle('alex:git:scanRepos', async (_event, roots, options) => {
  try {
...
ipcMain.handle('alex:terminal:start', async (event, payload = {}) => {
  if (!nodePty) {
    throw new Error('PTY support is unavailable. Reinstall desktop dependencies and restart Alex.')
  }
...
ipcMain.handle('alex:terminal:write', (_event, id, data) => {
...
ipcMain.handle('alex:terminal:resize', (_event, id, size = {}) => {
  const sessionInfo = terminalSessions.get(String(id || ''))
...
ipcMain.handle('alex:terminal:dispose', (_event, id) => disposeTerminalSession(String(id || '')))

ipcMain.handle('alex:updates:check', async () =>
...
ipcMain.handle('alex:updates:apply', async (_event, payload) =>
...
ipcMain.handle('alex:updates:branch:get', async () => readDesktopUpdateConfig())

ipcMain.handle('alex:updates:branch:set', async (_event, name) => {
  const branch = typeof name === 'string' && name.trim() ? name.trim() : DEFAULT_UPDATE_BRANCH
  writeDesktopUpdateConfig({ branch })
  return { branch }
})

// Resolve the canonical Alex version (the one `release.py` bumps in
// alex_cli/__init__.py + pyproject.toml) so the desktop About panel shows the
// real Alex version instead of the Electron app's own package.json version,
// which historically drifted (stuck at 0.0.2). Falls back to app.getVersion()
// when the source tree can't be read (e.g. a packaged build without the repo).
function resolveAlexVersion() {
  try {
    const root = resolveUpdateRoot()
    const initPath = path.join(root, 'alex_cli', '__init__.py')
...
// Re-resolve the live Alex version and push it into the native About panel
// just before showing it, so an in-place `alex update` is reflected without
...
ipcMain.handle('alex:version', async () => ({
  appVersion: resolveAlexVersion(),
  electronVersion: process.versions.electron,
  nodeVersion: process.versions.node,
  platform: process.platform,
  alexRoot: resolveUpdateRoot()
}))
...
// the actual removal via `alex uninstall …` so the cross-platform PATH /
// registry / service / node-symlink cleanup all lives in one place
// (alex_cli/uninstall.py + alex_cli/gui_uninstall.py).
//
...
async function getUninstallSummary() {
  ...
  const agentRoot = ACTIVE_ALEX_ROOT
  ...
  const fallback = () => ({
    alex_home: ALEX_HOME,
    agent_installed: isAlexSourceRoot(agentRoot) && fileExists(py),
    gui_installed: true,
  ...
  return new Promise(resolve => {
    ...
    try {
      const child = spawn(
        py,
        ['-m', 'alex_cli.main', 'uninstall', '--gui-summary'],
        hiddenWindowsChildOptions({
          cwd: agentRoot,
          env: { ...process.env, ALEX_HOME, NO_COLOR: '1' },
          stdio: ['ignore', 'pipe', 'ignore']
        })
      )
      child.stdout.on('data', chunk => {
...
async function runDesktopUninstall(mode) {
  ...
  if (!fileExists(venvPy)) {
    return {
      ok: false,
      error: 'agent-missing',
      message: `Can't run the uninstaller: no Alex agent venv at ${VENV_ROOT}.`
    }
  }
  ...
  // Python with PYTHONPATH=<agentRoot> so `import alex_cli` resolves from
  ...
  try {
    await releaseBackendLock(ACTIVE_ALEX_ROOT, 'uninstall')
  } catch (error) {
  ...
  try {
    if (IS_WINDOWS) {
      scriptPath = path.join(app.getPath('temp'), `alex-uninstall-${Date.now()}.cmd`)
      ...
      scriptPath = path.join(app.getPath('temp'), `alex-uninstall-${Date.now()}.sh`)
...
ipcMain.handle('alex:uninstall:summary', async () => getUninstallSummary())
ipcMain.handle('alex:uninstall:run', async (_event, payload) => {
...
ipcMain.handle('alex:vscode-theme:fetch', async (_event, id) => fetchMarketplaceThemes(String(id || '')))

// Search the Marketplace for color-theme extensions (empty query = top installs).
ipcMain.handle('alex:vscode-theme:search', async (_event, query) => searchMarketplaceThemes(String(query || ''), 20))

// ---------------------------------------------------------------------------
// alex:// deep links (e.g. alex://blueprint/morning-brief?time=08:00).
// A docs/dashboard "Send to App" button opens this URL; we route it into the
// running app's chat composer. Three delivery paths: macOS 'open-url',
// Win/Linux running-app 'second-instance' (argv), Win/Linux cold-start argv.
// ---------------------------------------------------------------------------
const ALEX_PROTOCOL = 'alex'
let _pendingDeepLink = null
let _rendererReadyForDeepLink = false

function _extractDeepLink(argv) {
  if (!Array.isArray(argv)) return null
  return argv.find(a => typeof a === 'string' && a.startsWith(`${ALEX_PROTOCOL}://`)) || null
}
...
function handleDeepLink(url) {
  ...
  // alex://blueprint/<key>?slot=val  -> host="blueprint", path="/<key>"
  ...
  try {
    ...
    mainWindow.focus()
    mainWindow.webContents.send('alex:deep-link', payload)
...
ipcMain.handle('alex:deep-link-ready', () => {
  _rendererReadyForDeepLink = true
  if (_pendingDeepLink) {
    const queued = _pendingDeepLink
    _pendingDeepLink = null
    handleDeepLink(
      `${ALEX_PROTOCOL}://${queued.kind}/${encodeURIComponent(queued.name)}` +
...
function registerDeepLinkProtocol() {
  try {
    if (process.defaultApp && process.argv.length >= 2) {
      ...
      app.setAsDefaultProtocolClient(ALEX_PROTOCOL, process.execPath, [path.resolve(process.argv[1])])
    } else {
      app.setAsDefaultProtocolClient(ALEX_PROTOCOL)
    }
  } catch (err) {
...
// second-instance argv. Without the lock a second `alex://` launch spawns a
...
app.whenReady().then(() => {
  ...
  // Win/Linux cold start: the launching alex:// URL is in our own argv.
