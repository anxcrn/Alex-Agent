 ...
 *
 * Stamps the Alex icon + identity onto the packed Windows Alex.exe via
 * rcedit (delegated to set-exe-identity.cjs). This runs for EVERY packed build
 * — first install, `alex desktop`, the installer's --update rebuild, and a
 ...
 *   - packager.appInfo.productFilename: the exe basename (e.g. 'Alex')
 */
...
exports.default = async function afterPack(context) {
  ...
  const productName = context.packager?.appInfo?.productFilename || 'Alex'
  ...
  try {
    ...
    console.warn(`[after-pack] exe identity stamp failed (${err.message}); Alex.exe keeps the stock Electron icon`)
  }
}
