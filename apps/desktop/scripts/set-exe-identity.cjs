#!/usr/bin/env node
// set-exe-identity.cjs — stamp the Alex icon + version metadata onto the
// built Alex.exe using rcedit, completely decoupled from electron-builder's
// signing path.
//
// WHY THIS EXISTS
// ---------------
...
// skips rcedit, so the unpacked Alex.exe keeps the stock Electron icon and
...
// so EVERY packed build — first install, `alex desktop`, the installer's
...
// path (which rebuilds via `alex desktop --build-only`, never install.ps1)
// shipped a stock "Electron" exe. Keeping it in afterPack closes that gap.
//
// Also runnable standalone for ad-hoc re-stamping:
//   node scripts/set-exe-identity.cjs <path-to-Alex.exe>
//
...
// Stamp the Alex icon + identity onto `exe`. Resolves on success, throws on
...
async function stampExeIdentity(exe, desktopRoot = path.resolve(__dirname, '..')) {
  ...
  await rcedit(exe, {
    ...
    'version-string': {
      ...
      CompanyName: 'Charan Vankudoth',
      LegalCopyright: 'Copyright (c) 2026 Charan Vankudoth'
    }
  })

  console.log('[set-exe-identity] done — Alex icon + identity stamped')
}
