...
// no longer bundles the Alex Agent Python payload (that's fetched at first
// launch via install.ps1 / install.sh, per the Phase 1 thin-installer flow).
const APP = (() => {
  if (PLATFORM === 'darwin') {
    const appPath = path.join(RELEASE_ROOT, `mac-${ARCH}`, 'Alex.app')
    return {
      appPath,
      binary: path.join(appPath, 'Contents', 'MacOS', 'Alex'),
  ...
  if (PLATFORM === 'win32') {
    ...
    return {
      appPath: unpacked,
      binary: path.join(unpacked, 'Alex.exe'),
  ...
  return {
    appPath: unpacked,
    binary: path.join(unpacked, 'alex'),
    resourcesPath: path.join(unpacked, 'resources'),
    asarPath: path.join(unpacked, 'resources', 'app.asar'),
    unpackedDistIndex: path.join(unpacked, 'resources', 'app.asar.unpacked', 'dist', 'index.html')
  }
...
// Default ALEX_HOME for non-sandboxed runs -- matches main.cjs's
// resolveAlexHome(). On Windows it's %LOCALAPPDATA%\alex; elsewhere
// it's ~/.alex. The fresh-install sandbox launchFresh() sets its own
// ALEX_HOME and never touches this.
const DEFAULT_ALEX_HOME = (() => {
  if (PLATFORM === 'win32' && process.env.LOCALAPPDATA) {
    return path.join(process.env.LOCALAPPDATA, 'alex')
  }
  return path.join(os.homedir(), '.alex')
})()
const VENV_ROOT = path.join(DEFAULT_ALEX_HOME, 'alex-agent', 'venv')
const FRESH_SANDBOX_ROOT = path.join(os.tmpdir(), 'alex-desktop-fresh-install')
...
function ensurePackagedApp() {
  if (process.env.ALEX_DESKTOP_SKIP_BUILD === '1' && exists(APP.binary)) {
    return
  }
...
function resolveDmgPath() {
  if (!exists(RELEASE_ROOT)) {
    return path.join(RELEASE_ROOT, `Alex-${PACKAGE_JSON.version}-${ARCH}.dmg`)
  }

  const prefix = `Alex-${PACKAGE_JSON.version}`
  const candidates = fs
  ...
  return candidates.length > 0
    ...
    : path.join(RELEASE_ROOT, `Alex-${PACKAGE_JSON.version}-${ARCH}.dmg`)
}

function resolveNsisPath() {
  // electron-builder NSIS artifactName template is 'Alex-${version}-${os}-${arch}.${ext}'
...
function ensureDmg() {
  ...
  if (process.env.ALEX_DESKTOP_SKIP_BUILD === '1' && exists(resolveDmgPath())) {
    return
  }
...
function ensureNsis() {
  ...
  if (process.env.ALEX_DESKTOP_SKIP_BUILD === '1' && resolveNsisPath()) {
    return
  }
...
function launchFresh() {
  ...
  const alexHome = path.join(sandbox, 'alex-home')
  const cwd = path.join(sandbox, 'workspace')

  fs.mkdirSync(userDataDir, { recursive: true })
  fs.mkdirSync(alexHome, { recursive: true })
  ...
  env.ALEX_DESKTOP_CWD = cwd
  env.ALEX_DESKTOP_IGNORE_EXISTING = '1'
  env.ALEX_DESKTOP_TEST_MODE = 'fresh-install'
  env.ALEX_DESKTOP_USER_DATA_DIR = userDataDir
  env.ALEX_HOME = alexHome
  delete env.ALEX_DESKTOP_ALEX
  delete env.ALEX_DESKTOP_ALEX_ROOT
  ...
  console.log(`  ALEX_HOME: ${alexHome}`)
  console.log(`  cwd: ${cwd}`)

  return { runtimeRoot: path.join(alexHome, 'alex-agent', 'venv') }
}

// Validate the packaged bundle matches the thin-installer architecture:
//   - The Alex Agent Python payload is NOT shipped (it's fetched at first
...
function validateBundle() {
  ...
  // present anymore. If a stray ship of alex_cli sneaks back in we want
  // to fail loudly rather than re-introduce the 400MB delta we just removed.
  const staleFactoryMarker = path.join(APP.resourcesPath, 'alex-agent', 'alex_cli', 'main.py')
...
function help() {
  console.log(`Usage:
  npm run test:desktop:existing  # build packaged app, launch with normal PATH/existing Alex
  npm run test:desktop:fresh     # build packaged app, launch with temp userData + ALEX_HOME
...
Fast rerun (skip rebuild if the packaged app already exists):
  ALEX_DESKTOP_SKIP_BUILD=1 npm run test:desktop:fresh
`)
}
