 ...
 * product name (`Alex`). If a PREVIOUS `npm run pack` was interrupted
 ...
 *   '.../release/linux-unpacked/electron' -> '.../release/linux-unpacked/Alex'
 *
 * This is a hard failure with no obvious cause for the user — `alex desktop`
 ...
 * (the mac-unpacked Alex.app bundle) and Windows (win-unpacked), so we
