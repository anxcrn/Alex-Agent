# Alex Agent Security Policy

This document describes Alex Agent's trust model, names the one
security boundary the project treats as load-bearing, and defines the
scope for vulnerability reports.

## 1. Reporting a Vulnerability

Report privately via [GitHub Security Advisories](https://github.com/charan vankudoth/alex-agent/security/advisories/new)
or **security@charan vankudoth.com**. Do not open public issues for
security vulnerabilities. **Alex Agent does not operate a bug
bounty program.**
...
- Environment details (`alex version`, commit SHA, OS, Python
  version).
...
- **Trust envelope.** The set of resources an operator has implicitly
  granted Alex Agent access to by running it — typically, whatever
  the operator's own user account can reach on the host.
- **Stance.** An explicit statement in Alex Agent's documentation
...
Alex Agent supports two OS-level isolation postures. They address
...
Alex Agent supports this in two ways:

- **Alex Agent's own Docker image and Compose setup.** Lighter-
...
Under a whole-process wrapper, Alex Agent's in-process heuristics
...
Alex Agent filters the environment it passes to its lower-trust
...
A malicious or buggy plugin is not a vulnerability in Alex Agent
itself. Bugs in Alex Agent's plugin-install or plugin-discovery
...
4. **Within the authorized set, all callers are equally trusted.**
   Alex Agent does not model per-caller capabilities inside a
...
- Trust-model documentation violations: code behaving contrary to
  what this policy, Alex Agent's own documentation, or reasonable
  operator expectations would predict — including cases where
  Alex Agent has documented a stance about how its output should
...
- **Documented break-glass settings.** Operator-selected trade-offs
  ...
  alex-home security, and similar. Reports against those
...
- **Community-contributed skills and plugins.** Third-party skills
  ...
  plugins are in the operator's review surface, not Alex Agent's
  trust surface (§2.4, §2.5). A skill or plugin doing something
  malicious is the expected failure mode of one that wasn't
  reviewed, not a vulnerability in Alex Agent. Bugs in Alex
...
- Alex Agent includes supply-chain guards for MCP server
