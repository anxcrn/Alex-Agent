...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("ALEX_HOME", os.path.expanduser("~/.alex")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("ALEX_HOME", Path.home() / ".alex")) / "skills" / "red-teaming" / "godmode"
...
ALEX_HOME = Path(os.getenv("ALEX_HOME", Path.home() / ".alex"))
CONFIG_PATH = ALEX_HOME / "config.yaml"
PREFILL_PATH = ALEX_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/Alex models — already uncensored, just needs clean prompt
    "alex": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "alex" in model_lower or "nous" in model_lower:
        return "alex"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Alex config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.alex/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Alex for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Alex for changes to take effect.")
