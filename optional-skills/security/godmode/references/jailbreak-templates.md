...
## 5. GODMODE FAST — Alex 4 405B (Zero Refusal)

**Model:** `charan vankudoth/alex-4-405b`
**Strategy:** Alex models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with Alex Agent

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `~/.alex/config.yaml`:
