...
Usage (via cron with --no-agent):
    ...
    alex cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$ALEX_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "Alex-Watcher/1.0"})
