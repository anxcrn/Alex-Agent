#!/usr/bin/env python3
"""Canvas LMS API CLI for Alex Agent.
...
def _check_config():
    ...
    if missing:
        alex_env = os.path.join(
            os.environ.get("ALEX_HOME", os.path.expanduser("~/.alex")), ".env"
        )
        print(
            f"Missing required environment variables: {', '.join(missing)}\n"
            f"Set them in {alex_env} or export them in your shell.\n"
...
def main():
    parser = argparse.ArgumentParser(
        description="Canvas LMS API CLI for Alex Agent"
    )
