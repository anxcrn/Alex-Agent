#!/usr/bin/env python3
"""
Monitor a running video-production kanban. Polls `alex kanban list` and
...
def alex_available() -> bool:
    return shutil.which("alex") is not None


def kanban_list(tenant: str) -> list[dict]:
    """Returns parsed task rows. Falls back to plain stdout parsing if JSON
    output isn't supported by the installed alex CLI."""
    try:
        out = subprocess.run(
            ["alex", "kanban", "list", "--tenant", tenant, "--json"],
        ...
        pass
    # Fallback: textual parse of `alex kanban list`
    out = subprocess.run(
        ["alex", "kanban", "list", "--tenant", tenant],
        capture_output=True, text=True, check=False,
    )
    rows = []
...
def main():
    ...
    if not alex_available():
        print("ERROR: 'alex' CLI not found in PATH", file=sys.stderr)
        sys.exit(1)
