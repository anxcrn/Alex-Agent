...
the actual `setup.sh` that creates the project workspace, configures Alex
...
> [charan vankudoth/kanban-video-pipeline](https://github.com/charan vankudoth/kanban-video-pipeline).
...
2. **Create profiles** — `alex profile create <name> --clone`
3. **Configure profiles** — patch each profile's
   `~/.alex/profiles/<name>/config.yaml` to set toolsets, always_load skills,
   and `cwd`
4. **Write SOUL.md per profile** — the personality + role definition
5. **Copy any provided assets + write `brief.md`, `TEAM.md`, and `taste/`**
6. **Fire the initial kanban task** — `alex kanban create` assigned to the director
...
```bash
alex profile create director --clone 2>/dev/null || true
```
...
Each profile has a YAML config at `~/.alex/profiles/<name>/config.yaml`. The
...
p = os.path.expanduser(f"~/.alex/profiles/{profile}/config.yaml")
with open(p) as f:
...
PyYAML must be installed in the user's Python (it ships with most Alex
...
Each profile gets a `SOUL.md` at `~/.alex/profiles/<name>/SOUL.md` that
...
```bash
alex kanban create "Direct production of <video title>" \
    --assignee director \
...
the Alex `.env` (`${ALEX_HOME:-$HOME/.alex}/.env`) and macOS Keychain
(if on macOS):
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    local _alex_env="${ALEX_HOME:-$HOME/.alex}/.env"
    if grep -q "^${var}=" "$_alex_env" 2>/dev/null && \
       [ -n "$(grep "^${var}=" "$_alex_env" | cut -d= -f2-)" ]; then
        return 0
    fi
    ...
    echo "ERROR: ${var} not set in ${_alex_env} or Keychain (${kc_account}/${kc_service})"
    return 1
}

check_key ELEVENLABS_API_KEY alex ELEVENLABS_API_KEY || exit 1
check_key OPENROUTER_API_KEY alex OPENROUTER_API_KEY || exit 1
# ...
```
