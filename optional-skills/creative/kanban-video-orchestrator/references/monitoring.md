...
alex kanban watch --tenant <project-slug>

# Snapshot of the board
alex kanban list --tenant <project-slug>
alex kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
alex kanban stats --tenant <project-slug>

# Visual dashboard (browser)
alex dashboard

# Inspect a specific task (includes comments + events)
alex kanban show <task-id>

# Follow a single task's event stream
alex kanban tail <task-id>
```

Verify available subcommands with `alex kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `alex kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `alex kanban block <id>` then `alex kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `alex kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `alex kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `alex profile list` (profile exists?), `alex status` (gateway/dispatcher up?), and verify tenant. |
...
NEW_TASK_ID=$(alex kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
alex kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
alex kanban block <task-id>
alex kanban archive <task-id>

# Diagnose what happened
alex kanban show <task-id>      # task body, comments, recent events
alex kanban tail <task-id>      # follow the live event stream
alex kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `alex kanban create` for the director
...
while true; do
    clear
    alex kanban list --tenant <slug>
    echo "---"
    alex kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `alex kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `alex profile list` and start
