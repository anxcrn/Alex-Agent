...
Maps each role archetype to the Alex skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public alex-agent
...
## Alex skills relevant to video production

### Visual / rendering skills (`alex-agent/skills/creative/`)
...
### Design / pre-production skills (`alex-agent/skills/creative/`)
...
### Audio / media skills (`alex-agent/skills/creative/` + `skills/media/`)
...
These are **not** Alex skills but external CLIs / APIs that profiles invoke.
...
## Built-in Alex tools for media review

These are native Alex tools — not invoked via terminal but through their own
...
key is present in `${ALEX_HOME:-~/.alex}/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Alex profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `${ALEX_HOME:-~/.alex}/.env` → environment variable.
...
When a new Alex-public video skill ships:
