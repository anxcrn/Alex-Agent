...
Plus: channels_list (Alex-specific extra)

Usage:
    alex mcp serve
    alex mcp serve --verbose
...
logger = logging.getLogger("alex.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using ALEX_HOME."""
    try:
        from alex_constants import get_alex_home
        return get_alex_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("ALEX_HOME", Path.home() / ".alex")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from alex_constants import get_alex_home
        directory_file = get_alex_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("ALEX_HOME", Path.home() / ".alex")
...
class EventBridge:
    ...
    This is the Alex equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            from alex_constants import get_alex_home
            db_file = get_alex_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("ALEX_HOME", Path.home() / ".alex")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Alex MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "alex",
        instructions=(
            "Alex Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Alex MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
