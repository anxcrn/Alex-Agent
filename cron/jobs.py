...
Jobs are stored in ~/.alex/cron/jobs.json
Output is saved to ~/.alex/cron/output/{job_id}/{timestamp}.md
"""
...
# store under its own ALEX_HOME, and a profile-scoped gateway runs that
# profile's jobs under that same ALEX_HOME — so a job authored in profile
# `coder` lives in `~/.alex/profiles/coder/cron/jobs.json` and executes with
# `coder`'s `.env`, `config.yaml`, and skills. We deliberately anchor on
# `get_alex_home()` (the active profile home), NOT `get_default_alex_root()`
# (the shared root). Anchoring at the root would funnel every profile's jobs
# into one shared `jobs.json` and run them under whatever ALEX_HOME the
...
# `_get_alex_home()` / `_get_lock_paths()` resolution in cron/scheduler.py.
ALEX_DIR = get_alex_home().resolve()
CRON_DIR = ALEX_DIR / "cron"
JOBS_FILE = CRON_DIR / "jobs.json"
# Heartbeat file the in-process ticker touches on every loop iteration. The
# gateway process and the (separate) ``alex cron status`` process share it
...
# threshold in `alex cron status` (alex_cli/cron.py), so the two never
# drift apart.
...
def _jobs_lock():
    ...
    and standalone ``alex`` CLI invocations, which previously shared no lock
...
def parse_schedule(schedule: str) -> Dict[str, Any]:
    ...
    if 'T' in schedule or re.match(r'^\d{4}-\d{2}-\d{2}', schedule):
        try:
            ...
            #
            # Anchor to the CONFIGURED Alex timezone, not the server's local
            # timezone. The due-check (`get_due_jobs`) compares `next_run_at`
            # against `alex_time.now()`, which uses the configured zone. If a
            ...
            if dt.tzinfo is None:
                alex_tz = _alex_now().tzinfo
                dt = dt.replace(tzinfo=alex_tz)
            return {
                "kind": "once",
                "run_at": dt.isoformat(),
    ...
    try:
        ...
        run_at = _alex_now() + timedelta(minutes=minutes)
        return {
            "kind": "once",
...
def _ensure_aware(dt: datetime) -> datetime:
    """Return a timezone-aware datetime in Alex configured timezone.

    Backward compatibility:
    - Older stored timestamps may be naive.
    - Naive values are interpreted as *system-local wall time* (the timezone
      `datetime.now()` used when they were created), then converted to the
      configured Alex timezone.

    This preserves relative ordering for legacy naive timestamps across
    timezone changes and avoids false not-due results.
    """
    target_tz = _alex_now().tzinfo
    if dt.tzinfo is None:
...
def _stored_wall_clock_is_future(stored: datetime, current: datetime) -> bool:
    ...
    Cron schedules express local wall-clock intent. If Alex/system local time
    ...
    if schedule["kind"] == "once":
        ...
        if not HAS_CRONITER:
            logger.warning(
                ...
                "reinstall alex-agent or run 'pip install croniter' in your "
                "runtime env.",
                schedule.get("expr"),
            )
            return None
...
# =============================================================================
# Ticker heartbeat (liveness signal for `alex cron status`)
# =============================================================================
...
def _atomic_write_epoch(path: Path) -> None:
    ...
    concurrent reader in another process (``alex cron status``) never sees a
...
def record_ticker_heartbeat(success: bool = False) -> None:
    ...
    `alex cron status` can tell a thread that is merely *alive and looping*
...
def _save_jobs_unlocked(jobs: List[Dict[str, Any]]):
    ...
    try:
        with os.fdopen(fd, 'w', encoding='utf-8') as f:
            json.dump({"jobs": jobs, "updated_at": _alex_now().isoformat()}, f, indent=2)
            f.flush()
            os.fsync(f.fileno())
...
def _resolve_default_model_snapshot() -> Optional[str]:
    ...
    try:
        ...
        cfg_path = get_alex_home() / "config.yaml"
...
def create_job(
    ...
    Args:
        ...
        script: Optional path to a script whose stdout feeds the job. With
                ...
                ~/.alex/scripts/; ``.sh`` / ``.bash`` files run via bash,
...
def trigger_job(job_id: str) -> Optional[Dict[str, Any]]:
    ...
    return update_job(
        job["id"],
        {
            ...
            "paused_at": None,
            "paused_reason": None,
            "next_run_at": _alex_now().isoformat(),
        },
    )
...
def _machine_id() -> str:
    ...
    Uses ``ALEX_MACHINE_ID`` if set, else hostname + pid. The CAS correctness
    comes from the file lock + the fresh-claim check, not from this value.
    """
    explicit = os.getenv("ALEX_MACHINE_ID", "").strip()
    if explicit:
        return explicit
    try:
        import socket
...
# execution. Unlike the quick-snapshot store (`alex_cli.backup`, capped at 20)
...
def _prune_job_output(job_output_dir: Path, keep: int) -> int:
    ...
    Mirrors the quick-snapshot retention in ``alex_cli.backup._prune_quick_snapshots``:
...
def save_job_output(job_id: str, output: str):
    ...
    timestamp = _alex_now().strftime("%Y-%m-%d_%H-%M-%S")
