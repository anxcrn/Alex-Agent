...
# 5. Symlinks the 'alex' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⚕ Alex Agent Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/alex-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — symlink alex into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up alex command..."

ALEX_BIN="$SCRIPT_DIR/venv/bin/alex"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$ALEX_BIN" "$COMMAND_LINK_DIR/alex"
echo -e "${GREEN}✓${NC} Symlinked alex → $COMMAND_LINK_DISPLAY_DIR/alex"
...
# ============================================================================
# Seed bundled skills into ~/.alex/skills/
# ============================================================================

ALEX_SKILLS_DIR="${ALEX_HOME:-$HOME/.alex}/skills"
mkdir -p "$ALEX_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.alex/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$ALEX_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  alex status        # Check configuration"
if is_termux; then
    echo "  alex gateway       # Run gateway in foreground"
else
    echo "  alex gateway install # Install gateway service (messaging + cron)"
fi
echo "  alex cron list     # View scheduled jobs"
echo "  alex doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m alex_cli.main setup
fi
