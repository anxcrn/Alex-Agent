...
#
# Wired into the image as /etc/cont-init.d/01-alex-setup by the
...
ALEX_HOME="${ALEX_HOME:-/opt/data}"
INSTALL_DIR="/opt/alex"

# Drop to alex via s6-setuidgid, but skip it when already non-root.
as_alex() { [ "$(id -u)" = 0 ] || { "$@"; return; }; s6-setuidgid alex "$@"; }
...
# starts non-root. The baked install tree under /opt/alex is intentionally
# root-owned and non-writable; mutable runtime state must live under
# $ALEX_HOME. An arbitrary `--user` UID therefore cannot repair or populate
...
# default) and pass ALEX_UID/ALEX_GID — or the PUID/PGID aliases — which the
...
# `id -u alex` is the unremapped build UID (10000) because no root-only remap
# could run. root starts (id -u = 0) and the normal supervised drop to the
# alex UID are both unaffected.
cur_uid="$(id -u)"
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u alex)" ]; then
    cat >&2 <<EOF
[stage2] ERROR: container started with --user $cur_uid (an arbitrary, non-alex UID).
...
and the baked /opt/alex install tree is intentionally root-owned and
...
Start the container as root (the default) and pass your host UID/GID instead:
    ...
    docker run -e ALEX_UID=\$(id -u) -e ALEX_GID=\$(id -g) ...

NAS users (Synology / unRAID / UGOS) can use the PUID/PGID aliases:

    docker run -e PUID=\$(id -u) -e PGID=\$(id -g) ...

The image remaps the alex user to that UID/GID at boot and chowns the data
...
# --- Bootstrap ALEX_HOME as root ---
# Create the directory (and any missing parents) while we still have root
# privileges so the chown checks below see real metadata and the later
# `s6-setuidgid alex mkdir -p` block doesn't EACCES on root-owned
# ancestors. Without this, custom ALEX_HOME paths whose parents only
# root can create (e.g. `ALEX_HOME=/home/alex/.alex` in a Compose
...
# --- UID/GID remap ---
# Accept PUID/PGID as aliases for ALEX_UID/ALEX_GID.  NAS users (UGOS,
...
# UID 10000 leaves the runtime unable to read the volume.  ALEX_UID/
# ALEX_GID still win when both are set.  See #15290, salvages #25872.
ALEX_UID="${ALEX_UID:-${PUID:-}}"
ALEX_GID="${ALEX_GID:-${PGID:-}}"

if [ -n "${ALEX_UID:-}" ] && validate_uid_gid "$ALEX_UID" && [ "$ALEX_UID" != "$(id -u alex)" ]; then
    echo "[stage2] Changing alex UID to $ALEX_UID"
    usermod -u "$ALEX_UID" alex
fi
if [ -n "${ALEX_GID:-}" ] && validate_uid_gid "$ALEX_GID" && [ "$ALEX_GID" != "$(id -g alex)" ]; then
    echo "[stage2] Changing alex GID to $ALEX_GID"
    # -o allows non-unique GID (e.g. macOS GID 20 "staff" may already
    # exist as "dialout" in the Debian-based container image).
    groupmod -o -g "$ALEX_GID" alex 2>/dev/null || true
fi
...
# host's `docker` group (or root). The supervised alex user (UID 10000)
...
# alex process with `Groups: 10000` (998 gone); after this hook adds
...
# matching entry that includes alex. Idempotent across container
...
#   - alex is already a member of the right group (idempotent restart)
...
for sock in /var/run/docker.sock /run/docker.sock; do
    ...
    if id -G alex 2>/dev/null | tr ' ' '\n' | grep -qx "$sock_gid"; then
        echo "[stage2] alex already in group $sock_gid for $sock"
        break
    fi
    ...
    if usermod -aG "$sock_group" alex 2>/dev/null; then
        echo "[stage2] Added alex to group $sock_group (GID $sock_gid) for $sock"
    else
        echo "[stage2] Warning: usermod -aG $sock_group alex failed; docker backend may fail with EACCES"
    fi
    break
done

# --- Fix ownership of data volume ---
# When ALEX_UID is remapped or the top-level $ALEX_HOME isn't owned by
# the runtime alex UID, restore ownership to alex — but ONLY for the
# directories alex actually writes to. The full $ALEX_HOME may be a
# host-mounted bind containing unrelated user files; `chown -R` would
# silently destroy host ownership of those (see issue #19788).
#
# The canonical list of alex-owned subdirs is the same one the s6-setuidgid
# mkdir -p block below seeds. Keep them in sync if the seed list changes.
actual_alex_uid=$(id -u alex)
...
chown_alex_tree() {
    ...
    chown -R alex:alex "$target" 2>/dev/null || \
        echo "[stage2] Warning: chown $target failed (rootless container?) — continuing"
}

needs_chown=false
if [ "$(stat -c %u "$ALEX_HOME" 2>/dev/null)" != "$actual_alex_uid" ]; then
    needs_chown=true
fi
if [ "$needs_chown" = true ]; then
    echo "[stage2] Fixing ownership of $ALEX_HOME (targeted) to alex ($actual_alex_uid)"
    # In rootless Podman the container's "root" is mapped to an
    # unprivileged host UID — chown will fail. That's fine: the volume
    # is already owned by the mapped user on the host side.
    #
    # Top-level $ALEX_HOME: chown the directory itself (not its contents)
    # so alex can mkdir new subdirs but bind-mounted host files keep
    # their existing ownership.
    if refuse_symlinked_path "chown" "$ALEX_HOME"; then
        :
    else
        chown alex:alex "$ALEX_HOME" 2>/dev/null || \
            echo "[stage2] Warning: chown $ALEX_HOME failed (rootless container?) — continuing"
    fi
    # Alex-owned subdirs: recursive chown is safe here because these are
    # created and managed exclusively by alex (see the s6-setuidgid mkdir
    ...
    for sub in cron sessions logs hooks memories skills skins plans workspace home profiles pairing platforms/pairing lazy-packages; do
        if [ -e "$ALEX_HOME/$sub" ]; then
            chown_alex_tree "$ALEX_HOME/$sub"
        fi
    done
fi
...
# alex user. Hosted/container instances keep mutable state under
# $ALEX_HOME (/opt/data) and run with PYTHONDONTWRITEBYTECODE plus
# ALEX_DISABLE_LAZY_INSTALLS=1. Keeping /opt/alex root-owned and
...
# $ALEX_HOME/lazy-packages dir on the data volume (Dockerfile sets
# ALEX_LAZY_INSTALL_TARGET). That dir is appended to the END of sys.path,
...
# even though installs are re-enabled. The dir is seeded + chowned to alex
...
# Always reset ownership of $ALEX_HOME/profiles to alex on every
# boot. Profile dirs and files can land owned by root when commands
# are invoked via `docker exec <container> alex …` (which defaults
# to root unless `-u` is passed), and that breaks the cont-init
# reconciler (02-reconcile-profiles) which runs as alex and walks
# the profiles dir. Idempotent; skipped on rootless containers where
# chown would fail.
if [ -d "$ALEX_HOME/profiles" ]; then
    chown_alex_tree "$ALEX_HOME/profiles"
fi

# Always reset ownership of $ALEX_HOME/cron on every boot for the same
# docker-exec/root-write reason as profiles/. The cron scheduler state
# (jobs.json) must stay readable by the unprivileged alex runtime even
# after root-context maintenance commands or scheduler writes.
if [ -d "$ALEX_HOME/cron" ]; then
    chown_alex_tree "$ALEX_HOME/cron"
fi

# Reset ownership of alex-owned top-level state files on every boot.
# The targeted data-volume chown above only covers alex-owned
# *subdirectories*; loose state files living directly under $ALEX_HOME
# are missed. When those files are created or rewritten by
# `docker exec <container> alex …` (root unless `-u` is passed) they
# land root-owned, and the unprivileged alex runtime then hits
...
# sweep so host-owned files in a bind-mounted $ALEX_HOME are never
...
# entries of alex_cli.profile_distribution.USER_OWNED_EXCLUDE plus the
...
for f in \
    ...
    active_profile; do
    if [ -e "$ALEX_HOME/$f" ]; then
        if refuse_symlinked_path "chown" "$ALEX_HOME/$f"; then
            :
        else
            chown alex:alex "$ALEX_HOME/$f" 2>/dev/null || true
        fi
    fi
done

# --- config.yaml permissions ---
# Ensure config.yaml is readable by the alex runtime user even if it
# was edited on the host after initial ownership setup.
if [ -f "$ALEX_HOME/config.yaml" ]; then
    if refuse_symlinked_path "chown/chmod" "$ALEX_HOME/config.yaml"; then
        :
    else
        chown alex:alex "$ALEX_HOME/config.yaml" 2>/dev/null || true
        chmod 640 "$ALEX_HOME/config.yaml" 2>/dev/null || true
    fi
fi

# --- Seed directory structure as alex user ---
# Run as alex via s6-setuidgid so dirs end up owned correctly (matters
# under rootless Podman where chown back to root would fail).
#
# Use direct `mkdir -p` invocation (no `sh -c "..."` wrapper) so the
# shell isn't a second interpreter — defends against $ALEX_HOME values
# containing shell metacharacters. PR #30136 review item O2.
as_alex mkdir -p \
    "$ALEX_HOME/cron" \
    "$ALEX_HOME/sessions" \
    "$ALEX_HOME/logs" \
    "$ALEX_HOME/logs/gateways" \
    "$ALEX_HOME/hooks" \
    "$ALEX_HOME/memories" \
    "$ALEX_HOME/skills" \
    "$ALEX_HOME/skins" \
    ...
    "$ALEX_HOME/home" \
    "$ALEX_HOME/pairing" \
    "$ALEX_HOME/platforms/pairing" \
    "$ALEX_HOME/lazy-packages"

# --- Install-method stamp ---
# The 'docker' stamp is baked into the immutable install tree at
# /opt/alex/.install_method (see Dockerfile), NOT written here into
# $ALEX_HOME. detect_install_method() reads the code-scoped stamp first.
#
# Why we no longer stamp $ALEX_HOME: it is a shared DATA volume, commonly
# bind-mounted from the host (~/.alex:/opt/data) and sometimes shared with a
...
# 'alex update'. To heal homes already poisoned by older images, remove a
# stale 'docker' stamp from $ALEX_HOME if one is present (the host install's
# own installer re-creates its code-scoped stamp; a genuine container relies on
# the baked /opt/alex stamp, so deleting the data-dir copy is safe).
if [ -f "$ALEX_HOME/.install_method" ]; then
    stamped="$(tr -d '[:space:]' < "$ALEX_HOME/.install_method" 2>/dev/null || true)"
    if [ "$stamped" = "docker" ]; then
        rm -f "$ALEX_HOME/.install_method" 2>/dev/null || true
    fi
fi
...
seed_one() {
    dest=$1
    src=$2
    if [ ! -f "$ALEX_HOME/$dest" ] && [ -f "$INSTALL_DIR/$src" ]; then
        if refuse_symlinked_path "seed" "$ALEX_HOME/$dest"; then
            :
        else
            as_alex cp "$INSTALL_DIR/$src" "$ALEX_HOME/$dest"
        fi
    fi
}
...
if [ -f "$ALEX_HOME/.env" ]; then
    if refuse_symlinked_path "chown/chmod" "$ALEX_HOME/.env"; then
        :
    else
        chown alex:alex "$ALEX_HOME/.env" 2>/dev/null || true
        chmod 600 "$ALEX_HOME/.env" 2>/dev/null || true
    fi
fi

# --- Migrate persisted config schema ---
# Docker image upgrades replace the code under $INSTALL_DIR but preserve
# $ALEX_HOME on the mounted volume. Run the same safe, non-interactive
# config-schema migrations that `alex update` runs for non-Docker installs,
# after first-boot seeding and before supervised gateway services start.
# Set ALEX_SKIP_CONFIG_MIGRATION=1 for controlled/manual migrations.
if [ -f "$ALEX_HOME/config.yaml" ]; then
    s6-setuidgid alex "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/scripts/docker_config_migrate.py" \
...
if [ ! -f "$ALEX_HOME/auth.json" ] && [ -n "${ALEX_AUTH_JSON_BOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "seed" "$ALEX_HOME/auth.json"; then
        :
    else
        printf '%s' "$ALEX_AUTH_JSON_BOOTSTRAP" > "$ALEX_HOME/auth.json"
        chown alex:alex "$ALEX_HOME/auth.json" 2>/dev/null || true
        chmod 600 "$ALEX_HOME/auth.json"
    fi
fi
...
# can set ALEX_GATEWAY_BOOTSTRAP_STATE=running; we seed the state file
...
# ALEX_AUTH_JSON_BOOTSTRAP seeds auth.json. The [ ! -f ] guard is the
...
if [ ! -f "$ALEX_HOME/gateway_state.json" ] && \
        [ "${ALEX_GATEWAY_BOOTSTRAP_STATE:-}" = "running" ]; then
    if refuse_symlinked_path "seed" "$ALEX_HOME/gateway_state.json"; then
        :
    else
        printf '{"gateway_state":"running"}\n' > "$ALEX_HOME/gateway_state.json"
        chown alex:alex "$ALEX_HOME/gateway_state.json" 2>/dev/null || true
        chmod 644 "$ALEX_HOME/gateway_state.json"
    fi
fi
...
if [ -d "$INSTALL_DIR/skills" ]; then
    as_alex "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" \
        || echo "[stage2] Warning: skills_sync.py failed; continuing"
fi

# --- Discover agent-browser's Chromium binary ---
# The image's Dockerfile runs `npx playwright install chromium`, which
# populates ``$PLAYWRIGHT_BROWSERS_PATH`` (=/opt/alex/.playwright) with
# a ``chromium_headless_shell-<build>/chrome-headless-shell-linux64/``
# directory. agent-browser (the runtime CLI Alex spawns for the
...
# main-wrapper.sh propagates it into the supervised ``alex`` process
...
if [ -z "${AGENT_BROWSER_EXECUTABLE_PATH:-}" ] && \
    ...
    if [ -n "$browser_bin" ]; then
        ...
        # up for all supervised services (main-alex, dashboard, etc.).
