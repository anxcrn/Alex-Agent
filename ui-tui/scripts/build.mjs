...
await build({
  ...
  // Skip the prebuilt @alex/ink bundle — esbuild's __esm helper doesn't
  // await nested async init, which breaks lazy-initialized exports like
  // `render`. Bundling from source sidesteps that.
  alias: { '@alex/ink': resolve(root, 'packages/alex-ink/src/entry-exports.ts') },
...
// the `node` token, leaving a broken interpreter). The alex_cli launcher
