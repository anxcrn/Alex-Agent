...
React + Ink terminal UI for Alex. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `ALEX_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- `src/app/setupHandoff.ts` — launches external `alex setup`, suspends Ink while it runs, opens a new session on success
...
- Input history is stored in `~/.alex/.alex_history` or under `ALEX_HOME`.
...
### Setup (`setup.ts`)
`/setup` — launches external `alex setup` wizard, suspends Ink while it runs
...
```text
ui-tui/
  packages/alex-ink/   forked Ink renderer (local dep)
  src/
    ...
    app/
      ...
      setupHandoff.ts               launches external alex setup, suspends Ink while it runs
    ...
    lib/
      ...
      parentLog.ts               append-only log to ~/.alex/tui-parent.log
    ...
    types/
      alex-ink.d.ts            type declarations for @alex/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent AlexCLI subprocess for slash commands
```