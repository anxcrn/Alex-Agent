...
const logDir = join(process.env.ALEX_HOME?.trim() || join(homedir(), '.alex'), 'logs')
...
// a real ~/.alex (tests must stay hermetic — see AGENTS.md).
...
export function recordParentLifecycle(line: string): void {
  ...
  try {
    ...
    if (!warned) {
      warned = true
      process.stderr.write('alex-tui: parent lifecycle log unavailable\n')
    }
  }
}
