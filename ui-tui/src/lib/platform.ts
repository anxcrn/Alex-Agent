 ...
 * alex-ink sets ``key.meta`` for plain Alt/Option on every platform
 ...
 * (``alex_cli/voice.py::normalize_voice_record_key_for_prompt_toolkit``)
 ...
 * modifier — but alex-ink reports Alt as ``key.meta`` on many
...
export const parseVoiceRecordKey = (raw: unknown): ParsedVoiceRecordKey => {
  ...
  // On macOS alex-ink reports Alt as ``key.meta``, which ``isActionMod``
...
export const isVoiceToggleKey = (
  ...
  switch (configured.mod) {
    case 'alt':
      ...
      // alt binding.
      //
      // Bare Escape on alex-ink can arrive as ``key.meta=true`` on some
    ...
    case 'ctrl':
      ...
      // ``key.meta`` is alex-ink's Alt signal and accepting it would
