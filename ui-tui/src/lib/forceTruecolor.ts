...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.ALEX_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep Alex on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via ALEX_TUI_TRUECOLOR=1.
