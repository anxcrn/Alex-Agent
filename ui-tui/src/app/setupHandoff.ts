import type { RunExternalProcess } from '@alex/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`alex ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching alex: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`alex ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
